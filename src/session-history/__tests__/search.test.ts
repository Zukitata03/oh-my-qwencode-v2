...
describe('searchSessionHistory', () => {
  it('returns structured matches with snippets from rollout transcripts', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-session-search-'));
  ...
  it('supports session, project, and limit filters', async () => {
    ...
    try {
      ...
      await writeRollout(qwenHomeDir, '2026-03-09T12:00:00.000Z', 'rollout-2026-03-09T12-00-00-session-b.jsonl', [
        {
          type: 'session_meta',
          payload: {
            id: 'session-b',
