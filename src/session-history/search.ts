    ...
    for await (const line of reader) {
      ...
      for (const candidate of extractSearchableTexts(parsed, line)) {
        ...
        results.push({
          ...
          transcript_path_relative: relative(options.qwenHomeDir, filePath),
...
export async function searchSessionHistory(options: SessionSearchOptions): Promise<SessionSearchReport> {
  ...
  const qwenHomeDir = options.qwenHomeDir ?? qwenHome();
  ...
  const rolloutRoot = join(qwenHomeDir, 'sessions');
