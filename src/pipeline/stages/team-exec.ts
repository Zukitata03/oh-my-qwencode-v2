 ...
 *
 * Wraps the existing team mode (tmux-based Qwen Code workers) into a
 * PipelineStage. The execution backend is always teams — this is the
 * canonical OMQ execution surface.
 */
...
export interface TeamExecStageOptions {
  /** Number of Qwen Code workers to launch. Defaults to 2. */
  workerCount?: number;
 ...
 *
 * This stage delegates to the existing `omq team` infrastructure, which
 * starts real Qwen Code workers in tmux panes. The stage collects the
...
/**
 * Build the `omq team` CLI instruction from a descriptor.
 */
export function buildTeamInstruction(descriptor: TeamExecDescriptor): string {
  const launchCommand = `omq team ${descriptor.workerCount}:${descriptor.agentType} ${JSON.stringify(descriptor.task)}`;
