/**
 * Config.toml generator/merger for oh-my-qwencode
 * Merges OMQ MCP server entries and feature flags into existing config.toml
 *
...
// ---------------------------------------------------------------------------
// Top-level OMQ keys (must live before any [table] header)
// ---------------------------------------------------------------------------

/** Keys we own at the TOML root level. Used for upsert + strip. */
const OMQ_TOP_LEVEL_KEYS = [
  "notify",
...
const SHARED_MCP_REGISTRY_MARKER = "oh-my-qwencode (OMQ) Shared MCP Registry Sync";
const SHARED_MCP_REGISTRY_END_MARKER =
  "# End oh-my-qwencode shared MCP registry sync";
const OMQ_AGENTS_MAX_THREADS = 6;
const OMQ_AGENTS_MAX_DEPTH = 2;
const OMQ_EXPLORE_ROUTING_DEFAULT = '1';
const OMQ_EXPLORE_CMD_ENV = 'USE_OMQ_EXPLORE_CMD';
const OMQ_TUI_STATUS_LINE =
...
function stripRootLevelKeys(config: string, keys: readonly string[]): string {
  ...
  if (
    keys.some((key) =>
      OMQ_TOP_LEVEL_KEYS.includes(key as (typeof OMQ_TOP_LEVEL_KEYS)[number]),
    )
  ) {
    lines = lines.filter(
      (l) =>
...
/**
 * Remove any existing OMQ-owned top-level keys so we can re-insert them
 * cleanly. Also removes the comment line that precedes them.
 */
export function stripOmqTopLevelKeys(config: string): string {
  return stripRootLevelKeys(config, OMQ_TOP_LEVEL_KEYS);
}
...
function upsertEnvSettings(config: string): string {
  ...
  if (envStart < 0) {
    ...
    const envBlock = [
      "[env]",
      `${OMQ_EXPLORE_CMD_ENV} = "${OMQ_EXPLORE_ROUTING_DEFAULT}"`,
      "",
    ].join("\n");
  ...
  for (let i = envStart + 1; i < sectionEnd; i++) {
    if (new RegExp(`^\\s*${OMQ_EXPLORE_CMD_ENV}\\s*=`).test(lines[i])) {
      exploreRoutingIdx = i;
      break;
    }
  }
...
function upsertAgentsSettings(config: string): string {
  ...
  if (agentsStart < 0) {
    ...
    const agentsBlock = [
      "[agents]",
      `max_threads = ${OMQ_AGENTS_MAX_THREADS}`,
      `max_depth = ${OMQ_AGENTS_MAX_DEPTH}`,
      "",
    ].join("\n");
  ...
  if (maxThreadsIdx < 0) {
    lines.splice(sectionEnd, 0, `max_threads = ${OMQ_AGENTS_MAX_THREADS}`);
    sectionEnd += 1;
  }
  if (maxDepthIdx < 0) {
    lines.splice(sectionEnd, 0, `max_depth = ${OMQ_AGENTS_MAX_DEPTH}`);
  }
...
/**
 * Remove OMQ-owned feature flags from the [features] section.
 * If the section becomes empty after removal, remove the section header too.
 */
export function stripOmqFeatureFlags(config: string): string {
  const lines = config.split(/\r?\n/);
  const featuresStart = lines.findIndex((line) =>
    /^\s*\[features\]\s*$/.test(line),
  );
  ...
  const omqFlags = ["multi_agent", "child_agents_md", "collab"];
  const filtered: string[] = [];
  for (let i = 0; i < lines.length; i++) {
    if (i > featuresStart && i < sectionEnd) {
      const isOmqFlag = omqFlags.some((f) =>
        new RegExp(`^\\s*${f}\\s*=`).test(lines[i]),
      );
      if (isOmqFlag) continue;
    }
...
export function stripOmqEnvSettings(config: string): string {
  const lines = config.split(/\r?\n/);
  const envStart = lines.findIndex((line) => /^\s*\[env\]\s*$/.test(line));
  ...
  for (let i = 0; i < lines.length; i++) {
    if (i > envStart && i < sectionEnd) {
      const isOmqEnvKey = new RegExp(
        `^\\s*${OMQ_EXPLORE_CMD_ENV}\\s*=`,
      ).test(lines[i]);
      if (isOmqEnvKey) continue;
    }
    filtered.push(lines[i]);
  }
...
// ---------------------------------------------------------------------------
// Orphaned OMQ table sections (no marker block)
// ---------------------------------------------------------------------------

/**
 * Check whether a TOML table name belongs to a legacy OMQ-managed agent entry.
 * Handles both `agents.name` and `agents."name"` forms.
 */
function isLegacyOmqAgentSection(tableName: string): boolean {
...
/**
 * Strip OMQ-owned table sections that exist outside the marker block.
 ...
 *
 * Targets: [mcp_servers.omq_*], legacy [agents.<name>] entries, [tui]
 */
function stripOrphanedOmqSections(config: string): string {
  const lines = config.split(/\r?\n/);
  const result: string[] = [];
  ...
  while (i < lines.length) {
    ...
    if (tableMatch) {
      ...
      // The marker-based stripExistingOmqBlocks already handles [tui]
      // when it lives inside the OMQ marker block.
      const isOmqSection =
        /^mcp_servers\.omq_/.test(tableName) ||
        isLegacyOmqAgentSection(tableName);

      if (isOmqSection) {
        // Remove preceding OMQ comment lines and blank lines
        while (result.length > 0) {
          const last = result[result.length - 1];
          if (last.trim() === "" || /^#\s*(OMQ|oh-my-qwencode)/i.test(last)) {
            result.pop();
          } else {
            break;
          }
...
function upsertTuiStatusLine(config: string): {
  ...
  const mergedSection = ["[tui]", ...preservedKeyLines, OMQ_TUI_STATUS_LINE];
...
// ---------------------------------------------------------------------------
// OMQ [table] sections block (appended at end of file)
// ---------------------------------------------------------------------------

export function stripExistingOmqBlocks(config: string): {
  cleaned: string;
  removed: number;
} {
  const marker = "oh-my-qwencode (OMQ) Configuration";
  const endMarker = "# End oh-my-qwencode";
  let cleaned = config;
  let removed = 0;
...
function getSharedMcpRegistryBlock(
  ...
  const lines = [
    ...
    "# Managed by omq setup - edit the registry file instead",
  ];
  if (sourcePath) {
...
/**
 * OMQ table-section block (MCP servers, TUI).
 * Contains ONLY [table] sections — no bare keys.
 */
function getOmqTablesBlock(pkgRoot: string, includeTui = true): string {
  ...
  return [
    ...
    "startup_timeout_sec = 5",
    "",
    "# OMQ Code Intelligence MCP Server (LSP diagnostics, AST search)",
    ...
    "",
    "# OMQ Trace MCP Server (agent flow timeline & statistics)",
    ...
    "startup_timeout_sec = 5",
    "",
    "# OMQ Team MCP Server (team job lifecycle: start, status, wait, cleanup)",
    "[mcp_servers.omq_team_run]",
    'command = "node"',
    `args = ["${teamServerPath}"]`,
    "enabled = true",
    "startup_timeout_sec = 5",
...
/**
 * Merge OMQ config into existing config.toml
 * Preserves existing user settings, appends OMQ block if not present.
 *
 * Layout:
 *   1. OMQ top-level keys (notify, model_reasoning_effort, developer_instructions)
 *   2. [features] with multi_agent + child_agents_md
 *   3. [env] with defaulted explore-routing opt-in
 *   4. … user sections …
 *   5. OMQ [table] sections (mcp_servers, tui)
 */
export function buildMergedConfig(
  ...
  if (existing.includes("oh-my-qwencode (OMQ) Configuration")) {
    const stripped = stripExistingOmqBlocks(existing);
  ...
  const topLines = getOmqTopLevelLines(
    pkgRoot,
    existing,
    options.modelOverride,
  );
  const tablesBlock = getOmqTablesBlock(
    pkgRoot,
 ...
 *
 * After an omq version upgrade the OLD setup code (still loaded in memory)
 * may write a config with duplicate [tui] sections.  The Qwen Code TOML
...
export async function mergeConfig(
  ...
  if (existing.includes("oh-my-qwencode (OMQ) Configuration")) {
    ...
    if (options.verbose && stripped.removed > 0) {
      console.log("  Updating existing OMQ config block.");
    }
  }
