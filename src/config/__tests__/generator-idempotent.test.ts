 ...
 * Verifies that repeated `omq setup` runs do not duplicate OMQ sections.
 */
...
/** Assert the OMQ block appears exactly once */
function assertSingleOmqBlock(toml: string): void {
  assert.equal(
    count(toml, /# oh-my-qwencode \(OMQ\) Configuration/g),
    1,
    "OMQ marker should appear once",
  );
  assert.equal(
    count(toml, /# End oh-my-qwencode/g),
    1,
    "End marker should appear once",
  );
  assert.equal(
    count(toml, /^\[mcp_servers\.omq_state\]$/gm),
    1,
    "[mcp_servers.omq_state] should appear once",
  );
  assert.equal(
    count(toml, /^\[mcp_servers\.omq_memory\]$/gm),
    1,
    "[mcp_servers.omq_memory] should appear once",
  );
  assert.equal(
    count(toml, /^\[mcp_servers\.omq_code_intel\]$/gm),
    1,
    "[mcp_servers.omq_code_intel] should appear once",
  );
  assert.equal(
    count(toml, /^\[mcp_servers\.omq_trace\]$/gm),
    1,
    "[mcp_servers.omq_trace] should appear once",
  );
  ...
  assert.equal(
    count(toml, /^USE_OMQ_EXPLORE_CMD = "1"$/gm),
    1,
    "USE_OMQ_EXPLORE_CMD should appear once",
  );
}

describe("config generator idempotency (#384)", () => {
  it("first run creates config with all OMQ sections", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-idem-"));
    try {
      const configPath = join(wd, "config.toml");
      await mergeConfig(configPath, wd);
      const toml = await readFile(configPath, "utf-8");

      assertSingleOmqBlock(toml);
  ...
  it("second run updates without duplicating any section", async () => {
    ...
    try {
      ...
      assertSingleOmqBlock(first);

      // Second run
      await mergeConfig(configPath, wd);
      const second = await readFile(configPath, "utf-8");
      assertSingleOmqBlock(second);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it("cleans up legacy config without markers", async () => {
    ...
    try {
      const configPath = join(wd, "config.toml");
      // Simulate a legacy config written without OMQ markers
  ...
  it("cleans up orphaned OMQ sections outside marker block", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-idem-"));
    try {
      const configPath = join(wd, "config.toml");
  ...
  it("preserves user content between OMQ re-runs", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-idem-"));
    try {
      const configPath = join(wd, "config.toml");
  ...
  it("handles config with only orphaned agents sections", async () => {
    ...
    try {
      ...
      const orphanedAgents = [
        ...
        "multi_agent = true",
        "",
        "# OMQ Native Agent Roles (Qwen Code multi-agent)",
        "",
        "[agents.executor]",
      ...
      assert.doesNotMatch(toml, /^\[agents\.executor\]$/m, "legacy OMQ agent entry removed");
      assert.doesNotMatch(toml, /^\[agents\.explore\]$/m, "legacy OMQ agent entry removed");
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });

  it("preserves non-OMQ agent sections", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-idem-"));
    try {
      const configPath = join(wd, "config.toml");
      const userAgents = [
  ...
  it("merges OMQ status_line into an existing user [tui] section without duplicating the table", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-idem-"));
    try {
      const configPath = join(wd, "config.toml");
      const userTui = [
  ...
  it("skips emitting an OMQ [tui] table when includeTui is disabled", () => {
    const toml = buildMergedConfig("", "/tmp/omq", {
      includeTui: false,
    });

    assert.doesNotMatch(toml, /^\[tui\]$/m);
    assert.match(toml, /^\[mcp_servers\.omq_state\]$/m);
    assert.match(toml, /^\[env\]$/m);
    assert.match(toml, /^USE_OMQ_EXPLORE_CMD = "1"$/m);
  });

  it('seeds USE_OMQ_EXPLORE_CMD=1 into generated config by default', () => {
    const toml = buildMergedConfig('', '/tmp/omq');
  ...
  it('preserves existing [env] keys and explicit explore routing opt-outs', () => {
    const toml = buildMergedConfig(
      ['[env]', 'FOO = "bar"', 'USE_OMQ_EXPLORE_CMD = "0"', ''].join('\n'),
      '/tmp/omq',
    );

    assert.match(toml, /^\[env\]$/m);
    assert.match(toml, /^FOO = "bar"$/m);
    assert.match(toml, /^USE_OMQ_EXPLORE_CMD = "0"$/m);
  });

  it("replaces an existing OMQ notify entry without leaving orphan fragments behind", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-idem-"));
    try {
      const configPath = join(wd, "config.toml");
      const existing = [
  ...
  it("seeds context keys when root model is missing and both context keys are absent", async () => {
    ...
    try {
      ...
      assert.match(toml, /^model = "qwen3.6-plus"$/m);
  ...
  it("can override qwen3.5-plus to qwen3.6-plus and seed 1M context defaults", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-idem-"));
    try {
      const toml = buildMergedConfig('model = \"qwen3.5-plus\"\n', wd, {
        modelOverride: "qwen3.6-plus",
      });

      assert.match(toml, /^model = "qwen3.6-plus"$/m);
      assert.doesNotMatch(toml, /^model = "gpt-5\.3-qwen"$/m);
      assert.match(toml, /^model_context_window = 1000000$/m);
      assert.match(toml, /^model_auto_compact_token_limit = 900000$/m);
    } finally {
      await rm(wd, { recursive: true, force: true });
  ...
  it("does not seed 1M context defaults for non-qwen3.6-plus models", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-idem-"));
    try {
      const configPath = join(wd, "config.toml");
  ...
  it("preserves partial user context config without backfilling the missing partner key", async () => {
    ...
    try {
      ...
      await writeFile(
        configPath,
        ['model = "qwen3.6-plus"', "model_context_window = 640000", ""].join("\n"),
      );
  ...
  it("does not duplicate seeded model defaults across reruns", async () => {
    ...
    try {
      ...
      const toml = await readFile(configPath, "utf-8");
      assert.equal(
        count(toml, /^model = "qwen3.6-plus"$/gm),
        1,
  ...
  it("repairs config with duplicate [tui] sections from upgrade", async () => {
    ...
    try {
      const configPath = join(wd, "config.toml");
      // Simulate a broken config left by an older omq setup: an orphaned
      // [tui] outside the OMQ block AND another [tui] inside the block.
      const broken = [
        '[mcp_servers.figma]',
  ...
  it("syncs shared MCP registry entries in a dedicated managed block", async () => {
    ...
    try {
      const first = buildMergedConfig("", wd, {
        ...
        sharedMcpRegistrySource: "/tmp/.omq/mcp-registry.json",
      });
      ...
      assert.equal(
        count(second, /oh-my-qwencode \(OMQ\) Shared MCP Registry Sync/g),
        1,
      ...
      assert.match(second, /# Source: \/tmp\/\.omq\/mcp-registry\.json/);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it("skips shared MCP entries that already exist in user config", () => {
    ...
    const merged = buildMergedConfig(existing, "/tmp/omq", {
      sharedMcpServers: [
        {
          name: "gitnexus",
          command: "gitnexus",
