...
describe('config generator', () => {
  ...
  it('seeds qwen3.6-plus model and context defaults for fresh configs', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-config-gen-'));
    try {
      const configPath = join(wd, 'config.toml');
      await mergeConfig(configPath, wd);
  ...
  it('writes model_reasoning_effort and strengthened developer_instructions', async () => {
    ...
    try {
      ...
      assert.match(toml, /Qwen Code native subagents are available via \.qwen\/agents/);
  ...
  it('handles paths with spaces in notify array', async () => {
    const base = await mkdtemp(join(tmpdir(), 'omq config gen space-'));
  ...
  it('re-runs setup replacing OMQ config cleanly', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-config-gen-'));
    try {
      const configPath = join(wd, 'config.toml');
      await mergeConfig(configPath, wd);
      ...
      // OMQ block appears exactly once
      assert.equal(
        (rerun.match(/# oh-my-qwencode \(OMQ\) Configuration/g) ?? []).length,
        1
      );
      assert.equal((rerun.match(/# End oh-my-qwencode/g) ?? []).length, 1);
  ...
  it('does not seed 1M context keys for non-qwen3.6-plus models', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-config-gen-'));
    try {
      const configPath = join(wd, 'config.toml');
  ...
  it('writes a global [agents] section with OMQ defaults', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-config-gen-'));
    try {
      const configPath = join(wd, 'config.toml');
      await mergeConfig(configPath, wd);
  ...
  it('migrates a legacy OMQ block and preserves user settings', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-config-gen-'));
    try {
      const configPath = join(wd, 'config.toml');
      const legacy = [
      ...
      assert.equal(
        (toml.match(/oh-my-qwencode \(OMQ\) Configuration/g) ?? []).length,
        1
      );
  ...
  it('escapes Windows-style backslashes for MCP server args', async () => {
    ...
    try {
      const configPath = join(wd, 'config.toml');
      const windowsPkgRoot = 'C:\\Users\\alice\\oh-my-qwencode';
      await mergeConfig(configPath, windowsPkgRoot);
      const toml = await readFile(configPath, 'utf-8');

      assert.match(
        toml,
        /args = \["C:\\\\Users\\\\alice\\\\oh-my-qwencode\/dist\/mcp\/state-server\.js"\]/,
      );
      assert.match(
        toml,
        /args = \["C:\\\\Users\\\\alice\\\\oh-my-qwencode\/dist\/mcp\/memory-server\.js"\]/,
      );
      assert.match(
        toml,
        /args = \["C:\\\\Users\\\\alice\\\\oh-my-qwencode\/dist\/mcp\/code-intel-server\.js"\]/,
      );
      assert.match(
        toml,
        /args = \["C:\\\\Users\\\\alice\\\\oh-my-qwencode\/dist\/mcp\/trace-server\.js"\]/,
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
