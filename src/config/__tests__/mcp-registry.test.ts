...
describe("unified MCP registry loader", () => {
  it("prefers ~/.omq/mcp-registry.json over ~/.omc/mcp-registry.json", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-mcp-registry-"));
    try {
      const omqPath = join(wd, ".omq", "mcp-registry.json");
      ...
      assert.equal(result.sourcePath, omqPath);
  ...
  it("falls back to ~/.omc/mcp-registry.json when ~/.omq registry is absent", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-mcp-registry-"));
    try {
      const omcPath = join(wd, ".omc", "mcp-registry.json");
      await mkdir(join(wd, ".omc"), { recursive: true });
  ...
  it("returns canonical home-based registry candidates", () => {
    ...
    assert.deepEqual(candidates, [
      "/tmp/home/.omq/mcp-registry.json",
