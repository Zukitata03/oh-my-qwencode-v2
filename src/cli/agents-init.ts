...
export const AGENTS_INIT_USAGE = [
  "Usage: omq agents-init [path] [--dry-run] [--force] [--verbose]",
  "       omq deepinit [path] [--dry-run] [--force] [--verbose]",
  "",
...
const MANAGED_MARKER = "<!-- OMQ:AGENTS-INIT:MANAGED -->";
const MANUAL_START = "<!-- OMQ:AGENTS-INIT:MANUAL:START -->";
const MANUAL_END = "<!-- OMQ:AGENTS-INIT:MANUAL:END -->";
...
export async function agentsInit(
  ...
  console.log("oh-my-qwencode AGENTS bootstrap");
  console.log("===========================\n");
  ...
  for (let index = 0; index < plannedDirs.length; index += 1) {
    ...
    const decision = await syncManagedAgentsFile(
      ...
      backupRoot,
      rootOverlayRisk
        ? "active omq session detected for project root AGENTS.md"
        : undefined,
    );
  ...
  console.log(
    "- Preserves manual notes only for files already managed by omq agents-init.",
  );
  console.log(
    "- Never overwrites unmanaged AGENTS.md files unless you pass --force.",
  );
  console.log(
    "- Avoids rewriting project-root AGENTS.md while an active omq session is running.\n",
  );
