...
const HELP = `omq session - Search prior local session history

Usage:
  omq session search <query> [options]
...
Examples:
  omq session search "worker inbox path"
  omq session search all_workers_idle --since 7d --limit 5
  omq session search "team api" --project current --json
`;
