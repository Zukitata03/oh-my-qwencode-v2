...
const OMQ_SPARKSHELL_BIN_ENV = SPARKSHELL_BIN_ENV_SHARED;

export const SPARKSHELL_USAGE = [
  'Usage: omq sparkshell <command> [args...]',
  '   or: omq sparkshell --tmux-pane <pane-id> [--tail-lines <100-1000>]',
  'Runs the native omq-sparkshell sidecar with direct argv execution or explicit tmux pane summarization.',
...
export function sparkshellBinaryName(platform: NodeJS.Platform = process.platform): string {
  return platform === 'win32' ? 'omq-sparkshell.exe' : 'omq-sparkshell';
}
...
export function nestedRepoLocalSparkShellBinaryPath(
  ...
  return join(packageRoot, 'native', 'omq-sparkshell', 'target', 'release', sparkshellBinaryName(platform));
}
...
export function resolveSparkShellBinaryPath(options: ResolveSparkShellBinaryPathOptions = {}): string {
  ...
  const override = env[OMQ_SPARKSHELL_BIN_ENV]?.trim();
  if (override) {
  ...
  throw new Error(
    `[sparkshell] native binary not found. Checked ${packagedCandidates.join(', ')}, ${repoLocal}, and ${nestedRepoLocal}. `
      + `Set ${OMQ_SPARKSHELL_BIN_ENV} to override the path.`
  );
}
...
export async function resolveSparkShellBinaryPathWithHydration(
  ...
  for (const cached of resolveCachedNativeBinaryCandidatePaths('omq-sparkshell', version, platform, arch, env, {
  ...
  const hydrated = await hydrateNativeBinary('omq-sparkshell', { packageRoot, env, platform, arch });
  if (hydrated) return hydrated;

  throw new Error(
    `[sparkshell] native binary not found. Checked cached/native candidates under ${packageRoot}, ${repoLocal}, and ${nestedRepoLocal}. `
      + `Reconnect to the network so OMQ can fetch the release asset, or set ${OMQ_SPARKSHELL_BIN_ENV} to override the path.`
  );
}
...
export function runSparkShellBinary(
  ...
  const configEnvOverrides = readConfiguredEnvOverrides(env.QWEN_HOME);
...
export async function sparkshellCommand(args: string[]): Promise<void> {
  ...
  const hasExplicitOverride = typeof process.env[OMQ_SPARKSHELL_BIN_ENV] === 'string'
    && process.env[OMQ_SPARKSHELL_BIN_ENV]!.trim().length > 0;
