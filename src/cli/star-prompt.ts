/**
 * One-time GitHub star prompt shown at OMQ startup.
 * Skipped when no TTY or when gh CLI is not installed.
 * State stored globally (~/.omq/state/star-prompt.json) so it shows once per user.
 */
...
const REPO = 'Yeachan-Heo/oh-my-qwencode';
...
export function starPromptStatePath(): string {
  return join(homedir(), '.omq', 'state', 'star-prompt.json');
}
...
export async function markPrompted(): Promise<void> {
  const stateDir = join(homedir(), '.omq', 'state');
...
export async function maybePromptGithubStar(deps: MaybePromptGithubStarDeps = {}): Promise<void> {
  ...
  const approved = await askYesNoImpl('[omq] Enjoying oh-my-qwencode? Star it on GitHub? [Y/n] ');
  if (!approved) return;
  ...
  if (star.ok) {
    ...
    log('[omq] Thanks for the star!');
    return;
  }
  const warn = deps.warnFn ?? console.warn;
  warn(`[omq] Could not star repository automatically: ${star.error}`);
}
