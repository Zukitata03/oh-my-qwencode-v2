/**
 * omq doctor - Validate oh-my-qwencode installation
 */
...
import {
  qwenHome, qwenConfigPath, qwenPromptsDir,
  userSkillsDir, projectSkillsDir, omqStateDir, detectLegacySkillRootOverlap,
} from '../utils/paths.js';
...
import { OMQ_EXPLORE_CMD_ENV, isExploreCommandRoutingEnabled } from '../hooks/explore-routing.js';
...
function resolveDoctorPaths(cwd: string, scope: DoctorSetupScope): DoctorPaths {
  if (scope === 'project') {
    const qwenHomeDir = join(cwd, '.qwen');
    return {
      qwenHomeDir,
      configPath: join(qwenHomeDir, 'config.toml'),
      promptsDir: join(qwenHomeDir, 'prompts'),
      skillsDir: projectSkillsDir(cwd),
      stateDir: omqStateDir(cwd),
    };
  }
...
export async function doctor(options: DoctorOptions = {}): Promise<void> {
  ...
  const scopeSourceMessage = scopeResolution.source === 'persisted'
    ? ' (from .omq/setup-scope.json)'
    : '';

  console.log('oh-my-qwencode doctor');
  console.log('==================\n');
  console.log(`Resolved setup scope: ${scopeResolution.scope}${scopeSourceMessage}\n`);

  const checks: Check[] = [];

  // Check 1: Qwen Code installed
  checks.push(checkQwenCli());
  ...
  // Check 3: Qwen Code home directory
  checks.push(checkDirectory('Qwen Code home', paths.qwenHomeDir));
  ...
  checks.push(checkAgentsMd(scopeResolution.scope, paths.qwenHomeDir));
  ...
  if (failCount > 0) {
    console.log('\nRun "omq setup" to fix installation issues.');
  } else if (warnCount > 0) {
    console.log('\nRun "omq setup --force" to refresh all components.');
  } else {
    console.log('\nAll checks passed! oh-my-qwencode is ready.');
  }
}
...
async function doctorTeam(): Promise<void> {
  console.log('oh-my-qwencode doctor --team');
  console.log('=========================\n');
...
async function collectTeamDoctorIssues(cwd: string): Promise<TeamDoctorIssue[]> {
  ...
  for (const teamName of teamDirs) {
    ...
    let tmuxSession = `omq-team-${teamName}`;
  ...
  if ((existsSync(hudStatePath) || existsSync(leaderActivityPath)) && teamDirs.length > 0) {
    try {
      ...
      if (leaderIsStale && !tmuxUnavailable) {
        ...
        for (const teamName of teamDirs) {
          const session = knownTeamSessions.has(`omq-team-${teamName}`)
            ? `omq-team-${teamName}`
...
function listTeamTmuxSessions(): Set<string> | null {
  ...
  const sessions = (res.stdout || '')
    .split('\n')
    .map((s) => s.trim())
    .filter((s) => s.startsWith('omq-team-'));
  return new Set(sessions);
}

function checkQwenCli(): Check {
  ...
  if (result.error) {
    ...
    if (kind === 'missing') {
      return { name: 'Qwen Code', status: 'fail', message: 'not found - install from https://github.com/openai/qwen' };
    }
    if (kind === 'blocked') {
      return {
        name: 'Qwen Code',
  ...
  if (result.status === 0) {
    ...
    return { name: 'Qwen Code', status: 'pass', message: `installed (${version})` };
  }
...
function checkExploreHarness(): Check {
  ...
  const manifestPath = join(packageRoot, 'crates', 'omq-explore', 'Cargo.toml');
  if (!existsSync(manifestPath)) {
    return {
      name: 'Explore Harness',
      status: 'warn',
      message: 'Rust harness sources not found in this install (omq explore unavailable until packaged or OMQ_EXPLORE_BIN is set)',
    };
  }
  ...
  if (override) {
    ...
    return {
      name: 'Explore Harness',
      status: 'warn',
      message: `OMQ_EXPLORE_BIN is set but path was not found (${override})`,
    };
  }
  ...
  if (result.error) {
    ...
    if (kind === 'missing') {
      return {
        name: 'Explore Harness',
        status: 'warn',
        message: `Rust harness sources are packaged, but no compatible packaged prebuilt or cargo was found (install Rust or set ${EXPLORE_BIN_ENV} for omq explore)`,
      };
    }
    return {
      name: 'Explore Harness',
...
async function checkConfig(configPath: string): Promise<Check> {
  ...
  try {
    ...
    const hasOmq = content.includes('omq_') || content.includes('oh-my-qwencode');
    if (hasOmq) {
      return { name: 'Config', status: 'pass', message: 'config.toml has OMQ entries' };
    }

    return {
      name: 'Config',
      status: 'warn',
      message: 'config.toml exists but no OMQ entries yet (expected before first setup; run "omq setup --force" once)',
    };
  } catch {
...
async function checkExploreRouting(configPath: string): Promise<Check> {
  const envValue = process.env[OMQ_EXPLORE_CMD_ENV];
  if (typeof envValue === 'string' && !isExploreCommandRoutingEnabled(process.env)) {
    return {
      name: 'Explore routing',
      status: 'warn',
      message:
        'disabled by environment override; enable with USE_OMQ_EXPLORE_CMD=1 (or remove the explicit opt-out)',
    };
  }
  ...
  try {
    ...
    const configuredValue = parsed?.env?.USE_OMQ_EXPLORE_CMD;

    if (
      typeof configuredValue === 'string' &&
      !isExploreCommandRoutingEnabled({
        USE_OMQ_EXPLORE_CMD: configuredValue,
      })
    ) {
      return {
        name: 'Explore routing',
        status: 'warn',
        message:
          'disabled in config.toml [env]; set USE_OMQ_EXPLORE_CMD = "1" to restore default explore-first routing',
      };
    }
...
async function checkLegacySkillRootOverlap(): Promise<Check> {
  ...
  if (overlap.overlappingSkillNames.length === 0) {
    return {
      ...
      status: 'warn',
      message:
        `legacy ~/.agents/skills still exists (${overlap.legacySkillCount} skills) alongside canonical ${overlap.canonicalDir}; remove or archive it if Qwen Code shows duplicate entries`,
    };
  }
  ...
  return {
    ...
    status: 'warn',
    message:
      `${overlap.overlappingSkillNames.length} overlapping skill names between ${overlap.canonicalDir} and ${overlap.legacyDir}${mismatchMessage}; Qwen Code Enable/Disable Skills may show duplicates until ~/.agents/skills is cleaned up`,
  };
}
...
function checkAgentsMd(scope: DoctorSetupScope, qwenHomeDir: string): Check {
  if (scope === 'user') {
    const userAgentsMd = join(qwenHomeDir, 'AGENTS.md');
    ...
    return {
      name: 'AGENTS.md',
      status: 'warn',
      message: `not found in ${userAgentsMd} (run omq setup --scope user)`,
    };
  }
  ...
  return {
    name: 'AGENTS.md',
    status: 'warn',
    message: 'not found in project root (run omq agents-init . or omq setup --scope project)',
  };
}
...
async function checkMcpServers(configPath: string): Promise<Check> {
  ...
  try {
    ...
    if (mcpCount > 0) {
      const hasOmq = content.includes('omq_state') || content.includes('omq_memory');
      if (hasOmq) {
        return { name: 'MCP Servers', status: 'pass', message: `${mcpCount} servers configured (OMQ present)` };
      }
      return {
        name: 'MCP Servers',
        status: 'warn',
        message: `${mcpCount} servers but no OMQ servers yet (expected before first setup; run "omq setup --force" once)`,
      };
    }
