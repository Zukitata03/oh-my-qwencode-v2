/**
 * oh-my-qwencode CLI
 * Multi-agent orchestration for Qwen Code
 */
...
import { qwenConfigPath } from "../utils/paths.js";
...
const HELP = `
oh-my-qwencode (omq) - Multi-agent orchestration for Qwen Code

Usage:
  omq           Launch Qwen Code (HUD auto-attaches only when already inside tmux)
  omq exec      Run qwen exec non-interactively with OMQ AGENTS/overlay injection
...
Options:
  ...
  --high        Launch Qwen Code with high reasoning effort
                (Note: Qwen Code 0.14.0+ requires configuring model_reasoning_effort in ~/.qwen/settings.json)
  --xhigh       Launch Qwen Code with xhigh reasoning effort
                (Note: Qwen Code 0.14.0+ requires configuring model_reasoning_effort in ~/.qwen/settings.json)
  --madmax      DANGEROUS: bypass Qwen Code approvals and sandbox
                (alias for --approval-mode yolo)
  ...
  --purge       Remove .omq/ cache directory during uninstall
  --verbose     Show detailed output
  --scope       Setup scope for "omq setup" only:
                user | project
  --skill-target
                User-scope skills target for "omq setup" only:
                qwen-home
`;

const REASONING_KEY = "model_reasoning_effort";
const MODEL_INSTRUCTIONS_FILE_KEY = "model_instructions_file";
const TEAM_WORKER_LAUNCH_ARGS_ENV = "OMQ_TEAM_WORKER_LAUNCH_ARGS";
const TEAM_INHERIT_LEADER_FLAGS_ENV = "OMQ_TEAM_INHERIT_LEADER_FLAGS";
...
const OMQ_RALPH_APPEND_INSTRUCTIONS_FILE_ENV =
  "OMQ_RALPH_APPEND_INSTRUCTIONS_FILE";
const OMQ_AUTORESEARCH_APPEND_INSTRUCTIONS_FILE_ENV =
  "OMQ_AUTORESEARCH_APPEND_INSTRUCTIONS_FILE";
const REASONING_MODES = ["low", "medium", "high", "xhigh"] as const;
type ReasoningMode = (typeof REASONING_MODES)[number];
const REASONING_MODE_SET = new Set<string>(REASONING_MODES);
const REASONING_USAGE = "Usage: omq reasoning <low|medium|high|xhigh>";
...
const QWEN_VERSION_FLAGS = new Set(["--version", "-V"]);
...
export function readPersistedSetupPreferences(
  ...
  const scopePath = join(cwd, ".omq", "setup-scope.json");
...
export function resolveQwenHomeForLaunch(
  cwd: string,
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  if (env.QWEN_HOME && env.QWEN_HOME.trim() !== "") return env.QWEN_HOME;
  const persistedScope = readPersistedSetupScope(cwd);
  if (persistedScope === "project") {
    return join(cwd, ".qwen");
  }
  return undefined;
}
...
export interface QwenExecFailureClassification {
...
export function classifyQwenExecFailure(
  error: unknown,
): QwenExecFailureClassification {
...
function runQwenBlocking(
  ...
  const { result } = spawnPlatformCommandSync("qwen", launchArgs, {
    cwd,
    stdio: "inherit",
    env: qwenEnv,
    encoding: "utf-8",
  ...
  if (result.error) {
    ...
    if (kind === "missing") {
      console.error(
        "[omq] failed to launch qwen: executable not found in PATH",
      );
    } else if (kind === "blocked") {
      console.error(
        `[omq] failed to launch qwen: executable is present but blocked in the current environment (${errno.code || "blocked"})`,
      );
    } else {
      console.error(`[omq] failed to launch qwen: ${errno.message}`);
    }
    throw result.error;
  }
  ...
  if (result.status !== 0) {
    ...
    if (result.signal) {
      console.error(`[omq] qwen exited due to signal ${result.signal}`);
    }
  }
}
...
async function reasoningCommand(args: string[]): Promise<void> {
  const mode = args[0];
  const configPath = qwenConfigPath();
...
export async function launchWithHud(args: string[]): Promise<void> {
  if (isNativeWindows()) {
    ...
    if (result.error) {
      ...
      if (kind === "missing") {
        console.warn(
          "[omq] warning: tmux was not found on native Windows. Continuing without tmux/HUD.\n" +
            "[omq] To enable tmux-backed features, install psmux:\n" +
            "[omq]   winget install psmux\n" +
            "[omq] See: https://github.com/marlocarlo/psmux",
        );
      } else {
        console.warn(
          `[omq] warning: tmux probe failed on native Windows (${errno.code || errno.message}). Continuing without tmux/HUD.`,
        );
      }
    } else if (result.status !== 0 && !isTmuxAvailable()) {
      const stderr = (result.stderr || "").trim();
      console.warn(
        `[omq] warning: tmux reported an error on native Windows${stderr ? ` (${stderr})` : ""}. Continuing without tmux/HUD.`,
      );
    }
  }
  ...
  const qwenHomeOverride = resolveQwenHomeForLaunch(launchCwd, process.env);
  const launchPolicy = resolveQwenLaunchPolicy(
    process.env,
    process.platform,
    undefined,
    isNativeWindows(),
  ...
  const normalizedArgs = normalizeQwenLaunchArgs(
    notifyTempResult.passthroughArgs,
  );
  let cwd = launchCwd;
  ...
  const sessionId = `omq-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  ...
  // have written a config.toml with duplicate [tui] sections.  Qwen Code's
  ...
  try {
    ...
    // preLaunch errors must NOT prevent Qwen Code from starting
    console.error(
...
export async function execWithOverlay(args: string[]): Promise<void> {
  ...
  try {
    ...
    const qwenEnvBase = qwenHomeOverride
      ? { ...process.env, QWEN_HOME: qwenHomeOverride }
      : process.env;
    const qwenEnv = notifyTempContractRaw
      ? {
          ...qwenEnvBase,
          [OMQ_NOTIFY_TEMP_CONTRACT_ENV]: notifyTempContractRaw,
        }
      : qwenEnvBase;
...
export function normalizeQwenLaunchArgs(args: string[]): string[] {
...
function shouldBypassDefaultSystemPrompt(env: NodeJS.ProcessEnv): boolean {
  return env[OMQ_BYPASS_DEFAULT_SYSTEM_PROMPT_ENV] !== "0";
}
...
export function collectInheritableTeamWorkerArgs(
  ...
  return collectInheritableTeamWorkerArgsShared(qwenArgs);
}
...
export function resolveTeamWorkerLaunchArgsEnv(
  ...
  const inheritedArgs = inheritLeaderFlags
    ? collectInheritableTeamWorkerArgs(qwenArgs)
    : [];
...
export function buildTmuxSessionName(cwd: string, sessionId: string): string {
  ...
  const repoDir = parentDir.endsWith(".omq-worktrees")
    ? parentDir.slice(0, -".omq-worktrees".length)
    : parentDir === "worktrees" && grandparentDir === ".omq"
  ...
  const sessionToken = sanitizeTmuxToken(sessionId.replace(/^omq-/, ""));
  const name = `omq-${dirToken}-${branchToken}-${sessionToken}`;
...
function buildDetachedSessionLeaderCommand(
  ...
  const wrapped = [`trap '${cleanupTrap}' 0 INT TERM HUP;`, qwenCmd].join(" ");
...
export function buildDetachedSessionBootstrapSteps(
  ...
  const detachedLeaderCmd = nativeWindows
    ? "powershell.exe"
    : buildDetachedSessionLeaderCommand(sessionName, qwenCmd);
  const newSessionArgs: string[] = [
    ...
    ...(workerLaunchArgs
      ...
      : []),
    ...(qwenHomeOverride ? ["-e", `QWEN_HOME=${qwenHomeOverride}`] : []),
    ...(notifyTempContractRaw
      ? ["-e", `${OMQ_NOTIFY_TEMP_CONTRACT_ENV}=${notifyTempContractRaw}`]
      : []),
    detachedLeaderCmd,
  ];
...
async function readLaunchAppendInstructions(): Promise<string> {
  const appendixCandidates = [
    process.env[OMQ_RALPH_APPEND_INSTRUCTIONS_FILE_ENV]?.trim(),
    process.env[OMQ_AUTORESEARCH_APPEND_INSTRUCTIONS_FILE_ENV]?.trim(),
  ].filter(
...
export function buildNotifyFallbackWatcherEnv(
  ...
  return {
    ...nextEnv,
    ...(options.qwenHomeOverride ? { QWEN_HOME: options.qwenHomeOverride } : {}),
    OMQ_HUD_AUTHORITY: options.enableAuthority ? "1" : "0",
  };
}
...
${launchAppendix}`
  ...
  try {
    if (notifyTempContract?.active) {
      process.env[OMQ_NOTIFY_TEMP_CONTRACT_ENV] =
      ...
      delete process.env[OMQ_NOTIFY_TEMP_CONTRACT_ENV];
    }
...
/**
 * runQwen: Launch Qwen Code (blocks until exit).
...
function runQwen(
  ...
  const qwenEnvWithNotify = notifyTempContractRaw
    ? { ...qwenEnv, [OMQ_NOTIFY_TEMP_CONTRACT_ENV]: notifyTempContractRaw }
    : qwenEnv;
  ...
  if (isQwenVersionRequest(launchArgs)) {
    runQwenBlocking(cwd, launchArgs, qwenEnvWithNotify);
    return;
  }

  if (launchPolicy === "inside-tmux") {
    // Already in tmux: launch qwen in current pane, HUD in bottom split
    ...
    // Opt-out: set OMQ_MOUSE=0. (closes #128)
    if (process.env.OMQ_MOUSE !== "0") {
      try {
    ...
    // Not in tmux: create a new tmux session with qwen + HUD pane
    const qwenCmd = buildTmuxPaneCommand("qwen", launchArgs);
    const detachedWindowsQwenCmd = nativeWindows
      ? buildWindowsPromptCommand("qwen", launchArgs)
      : null;
    ...
    try {
      ...
      // tmux not available or failed, just run qwen directly
      runQwenBlocking(cwd, launchArgs, qwenEnvWithNotify);
    }
  }
}
...
function isQwenVersionRequest(args: string[]): boolean {
  return args.some((arg) => QWEN_VERSION_FLAGS.has(arg));
}
...
async function postLaunch(
  ...
  // 0. Flush fallback watcher once to reduce race with fast qwen exit.
  try {
  ...
  try {
    ...
    const errorSummary =
      normalizedEvent === "failed"
        ? `qwen exited with code ${process.exitCode}`
        : undefined;
...
function notifyFallbackPidPath(cwd: string): string {
  return join(cwd, ".omq", "state", "notify-fallback.pid");
}

function hookDerivedWatcherPidPath(cwd: string): string {
  return join(cwd, ".omq", "state", "hook-derived-watcher.pid");
}
...
async function startNotifyFallbackWatcher(
  ...
  if (process.env.OMQ_NOTIFY_FALLBACK === "0") return;
  ...
  if (existsSync(pidPath)) {
    try {
      ...
      if (!hasErrnoCode(error, "ESRCH")) {
        console.warn(
          "[omq] warning: failed to stop stale notify fallback watcher",
          {
            path: pidPath,
  ...
  await mkdir(join(cwd, ".omq", "state"), { recursive: true }).catch(
    (error: unknown) => {
      console.warn(
  ...
  const child = spawn(
    ...
    [
      ...
      "--parent-pid",
      String(process.pid),
      ...(process.env.OMQ_NOTIFY_FALLBACK_MAX_LIFETIME_MS
        ? ["--max-lifetime-ms", process.env.OMQ_NOTIFY_FALLBACK_MAX_LIFETIME_MS]
        : []),
    ],
    {
      cwd,
...
async function startHookDerivedWatcher(cwd: string): Promise<void> {
  if (process.env.OMQ_HOOK_DERIVED_SIGNALS !== "1") return;
  ...
  if (existsSync(pidPath)) {
    try {
      ...
      console.warn("[omq] warning: failed to stop stale hook-derived watcher", {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      });
    }
  ...
  await mkdir(join(cwd, ".omq", "state"), { recursive: true }).catch(
    (error: unknown) => {
      console.warn(
        "[omq] warning: failed to create hook-derived watcher state directory",
        {
          cwd,
          error: error instanceof Error ? error.message : String(error),
        },
  ...
  await writeFile(
    ...
    console.warn(
      "[omq] warning: failed to write hook-derived watcher pid file",
      {
        path: pidPath,
        error: error instanceof Error ? error.message : String(error),
      },
