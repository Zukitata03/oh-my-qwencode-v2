...
import { qwenAgentsDir, projectQwenAgentsDir } from '../utils/paths.js';

export const RESERVED_NATIVE_AGENT_NAMES = new Set(['default', 'worker', 'explorer']);
const DEFAULT_AGENT_MODEL = 'qwen3.6-plus';
const AGENTS_USAGE = [
  'Usage:',
  '  omq agents list [--scope user|project]',
  '  omq agents add <name> [--scope user|project] [--force]',
  '  omq agents edit <name> [--scope user|project]',
  '  omq agents remove <name> [--scope user|project] [--force]',
  '',
  'Manage Qwen Code native agent TOML files under ~/.qwen/agents/ or ./.qwen/agents/.',
  '',
  'Notes:',
...
function normalizeAgentName(name: string): string {
  ...
  if (isReservedNativeAgentName(trimmed)) {
    throw new Error(`"${trimmed}" is reserved by Qwen Code built-in agents`);
  }
  return trimmed;
}

function resolveAgentsDir(scope: AgentScope, cwd = process.cwd()): string {
  return scope === 'project' ? projectQwenAgentsDir(cwd) : qwenAgentsDir();
}
...
function inferMutationScope(cwd = process.cwd()): AgentScope {
  const persistedScopePath = join(cwd, '.omq', 'setup-scope.json');
  ...
  return existsSync(join(cwd, '.qwen')) ? 'project' : 'user';
}
...
function scaffoldAgentToml(name: string): string {
  ...
  return [
    `# Qwen Code native agent: ${normalized}`,
    `name = "${normalized}"`,
...
export async function agentsCommand(args: string[]): Promise<void> {
  ...
  switch (subcommand) {
    ...
    case 'add': {
      const name = args[1];
      assert.ok(name, 'Usage: omq agents add <name>');
    ...
    case 'edit': {
      const name = args[1];
      assert.ok(name, 'Usage: omq agents edit <name>');
    ...
    case 'remove': {
      const name = args[1];
      assert.ok(name, 'Usage: omq agents remove <name>');
