...
export const EXPLORE_USAGE = [
  'Usage: omq explore --prompt "<prompt>"',
  '   or: omq explore --prompt-file <file>',
].join('\n');

const PROMPT_FLAG = '--prompt';
const PROMPT_FILE_FLAG = '--prompt-file';
export const EXPLORE_BIN_ENV = EXPLORE_BIN_ENV_SHARED;
const EXPLORE_SPARK_MODEL_ENV = 'OMQ_EXPLORE_SPARK_MODEL';
...
export function packagedExploreHarnessBinaryName(platform: NodeJS.Platform = process.platform): string {
  return platform === 'win32' ? 'omq-explore-harness.exe' : 'omq-explore-harness';
}
...
export function resolvePackagedExploreHarnessCommand(
  ...
  const metadataPath = join(packageRoot, 'bin', 'omq-explore-harness.meta.json');
...
export async function resolveExploreHarnessCommandWithHydration(
  ...
  for (const cached of resolveCachedNativeBinaryCandidatePaths('omq-explore-harness', version, process.platform, process.arch, env)) {
  ...
  if (!isRepositoryCheckout(packageRoot)) {
    const hydrated = await hydrateNativeBinary('omq-explore-harness', { packageRoot, env });
    if (hydrated) return { command: hydrated, args: [] };
    throw new Error('[explore] no compatible native harness is available for this install. Reconnect to the network so OMQ can fetch the release asset, or set OMQ_EXPLORE_BIN to a prebuilt harness binary.');
  }
...
export async function exploreCommand(args: string[]): Promise<void> {
  ...
  if (sparkShellRoute) {
    try {
      ...
      process.stderr.write(`[omq explore] sparkshell backend unavailable (${message}). Falling back to the explore harness.\n`);
    }
  }
  ...
  if (result.error) {
    ...
    if (harness.command === 'cargo' && errno.code === 'ENOENT') {
      throw new Error('[explore] cargo was not found. Install a Rust toolchain, use a compatible packaged omq-explore prebuilt, or set OMQ_EXPLORE_BIN to a prebuilt harness binary.');
    }
