...
export const ASK_USAGE = [
  'Usage: omq ask <claude|gemini> <question or task>',
  '   or: omq ask <claude|gemini> -p "<prompt>"',
  '   or: omq ask claude --print "<prompt>"',
  '   or: omq ask gemini --prompt "<prompt>"',
  '   or: omq ask <claude|gemini> --agent-prompt <role> "<prompt>"',
  '   or: omq ask <claude|gemini> --agent-prompt=<role> --prompt "<prompt>"',
].join('\n');
...
const ASK_ADVISOR_SCRIPT_ENV = 'OMQ_ASK_ADVISOR_SCRIPT';
...
function resolveAskPromptsDir(cwd: string, env: NodeJS.ProcessEnv = process.env): string {
  const qwenHomeOverride = env.QWEN_HOME?.trim();
  if (qwenHomeOverride) {
    return join(qwenHomeOverride, 'prompts');
  }
...
async function resolveAgentPromptContent(
  ...
  if (!existsSync(promptsDir)) {
    throw new Error(`[ask] prompts directory not found: ${promptsDir}. Run "omq setup" to install prompts.`);
  }
