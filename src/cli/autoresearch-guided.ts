...
export function buildAutoresearchDeepInterviewPrompt(
  ...
  return [
    ...
    'Run the deep-interview skill in autoresearch mode for `omq autoresearch`.',
    'Guide the user through research topic definition, evaluator readiness, keep policy, and slug/session naming.',
    'Do not launch tmux or run `omq autoresearch` yourself.',
    'When the user confirms launch and the evaluator is concrete, write/update these canonical artifacts under `.omq/specs/`:',
...
export function spawnAutoresearchTmux(missionDir: string, slug: string): void {
  ...
  const cmd = `${shellQuote(process.execPath)} ${shellQuote(omqPath)} autoresearch ${shellQuote(missionDir)}`;
