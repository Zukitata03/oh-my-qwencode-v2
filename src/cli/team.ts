...
function readPersistedTeamFollowupState(cwd: string): {
  ...
  const path = join(cwd, '.omq', 'state', 'team-state.json');
...
const TEAM_HELP = `
Usage: omq team [N:agent-type] "<task description>"
       omq team status <team-name> [--json] [--tail-lines <100-1000>]
       omq team await <team-name> [--timeout-ms <ms>] [--after-event-id <id>] [--json]
...
Notes:
  ...
  --worktree is deprecated for omq team and is now only a backward-compatible no-op override.
  use native Qwen Code subagents for small in-session fanout; use omq team for durable tmux/state/worktree coordination.

Examples:
  omq team 3:executor "fix failing tests"
  omq team status my-team
  omq team status my-team --json
  omq team status my-team --tail-lines 600
...
const TEAM_API_OPERATION_NOTES: Partial<Record<TeamApiOperation, string>> = {
  ...
  'read-events': 'Events are returned in canonical form; worker_idle log entries normalize to type worker_state_changed with source_type worker_idle. wakeable_only defaults to false; set wakeable_only=true to mirror omq team await semantics (wakeable events now include merge conflicts and per-signal stale alerts).',
  'await-event': 'Waits for the next matching event and returns status=timeout when no matching event arrives before timeout_ms. wakeable_only defaults to false; set wakeable_only=true to mirror omq team await semantics (wakeable events now include merge conflicts and per-signal stale alerts).',
  ...
  switch (field) {
    ...
    case 'metadata':
      return {
        ...
        diff_path: '/tmp/team/worktrees/worker-1/.omq/diff.md',
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (token === '--tail-lines') {
      ...
      if (!Number.isFinite(parsed) || parsed < MIN_SPARKSHELL_TAIL_LINES || parsed > MAX_SPARKSHELL_TAIL_LINES) {
        throw new Error(`Usage: omq team status <team-name> [--json] [--tail-lines <${MIN_SPARKSHELL_TAIL_LINES}-${MAX_SPARKSHELL_TAIL_LINES}>]`);
      }
      return parsed;
    }
...
async function readTeamPaneStatus(
  ...
  const sparkshellCommands = Object.fromEntries(
    [
      leaderPaneId ? ['leader', `omq sparkshell --tmux-pane ${leaderPaneId} --tail-lines ${tailLines}`] : null,
      hudPaneId ? ['hud', `omq sparkshell --tmux-pane ${hudPaneId} --tail-lines ${tailLines}`] : null,
      ...Object.entries(workerPanes).map(([workerName, paneId]) => [
        workerName,
        `omq sparkshell --tmux-pane ${paneId} --tail-lines ${tailLines}`,
      ] as const),
    ].filter((entry): entry is [string, string] => entry !== null),
  );
  ...
  const recommendedInspectTaskClaimLockPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => {
      const taskId = recommendedInspectTasks[target];
      return [target, taskId && snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'claims', `task-${taskId}.lock`) : null];
    }),
  );
  ...
  const recommendedInspectTaskPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => {
      const taskId = recommendedInspectTasks[target];
      return [target, taskId && snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'tasks', `task-${taskId}.json`) : null];
    }),
  );
  const recommendedInspectApprovalPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => {
      const taskId = recommendedInspectTasks[target];
      return [target, taskId && snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'approvals', `task-${taskId}.json`) : null];
    }),
  );
  const recommendedInspectWorkerStateDirs = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'workers', target) : null,
    ]),
  );
  const recommendedInspectWorkerStatusPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'workers', target, 'status.json') : null,
    ]),
  );
  const recommendedInspectWorkerHeartbeatPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'workers', target, 'heartbeat.json') : null,
    ]),
  );
  const recommendedInspectWorkerIdentityPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'workers', target, 'identity.json') : null,
    ]),
  );
  const recommendedInspectWorkerInboxPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'workers', target, 'inbox.md') : null,
    ]),
  );
  const recommendedInspectWorkerMailboxPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'mailbox', `${target}.json`) : null,
    ]),
  );
  const recommendedInspectWorkerShutdownRequestPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'workers', target, 'shutdown-request.json') : null,
    ]),
  );
  const recommendedInspectWorkerShutdownAckPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'workers', target, 'shutdown-ack.json') : null,
    ]),
  );
  const recommendedInspectTeamConfigPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'config.json') : null,
    ]),
  );
  const recommendedInspectTeamManifestPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'manifest.v2.json') : null,
    ]),
  );
  const recommendedInspectTeamEventsPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'events', 'events.ndjson') : null,
    ]),
  );
  const recommendedInspectTeamDispatchPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'dispatch', 'requests.json') : null,
    ]),
  );
  const recommendedInspectTeamDirPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName) : null,
    ]),
  );
  const recommendedInspectTeamPhasePaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'phase.json') : null,
    ]),
  );
  const recommendedInspectTeamMonitorSnapshotPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'monitor-snapshot.json') : null,
    ]),
  );
  const recommendedInspectTeamSummarySnapshotPaths = Object.fromEntries(
    recommendedInspectTargets.map((target) => [
      target,
      snapshot?.teamName ? join(cwd, '.omq', 'state', 'team', snapshot.teamName, 'summary-snapshot.json') : null,
    ]),
  );
  ...
  return {
    ...
    sparkshell_hint: Object.keys(workerPanes).length > 0
      ? 'omq sparkshell --tmux-pane <pane-id> --tail-lines 400'
      : null,
...
function renderTeamPaneStatus(
  ...
  if (paneStatus.sparkshell_hint) {
    console.log('sparkshell_hint: omq sparkshell --tmux-pane <pane-id> --tail-lines 400');
  }
...
function parseTeamArgs(args: string[], cwd: string = process.cwd()): ParsedTeamArgs {
  ...
  if (tokens[0]?.toLowerCase() === 'ralph') {
    throw new Error('Deprecated usage: `omq team ralph ...` has been removed. Use `omq team ...` or run `omq ralph ...` separately.');
  }
...
export function buildLeaderMonitoringHints(teamName: string): string[] {
  ...
  return [
    `leader_check: omq team status ${sanitized}`,
    `leader_loop_hint: while ON, keep checking state (example: sleep 30 && omq team status ${sanitized})`,
  ];
}
...
export async function teamCommand(args: string[], _options: TeamCliOptions = {}): Promise<void> {
  ...
  if (subcommand === 'status') {
    ...
    const wantsJson = teamArgs.includes('--json');
    if (!name) throw new Error('Usage: omq team status <team-name> [--json]');
  ...
  if (subcommand === 'await') {
    const name = teamArgs[1];
    if (!name) throw new Error('Usage: omq team await <team-name> [--timeout-ms <ms>] [--after-event-id <id>] [--json]');
    const wantsJson = teamArgs.includes('--json');
  ...
  if (subcommand === 'resume') {
    const name = teamArgs[1];
    if (!name) throw new Error('Usage: omq team resume <team-name>');
  ...
  if (subcommand === 'shutdown') {
    ...
    await updateModeState('team', {
      ...
      console.warn('[omq] warning: failed to persist team mode shutdown state', {
        team: name,
