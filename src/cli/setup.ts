/**
 * omq setup - Automated installation of oh-my-qwencode
...
const PROJECT_OMQ_GITIGNORE_ENTRY = ".omq/";
...
function applyScopePathRewritesToAgentsTemplate(
  ...
  return content.replaceAll("~/.qwen", "./.qwen");
}
...
function getBackupContext(
  ...
  if (scope === "project") {
    return {
      backupRoot: join(projectRoot, ".omq", "backups", "setup", timestamp),
      baseRoot: projectRoot,
    };
  }
  return {
    backupRoot: join(homedir(), ".omq", "backups", "setup", timestamp),
    baseRoot: homedir(),
  };
}
...
async function buildLegacySkillOverlapNotice(
  ...
  if (overlap.overlappingSkillNames.length === 0) {
    return {
      shouldWarn: true,
      message:
        `Legacy ~/.agents/skills still exists (${overlap.legacySkillCount} skills) alongside canonical ${overlap.canonicalDir}. Qwen Code may still discover both roots; archive or remove ~/.agents/skills if Enable/Disable Skills shows duplicates.`,
    };
  }
  ...
  return {
    shouldWarn: true,
    message:
      `Detected ${overlap.overlappingSkillNames.length} overlapping skill names between canonical ${overlap.canonicalDir} and legacy ${overlap.legacyDir}.${mismatchSuffix} Remove or archive ~/.agents/skills after confirming ${overlap.canonicalDir} is the version you want Qwen Code to load.`,
  };
}
...
function getScopeFilePath(projectRoot: string): string {
  return join(projectRoot, ".omq", "setup-scope.json");
}
...
export function resolveScopeDirectories(
  ...
  if (scope === "project") {
    const qwenHomeDir = join(projectRoot, ".qwen");
    console.log(`qwenHomeDir is ${qwenHomeDir}`)
    return {
      qwenConfigFile: join(qwenHomeDir, "config.toml"),
      qwenHomeDir,
      nativeAgentsDir: join(qwenHomeDir, "agents"),
      promptsDir: join(qwenHomeDir, "prompts"),
      skillsDir: join(qwenHomeDir, "skills"),
    };
  }
  return {
    qwenConfigFile: qwenConfigPath(),
    qwenHomeDir: qwenHome(),
    nativeAgentsDir: qwenAgentsDir(),
    promptsDir: qwenPromptsDir(),
...
async function readPersistedSetupPreferences(
  ...
  try {
    ...
    if (parsed && typeof parsed.scope === "string") {
      ...
      if (migrated) {
        console.warn(
          `[omq] Migrating persisted setup scope "${parsed.scope}" → "${migrated}" ` +
...
async function promptForSetupScope(
  ...
  try {
    ...
    console.log(
      `  1) user (default) — installs to ~/.qwen (skills default to ~/.qwen/skills)`,
    );
    console.log("  2) project — installs to ./.qwen (local to project)");
...
function probeInstalledQwenVersion(): string | null {
  const { result } = spawnPlatformCommandSync("qwen", ["--version"], {
    encoding: "utf-8",
...
function shouldOmqManageTuiFromQwenVersion(versionOutput: string | null): boolean {
  if (!versionOutput) return true;
  const parsed = parseSemverTriplet(versionOutput);
  if (!parsed) return true;
  return !semverGte(parsed, TUI_OWNED_BY_QWEN_VERSION);
}
...
async function ensureProjectOmqGitignore(
  projectRoot: string,
  backupContext: SetupBackupContext,
  options: Pick<SetupOptions, "dryRun" | "verbose">,
  ...
  if (hasGitignoreEntry(existing, PROJECT_OMQ_GITIGNORE_ENTRY)) {
    return "unchanged";
  }

  const nextContent = destinationExists
    ? `${existing}${existing.endsWith("\n") || existing.length === 0 ? "" : "\n"}${PROJECT_OMQ_GITIGNORE_ENTRY}\n`
    : `${PROJECT_OMQ_GITIGNORE_ENTRY}\n`;
  ...
  if (options.verbose) {
    console.log(
      `  ${options.dryRun ? "would update" : destinationExists ? "updated" : "created"} .gitignore (${PROJECT_OMQ_GITIGNORE_ENTRY})`,
    );
  }
...
export async function setup(options: SetupOptions = {}): Promise<void> {
  ...
  const scopeSourceMessage =
    resolvedScope.source === "persisted" ? " (from .omq/setup-scope.json)" : "";
  const backupContext = getBackupContext(resolvedScope.scope, projectRoot);

  console.log("oh-my-qwencode setup");
  console.log("=================\n");
  console.log(
  ...
  const dirs = [
    scopeDirs.qwenHomeDir,
    scopeDirs.promptsDir,
    scopeDirs.skillsDir,
    scopeDirs.nativeAgentsDir,
    omqStateDir(projectRoot),
    omqPlansDir(projectRoot),
    omqLogsDir(projectRoot),
  ];
  ...
  if (resolvedScope.scope === "project") {
    const gitignoreResult = await ensureProjectOmqGitignore(
      projectRoot,
      backupContext,
      { dryRun, verbose },
    );
    if (gitignoreResult === "created") {
      console.log(
        "  Created .gitignore with .omq/ so local OMQ runtime state stays out of source control.\n",
      );
    } else if (gitignoreResult === "updated") {
      console.log(
        "  Added .omq/ to .gitignore so local OMQ runtime state stays out of source control.\n",
      );
    }
  }
  ...
  const managedConfig = await updateManagedConfig(
    scopeDirs.qwenConfigFile,
    pkgRoot,
    sharedMcpRegistry,
    summary.config,
    backupContext,
    { qwenVersionProbe: options.qwenVersionProbe, dryRun, verbose, modelUpgradePrompt },
  );
  ...
  console.log(`  Config refresh complete (${scopeDirs.qwenConfigFile}).\n`);
  ...
  if (teamToolsCheck.ok) {
    console.log("  omq team api command detected (CLI-first interop ready)");
  } else {
    console.log(`  WARNING: ${teamToolsCheck.message}`);
    console.log("  Run `npm run build` and then re-run `omq setup`.");
  }
  console.log();
  ...
  if (existsSync(agentsMdSrc)) {
    ...
    const modelTableContext = resolveAgentsModelTableContext(resolvedConfig, {
      qwenHomeOverride: scopeDirs.qwenHomeDir,
    });
    ...
    if (agentsMdExists) {
      ...
      if (isOmqGeneratedAgentsMd(existing)) {
    ...
    if (
      ...
      console.log(
        "  WARNING: Active omq session detected (pid " +
          activeSession?.pid +
          ").",
      );
      console.log(
      ...
      console.log(
        resolvedScope.scope === "project"
          ...
          : `  Refreshed AGENTS.md model capability table in ${scopeDirs.qwenHomeDir}.`,
      );
    } else {
      ...
      if (result === "updated") {
        console.log(
          resolvedScope.scope === "project"
            ...
            : `  Generated AGENTS.md in ${scopeDirs.qwenHomeDir}.`,
        );
      } else if (result === "unchanged") {
        console.log(
          resolvedScope.scope === "project"
            ? "  AGENTS.md already up to date in project root."
            : `  AGENTS.md already up to date in ${scopeDirs.qwenHomeDir}.`,
        );
  ...
  const hudConfigPath = join(projectRoot, ".omq", "hud-config.json");
  ...
  console.log('Setup complete! Run "omq doctor" to verify installation.');
  console.log("\nNext steps:");
  console.log("  1. Start Qwen Code in your project directory");
  console.log(
    "  2. Use role/workflow keywords like $architect, $executor, and $plan in Qwen Code",
  );
  ...
  console.log(
    "  5. Native agent defaults configured in config.toml [agents] and TOML files written to .qwen/agents/",
  );
  console.log(
    '  6. "omq explore" and "omq sparkshell" can hydrate native release binaries on first use; source installs still allow repo-local fallbacks and OMQ_EXPLORE_BIN / OMQ_SPARKSHELL_BIN overrides',
  );
  if (isGitHubCliConfigured()) {
    console.log("\nSupport the project: gh repo star Yeachan-Heo/oh-my-qwencode");
  }
}
...
async function syncManagedAgentsContent(
  ...
  if (destinationExists && !options.force) {
    ...
    if (!shouldOverwrite) {
      summary.skipped += 1;
      if (options.verbose) {
        const managedLabel = isOmqGeneratedAgentsMd(existing)
          ? "managed"
          : "unmanaged";
...
async function refreshNativeAgentConfigs(
  ...
  for (const [name, agent] of Object.entries(AGENT_DEFINITIONS)) {
    ...
    const toml = generateAgentToml(agent, promptContent, {
      qwenHomeOverride: join(agentsDir, ".."),
    });
...
async function updateManagedConfig(
  ...
  options: Pick<
    SetupOptions,
    "qwenVersionProbe" | "dryRun" | "verbose" | "modelUpgradePrompt"
  >,
  ...
  const qwenVersion =
    options.qwenVersionProbe?.() ?? probeInstalledQwenVersion();
  const omqManagesTui = shouldOmqManageTuiFromQwenVersion(qwenVersion);
  ...
  if (!changed) {
    summary.unchanged += 1;
    return { finalConfig, omqManagesTui };
  }
