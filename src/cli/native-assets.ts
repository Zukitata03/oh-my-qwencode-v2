...
export type NativeProduct = 'omq-explore-harness' | 'omq-sparkshell';
...
const NATIVE_AUTO_FETCH_ENV = 'OMQ_NATIVE_AUTO_FETCH';
const NATIVE_MANIFEST_URL_ENV = 'OMQ_NATIVE_MANIFEST_URL';
const NATIVE_RELEASE_BASE_URL_ENV = 'OMQ_NATIVE_RELEASE_BASE_URL';
const NATIVE_CACHE_DIR_ENV = 'OMQ_NATIVE_CACHE_DIR';
export const EXPLORE_BIN_ENV = 'OMQ_EXPLORE_BIN';
export const SPARKSHELL_BIN_ENV = 'OMQ_SPARKSHELL_BIN';
...
export function resolveNativeCacheRoot(env: NodeJS.ProcessEnv = process.env): string {
  ...
  if (process.platform === 'win32') {
    return resolve(env.LOCALAPPDATA?.trim() || join(homedir(), 'AppData', 'Local'), 'oh-my-qwencode', 'native');
  }
  return resolve(env.XDG_CACHE_HOME?.trim() || join(homedir(), '.cache'), 'oh-my-qwencode', 'native');
}
