...
function withoutTeamTestWorkerEnv<T>(fn: () => T): T {
  const previousTeamWorker = process.env.OMQ_TEAM_WORKER;
  const previousTeamStateRoot = process.env.OMQ_TEAM_STATE_ROOT;
  delete process.env.OMQ_TEAM_WORKER;
  delete process.env.OMQ_TEAM_STATE_ROOT;

  let restoreImmediately = true;
  const restore = () => {
    if (typeof previousTeamWorker === 'string') process.env.OMQ_TEAM_WORKER = previousTeamWorker;
    else delete process.env.OMQ_TEAM_WORKER;

    if (typeof previousTeamStateRoot === 'string') process.env.OMQ_TEAM_STATE_ROOT = previousTeamStateRoot;
...
describe('parseTeamStartArgs', () => {
  ...
  it('rejects deprecated omq team ralph syntax', () => {
    assert.throws(
      () => parseTeamStartArgs(['ralph', '--worktree=feature/demo', '4:executor', 'ship', 'it']),
      /Deprecated usage: `omq team ralph \.\.\.` has been removed/,
    );
  });
  ...
  it('reuses the approved team launch hint for a short English follow-up', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-followup-en-'));
    const previousCwd = process.cwd();
    try {
      process.chdir(wd);
      await mkdir(join(wd, '.omq', 'plans'), { recursive: true });
      await writeFile(
        join(wd, '.omq', 'plans', 'prd-issue-831.md'),
        '# Approved plan\n\nLaunch via omq team 3:executor "Execute approved issue 831 plan"\n',
      );
      await writeFile(join(wd, '.omq', 'plans', 'test-spec-issue-831.md'), '# Test spec\n');
  ...
  it('reuses the approved team launch hint for a short Korean follow-up', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-followup-ko-'));
    const previousCwd = process.cwd();
    try {
      process.chdir(wd);
      await mkdir(join(wd, '.omq', 'plans'), { recursive: true });
  ...
  it('preserves explicit team staffing overrides while reusing the approved plan task', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-followup-override-'));
    const previousCwd = process.cwd();
    try {
      process.chdir(wd);
      await mkdir(join(wd, '.omq', 'plans'), { recursive: true });
...
describe('teamCommand api', () => {
  it('builds leader monitoring hints that keep team status visible while ON', () => {
    ...
    assert.equal(hints[0], 'leader_check: omq team status my-team');
    assert.match(hints[1] ?? '', /while ON, keep checking state/);
    assert.match(hints[1] ?? '', /sleep 30 && omq team status my-team/);
  });

  it('prints team-specific help for omq team --help', async () => {
    const logs: string[] = [];
    ...
    try {
      ...
      assert.match(logs[0] ?? '', /Usage: omq team \[N:agent-type\]/);
      assert.match(logs[0] ?? '', /omq team api <operation>/);
      assert.match(logs[0] ?? '', /dedicated worktrees automatically by default/);
      assert.match(logs[0] ?? '', /--worktree is deprecated/);
      assert.match(logs[0] ?? '', /native Qwen Code subagents for small in-session fanout/);
    } finally {
      console.log = originalLog;
    }
  });

  it('prints team-specific help for omq team help alias', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
  ...
  it('prints team-api-specific help for omq team api --help', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
      ...
      assert.equal(logs.length, 1);
      assert.match(logs[0] ?? '', /Usage: omq team api <operation>/);
  ...
  it('prints team-api-specific help for omq team api help alias', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
  ...
  it('prints operation-specific help for omq team api <operation> --help', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
      ...
      assert.equal(logs.length, 1);
      assert.match(logs[0] ?? '', /Usage: omq team api send-message --input <json> \[--json\]/);
  ...
  it('prints operation-specific help for omq team api <operation> help alias', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
      ...
      assert.equal(logs.length, 1);
      assert.match(logs[0] ?? '', /Usage: omq team api claim-task --input <json> \[--json\]/);
  ...
  it('prints event query help for omq team api read-events help alias', async () => {
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      console.log = (...args: unknown[]) => logs.push(args.map(String).join(' '));
      ...
      assert.equal(logs.length, 1);
      assert.match(logs[0] ?? '', /Usage: omq team api read-events --input <json> \[--json\]/);
  ...
  it('executes read-events via CLI api with canonical JSON results', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-api-read-events-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      assert.equal(envelope.command, 'omq team api read-events');
  ...
  it('executes read-idle-state via CLI api with structured JSON results', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-api-read-idle-state-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      assert.equal(envelope.command, 'omq team api read-idle-state');
      assert.equal(envelope.ok, true);
  ...
  it('executes read-stall-state via CLI api with structured JSON results', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-api-read-stall-state-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      assert.equal(envelope.command, 'omq team api read-stall-state');
      assert.equal(envelope.ok, true);
  ...
  it('executes CLI interop operation with stable JSON envelope', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-api-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      assert.equal(envelope.command, 'omq team api send-message');
      assert.equal(envelope.ok, true);
  ...
  it('returns deterministic JSON errors for invalid api usage with --json', async () => {
    ...
    try {
      ...
      assert.equal(envelope.schema_version, '1.0');
      assert.equal(typeof envelope.timestamp, 'string');
      assert.equal(envelope.command, 'omq team api');
      assert.equal(envelope.ok, false);
      assert.equal(envelope.operation, 'unknown');
      assert.equal(envelope.error?.code, 'invalid_input');
      assert.match(envelope.error?.message ?? '', /Usage: omq team api/);
  ...
  it('supports claim-safe lifecycle via CLI api (create -> claim -> transition)', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-api-lifecycle-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
  ...
  it('accepts new canonical event types via CLI api append-event', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-api-event-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
...
describe('teamCommand status', () => {
  it('prints pane ids and sparkshell hint when tmux panes are recorded', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-status-panes-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      await writeFile(
        join(wd, '.omq', 'state', 'team', 'pane-team', 'tasks', 'task-1.json'),
        `${JSON.stringify({
          ...JSON.parse(await readFile(join(wd, '.omq', 'state', 'team', 'pane-team', 'tasks', 'task-1.json'), 'utf-8')) as Record<string, unknown>,
      ...
      await writeFile(
        join(wd, '.omq', 'state', 'team', 'pane-team', 'tasks', 'task-2.json'),
        `${JSON.stringify({
          ...JSON.parse(await readFile(join(wd, '.omq', 'state', 'team', 'pane-team', 'tasks', 'task-2.json'), 'utf-8')) as Record<string, unknown>,
          created_at: '2026-03-10T23:56:00.000Z',
          completed_at: '2026-03-11T00:06:00.000Z',
        }, null, 2)}\n`,
      );
      config.workers[0]!.worker_cli = 'qwen';
      ...
      config.workers[0]!.team_state_root = '/tmp/pane-team/.omq/state';
      config.workers[1]!.team_state_root = '/tmp/pane-team/.omq/state';
      ...
      const manifestPath = join(wd, '.omq', 'state', 'team', 'pane-team', 'manifest.v2.json');
      ...
      await writeFile(
        join(wd, '.omq', 'state', 'team', 'pane-team', 'config.json'),
      ...
      assert.match(output, /sparkshell_hint: omq sparkshell --tmux-pane <pane-id> --tail-lines 400/);
      assert.match(output, /inspect_leader: omq sparkshell --tmux-pane %10 --tail-lines 400/);
      assert.match(output, /inspect_hud: omq sparkshell --tmux-pane %11 --tail-lines 400/);
      assert.match(output, /inspect_worker-1: omq sparkshell --tmux-pane %21 --tail-lines 400/);
      assert.match(output, /inspect_worker-2: omq sparkshell --tmux-pane %22 --tail-lines 400/);
    } finally {
      console.log = originalLog;
      process.chdir(previousCwd);
      await rm(wd, { recursive: true, force: true });
  ...
  it('returns pane ids and sparkshell hint in JSON mode', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-status-json-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      await writeFile(
        join(wd, '.omq', 'state', 'team', 'pane-json-team', 'tasks', 'task-1.json'),
        `${JSON.stringify({
          ...JSON.parse(await readFile(join(wd, '.omq', 'state', 'team', 'pane-json-team', 'tasks', 'task-1.json'), 'utf-8')) as Record<string, unknown>,
      ...
      config.workers[0]!.team_state_root = '/tmp/pane-json-team/.omq/state';
      ...
      const manifestPath = join(wd, '.omq', 'state', 'team', 'pane-json-team', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as {
        leader_pane_id?: string | null;
        hud_pane_id?: string | null;
        workers?: Array<{ pane_id?: string }>;
      ...
      await writeFile(
        join(wd, '.omq', 'state', 'team', 'pane-json-team', 'config.json'),
        `${JSON.stringify(config, null, 2)}\n`,
      );
      await writeFile(
        manifestPath,
      ...
      assert.equal(payload.command, 'omq team status');
      ...
      assert.deepEqual(payload.panes?.recommended_inspect_team_state_roots, { 'worker-1': '/tmp/pane-json-team/.omq/state' });
      ...
      assert.deepEqual(payload.panes?.recommended_inspect_task_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/tasks/task-1.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_approval_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/approvals/task-1.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_state_dirs, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/workers/worker-1` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_status_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/workers/worker-1/status.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_heartbeat_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/workers/worker-1/heartbeat.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_identity_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/workers/worker-1/identity.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_inbox_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/workers/worker-1/inbox.md` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_mailbox_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/mailbox/worker-1.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_shutdown_request_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/workers/worker-1/shutdown-request.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_worker_shutdown_ack_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/workers/worker-1/shutdown-ack.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_dir_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_config_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/config.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_manifest_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/manifest.v2.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_events_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/events/events.ndjson` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_dispatch_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/dispatch/requests.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_phase_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/phase.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_monitor_snapshot_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/monitor-snapshot.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_team_summary_snapshot_paths, { 'worker-1': `${wd}/.omq/state/team/pane-json-team/summary-snapshot.json` });
      assert.deepEqual(payload.panes?.recommended_inspect_panes, { 'worker-1': '%41' });
      assert.equal(payload.panes?.recommended_inspect_command, 'omq sparkshell --tmux-pane %41 --tail-lines 400');
      assert.deepEqual(payload.panes?.recommended_inspect_commands, ['omq sparkshell --tmux-pane %41 --tail-lines 400']);
      assert.equal(payload.panes?.recommended_inspect_summary, 'target=worker-1 pane=%41 cli=claude role=executor alive=false turn_count=5 turns_without_progress=0 reason=dead_worker state=working task=1 subject=Recover worker-1 progress command=omq sparkshell --tmux-pane %41 --tail-lines 400');
      assert.deepEqual(payload.panes?.recommended_inspect_items, [{
        ...
        team_state_root: '/tmp/pane-json-team/.omq/state',
        ...
        task_claim_lock_path: `${wd}/.omq/state/team/pane-json-team/claims/task-1.lock`,
        ...
        task_path: `${wd}/.omq/state/team/pane-json-team/tasks/task-1.json`,
        approval_path: `${wd}/.omq/state/team/pane-json-team/approvals/task-1.json`,
        worker_state_dir: `${wd}/.omq/state/team/pane-json-team/workers/worker-1`,
        worker_status_path: `${wd}/.omq/state/team/pane-json-team/workers/worker-1/status.json`,
        worker_heartbeat_path: `${wd}/.omq/state/team/pane-json-team/workers/worker-1/heartbeat.json`,
        worker_identity_path: `${wd}/.omq/state/team/pane-json-team/workers/worker-1/identity.json`,
        worker_inbox_path: `${wd}/.omq/state/team/pane-json-team/workers/worker-1/inbox.md`,
        worker_mailbox_path: `${wd}/.omq/state/team/pane-json-team/mailbox/worker-1.json`,
        worker_shutdown_request_path: `${wd}/.omq/state/team/pane-json-team/workers/worker-1/shutdown-request.json`,
        worker_shutdown_ack_path: `${wd}/.omq/state/team/pane-json-team/workers/worker-1/shutdown-ack.json`,
        team_dir_path: `${wd}/.omq/state/team/pane-json-team`,
        team_config_path: `${wd}/.omq/state/team/pane-json-team/config.json`,
        team_manifest_path: `${wd}/.omq/state/team/pane-json-team/manifest.v2.json`,
        team_events_path: `${wd}/.omq/state/team/pane-json-team/events/events.ndjson`,
        team_dispatch_path: `${wd}/.omq/state/team/pane-json-team/dispatch/requests.json`,
        team_phase_path: `${wd}/.omq/state/team/pane-json-team/phase.json`,
        team_monitor_snapshot_path: `${wd}/.omq/state/team/pane-json-team/monitor-snapshot.json`,
        team_summary_snapshot_path: `${wd}/.omq/state/team/pane-json-team/summary-snapshot.json`,
        command: 'omq sparkshell --tmux-pane %41 --tail-lines 400',
      }]);
      ...
      assert.equal(payload.panes?.sparkshell_hint, 'omq sparkshell --tmux-pane <pane-id> --tail-lines 400');
      assert.deepEqual(payload.panes?.sparkshell_commands, {
        leader: 'omq sparkshell --tmux-pane %30 --tail-lines 400',
        hud: 'omq sparkshell --tmux-pane %31 --tail-lines 400',
        'worker-1': 'omq sparkshell --tmux-pane %41 --tail-lines 400',
      });
    } finally {
      console.log = originalLog;
      process.chdir(previousCwd);
  ...
  it('prints workspace_mode in text status output when present', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-status-workspace-mode-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      const teamDir = join(wd, '.omq', 'state', 'team', 'workspace-mode-team');
  ...
  it('returns workspace_mode in JSON status output when present', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-status-json-workspace-mode-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      config.workspace_mode = 'worktree';
      const teamDir = join(wd, '.omq', 'state', 'team', 'workspace-mode-json-team');
      const configPath = join(teamDir, 'config.json');
      const manifestPath = join(teamDir, 'manifest.v2.json');
      await mkdir(teamDir, { recursive: true });
      await writeFile(configPath, JSON.stringify(config, null, 2));
  ...
  it('returns a missing envelope in JSON mode when team state is absent', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-status-missing-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
  ...
  it('records leader runtime activity when team status is read', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-status-activity-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      const activity = JSON.parse(await readFile(join(wd, '.omq', 'state', 'leader-runtime-activity.json'), 'utf-8')) as {
  ...
  it('supports custom tail lines for generated sparkshell commands', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-status-tail-lines-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
      ...
      const manifestPath = join(wd, '.omq', 'state', 'team', 'pane-tail-team', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as {
        workers?: Array<{ pane_id?: string }>;
      };
      manifest.workers = config.workers.map((worker) => ({
      ...
      await writeFile(
        join(wd, '.omq', 'state', 'team', 'pane-tail-team', 'config.json'),
        `${JSON.stringify(config, null, 2)}\n`,
      );
      await writeFile(
        manifestPath,
      ...
      assert.match(logs.join('\n'), /inspect_worker-1: omq sparkshell --tmux-pane %51 --tail-lines 600/);
      ...
      assert.equal(payload.panes?.sparkshell_commands?.['worker-1'], 'omq sparkshell --tmux-pane %51 --tail-lines 550');
    } finally {
      console.log = originalLog;
      process.chdir(previousCwd);
      await rm(wd, { recursive: true, force: true });
...
describe('teamCommand await', () => {
  it('returns next canonical event for a team in JSON mode', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-await-'));
    const previousCwd = process.cwd();
    const logs: string[] = [];
    const originalLog = console.log;
    try {
  ...
  it('returns a dead-worker event for the prompt-launch smoke path instead of timing out', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-team-await-prompt-dead-'));
...
`,
  ...
  it('rejects legacy omq team ralph launches at command entry', async () => {
