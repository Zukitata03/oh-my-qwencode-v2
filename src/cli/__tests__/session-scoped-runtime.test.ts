...
function runOmq(cwd: string, ...args: string[]) {
  return spawnSync(process.execPath, [omqBin, ...args], {
    cwd,
    encoding: 'utf-8',
  });
}
...
describe('CLI session-scoped state parity', () => {
  it('status and cancel include session-scoped states', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-cli-session-scope-'));
    try {
      await mkdir(join(wd, '.omq', 'state'), { recursive: true });
      await writeFile(join(wd, '.omq', 'state', 'session.json'), JSON.stringify({ session_id: 'sess1' }));
      const scopedDir = join(wd, '.omq', 'state', 'sessions', 'sess1');
      ...
      const statusResult = runOmq(wd, 'status');
      ...
      const cancelResult = runOmq(wd, 'cancel');
  ...
  it('cancels linked ultrawork when Ralph is active', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-cli-ralph-link-'));
    try {
  ...
  it('does not mutate unrelated sessions when cancelling current session mode', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-cli-cross-session-'));
    try {
      const stateDir = join(wd, '.omq', 'state');
