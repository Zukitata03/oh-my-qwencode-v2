...
const CURRENT_SESSION_PROCESSES: ProcessEntry[] = [
  { pid: 700, ppid: 500, command: 'qwen' },
  { pid: 701, ppid: 700, command: 'node /repo/bin/omq.js cleanup --dry-run' },
  {
    pid: 710,
    ppid: 700,
    command: 'node /repo/oh-my-qwencode/dist/mcp/state-server.js',
  },
  {
    pid: 800,
    ppid: 1,
    command: 'node /tmp/oh-my-qwencode/dist/mcp/memory-server.js',
  },
  {
    pid: 810,
    ppid: 42,
  ...
  },
  {
    pid: 900,
    ppid: 1,
    command: 'node /tmp/not-omq/other-server.js',
  },
];

describe('findCleanupCandidates', () => {
  it('selects orphaned OMQ MCP processes while preserving the current session tree', () => {
    assert.deepEqual(
...
describe('cleanupOmqMcpProcesses', () => {
  it('supports dry-run without sending signals', async () => {
    const lines: string[] = [];
    let signalCount = 0;

    const result = await cleanupOmqMcpProcesses(['--dry-run'], {
      currentPid: 701,
    ...
    assert.match(lines.join('\n'), /Dry run: would terminate 3 orphaned OMQ MCP server process/);
    assert.match(lines.join('\n'), /PID 800/);
    assert.match(lines.join('\n'), /PID 810/);
  });
  ...
  it('sends SIGTERM, waits, and escalates with SIGKILL when needed', async () => {
    ...
    const result = await cleanupOmqMcpProcesses([], {
      currentPid: 701,
      listProcesses: () => [
    ...
    assert.match(lines.join('\n'), /Killed 2 orphaned OMQ MCP server process\(es\) \(1 required SIGKILL\)\./);
  });
});

describe('cleanupStaleTmpDirectories', () => {
  const tmpEntries = [
    { name: 'omq-stale-a', isDirectory: () => true },
    { name: 'omc-stale-b', isDirectory: () => true },
    { name: 'oh-my-qwencode-fresh', isDirectory: () => true },
    { name: 'oh-my-qwencode-file', isDirectory: () => false },
  ...
  it('supports dry-run and reports stale matching directories older than one hour', async () => {
    ...
    assert.match(
      lines.join('\n'),
      /Dry run: would remove 2 stale OMQ \/tmp directories:/,
    );
    assert.match(lines.join('\n'), /\/tmp\/omc-stale-b/);
    assert.match(lines.join('\n'), /\/tmp\/omq-stale-a/);
    assert.doesNotMatch(lines.join('\n'), /oh-my-qwencode-fresh/);
    assert.doesNotMatch(lines.join('\n'), /other-stale/);
  });
  ...
  it('removes only stale matching directories and returns the removed count', async () => {
    ...
    assert.deepEqual(removedPaths, ['/tmp/omc-stale-b', '/tmp/omq-stale-a']);
    assert.match(lines.join('\n'), /Removed stale \/tmp directory: \/tmp\/omc-stale-b/);
    assert.match(lines.join('\n'), /Removed stale \/tmp directory: \/tmp\/omq-stale-a/);
    assert.match(lines.join('\n'), /Removed 2 stale OMQ \/tmp directories\./);
  });
});
