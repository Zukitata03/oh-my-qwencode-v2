...
describe("omq setup refresh summary and dry-run behavior", () => {
  ...
  it("prints per-category summary and verbose changed-file detail", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-setup-refresh-"));
    try {
  ...
  it("does not overwrite or create backups during dry-run", async () => {
    ...
    try {
      ...
      assert.equal(existsSync(join(wd, ".omq", "backups", "setup")), false);
      assert.match(output, /skills: updated=/);
  ...
  it("creates .gitignore with a .omq/ entry during project-scoped setup", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-setup-refresh-"));
    try {
      await runSetupInTempDir(wd, { scope: "project" });

      assert.equal(existsSync(join(wd, ".omq", "state")), true);
      assert.equal(await readFile(join(wd, ".gitignore"), "utf-8"), ".omq/\n");
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });

  it("appends .omq/ to an existing project .gitignore without duplicating it", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-setup-refresh-"));
    try {
      ...
      assert.equal(gitignore.match(/^\.omq\/$/gm)?.length ?? 0, 1);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it("creates backup files under the scope-specific setup backup root when refreshing modified managed files", async () => {
    ...
    try {
      ...
      const promptPath = join(wd, ".qwen", "prompts", "executor.md");
  ...
  it("offers an upgrade from qwen3.5-plus to qwen3.6-plus when accepted", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-setup-refresh-"));
    try {
      await mkdir(join(wd, ".omq", "state"), { recursive: true });
      await mkdir(join(wd, ".qwen"), { recursive: true });
      await writeFile(
      ...
      await runSetupInTempDir(wd, {
        ...
        modelUpgradePrompt: async (currentModel, targetModel) => {
          promptCalls += 1;
          assert.equal(currentModel, "qwen3.5-plus");
          assert.equal(targetModel, "qwen3.6-plus");
          return true;
        },
      });

      const config = await readFile(join(wd, ".qwen", "config.toml"), "utf-8");
      assert.equal(promptCalls, 1);
      assert.match(config, /^model = "qwen3.6-plus"$/m);
      assert.doesNotMatch(config, /^model = "qwen3.5-plus"$/m);
  ...
  it("preserves qwen3.5-plus when the upgrade prompt is declined", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-setup-refresh-"));
    try {
      await mkdir(join(wd, ".omq", "state"), { recursive: true });
      await mkdir(join(wd, ".qwen"), { recursive: true });
      ...
      const config = await readFile(join(wd, ".qwen", "config.toml"), "utf-8");
      assert.match(config, /^model = "qwen3.5-plus"$/m);
  ...
  it("preserves qwen3.5-plus in non-interactive runs without prompting", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-setup-refresh-"));
    try {
      await mkdir(join(wd, ".omq", "state"), { recursive: true });
      await mkdir(join(wd, ".qwen"), { recursive: true });
  ...
  it("skips OMQ-managed [tui] writes for Qwen Code >= 0.107.0 and preserves an existing [tui] table", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-setup-refresh-"));
    try {
      await mkdir(join(wd, ".omq", "state"), { recursive: true });
      await mkdir(join(wd, ".qwen"), { recursive: true });
      await writeFile(
        join(wd, ".qwen", "config.toml"),
        ['model = "qwen3.6-plus"', "", "[tui]", 'theme = "night"', 'status_line = ["git-branch"]', ""].join("\n"),
      );

      const output = await runSetupWithCapturedLogs(wd, {
        scope: "project",
        qwenVersionProbe: () => "qwen-cli 0.107.0",
      });
      ...
      assert.match(
        output,
        /Qwen Code >= 0\.107\.0 manages \[tui\]; OMQ left that section untouched\./,
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it("keeps OMQ-managed [tui] writes for older Qwen Code versions", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-setup-refresh-"));
    try {
      await mkdir(join(wd, ".omq", "state"), { recursive: true });
      ...
      const output = await runSetupWithCapturedLogs(wd, {
        scope: "project",
        qwenVersionProbe: () => "qwen-cli 0.106.0",
      });
  ...
  it("syncs shared MCP registry entries into config.toml during setup", async () => {
    ...
    try {
      ...
      const config = await readFile(join(wd, ".qwen", "config.toml"), "utf-8");
      assert.match(config, /oh-my-qwencode \(OMQ\) Shared MCP Registry Sync/);
  ...
  it("ignores legacy ~/.omc/mcp-registry.json during setup unless candidates are passed explicitly", async () => {
    ...
    try {
      ...
      assert.match(output, /move it to .*\.omq\/mcp-registry\.json/i);
    } finally {
      if (typeof previousHome === "string") process.env.HOME = previousHome;
      else delete process.env.HOME;
      if (typeof previousQwenHome === "string") process.env.QWEN_HOME = previousQwenHome;
