...
function expectedLowComplexityModel(qwenHomeOverride?: string): string {
  return getTeamLowComplexityModel(qwenHomeOverride);
}
...
describe("normalizeQwenLaunchArgs", () => {
  ...
  it("does not forward --madmax and preserves other args", () => {
    assert.deepEqual(
      normalizeQwenLaunchArgs(["--model", "gpt-5", "--madmax", "--yolo"]),
      [
        "--model",
        "gpt-5",
        "--yolo",
  ...
  it("leaves unrelated args unchanged", () => {
    assert.deepEqual(normalizeQwenLaunchArgs(["--model", "gpt-5", "--yolo"]), [
      "--model",
      "gpt-5",
      "--yolo",
    ]);
  ...
  it("--spark is stripped from leader args (model goes to workers only)", () => {
    assert.deepEqual(normalizeQwenLaunchArgs(["--spark", "--yolo"]), [
      "--yolo",
    ]);
  });

  it("--spark alone produces no leader args", () => {
    assert.deepEqual(normalizeQwenLaunchArgs(["--spark"]), []);
  });
  ...
  it("--madmax-spark does not inject spark model into leader args", () => {
    const args = normalizeQwenLaunchArgs(["--madmax-spark"]);
    assert.ok(
      !args.includes("--model"),
  ...
  it("strips detached worktree flag from leader qwen args", () => {
    assert.deepEqual(normalizeQwenLaunchArgs(["--worktree", "--yolo"]), [
      "--yolo",
    ]);
  });

  it("strips named worktree flag from leader qwen args", () => {
    assert.deepEqual(
      normalizeQwenLaunchArgs(["--worktree=feature/demo", "--model", "gpt-5"]),
      ["--model", "gpt-5"],
    );
  });

  it("does not forward notify-temp flags/selectors to leader qwen args", () => {
    ...
    assert.deepEqual(normalizeQwenLaunchArgs(parsed.passthroughArgs), [
      "--model",
      "gpt-5",
    ]);
  });
...
describe("resolveNotifyTempContract", () => {
  ...
  it("activates from OMQ_NOTIFY_TEMP=1 env parity", () => {
...
describe("buildNotifyFallbackWatcherEnv", () => {
  it("enables watcher authority and propagates QWEN_HOME override when requested", () => {
    const env = buildNotifyFallbackWatcherEnv(
      { HOME: "/tmp/home", OMQ_HUD_AUTHORITY: "0", TMUX: "sock,1,0", TMUX_PANE: "%2" },
      { qwenHomeOverride: "/tmp/qwen-home", enableAuthority: true },
    );
    assert.equal(env.OMQ_HUD_AUTHORITY, "1");
    assert.equal(env.QWEN_HOME, "/tmp/qwen-home");
  ...
  it("disables watcher authority explicitly when not requested", () => {
    const env = buildNotifyFallbackWatcherEnv(
      { HOME: "/tmp/home", OMQ_HUD_AUTHORITY: "1", TMUX: "sock,1,0", TMUX_PANE: "%3" },
      { enableAuthority: false },
    );
    assert.equal(env.OMQ_HUD_AUTHORITY, "0");
    assert.equal(env.HOME, "/tmp/home");
    assert.equal(env.TMUX, undefined);
    assert.equal(env.TMUX_PANE, undefined);
  });
...
describe("resolveWorkerSparkModel", () => {
  ...
  it("reads low-complexity team model from config when qwenHomeOverride is provided", async () => {
    const qwenHome = await mkdtemp(join(tmpdir(), "omq-qwen-home-"));
    try {
      await writeFile(
        join(qwenHome, ".omq-config.json"),
        JSON.stringify({ models: { team_low_complexity: "gpt-4.1-mini" } }),
      );
      assert.equal(
        resolveWorkerSparkModel(["--spark"], qwenHome),
        "gpt-4.1-mini",
      );
    } finally {
...
describe("project launch scope helpers", () => {
  it("reads persisted setup scope when valid", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-launch-scope-"));
    try {
      await mkdir(join(wd, ".omq"), { recursive: true });
      await writeFile(
        join(wd, ".omq", "setup-scope.json"),
  ...
  it("ignores malformed persisted setup scope", async () => {
    ...
    try {
      await mkdir(join(wd, ".omq"), { recursive: true });
      await writeFile(join(wd, ".omq", "setup-scope.json"), "{not-json");
  ...
  it("uses project QWEN_HOME when persisted scope is project", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-launch-scope-"));
    try {
      await mkdir(join(wd, ".omq"), { recursive: true });
      await writeFile(
      ...
      assert.equal(resolveQwenHomeForLaunch(wd, {}), join(wd, ".qwen"));
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });

  it("keeps explicit QWEN_HOME override from env", async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-launch-scope-"));
    try {
      await mkdir(join(wd, ".omq"), { recursive: true });
      await writeFile(
      ...
      assert.equal(
        resolveQwenHomeForLaunch(wd, {
          QWEN_HOME: "/tmp/explicit-qwen-home",
        }),
        "/tmp/explicit-qwen-home",
      );
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it('resolves QWEN_HOME for legacy "project-local" persisted scope', async () => {
    const wd = await mkdtemp(join(tmpdir(), "omq-launch-scope-"));
    try {
      await mkdir(join(wd, ".omq"), { recursive: true });
      await writeFile(
...
describe("resolveQwenLaunchPolicy", () => {
  it("uses detached tmux on macOS when outside tmux and tmux is available", () => {
    assert.equal(
      resolveQwenLaunchPolicy({}, "darwin", true, false, true, true),
      "detached-tmux",
    );
  });
  ...
  it("uses detached tmux on non-macOS hosts when outside tmux and tmux is available", () => {
    assert.equal(
      resolveQwenLaunchPolicy({}, "linux", true, false, true, true),
      "detached-tmux",
    );
  });

  it("launches directly on native Windows even when tmux is available", () => {
    assert.equal(resolveQwenLaunchPolicy({}, "win32", true, true), "direct");
  });
  ...
  it("launches directly when stdin is not a tty outside tmux", () => {
    assert.equal(resolveQwenLaunchPolicy({}, "linux", true, false, false, true), "direct");
  });

  it("launches directly when stdout is not a tty outside tmux", () => {
    assert.equal(resolveQwenLaunchPolicy({}, "linux", true, false, true, false), "direct");
  });

  it("launches directly when tmux is unavailable outside tmux", () => {
    assert.equal(resolveQwenLaunchPolicy({}, "linux", false), "direct");
  });

  it("launches directly on native Windows when tmux is unavailable", () => {
    assert.equal(resolveQwenLaunchPolicy({}, "win32", false, true), "direct");
  });
});

describe("classifyQwenExecFailure", () => {
  it("classifies child process exit status as qwen exit", () => {
    const err = Object.assign(new Error("qwen exited 9"), { status: 9 });
    const classified = classifyQwenExecFailure(err);
    assert.equal(classified.kind, "exit");
    assert.equal(classified.exitCode, 9);
  });

  it("classifies signal termination as qwen exit and maps to signal-based exit code", () => {
  ...
  it("classifies ENOENT as launch error", () => {
    const err = Object.assign(new Error("spawn qwen ENOENT"), {
      code: "ENOENT",
    });
    const classified = classifyQwenExecFailure(err);
...
describe("tmux HUD pane helpers", () => {
  it("findHudWatchPaneIds detects stale HUD watch panes and excludes current pane", () => {
    const panes = parseTmuxPaneSnapshot(
      [
        "%1\tzsh\tzsh",
        "%2\tnode\tnode /tmp/bin/omq.js hud --watch",
        "%3\tnode\tnode /tmp/bin/omq.js hud --watch",
        "%4\tqwen\tqwen --model gpt-5",
      ].join("\n"),
    );
...
describe("detached tmux new-session sequencing", () => {
  it("buildDetachedSessionBootstrapSteps uses shared HUD height and split-capture ordering", () => {
    ...
    assert.equal(steps[1]?.args[6], "omq-demo");
    ...
    assert.equal(
      steps[0]?.args.includes('OMQ_NOTIFY_TEMP_CONTRACT={\"active\":true}'),
      true,
    );
  });
    ...
    assert.equal(
      newSession!.args.includes("-e") &&
        newSession!.args.some((arg) =>
          arg.startsWith("OMQ_NOTIFY_TEMP_CONTRACT="),
        ),
      true,
    );
  });
  ...
  it("buildDetachedSessionBootstrapSteps kills detached tmux session on exit and interrupt", () => {
    ...
    assert.match(leaderCmd!, /"omq-demo"/);
  ...
  it("buildDetachedSessionRollbackSteps unregisters hooks before killing session", () => {
    const steps = buildDetachedSessionRollbackSteps(
      "omq-demo",
      "omq-demo:0",
      "omq_resize_launch_demo_0_12",
      "omq_attached_launch_demo_0_12",
    );
    assert.deepEqual(
      steps.map((step) => step.name),
      [
    ...
    assert.equal(steps[0]?.args[3], "omq-demo:0");
    assert.match(steps[0]?.args[4] ?? "", /^client-attached\[\d+\]$/);
    assert.match(steps[1]?.args[4] ?? "", /^client-resized\[\d+\]$/);
    assert.deepEqual(steps[2]?.args, ["kill-session", "-t", "omq-demo"]);
  });
...
describe("buildTmuxPaneCommand", () => {
  ...
  it("wraps command with bash profile sourcing for bash shell", () => {
    const result = buildTmuxPaneCommand("qwen", [], "/bin/bash");
    assert.ok(
  ...
  it("skips rc sourcing for unknown shells but still uses login flag", () => {
    const result = buildTmuxPaneCommand("qwen", [], "/bin/fish");
    assert.ok(
  ...
  it("falls back to /bin/sh when shell path is empty", () => {
    const result = buildTmuxPaneCommand("qwen", [], "");
    assert.ok(
...
describe("buildTmuxSessionName", () => {
  it("uses detached fallback quietly outside git repos", () => {
    const name = buildTmuxSessionName(
      "/tmp/My Repo",
      "omq-1770992424158-abc123",
    );
    assert.equal(name, "omq-my-repo-detached-1770992424158-abc123");
  });

  it("sanitizes invalid characters", () => {
    const name = buildTmuxSessionName("/tmp/@#$", "omq-+++");
    assert.match(
      name,
      /^omq-(unknown|[a-z0-9-]+)-[a-z0-9-]+-(unknown|[a-z0-9-]+)$/,
    );
    assert.equal(name.includes("_"), false);
    assert.equal(name.includes(" "), false);
  });

  it("includes repo name when cwd is inside .omq-worktrees", () => {
    const name = buildTmuxSessionName(
      "/home/user/my-repo.omq-worktrees/launch-feature-x",
      "omq-123-abc",
    );
    assert.match(name, /^omq-my-repo-launch-feature-x-/);
  });

  it("includes repo name for detached worktree paths", () => {
    const name = buildTmuxSessionName(
      "/projects/cool-project.omq-worktrees/launch-detached",
      "omq-456-def",
    );
    assert.match(name, /^omq-cool-project-launch-detached-/);
  });

  it("includes repo name when cwd is inside .omq/worktrees", () => {
    const name = buildTmuxSessionName(
      "/home/user/my-repo/.omq/worktrees/autoresearch-demo",
      "omq-789-ghi",
    );
    assert.match(name, /^omq-my-repo-autoresearch-demo-/);
  });
});
...
describe("team worker launch arg inheritance helpers", () => {
  ...
  it("collectInheritableTeamWorkerArgs supports --model=<value> syntax", () => {
    assert.deepEqual(
      collectInheritableTeamWorkerArgs(["--model=qwen3.5-plus"]),
      ["--model", "qwen3.5-plus"],
    );
  });
  ...
  it("resolveTeamWorkerLaunchArgsEnv uses inherited model when env model is absent", () => {
    assert.equal(
      ...
      "--no-alt-screen --model qwen3.5-plus",
    );
  });
...
describe("injectModelInstructionsBypassArgs", () => {
  ...
  it("does not append when bypass is disabled via env", () => {
    const args = injectModelInstructionsBypassArgs(
      "/tmp/my-project",
      ["--model", "gpt-5"],
      { OMQ_BYPASS_DEFAULT_SYSTEM_PROMPT: "0" },
    );
  ...
  it("uses session-scoped default model_instructions_file when provided", () => {
    const args = injectModelInstructionsBypassArgs(
      ...
      ["--model", "gpt-5"],
      {},
      "/tmp/my-project/.omq/state/sessions/session-1/AGENTS.md",
    );
    assert.deepEqual(args, [
      "--model",
      "gpt-5",
