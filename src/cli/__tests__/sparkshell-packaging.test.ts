...
describe('sparkshell packaging scaffold', () => {
  it('registers native helper scripts but keeps staged native artifacts out of npm releases', () => {
    ...
    const stagedRoot = mkdtempSync(join(tmpdir(), 'omq-sparkshell-stage-'));
    ...
    assert.match(testScriptSource, /'crates', 'omq-sparkshell', 'Cargo\.toml'/);
    assert.doesNotMatch(testScriptSource, /'native', 'omq-sparkshell', 'Cargo\.toml'/);
    ...
    try {
      ...
      const buildResult = spawnSync(process.execPath, [buildScriptPath], {
        ...
        env: {
          ...process.env,
          OMQ_SPARKSHELL_MANIFEST: join(process.cwd(), 'crates', 'omq-sparkshell', 'Cargo.toml'),
          OMQ_SPARKSHELL_STAGE_DIR: stagedRoot,
        },
      });
