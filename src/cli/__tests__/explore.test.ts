...
function runOmq(
  ...
  const nodeWrapper = join(cwd, '.omq-test-node.sh');
  if (!existsSync(nodeWrapper)) {
    writeFileSync(nodeWrapper, '#!/bin/sh\nexec node "$@"\n');
    chmodSync(nodeWrapper, 0o755);
  }
  const r = spawnSync(nodeWrapper, [omqBin, ...argv], {
    cwd,
    encoding: 'utf-8',
...
async function writeEnvNodeQwenStub(wd: string, capturePath: string): Promise<string> {
  const stub = join(wd, 'qwen-stub.sh');
  const argvPath = join(wd, 'qwen-argv.txt');
...
async function writeScenarioQwenStub(wd: string, body: string): Promise<string> {
  const stub = join(wd, 'qwen-scenario-stub.sh');
  await writeFile(
    stub,
    `#!/bin/sh
set -eu
...
describe('loadExplorePrompt', () => {
  it('reads prompt file content', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-prompt-'));
    try {
...
describe('resolvePackagedExploreHarnessCommand', () => {
  it('uses a packaged native binary when metadata matches the current platform', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-packaged-'));
    try {
      const binDir = join(wd, 'bin');
      await mkdir(binDir, { recursive: true });
      await writeFile(join(wd, 'package.json'), '{}\n');
      await writeFile(join(binDir, 'omq-explore-harness.meta.json'), JSON.stringify({
  ...
  it('ignores packaged binaries built for a different platform', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-packaged-mismatch-'));
    try {
      const binDir = join(wd, 'bin');
      await mkdir(binDir, { recursive: true });
      await writeFile(join(wd, 'package.json'), '{}\n');
...
describe('resolveExploreHarnessCommand', () => {
  it('uses env override when provided', () => {
    const resolved = resolveExploreHarnessCommand('/repo', { OMQ_EXPLORE_BIN: '/tmp/omq-explore-stub' } as NodeJS.ProcessEnv);
    assert.deepEqual(resolved, { command: '/tmp/omq-explore-stub', args: [] });
  });

  it('prefers a packaged native harness binary when present', async () => {
    await withPackagedExploreHarnessLock(async () => {
      const wd = await mkdtemp(join(tmpdir(), 'omq-explore-native-'));
      try {
        const binDir = join(wd, 'bin');
        await mkdir(binDir, { recursive: true });
        await writeFile(join(wd, 'package.json'), '{}\n');
  ...
  it('uses an existing repo-built native harness before cargo fallback', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-target-'));
    try {
      ...
      await mkdir(join(wd, 'crates', 'omq-explore'), { recursive: true });
      await writeFile(join(wd, 'crates', 'omq-explore', 'Cargo.toml'), '[package]\nname="omq-explore-harness"\nversion="0.0.0"\n');
  ...
  it('builds cargo fallback command otherwise', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-fallback-'));
    try {
      const crateDir = join(wd, 'crates', 'omq-explore');
      ...
      assert.ok(resolved.args.includes(join(wd, 'crates', 'omq-explore', 'Cargo.toml')));
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it('hydrates a native harness for packaged installs before attempting cargo fallback', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-hydrated-'));
    try {
      ...
      const archivePath = join(assetRoot, 'omq-explore-harness-x86_64-unknown-linux-musl.tar.gz');
      ...
      try {
        await writeFile(join(assetRoot, 'native-release-manifest.json'), JSON.stringify({
          version: '0.8.15',
          assets: [{
            product: 'omq-explore-harness',
            version: '0.8.15',
            platform: 'linux',
            arch: 'x64',
            archive: 'omq-explore-harness-x86_64-unknown-linux-musl.tar.gz',
            binary: 'omq-explore-harness',
            binary_path: 'omq-explore-harness',
            sha256: checksum,
            size: archiveBuffer.length,
            download_url: `${server.baseUrl}/omq-explore-harness-x86_64-unknown-linux-musl.tar.gz`,
          }],
        }, null, 2));
  ...
  it('reports a clean fallback error when the native manifest is unavailable for packaged installs', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-missing-manifest-'));
    try {
      await writeFile(join(wd, 'package.json'), JSON.stringify({
        version: '0.8.15',
        repository: { url: 'git+https://github.com/Yeachan-Heo/oh-my-qwencode.git' },
...
describe('buildExploreHarnessArgs', () => {
  it('includes cwd, prompt, prompt contract, and constrained model settings', () => {
    const args = buildExploreHarnessArgs('find auth', '/repo', {
      OMQ_EXPLORE_SPARK_MODEL: 'spark-model',
...
describe('exploreCommand', () => {
  it('routes qualifying read-only shell commands through sparkshell instead of the direct harness', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-sparkshell-route-'));
    try {
      ...
      const result = runOmq(wd, ['explore', '--prompt', 'git log --oneline'], {
        OMQ_SPARKSHELL_BIN: sparkshellStub,
        OMQ_EXPLORE_BIN: harnessStub,
      });
  ...
  it('falls back to the explore harness when sparkshell backend is unavailable', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-sparkshell-fallback-'));
    try {
      const harnessStub = join(wd, 'explore-stub.sh');
      await writeFile(
        harnessStub,
      ...
      const result = runOmq(wd, ['explore', '--prompt', 'git log --oneline'], {
        OMQ_SPARKSHELL_BIN: join(wd, 'missing-sparkshell'),
        OMQ_EXPLORE_BIN: harnessStub,
      });
      if (shouldSkipForSpawnPermissions(result.error)) return;
  ...
  it('falls back to the explore harness when sparkshell is GLIBC-incompatible', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-sparkshell-glibc-'));
    try {
      const sparkshellStub = join(wd, 'sparkshell-stub.sh');
      const harnessStub = join(wd, 'explore-stub.sh');
      await writeFile(
  ...
  it('passes prompt to harness and preserves markdown stdout', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-cmd-'));
    try {
      ...
      const originalEnv = process.env.OMQ_EXPLORE_BIN;
      process.env.OMQ_EXPLORE_BIN = stub;
      const originalCwd = process.cwd();
      process.chdir(wd);
      try {
        ...
        process.chdir(originalCwd);
        if (originalEnv === undefined) delete process.env.OMQ_EXPLORE_BIN;
        else process.env.OMQ_EXPLORE_BIN = originalEnv;
        process.stdout.write = originalStdout;
        process.stderr.write = originalStderr;
      }
  ...
  it('works end-to-end through omq explore', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-e2e-'));
    try {
      const stub = join(wd, 'explore-stub.sh');
      await writeFile(
        stub,
      ...
      const result = runOmq(wd, ['explore', '--prompt', 'find auth'], { OMQ_EXPLORE_BIN: stub });
      if (shouldSkipForSpawnPermissions(result.error)) return;
      assert.equal(result.status, 0, result.stderr || result.stdout);
      assert.equal(result.stdout, '# Answer\nReady to proceed\n');
    } finally {
  ...
  it('launches an env-node qwen binary while keeping model shell commands allowlisted', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-harness-e2e-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const capturePath = join(wd, 'capture.json');
        const qwenStub = await writeEnvNodeQwenStub(wd, capturePath);
        const testPath = await createExploreTestPath(wd);

        const result = runOmq(wd, ['explore', '--prompt', 'find buildTmuxPaneCommand'], {
          OMQ_EXPLORE_QWEN_BIN: qwenStub,
          PATH: testPath,
        });
        if (shouldSkipForSpawnPermissions(result.error)) return;
        ...
        assert.match(captured, /PATH=.*omq-explore-allowlist-/);
        assert.match(captured, /SHELL=.*omq-explore-allowlist-.*\/bin\/bash$/m);
        ...
        assert.match(captured, /--BLOCKED_STDERR--[\s\S]*not on the omq explore allowlist/);
      });
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it('supports --prompt-file end-to-end with the harness', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-harness-prompt-file-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const capturePath = join(wd, 'capture.json');
        const qwenStub = await writeEnvNodeQwenStub(wd, capturePath);
        ...
        const result = runOmq(wd, ['explore', '--prompt-file', promptPath], {
          OMQ_EXPLORE_QWEN_BIN: qwenStub,
          PATH: testPath,
        });
        if (shouldSkipForSpawnPermissions(result.error)) return;
  ...
  it('preserves must-preserve facts in a long noisy summary fixture', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-fidelity-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const harnessStub = await writeExploreHarnessScenarioStub(
          wd,
          `
printf '%s\n' '# Answer' '## Critical facts' '- MUST: summary mode stayed read-only' '- MUST: blocked command stayed node --version' '- MUST: next command is omq team status <team-name>' '' '## Noise'
i=0
while [ "$i" -lt 80 ]; do
  printf '%s\n' "- distractor line $i"
  i=$((i + 1))
...
`,
        ...
        assert.match(result.stdout, /MUST: next command is omq team status <team-name>/);
      });
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it('preserves buried critical facts in adversarial noisy output', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-adversarial-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const harnessStub = await writeExploreHarnessScenarioStub(
          wd,
...
`,
  ...
  it('falls back after spark failure and preserves actionable stderr guidance', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-fallback-success-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const harnessStub = await writeExploreHarnessScenarioStub(
          wd,
          `
printf '[omq explore] spark model \`%s\` unavailable or failed (exit 17). Falling back to \`qwen3.6-plus\`.\n' "\${OMQ_EXPLORE_SPARK_MODEL:-spark-test-model}" >&2
printf '[omq explore] spark stderr: spark timed out; retry with the frontier fallback\n' >&2
...
`,
        ...
        const result = await runExploreCommandForTest(wd, ['--prompt', 'validate fallback recovery'], {
          OMQ_EXPLORE_BIN: harnessStub,
          OMQ_EXPLORE_SPARK_MODEL: 'spark-test-model',
        });
  ...
  it('reports both failed attempts with codes and final actionable stderr end-to-end', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-explore-fallback-failure-'));
    try {
      await withPackagedExploreHarnessHidden(async () => {
        const harnessStub = await writeExploreHarnessScenarioStub(
          wd,
          `
printf '[omq explore] spark model \`%s\` unavailable or failed (exit 23). Falling back to \`qwen3.6-plus\`.\n' "\${OMQ_EXPLORE_SPARK_MODEL:-spark-test-model}" >&2
printf '[omq explore] spark stderr: spark backend unavailable; install the fallback runtime\n' >&2
printf '[omq explore] both spark (\`%s\`) and fallback (\`qwen3.6-plus\`) attempts failed (codes 23 / 29). Last stderr: fallback backend unavailable; set OMQ_EXPLORE_BIN to a working harness\n' "\${OMQ_EXPLORE_SPARK_MODEL:-spark-test-model}" >&2
exit 1
`,
        );
        ...
        assert.match(
          result.stderr,
          /both spark \(`spark-test-model`\) and fallback \(`qwen3.6-plus`\) attempts failed \(codes 23 \/ 29\)\. Last stderr: fallback backend unavailable; set OMQ_EXPLORE_BIN to a working harness/,
        );
      });
    } finally {
      await rm(wd, { recursive: true, force: true });
