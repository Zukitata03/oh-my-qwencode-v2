...
describe('package bin contract', () => {
  it('declares omq with an explicit relative bin path and avoids packaging platform-specific native binaries', () => {
    const packageJsonPath = join(process.cwd(), 'package.json');
    const pkg = JSON.parse(readFileSync(packageJsonPath, 'utf-8')) as PackageJson;
    const binaryName = platform() === 'win32' ? 'omq-sparkshell.exe' : 'omq-sparkshell';
    ...
    assert.deepEqual(pkg.bin, { omq: 'dist/cli/omq.js' });
    assert.equal(pkg.scripts?.['build:explore'], 'cargo build -p omq-explore-harness');
    ...
    assert.equal(pkg.scripts?.['test:explore'], 'cargo test -p omq-explore-harness && node --test dist/cli/__tests__/explore.test.js dist/hooks/__tests__/explore-routing.test.js dist/hooks/__tests__/explore-sparkshell-guidance-contract.test.js');
    ...
    const binPath = join(process.cwd(), 'dist', 'cli', 'omq.js');
    ...
    const binEntry = results[0]?.files?.find((file) => file.path === 'dist/cli/omq.js');
    assert.ok(binEntry, 'expected npm pack output to include dist/cli/omq.js');

    const packagedHarnessPath = process.platform === 'win32' ? 'bin/omq-explore-harness.exe' : 'bin/omq-explore-harness';
    const packagedHarnessEntry = results[0]?.files?.find((file) => file.path === packagedHarnessPath);
    const packagedHarnessMetaEntry = results[0]?.files?.find((file) => file.path === 'bin/omq-explore-harness.meta.json');
    ...
    const crateManifestEntry = results[0]?.files?.find((file) => file.path === 'crates/omq-explore/Cargo.toml');
    const crateMainEntry = results[0]?.files?.find((file) => file.path === 'crates/omq-explore/src/main.rs');
    ...
    assert.ok(crateManifestEntry, 'expected npm pack output to include crates/omq-explore/Cargo.toml');
    assert.ok(crateMainEntry, 'expected npm pack output to include crates/omq-explore/src/main.rs');
  });
});
