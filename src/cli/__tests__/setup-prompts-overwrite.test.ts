...
describe('omq setup prompt/native-agent overwrite behavior', () => {
  ...
  it('installs only active/internal catalog prompts and native agents', async () => {
    ...
    try {
      ...
      const promptsDir = join(wd, '.qwen', 'prompts');
      const nativeAgentsDir = join(wd, '.qwen', 'agents');
      ...
      const codeReviewerToml = await readFile(join(wd, '.qwen', 'agents', 'code-reviewer.toml'), 'utf-8');
  ...
  it('removes stale merged/unlisted prompts on --force', async () => {
    ...
    try {
      ...
      for (const stalePrompt of stalePrompts) {
        const stalePath = join(wd, '.qwen', 'prompts', stalePrompt);
      ...
      for (const stalePrompt of stalePrompts) {
        assert.equal(existsSync(join(wd, '.qwen', 'prompts', stalePrompt)), false);
      }
      assert.equal(existsSync(join(wd, '.qwen', 'prompts', 'executor.md')), true);
    } finally {
      process.chdir(previousCwd);
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it('removes stale merged native agents on --force', async () => {
    ...
    try {
      ...
      for (const staleAgent of staleAgents) {
        const stalePath = join(wd, '.qwen', 'agents', staleAgent);
      ...
      for (const staleAgent of staleAgents) {
        assert.equal(existsSync(join(wd, '.qwen', 'agents', staleAgent)), false);
      }
      assert.equal(existsSync(join(wd, '.qwen', 'agents', 'executor.toml')), true);
    } finally {
      process.chdir(previousCwd);
      await rm(wd, { recursive: true, force: true });
    }
  ...
  it('removes stale native agents with the obsolete bridge field during normal setup', async () => {
    ...
    try {
      ...
      const stalePath = join(wd, '.qwen', 'agents', 'legacy-skill-agent.toml');
      await writeFile(
        stalePath,
        [
