...
async function writeRollout(
  ...
  const dir = join(qwenHomeDir, 'sessions', year, month, day);
  await mkdir(dir, { recursive: true });
  await writeFile(join(dir, fileName), `${lines.map((line) => JSON.stringify(line)).join('\n')}\n`, 'utf-8');
}

function runOmq(cwd: string, argv: string[], envOverrides: Record<string, string> = {}) {
...
describe('omq session search', () => {
  it('prints structured JSON results for matching transcripts', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-session-search-cli-'));
    const qwenHomeDir = join(cwd, '.qwen-home');
    try {
      await writeRollout(qwenHomeDir, '2026-03-10T12:00:00.000Z', 'rollout-2026-03-10T12-00-00-session-a.jsonl', [
        {
          type: 'session_meta',
          payload: {
            id: 'session-a',
      ...
      const result = runOmq(cwd, ['session', 'search', 'team api', '--project', 'current', '--json'], {
        QWEN_HOME: qwenHomeDir,
      });
