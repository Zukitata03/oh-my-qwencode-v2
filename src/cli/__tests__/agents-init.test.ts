...
describe('omq agents-init', () => {
  it('creates a managed root AGENTS.md plus direct-child AGENTS.md files while skipping ignored directories', async () => {
    ...
    try {
      ...
      assert.match(rootAgents, /OMQ:AGENTS-INIT:MANAGED/);
      ...
      assert.match(rootAgents, /# oh-my-qwencode - Intelligent Multi-Agent Orchestration/);
      assert.match(rootAgents, /\.\/\.qwen/);
  ...
  it('skips unmanaged AGENTS.md files by default but can adopt them with --force and a backup', async () => {
    ...
    try {
      ...
      assert.match(adopted, /OMQ:AGENTS-INIT:MANAGED/);
      const backupRoot = join(wd, '.omq', 'backups', 'agents-init');
  ...
  it('protects project-root AGENTS.md during an active OMQ session', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-agents-init-'));
    try {
      ...
      await writeFile(join(wd, 'src', 'index.ts'), 'export const x = 1;\n');
      await writeFile(
        join(wd, '.omq', 'state', 'session.json'),
        JSON.stringify({
