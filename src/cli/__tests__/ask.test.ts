...
describe('omq ask', () => {
  it('script usage documents provider-specific long flags from CLI help', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-ask-usage-'));
    try {
  ...
  it('preserves child stdout/stderr and exact non-zero exit code', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-ask-contract-'));
    try {
      const res = runOmq(wd, ['ask', 'claude', 'pass-through'], {
        OMQ_ASK_ADVISOR_SCRIPT: 'dist/scripts/fixtures/ask-advisor-stub.js',
        OMQ_ASK_STUB_STDOUT: 'artifact-path-from-stub.md\n',
        OMQ_ASK_STUB_STDERR: 'stub-warning-line\n',
        OMQ_ASK_STUB_EXIT_CODE: '7',
      });
      if (shouldSkipForSpawnPermissions(res.error)) return;
  ...
  it('resolves relative advisor override path from package root even on non-root cwd', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-ask-relative-'));
    try {
      const res = runOmq(wd, ['ask', 'gemini', 'relative-check'], {
        OMQ_ASK_ADVISOR_SCRIPT: 'dist/scripts/fixtures/ask-advisor-stub.js',
        OMQ_ASK_STUB_STDOUT: 'relative-override-ok\n',
        OMQ_ASK_STUB_EXIT_CODE: '0',
      });
      if (shouldSkipForSpawnPermissions(res.error)) return;
  ...
  it('uses package-root advisor script path from non-package cwd and still writes artifact', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-ask-nonroot-'));
    try {
      ...
      const res = runOmq(wd, ['ask', 'claude', 'non-root-default'], {
      ...
      assert.ok(artifactPath.startsWith(join(wd, '.omq', 'artifacts', 'claude-')));
  ...
  it('supports claude --print and gemini --prompt end-to-end through omq ask', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-ask-provider-flags-'));
    try {
      const fakeBin = join(wd, 'bin');
      await mkdir(fakeBin, { recursive: true });
      ...
      const claudeRes = runOmq(wd, ['ask', 'claude', '--print', 'claude-long-flag'], env);
      ...
      const geminiRes = runOmq(wd, ['ask', 'gemini', '--prompt', 'gemini-long-flag'], env);
  ...
  it('injects --agent-prompt content into final prompt while keeping Original task raw', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-ask-agent-prompt-'));
    try {
      const fakeBin = join(wd, 'bin');
      const qwenHome = join(wd, '.qwen-home');
      const promptsDir = join(qwenHome, 'prompts');
      await mkdir(fakeBin, { recursive: true });
      ...
      const res = runOmq(
        ...
        { PATH: `${fakeBin}:${process.env.PATH || ''}`, QWEN_HOME: qwenHome },
      );
      if (shouldSkipForSpawnPermissions(res.error)) return;
  ...
  it('fails clearly when --agent-prompt role is missing from prompts directory', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-ask-agent-prompt-missing-'));
    try {
      const fakeBin = join(wd, 'bin');
      const qwenHome = join(wd, '.qwen-home');
      const promptsDir = join(qwenHome, 'prompts');
