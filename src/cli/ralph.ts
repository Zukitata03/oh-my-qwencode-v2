...
export const RALPH_HELP = `omq ralph - Launch Qwen Code with ralph persistence mode active

Usage:
  omq ralph [task text...]
  omq ralph --prd "<task text>"
  omq ralph [ralph-options] [qwen-args...] [task text...]
...
PRD mode:
  Ralph initializes persistence artifacts in .omq/ so PRD and progress
  state can survive across Qwen Code sessions. Provide task text either as
  positional words or with --prd.

Common patterns:
  omq ralph "Fix flaky notify-hook tests"
  omq ralph --prd "Ship release checklist automation"
  omq ralph --model gpt-5 "Refactor state hydration"
  omq ralph -- --task-with-leading-dash
`;

const VALUE_TAKING_FLAGS = new Set(['--model', '--provider', '--config', '-c', '-i', '--images-dir']);
const RALPH_OMQ_FLAGS = new Set(['--prd', '--no-deslop']);
const RALPH_APPEND_ENV = 'OMQ_RALPH_APPEND_INSTRUCTIONS_FILE';
...
export function filterRalphQwenArgs(args: readonly string[]): string[] {
  const filtered: string[] = [];
  for (const token of args) {
    if (RALPH_OMQ_FLAGS.has(token.toLowerCase())) continue;
    filtered.push(token);
  }
  return filtered;
}
...
export function buildRalphAppendInstructions(
  ...
  return [
    ...
    'You are in OMQ Ralph persistence mode.',
    `Primary task: ${task}`,
    'Parallelism guidance:',
    '- Prefer Qwen Code native subagents for independent parallel subtasks.',
    '- Treat `.omq/state/subagent-tracking.json` as the native subagent activity ledger for this session.',
    ...
    options.noDeslop
      ...
      : `- Step 7.5 must run oh-my-qwencode:ai-slop-cleaner in standard mode on changed files only, using the repo-relative paths listed in \`${options.changedFilesPath}\`.`,
    options.noDeslop
...
async function writeRalphSessionFiles(
  ...
  await writeFile(
    instructionsPath,
    `${buildRalphAppendInstructions(task, { changedFilesPath: '.omq/ralph/changed-files.txt', noDeslop: options.noDeslop, approvedHint: options.approvedHint ?? null })}\n`,
  );
  return { instructionsPath, changedFilesPath: '.omq/ralph/changed-files.txt' };
}
...
export async function ralphCommand(args: string[]): Promise<void> {
  ...
  await updateModeState('ralph', {
    ...
    native_subagent_tracking_path: '.omq/state/subagent-tracking.json',
    native_subagent_policy: 'Parallel Qwen Code subagents are allowed for independent work, but phase completion must wait for active native subagent threads to finish.',
  ...
  console.log('[ralph] Ralph persistence mode active. Launching Qwen Code...');
  ...
  const qwenArgsBase = filterRalphQwenArgs(normalizedArgs);
  const qwenArgs = explicitTask === 'ralph-cli-launch' && approvedHint?.task
    ? [...qwenArgsBase, approvedHint.task]
    : qwenArgsBase;
