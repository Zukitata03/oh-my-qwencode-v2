...
const HELP = `
Usage:
  omq hooks init       Create .omq/hooks/sample-plugin.mjs scaffold
  omq hooks status     Show plugin directory + discovered plugins
  omq hooks validate   Validate plugin exports/signatures
  omq hooks test       Dispatch synthetic turn-complete event to plugins

Notes:
  - This command is additive. Existing \`omq tmux-hook\` behavior is unchanged.
  - Plugins are enabled by default. Disable with OMQ_HOOK_PLUGINS=0.
`;
...
async function initHooks(): Promise<void> {
  ...
  console.log('Plugins are enabled by default. Disable with OMQ_HOOK_PLUGINS=0.');
}
...
async function statusHooks(): Promise<void> {
  ...
  console.log(
    `Plugins enabled: ${isHookPluginsEnabled(process.env) ? 'yes' : 'no (disabled with OMQ_HOOK_PLUGINS=0)'}`,
  );
...
async function validateHooks(): Promise<void> {
  ...
  const plugins = await discoverHookPlugins(cwd);
  if (plugins.length === 0) {
    console.log('No plugins found. Run: omq hooks init');
    return;
  }
...
async function testHooks(): Promise<void> {
  ...
  const event = buildHookEvent('turn-complete', {
    source: 'native',
    context: {
      reason: 'omq-hooks-test',
    },
    session_id: 'omq-hooks-test',
  ...
  const logPath = join(cwd, '.omq', 'logs', `hooks-${new Date().toISOString().split('T')[0]}.jsonl`);
