/**
 * Launch-time update checks for oh-my-qwencode.
 * Non-fatal and throttled; can be disabled via OMQ_AUTO_UPDATE=0.
 */
...
const PACKAGE_NAME = 'oh-my-qwencode';
...
function updateStatePath(cwd: string): string {
  return join(cwd, '.omq', 'state', 'update-check.json');
}
...
export async function maybeCheckAndPromptUpdate(
  ...
  if (process.env.OMQ_AUTO_UPDATE === '0') return;
  ...
  const approved = await updateDependencies.askYesNo(
    `[omq] Update available: v${current} → v${latest}. Update now? [Y/n] `,
  );
  if (!approved) return;

  console.log(`[omq] Running: npm install -g ${PACKAGE_NAME}@latest`);
  const result = updateDependencies.runGlobalUpdate();

  if (result.ok) {
    await updateDependencies.setup({ force: true });
    console.log(`[omq] Updated to v${latest}. Restart to use new code.`);
  } else {
    console.log('[omq] Update failed. Run manually: npm install -g oh-my-qwencode@latest');
  }
}
