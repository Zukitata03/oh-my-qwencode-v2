...
const HELP = [
  'Usage: omq cleanup [--dry-run]',
  '',
  'Kill orphaned OMQ MCP server processes and remove stale OMQ /tmp directories left behind by previous Qwen Code App sessions.',
  '',
  'Options:',
...
const OMQ_MCP_SERVER_PATTERN = /(?:^|[\\/])dist[\\/]mcp[\\/](?:state|memory|code-intel|trace|team)-server\.(?:[cm]?js|ts)\b/i;
const QWEN_PROCESS_PATTERN = /(?:^|[\\/\s])qwen(?:\.js)?(?:\s|$)|@openai[\\/]qwen/i;
const OMQ_TMP_DIRECTORY_PATTERN = /^(omc|omq|oh-my-qwencode)-/;
...
export function isOmqMcpProcess(command: string): boolean {
  const normalized = normalizeCommand(command);
  return normalized.includes('oh-my-qwencode/dist/mcp') || OMQ_MCP_SERVER_PATTERN.test(normalized);
}
...
export function listOmqProcesses(): ProcessEntry[] {
...
function isQwenSessionProcess(command: string): boolean {
  return QWEN_PROCESS_PATTERN.test(normalizeCommand(command));
}
...
function resolveProtectedRootPid(
  ...
  while (typeof pid === 'number' && pid > 1) {
    ...
    if (command && isQwenSessionProcess(command)) return pid;
...
export function findCleanupCandidates(
  ...
  return processes
    ...
    .filter((processEntry) => isOmqMcpProcess(processEntry.command))
...
export async function cleanupOmqMcpProcesses(
  ...
  const listProcessesImpl = dependencies.listProcesses ?? listOmqProcesses;
  ...
  if (candidates.length === 0) {
    writeLine(dryRun
      ? 'Dry run: no orphaned OMQ MCP server processes found.'
      : 'No orphaned OMQ MCP server processes found.');
    return {
      dryRun,
      candidates,
      terminatedCount: 0,
  ...
  if (dryRun) {
    writeLine(`Dry run: would terminate ${candidates.length} orphaned OMQ MCP server process(es):`);
  ...
  writeLine(`Found ${candidates.length} orphaned OMQ MCP server process(es). Sending SIGTERM...`);
  ...
  writeLine(`Killed ${terminatedCount} orphaned OMQ MCP server process(es)${forceKilledCount > 0 ? ` (${forceKilledCount} required SIGKILL)` : ''}.`);
...
export async function cleanupStaleTmpDirectories(
  ...
  for (const entry of await listTmpEntries(tmpRoot)) {
    if (!entry.isDirectory() || !OMQ_TMP_DIRECTORY_PATTERN.test(entry.name)) continue;
  ...
  if (staleDirectories.length === 0) {
    writeLine(dryRun
      ? 'Dry run: no stale OMQ /tmp directories found.'
      : 'No stale OMQ /tmp directories found.');
    return 0;
  }

  const summaryTarget = formatPlural(
    staleDirectories.length,
    'stale OMQ /tmp directory',
    'stale OMQ /tmp directories',
  );
  if (dryRun) {
...
export async function cleanupCommand(
  ...
  const cleanupProcesses = dependencies.cleanupProcesses ?? cleanupOmqMcpProcesses;
