...
import { resolveQwenPane } from '../scripts/tmux-hook-engine.js';
...
const DEFAULT_CONFIG: TmuxHookConfig = {
  ...
  prompt_template: 'Continue from current mode state. [OMQ_TMUX_INJECT]',
  marker: '[OMQ_TMUX_INJECT]',
  dry_run: false,
  log_level: 'info',
...
const HELP = `
Usage:
  omq tmux-hook init       Create .omq/tmux-hook.json
  omq tmux-hook status     Show config + runtime state summary
  omq tmux-hook validate   Validate config and tmux target reachability
  omq tmux-hook test       Run a synthetic notify-hook turn (end-to-end)
`;
...
function omqDir(cwd = process.cwd()): string {
  return join(cwd, '.omq');
}

function tmuxHookConfigPath(cwd = process.cwd()): string {
  return join(omqDir(cwd), 'tmux-hook.json');
}

function tmuxHookStatePath(cwd = process.cwd()): string {
  return join(omqDir(cwd), 'state', 'tmux-hook-state.json');
}

function tmuxHookLogPath(cwd = process.cwd()): string {
  return join(omqDir(cwd), 'logs', `tmux-hook-${new Date().toISOString().split('T')[0]}.jsonl`);
}
...
async function readValidatedConfig(cwd = process.cwd()): Promise<TmuxHookConfig> {
  ...
  if (!existsSync(configPath)) {
    throw new Error('tmux-hook config missing. Run: omq tmux-hook init');
  }
  ...
  if (!existsSync(configPath)) {
    ...
    if (initResult.created) {
      ...
      if (initResult.usedPlaceholderTarget) {
        console.log('Could not auto-detect a tmux target. Edit `.omq/tmux-hook.json` when ready.');
...
function detectInitialTarget(): InitialTargetDetection | null {
  const canonicalPane = resolveQwenPane();
  if (canonicalPane) {
...
async function initTmuxHookConfig(opts?: { silent?: boolean; cwd?: string }): Promise<InitConfigResult> {
  ...
  const configPath = tmuxHookConfigPath(cwd);
  await mkdir(omqDir(cwd), { recursive: true });
...
async function testTmuxHook(args: string[]): Promise<void> {
  ...
  const payload = {
    ...
    'thread-id': threadId,
    'turn-id': turnId,
    'input-messages': ['omq tmux-hook test'],
  ...
  console.log('Check: .omq/logs/tmux-hook-YYYY-MM-DD.jsonl for skip/reason codes.');
}
