...
describe('ensureCanonicalRalphArtifacts', () => {
  it('keeps canonical files authoritative when they already exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-ralph-canonical-'));
    try {
      const canonicalPrd = join(cwd, '.omq', 'plans', 'prd-existing.md');
      const canonicalProgress = join(cwd, '.omq', 'state', 'ralph-progress.json');
      await mkdir(join(cwd, '.omq', 'plans'), { recursive: true });
      await mkdir(join(cwd, '.omq', 'state'), { recursive: true });
      ...
      await writeFile(join(cwd, '.omq', 'prd.json'), JSON.stringify({ project: 'legacy-project' }));
      await writeFile(join(cwd, '.omq', 'progress.txt'), 'legacy line\n');
  ...
  it('migrates legacy PRD/progress files one-way when canonical artifacts are absent', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-ralph-migrate-'));
    try {
      const legacyPrdPath = join(cwd, '.omq', 'prd.json');
      const legacyProgressPath = join(cwd, '.omq', 'progress.txt');
      await mkdir(join(cwd, '.omq'), { recursive: true });
      ...
      assert.match(canonicalPrd, /Migrated from legacy `.omq\/prd\.json`/);
      assert.equal(canonicalProgress.source, '.omq/progress.txt');
  ...
  it('records visual feedback with numeric and qualitative guidance for the next iteration', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-ralph-visual-feedback-'));
    try {
