...
const LEGACY_PRD_PATH = '.omq/prd.json';
const LEGACY_PROGRESS_PATH = '.omq/progress.txt';
...
async function writeMigrationMarker(
  ...
  const markerPath = join(cwd, '.omq', 'plans', 'ralph-migration-marker.json');
...
async function migrateLegacyPrdIfNeeded(
  ...
  const markdown = [
    `# ${title}`,
    '',
    '> Migrated from legacy `.omq/prd.json` (read-only compatibility import).',
    '',
    '## Migration Marker',
