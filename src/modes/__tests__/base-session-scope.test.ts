...
describe('modes/base session-scoped persistence', () => {
  it('writes mode state into the current session scope when session.json exists', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-mode-session-scope-'));
    try {
      await mkdir(join(wd, '.omq', 'state'), { recursive: true });
      await writeFile(join(wd, '.omq', 'state', 'session.json'), JSON.stringify({ session_id: 'sess-base-write' }));

      await startMode('ralplan', 'write in session scope', 5, wd);

      const scopedPath = join(wd, '.omq', 'state', 'sessions', 'sess-base-write', 'ralplan-state.json');
      assert.equal(existsSync(scopedPath), true);
      assert.equal(existsSync(join(wd, '.omq', 'state', 'ralplan-state.json')), false);
  ...
  it('prefers session-scoped reads over root fallback and writes updates back to the session scope', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-mode-session-read-'));
    try {
