...
export const DEFAULT_MARKER = '[OMQ_TMUX_INJECT]';
...
// Qwen Code agent commands — do NOT include 'claude' (that's Claude Code CLI, a different tool)
const AGENT_COMMANDS = new Set(['node', 'qwen', 'npx']);

function isHudStartCommand(startCommand: string): boolean {
  return /\bomq\b.*\bhud\b.*--watch/i.test(startCommand);
}

/**
 * Canonical qwen pane resolver. Finds the tmux pane running a qwen agent.
 *
 * Resolution order:
 * 1. TMUX_PANE env var — but only if the pane looks like a real agent pane, not HUD
 * 2. Scan all panes in the same tmux session for one started with qwen
 ...
 */
export function resolveQwenPane(): string {
  ...
  try {
    ...
    if (!SHELL_COMMANDS.has(base)) {
      ...
      // session scan so we can still reject HUD or locate a qwen pane.
    }
  } catch {
  ...
  try {
    ...
    for (const line of panes) {
      ...
      if (!paneId) continue;
      if (startCmd.includes('qwen') && !isHudStartCommand(startCmd)) {
        return paneId;
      }
    }
  } catch {
...
export function paneLooksReady(captured: any): boolean {
  ...
  const hasQwenPromptLine = lines.some((line) => /^\s*›\s*/u.test(line));
  const hasClaudePromptLine = lines.some((line) => /^\s*❯\s*/u.test(line));
  if (hasQwenPromptLine || hasClaudePromptLine) return true;

  const hasQwenWelcomePrompt = lines.some((line) => /\bhow can i help(?: you)?\b/i.test(line));
  if (hasQwenWelcomePrompt) return true;
...
export function buildSendKeysArgv({ paneTarget, prompt, dryRun, submitKeyPresses = 2 }: any): any {
  ...
  return {
    ...
    // Qwen Code generally prefers two presses; Claude typically needs one.
    submitArgv,
  };
}
