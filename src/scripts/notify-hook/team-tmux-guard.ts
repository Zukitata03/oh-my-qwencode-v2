...
export async function evaluatePaneInjectionReadiness(paneTarget: any, {
  ...
  // Canonical bypass: if resolveQwenPane confirms this is a qwen pane
  // (via pane_start_command), skip all readiness guards. The pane IS running
  // qwen even though tmux may report cmd=sh (shell wrapper).
  try {
    if (resolveQwenPane() === target) {
      return {
        ok: true,
        sent: false,
        reason: 'ok',
        paneTarget: target,
        paneCurrentCommand: 'qwen',
        paneCapture: '',
      };
    }
  } catch {
