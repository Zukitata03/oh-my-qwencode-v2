// @ts-nocheck
/**
 * Auto-nudge: detect Qwen Code "asking for permission" stall patterns and
...
function normalizeStallDetectionText(text) {
  return safeString(text)
    .replace(/\r[omq_tmux_inject]?/g, '[omq_tmux_inject]')
    .split('[omq_tmux_inject]')
    .filter((line) => !line.includes(DEFAULT_MARKER))
    .join('[omq_tmux_inject]')
    .toLowerCase()
    .replace(/[’‘`]/g, '\'');
}
...
function summarizePaneCaptureForLog(captured, maxLines = 6) {
  ...
  return lines.slice(-maxLines).join('[omq_tmux_inject]').slice(0, 600);
}
...
export async function loadAutoNudgeConfig() {
  const qwenHomePath = process.env.QWEN_HOME || join(homedir(), '.qwen');
  const configPath = join(qwenHomePath, '.omq-config.json');
...
export function detectStallPattern(text, patterns) {
  ...
  const lines = tail.split('[omq_tmux_inject]').filter((line) => line.trim());
  const hotZone = lines.slice(-3).join('[omq_tmux_inject]');
...
function resolveQwenPaneByCwdFallback(cwd) {
  ...
  try {
    ...
    for (const line of panes) {
      const [paneId, panePath = '', paneCommand = '', startCommand = ''] = line.split('[omq_tmux_inject]');
      ...
      if (/\bomq\b.*\bhud\b.*--watch/i.test(normalizedStart)) continue;
      if (normalizedStart.includes('qwen')) return paneId;
      if (normalizedCommand === 'qwen' || normalizedCommand === 'node' || normalizedCommand === 'npx') return paneId;
    }
  } catch {
...
async function resolveQwenPaneFromAnchor(anchorPane) {
  ...
  try {
    ...
    const panesResult = await runProcess(
      'tmux',
      ['list-panes', '-s', '-t', sessionName, '-F', '#{pane_id}[omq_tmux_inject]#{pane_current_command}[omq_tmux_inject]#{pane_start_command}'],
      2000,
    );
    const panes = safeString(panesResult.stdout).trim().split('[omq_tmux_inject]').filter(Boolean);
    for (const line of panes) {
      const [candidatePaneId, , rawStartCommand = ''] = line.split('[omq_tmux_inject]');
      const startCommand = safeString(rawStartCommand).toLowerCase();
      if (!candidatePaneId) continue;
      if (/\bomq\b.*\bhud\b.*--watch/i.test(startCommand)) continue;
      if (startCommand.includes('qwen')) return candidatePaneId;
    }
  } catch {
...
export async function resolveNudgePaneTarget(stateDir: any, cwd = '') {
  // Use canonical qwen pane resolver — validates pane is running an agent, not a shell
  const { resolveQwenPane } = await import('../tmux-hook-engine.js');
  const qwenPane = resolveQwenPane();
  if (qwenPane) return qwenPane;
      ...
      for (const f of files) {
        ...
        try {
          ...
          if (state && state.active && state.tmux_pane_id) {
            ...
            const upgradedPane = await resolveQwenPaneFromAnchor(anchoredPane);
  ...
  return resolveQwenPaneByCwdFallback(cwd);
}
