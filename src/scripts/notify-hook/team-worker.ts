...
export async function resolveTeamStateDirForWorker(cwd, parsedTeamWorker) {
  const explicitStateRoot = safeString(process.env.OMQ_TEAM_STATE_ROOT || '').trim();
  ...
  const leaderCwd = safeString(process.env.OMQ_TEAM_LEADER_CWD || '').trim();

  const candidateStateDirs = [];
  if (leaderCwd) {
    candidateStateDirs.push(join(resolvePath(leaderCwd), '.omq', 'state'));
  }
  candidateStateDirs.push(join(cwd, '.omq', 'state'));
...
export async function maybeNotifyLeaderAllWorkersIdle({ cwd, stateDir, logsDir, parsedTeamWorker }) {
  ...
  const nextAction = `Next: run omq team status ${teamName}, read unread worker messages, then decide whether to assign the next concrete task, reconcile results, or shut the team down.`;
  const message = `[OMQ] All ${N} worker${N === 1 ? '' : 's'} idle. ${nextAction} ${DEFAULT_MARKER}`;
...
export async function maybeNotifyLeaderWorkerIdle({ cwd, stateDir, logsDir, parsedTeamWorker }) {
  ...
  const parts = [`[OMQ] ${workerName} idle`];
