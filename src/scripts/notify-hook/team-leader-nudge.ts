...
export function resolveLeaderNudgeIntervalMs() {
  const raw = safeString(process.env.OMQ_TEAM_LEADER_NUDGE_MS || '');
...
export function resolveLeaderAllIdleNudgeCooldownMs() {
  const raw = safeString(process.env.OMQ_TEAM_LEADER_ALL_IDLE_COOLDOWN_MS || '');
  const parsed = asNumber(raw);
  // Default: 30 seconds.
...
export function resolveFallbackProgressStallThresholdMs() {
  const raw = safeString(process.env.OMQ_TEAM_PROGRESS_STALL_MS || '');
  const parsed = asNumber(raw);
...
export function resolveWorkerTurnStallThresholdMs() {
  const raw = safeString(process.env.OMQ_TEAM_WORKER_TURN_STALL_MS || '');
  const parsed = asNumber(raw);
...
function buildStatusCheckReminder(teamName) {
  return `Next: check messages; keep orchestrating; if done, gracefully shut down: omq team shutdown ${teamName}.`;
}

function buildMailboxCheckReminder(teamName) {
  return `Next: read messages; keep orchestrating; if done, gracefully shut down: omq team shutdown ${teamName}.`;
}

function buildWorkerStartEvidenceReminder(teamName, workerName) {
  return `Next: check ${workerName} msg/output, confirm task in omq team status ${teamName}, then reassign/nudge.`;
}
...
function buildLeaderActionGuidance(teamName, {
  ...
  if (leaderActionState === 'done_waiting_on_leader') {
    return `Next: decide whether to reconcile/merge results or gracefully shut down: omq team shutdown ${teamName}.`;
  }
  if (leaderActionState === 'stuck_waiting_on_leader') {
    return `Next: omq team status ${teamName}; read worker messages; unblock/reassign or shutdown.`;
  }
...
export async function emitTeamNudgeEvent(cwd, teamName, reason, nowIso) {
  const eventsDir = join(cwd, '.omq', 'state', 'team', teamName, 'events');
...
export async function maybeNudgeTeamLeader({ cwd, stateDir, logsDir, preComputedLeaderStale }) {
  ...
  for (const teamName of candidateTeamNames) {
    ...
    let workers = [];
    try {
      const manifestPath = join(omqDir, 'state', 'team', teamName, 'manifest.v2.json');
      const configPath = join(omqDir, 'state', 'team', teamName, 'config.json');
    ...
    let mailbox = null;
    try {
      const mailboxPath = join(omqDir, 'state', 'team', teamName, 'mailbox', 'leader-fixed.json');
    ...
    if (shouldSendAllIdleNudge) {
      ...
      const waitingText = leaderActionState === 'done_waiting_on_leader'
        ...
        : leaderActionState === 'stuck_waiting_on_leader'
          ...
          : '';
      text = `[OMQ] All ${N} worker${N === 1 ? '' : 's'} idle.${waitingText} ${leaderActionGuidance}`;
