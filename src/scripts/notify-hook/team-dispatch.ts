 ...
 * Disable entirely with OMQ_RUNTIME_BRIDGE=0.
 */
function runtimeExec(command) {
  if (process.env.OMQ_RUNTIME_BRIDGE === '0') return;
  try {
...
const ISSUE_DISPATCH_COOLDOWN_ENV = 'OMQ_TEAM_DISPATCH_ISSUE_COOLDOWN_MS';
const DEFAULT_DISPATCH_TRIGGER_COOLDOWN_MS = 30 * 1000;
const DISPATCH_TRIGGER_COOLDOWN_ENV = 'OMQ_TEAM_DISPATCH_TRIGGER_COOLDOWN_MS';
...
async function injectDispatchRequest(request, config, cwd) {
  ...
  for (let round = 0; round < INJECT_VERIFY_ROUNDS; round++) {
    ...
    try {
      ...
      // This avoids false confirmations when Qwen Code leaves the unsent draft just
...
export async function drainPendingTeamDispatch({
  cwd,
  stateDir = join(cwd, '.omq', 'state'),
  logsDir = join(cwd, '.omq', 'logs'),
  maxPerTick = 5,
  injector = injectDispatchRequest,
} = {}) {
  if (safeString(process.env.OMQ_TEAM_WORKER)) {
