...
function buildTmuxSessionName(cwd: any, sessionId: any): string {
  ...
  const dirToken = parentDir.endsWith('.omq-worktrees')
    ? sanitizeTmuxToken(`${parentDir.slice(0, -'.omq-worktrees'.length)}-${dirName}`)
  ...
  const sessionToken = sanitizeTmuxToken(safeString(sessionId).replace(/^omq-/, ''));
