...
function usage(): string {
  return [
    ...
    '',
    'Creates an npm tarball, installs it into an isolated prefix, and smoke tests the installed omq CLI.',
...
async function main(): Promise<void> {
  ...
  const tempRoot = mkdtempSync(join(tmpdir(), 'omq-packed-install-'));
  ...
  try {
    ...
    const omqPath = join(prefixDir, process.platform === 'win32' ? '' : 'bin', npmBinName('omq'));
    for (const argv of PACKED_INSTALL_SMOKE_CORE_COMMANDS) {
      run(omqPath, argv, { cwd: repoRoot });
    }
