...
const maxLifetimeMs = runOnce
  ...
  : Math.max(
    pollMs,
    asNumber(
      argValue('--max-lifetime-ms', process.env.OMQ_NOTIFY_FALLBACK_MAX_LIFETIME_MS || String(defaultMaxLifetimeMs)),
...
let lastDispatchDrain: DispatchDrainState = {
  leader_only: safeString(process.env.OMQ_TEAM_WORKER || '').trim() === '',
  last_tick_at: null,
  last_result: null,
  last_error: null,
};
...
const AUTO_NUDGE_STALL_MS = Math.max(
  pollMs,
  asNumber(process.env.OMQ_NOTIFY_FALLBACK_AUTO_NUDGE_STALL_MS || '5000', 5000),
);
...
async function runLeaderNudgeTick(): Promise<void> {
  ...
  const leaderOnly = safeString(process.env.OMQ_TEAM_WORKER || '').trim() === '';
