...
Available showcases:
  omq-self       -> missions/in-action-cat-shellout-demo
  ...
  all            -> omq-self ml-tabular bayesopt latent sorting

Examples:
  scripts/run-autoresearch-showcase.sh --list
  scripts/run-autoresearch-showcase.sh bayesopt
  scripts/run-autoresearch-showcase.sh omq-self ml-tabular
USAGE
}

mission_for() {
  case "$1" in
    omq-self)   printf '%s' 'missions/in-action-cat-shellout-demo' ;;
...
args=("$@")
if [[ " ${args[*]} " == *" all "* ]]; then
  args=(omq-self ml-tabular bayesopt latent sorting)
fi
