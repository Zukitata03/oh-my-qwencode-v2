...
/**
 * oh-my-qwencode Notification Hook
 * Qwen Code fires this after each agent turn via the `notify` config.
...
async function main() {
  ...
  const teamWorkerEnv = process.env.OMQ_TEAM_WORKER; // e.g., "fix-ts/worker-1"
  ...
  if (!isTeamWorker) {
    const metricsPath = join(omqDir, 'metrics.json');
    try {
      let metrics = {
        total_turns: 0,
        session_turns: 0,
  ...
  // 4. Write HUD state summary for `omq hud` (lead session only)
  if (!isTeamWorker) {
  ...
  // 9. Auto-nudge: detect Qwen Code stall patterns and automatically send a continuation prompt.
