...
test('resolveReplyListenerLiveEnv stays disabled until explicitly opted in', () => {
  const result = resolveReplyListenerLiveEnv({
    OMQ_DISCORD_NOTIFIER_BOT_TOKEN: 'discord-token',
    OMQ_DISCORD_NOTIFIER_CHANNEL: 'channel-1',
    OMQ_TELEGRAM_BOT_TOKEN: 'telegram-token',
    OMQ_TELEGRAM_CHAT_ID: 'chat-1',
  });
