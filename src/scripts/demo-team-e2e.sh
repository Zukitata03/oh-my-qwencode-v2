#!/usr/bin/env bash
#
# OMQ Team E2E Demo Script
#
# This script demonstrates the tmux-based multi-agent orchestration system
# with mixed Qwen Code/Claude workers. It performs a complete end-to-end test
...
#   OMQ_TEAM_WORKER_CLI             Worker CLI mode (default: auto)
#   OMQ_TEAM_WORKER_CLI_MAP         Comma-separated CLI assignments per worker
#   OMQ_TEAM_WORKER_LAUNCH_ARGS     Arguments passed to worker CLIs
#
# Example:
...
build_default_cli_map() {
  local count="$1"
  # More claude than qwen for mixed demos (emphasizes claude workers)
  # For 6 workers: 2 qwen, 4 claude
...
if [[ -n "${OMQ_TEAM_WORKER_CLI_MAP:-}" ]]; then
  IFS=',' read -ra CLI_MAP_ENTRIES <<< "$OMQ_TEAM_WORKER_CLI_MAP"
  if (("${#CLI_MAP_ENTRIES[@]}" != WORKER_COUNT)); then
    echo "error: OMQ_TEAM_WORKER_CLI_MAP has ${#CLI_MAP_ENTRIES[@]} entries but WORKER_COUNT is $WORKER_COUNT" >&2
    exit 1
  fi
fi
...
echo "OMQ_TEAM_WORKER_LAUNCH_ARGS=$OMQ_TEAM_WORKER_LAUNCH_ARGS"
...
omq team api mailbox-mark-notified --input "$MARK_INPUT" --json >/dev/null
omq team api mailbox-mark-delivered --input "$MARK_INPUT" --json >/dev/null
