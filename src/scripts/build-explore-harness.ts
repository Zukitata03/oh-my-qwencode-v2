...
const binaryName = process.platform === 'win32' ? 'omq-explore-harness.exe' : 'omq-explore-harness';
const sourcePath = join(root, 'target', 'release', binaryName);
const outputPath = join(binDir, binaryName);
const metadataPath = join(binDir, 'omq-explore-harness.meta.json');

const build = spawnSync('cargo', ['build', '--release', '-p', 'omq-explore-harness'], {
  cwd: root,
  encoding: 'utf-8',
