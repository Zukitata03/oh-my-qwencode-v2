export const OMQ_NOTIFY_TEMP_ENV = 'OMQ_NOTIFY_TEMP';
export const OMQ_NOTIFY_TEMP_CONTRACT_ENV = 'OMQ_NOTIFY_TEMP_CONTRACT';
...
export function parseNotifyTempContractFromArgs(
  ...
  const envActivated = env[OMQ_NOTIFY_TEMP_ENV] === '1';
...
export function isNotifyTempEnvActive(env: NodeJS.ProcessEnv = process.env): boolean {
  return env[OMQ_NOTIFY_TEMP_ENV] === '1';
}

export function readNotifyTempContractFromEnv(
  env: NodeJS.ProcessEnv = process.env,
): NotifyTempContract | null {
  const raw = env[OMQ_NOTIFY_TEMP_CONTRACT_ENV];
  if (!raw) return null;
  try {
