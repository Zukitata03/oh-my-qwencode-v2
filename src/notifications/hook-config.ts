 ...
 *
 * Reads hookTemplates from .omq-config.json for user-customizable message templates.
 * Config is stored under the notifications.hookTemplates key in qwenHome()/.omq-config.json.
 * Env var OMQ_HOOK_CONFIG overrides to a separate file path.
 */
 ...
 *
 * Primary source: notifications.hookTemplates key in qwenHome()/.omq-config.json
 * Env var override: OMQ_HOOK_CONFIG points to a separate file containing the
...
export function getHookConfig(): HookNotificationConfig | null {
  ...
  const envOverridePath = process.env.OMQ_HOOK_CONFIG;
  ...
  // Primary: read from notifications.hookTemplates in .omq-config.json
  const OMQ_CONFIG_PATH = join(qwenHome(), ".omq-config.json");
  if (!existsSync(OMQ_CONFIG_PATH)) {
    cachedConfig = null;
    return null;
  }

  try {
    const raw = JSON.parse(readFileSync(OMQ_CONFIG_PATH, "utf-8"));
 ...
 * - hook event `enabled: false` overrides .omq-config.json event `enabled: true`
 * - Platform credentials are NOT affected (they stay in .omq-config.json)
 */
