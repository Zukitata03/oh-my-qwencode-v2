 ...
 *
 * Reads notification config from .omq-config.json and provides
...
const CONFIG_FILE = join(qwenHome(), ".omq-config.json");
...
export function buildConfigFromEnv(): FullNotificationConfig | null {
  ...
  const discordMention = validateMention(process.env.OMQ_DISCORD_MENTION);

  const discordBotToken = process.env.OMQ_DISCORD_NOTIFIER_BOT_TOKEN;
  const discordChannel = process.env.OMQ_DISCORD_NOTIFIER_CHANNEL;
  ...
  const discordWebhook = process.env.OMQ_DISCORD_WEBHOOK_URL;
  if (discordWebhook) {
    config.discord = {
      enabled: true,
  ...
  const telegramToken =
    process.env.OMQ_TELEGRAM_BOT_TOKEN ||
    process.env.OMQ_TELEGRAM_NOTIFIER_BOT_TOKEN;
  const telegramChatId =
    process.env.OMQ_TELEGRAM_CHAT_ID ||
    process.env.OMQ_TELEGRAM_NOTIFIER_CHAT_ID ||
    process.env.OMQ_TELEGRAM_NOTIFIER_UID;
  ...
  const slackWebhook = process.env.OMQ_SLACK_WEBHOOK_URL;
  if (slackWebhook) {
    const slackMention = validateSlackMention(process.env.OMQ_SLACK_MENTION);
    config.slack = {
      enabled: true,
 ...
 *   2. OMQ_NOTIFY_PROFILE environment variable
...
export function resolveProfileConfig(
  ...
  const name =
    profileName ||
    process.env.OMQ_NOTIFY_PROFILE ||
...
export function getActiveProfileName(): string | null {
  if (process.env.OMQ_NOTIFY_PROFILE) {
    return process.env.OMQ_NOTIFY_PROFILE;
  }
  const raw = readRawConfig();
  if (!raw) return null;
  const notifications = raw.notifications as NotificationsBlock | undefined;
...
export function getNotificationConfig(
  ...
  if (raw) {
    ...
    if (notifications) {
      ...
      const envMention = validateMention(process.env.OMQ_DISCORD_MENTION);
      if (envMention) {
...
export function getVerbosity(config: FullNotificationConfig | null): VerbosityLevel {
  const envVal = process.env.OMQ_NOTIFY_VERBOSITY as string | undefined;
...
export function getReplyConfig(): import("./types.js").ReplyConfig | null {
  ...
  const enabled = process.env.OMQ_REPLY_ENABLED === "true" || replyRaw?.enabled === true;
  if (!enabled) return null;

  const authorizedDiscordUserIds = parseDiscordUserIds(
    process.env.OMQ_REPLY_DISCORD_USER_IDS,
  ...
  if (hasDiscordBot && authorizedDiscordUserIds.length === 0) {
    console.warn(
      ...
      "Set OMQ_REPLY_DISCORD_USER_IDS or add to .omq-config.json notifications.reply.authorizedDiscordUserIds"
    );
  }

  const pollIntervalMs = normalizeInteger(
    parseIntegerInput(process.env.OMQ_REPLY_POLL_INTERVAL_MS)
  ...
  const rateLimitPerMinute = normalizeInteger(
    parseIntegerInput(process.env.OMQ_REPLY_RATE_LIMIT)
  ...
  return {
    ...
    maxMessageLength,
    rateLimitPerMinute,
    includePrefix: process.env.OMQ_REPLY_INCLUDE_PREFIX !== "false" && (replyRaw?.includePrefix !== false),
