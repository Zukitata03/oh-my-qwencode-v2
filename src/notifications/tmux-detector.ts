 ...
 * Qwen Code, and inject text into panes. Used by the reply-listener daemon.
 */
...
export function analyzePaneContent(content: string): PaneAnalysis {
  ...
  return { hasQwen, hasRateLimitMessage, isBlocked, confidence: Math.min(confidence, 1) };
}
...
export function buildSendPaneArgvs(
  ...
  if (pressEnter) {
    // Qwen Code uses raw input mode; send C-m (carriage return) twice
 ...
 * Source of truth: Rust runtime's submit_presses_for_worker_cli (Claude=1, Qwen Code/Other=2).
...
export function getSubmitPresses(workerCli: string): number {
  if (process.env.OMQ_RUNTIME_BRIDGE === '0') {
    return workerCli.toLowerCase() === 'claude' ? 1 : 2;
  }
  // Rust-owned mapping: Claude=1, Qwen Code/Other=2
  return workerCli.toLowerCase() === 'claude' ? 1 : 2;
}
