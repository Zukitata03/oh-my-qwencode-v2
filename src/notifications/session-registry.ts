 ...
 *
 * Registry location: ~/.omq/state/reply-session-registry.jsonl (global, not worktree-local)
...
const REGISTRY_PATH = join(homedir(), '.omq', 'state', 'reply-session-registry.jsonl');
const REGISTRY_LOCK_PATH = join(homedir(), '.omq', 'state', 'reply-session-registry.lock');
