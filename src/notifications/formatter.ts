...
/** OMQ UI chrome: spinner/progress indicator characters */
...
/** OMQ HUD status lines: [OMQ#...] or [OMQ] (unversioned) */
const OMQ_HUD_RE = /\[OMQ[#\]]/;
...
export function parseTmuxTail(raw: string): string {
  ...
  for (const line of raw.split("\n")) {
    ...
    if (OMQ_HUD_RE.test(trimmed)) continue;
...
export function formatSessionIdle(payload: FullNotificationPayload): string {
  ...
  lines.push(`Qwen Code has finished and is waiting for input.`);
  lines.push("");
...
export function formatAskUserQuestion(payload: FullNotificationPayload): string {
  ...
  lines.push(`Qwen Code is waiting for your response.`);
  lines.push("");
  lines.push(buildFooter(payload, true));
