...
describe('formatSessionIdle', () => {
  it('should include idle header and waiting message', () => {
    ...
    assert.ok(result.includes('Qwen Code has finished and is waiting for input.'));
  });
...
describe('formatAskUserQuestion', () => {
  it('should include question text', () => {
    ...
    assert.ok(result.includes('Qwen Code is waiting for your response.'));
  });
});
...
describe('parseTmuxTail', () => {
  ...
  it('keeps wrapped Korean continuation lines in the same block', () => {
    const raw = [
      ...
      'block 10: 2. 아예 ~/.qwen/.omq-config.json에서 조절 가능하',
      '  게 로컬 패치하기',
    ].join('\n');

    const result = parseTmuxTail(raw);
    assert.ok(result.includes('block 10: 2. 아예 ~/.qwen/.omq-config.json에서 조절 가능하'));
    assert.ok(result.includes('  게 로컬 패치하기'));
    assert.ok(
      result.indexOf('block 10: 2. 아예 ~/.qwen/.omq-config.json에서 조절 가능하') <
  ...
  it('removes OMQ HUD status lines', () => {
    const raw = [
      '[OMQ#3] ultrawork active',
      '[OMQ] idle',
      'Normal output line',
    ].join('\n');
    const result = parseTmuxTail(raw);
    assert.ok(!result.includes('[OMQ'));
