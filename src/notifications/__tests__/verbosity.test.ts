...
describe('getVerbosity', () => {
  const origEnv = process.env.OMQ_NOTIFY_VERBOSITY;

  afterEach(() => {
    if (origEnv === undefined) {
      delete process.env.OMQ_NOTIFY_VERBOSITY;
    } else {
      process.env.OMQ_NOTIFY_VERBOSITY = origEnv;
    }
  });
