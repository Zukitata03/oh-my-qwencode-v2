...
describe('loadNotificationConfig', () => {
  it('returns null when config file does not exist', async () => {
    const fakePath = join(tmpdir(), `omq-test-${randomUUID()}`);
    const config = await loadNotificationConfig(fakePath);
    assert.equal(config, null);
  });

  it('returns parsed config when file exists', async () => {
    const tmpDir = join(tmpdir(), `omq-test-${randomUUID()}`);
    const omqDir = join(tmpDir, '.omq');
    ...
    writeFileSync(join(omqDir, 'notifications.json'), JSON.stringify(configData));
  ...
  it('returns null for invalid JSON', async () => {
    ...
    const omqDir = join(tmpDir, '.omq');
    mkdirSync(omqDir, { recursive: true });
    writeFileSync(join(omqDir, 'notifications.json'), 'not-json');
