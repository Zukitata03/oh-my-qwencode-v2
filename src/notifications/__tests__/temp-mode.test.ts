...
let tempQwenHome: string;

async function writeQwenConfig(contents: unknown): Promise<void> {
  await mkdir(tempQwenHome, { recursive: true });
  await writeFile(join(tempQwenHome, '.omq-config.json'), JSON.stringify(contents, null, 2));
}
...
describe('notification temp mode', () => {
  beforeEach(async () => {
    ...
    tempQwenHome = await mkdtemp(join(tmpdir(), 'omq-notify-temp-'));
  ...
  it('temp contract bypasses persistent file/profile routing', async () => {
    ...
    process.env.OMQ_NOTIFY_PROFILE = 'file-profile';
    process.env.OMQ_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/temp-only';
    process.env.OMQ_NOTIFY_TEMP_CONTRACT = JSON.stringify({
      active: true,
      selectors: ['slack'],
  ...
  it('shouldDispatchOpenClaw enforces temp-mode explicit selection and gateway matching', async () => {
    ...
    assert.equal(
      await shouldDispatchOpenClaw('session-end', activeWithOpenClaw, { OMQ_OPENCLAW: '0', QWEN_HOME: tempQwenHome }),
      false,
    );
  });
});
