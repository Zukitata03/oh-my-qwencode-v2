...
describe('analyzePaneContent', () => {
  it('returns zero confidence for empty content', () => {
    ...
    assert.equal(result.hasQwen, false);
    assert.equal(result.hasRateLimitMessage, false);
    assert.equal(result.isBlocked, false);
    assert.equal(result.confidence, 0);
  });

  it('detects "qwen" keyword', () => {
    const result = analyzePaneContent('Running Qwen Code agent...');
    assert.equal(result.hasQwen, true);
    assert.ok(result.confidence >= 0.5);
  });

  it('detects "omq" keyword', () => {
    const result = analyzePaneContent('omq session started');
    assert.equal(result.hasQwen, true);
  });

  it('detects "oh-my-qwencode" keyword', () => {
    const result = analyzePaneContent('oh-my-qwencode v1.0');
    assert.equal(result.hasQwen, true);
  });
  ...
  it('is case insensitive', () => {
    const result = analyzePaneContent('QWEN RUNNING');
    assert.equal(result.hasQwen, true);
  });
  ...
  it('gives high confidence for qwen content with prompt chars', () => {
    const result = analyzePaneContent('Qwen Code > Running agent task...');
    assert.equal(result.hasQwen, true);
    // qwen=0.5, >=0.1, agent/task=0.1, non-empty=0.1 = 0.8
    assert.ok(result.confidence >= 0.7, `Expected confidence >= 0.7, got ${result.confidence}`);
  });

  it('caps confidence at 1.0', () => {
    const result = analyzePaneContent('Qwen Code $ > agent task running omq');
    assert.ok(result.confidence <= 1.0);
  });

  it('gives some confidence for non-empty non-qwen content', () => {
