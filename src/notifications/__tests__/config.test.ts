...
const ENV_KEYS = [
  ...
  'OMQ_DISCORD_WEBHOOK_URL',
  'OMQ_DISCORD_MENTION',
  'OMQ_TELEGRAM_BOT_TOKEN',
  'OMQ_TELEGRAM_NOTIFIER_BOT_TOKEN',
  'OMQ_TELEGRAM_CHAT_ID',
  'OMQ_TELEGRAM_NOTIFIER_CHAT_ID',
  'OMQ_TELEGRAM_NOTIFIER_UID',
  'OMQ_SLACK_WEBHOOK_URL',
  'OMQ_REPLY_ENABLED',
...
describe('buildConfigFromEnv', () => {
  ...
  it('builds discord-bot config from env vars', () => {
    process.env.OMQ_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.OMQ_DISCORD_NOTIFIER_CHANNEL = '123456';
  ...
  it('includes validated mention in discord-bot config', () => {
    process.env.OMQ_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.OMQ_DISCORD_NOTIFIER_CHANNEL = '123456';
    process.env.OMQ_DISCORD_MENTION = '<@12345678901234567>';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('rejects invalid mention in env var', () => {
    process.env.OMQ_DISCORD_NOTIFIER_BOT_TOKEN = 'test-token';
    process.env.OMQ_DISCORD_NOTIFIER_CHANNEL = '123456';
    process.env.OMQ_DISCORD_MENTION = '@everyone';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds discord webhook config from env var', () => {
    process.env.OMQ_DISCORD_WEBHOOK_URL = 'https://discord.com/api/webhooks/test';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds telegram config from env vars', () => {
    process.env.OMQ_TELEGRAM_BOT_TOKEN = '123:abc';
    process.env.OMQ_TELEGRAM_CHAT_ID = '999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds slack config from env var', () => {
    process.env.OMQ_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/test';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('uses OMQ_TELEGRAM_NOTIFIER_BOT_TOKEN as fallback', () => {
    process.env.OMQ_TELEGRAM_NOTIFIER_BOT_TOKEN = '123:fallback';
    process.env.OMQ_TELEGRAM_CHAT_ID = '999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('uses OMQ_TELEGRAM_NOTIFIER_UID as fallback for chat ID', () => {
    process.env.OMQ_TELEGRAM_BOT_TOKEN = '123:abc';
    process.env.OMQ_TELEGRAM_NOTIFIER_UID = 'uid-999';
    const config = buildConfigFromEnv();
    assert.ok(config);
  ...
  it('builds config with multiple platforms from env', () => {
    ...
    process.env.OMQ_DISCORD_NOTIFIER_CHANNEL = 'channel-123';
    process.env.OMQ_TELEGRAM_BOT_TOKEN = '456:tg';
    process.env.OMQ_TELEGRAM_CHAT_ID = 'chat-789';
    process.env.OMQ_SLACK_WEBHOOK_URL = 'https://hooks.slack.com/services/test';
