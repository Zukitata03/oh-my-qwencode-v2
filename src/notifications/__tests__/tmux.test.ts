...
describe('getTeamTmuxSessions - session matching', () => {
  ...
  function makeFakeTmux(sessions: string[]): void {
    const fakeBinDir = mkdtempSync(join(tmpdir(), 'omq-tmux-team-test-'));
  ...
  it('returns canonical session name (omq-team-alpha) for team "alpha"', () => {
    makeFakeTmux(['omq-team-alpha', 'other-session']);
    assert.deepEqual(getTeamTmuxSessions('alpha'), ['omq-team-alpha']);
  });

  it('returns prefixed worker sessions (omq-team-alpha-worker1)', () => {
    makeFakeTmux(['omq-team-alpha-worker1', 'omq-team-alpha-worker2']);
    assert.deepEqual(getTeamTmuxSessions('alpha'), ['omq-team-alpha-worker1', 'omq-team-alpha-worker2']);
  });

  it('does NOT return sessions for a different team', () => {
    makeFakeTmux(['omq-team-beta', 'omq-team-beta-worker1']);
...
describe('captureTmuxPane', () => {
  ...
  it('captures output for valid pane id and sanitizes/clamps lines', () => {
    const fakeBinDir = mkdtempSync(join(tmpdir(), 'omq-tmux-test-'));
    tmpDirs.push(fakeBinDir);
    const tmuxPath = join(fakeBinDir, 'tmux');
    writeFileSync(
      tmuxPath,
