/**
 * Notification system for oh-my-qwencode
...
/**
 * Load notification config from .omq/notifications.json
 */
export async function loadNotificationConfig(projectRoot?: string): Promise<NotificationConfig | null> {
  const configPath = join(projectRoot || process.cwd(), '.omq', 'notifications.json');
...
export function _buildDesktopArgs(
  ...
  if (platform === 'darwin') {
    ...
    const ps =
      ...
      `[Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('oh-my-qwencode').Show($xml)`;
...
async function sendDiscordNotification(payload: NotificationPayload, webhookUrl: string): Promise<void> {
  ...
  const body = JSON.stringify({
    embeds: [{
      title: `[OMQ] ${payload.title}`,
      description: payload.message,
      color: colorMap[payload.type],
      footer: { text: `oh-my-qwencode | ${payload.mode || 'general'}` },
