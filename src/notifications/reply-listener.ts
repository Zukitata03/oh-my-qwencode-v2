...
const DEFAULT_STATE_DIR = join(homedir(), '.omq', 'state');
...
const REPLY_ACK_PREFIX = 'Injected into Qwen Code session.';
...
function injectReply(
  ...
  if (analysis.confidence < 0.4) {
    log(`WARN: Pane ${paneId} does not appear to be running Qwen Code (confidence: ${analysis.confidence}). Skipping injection, removing stale mapping.`);
