 ...
 *
 * Config key : notifications.dispatchCooldownSeconds in ~/.qwen/.omq-config.json
 * Env var    : OMQ_DISPATCH_COOLDOWN_SECONDS  (overrides config)
 * State file : .omq/state/dispatch-notif-cooldown.json
 ...
 *
 * Resolution order:
 *   1. OMQ_DISPATCH_COOLDOWN_SECONDS env var
 *   2. notifications.dispatchCooldownSeconds in ~/.qwen/.omq-config.json
 *   3. Default: 60 seconds
 */
export function getDispatchNotificationCooldownSeconds(): number {
  // 1. Environment variable override
  const envVal = process.env.OMQ_DISPATCH_COOLDOWN_SECONDS;
  if (envVal !== undefined) {
  ...
  // 2. Config file
  try {
    const configPath = join(qwenHome(), '.omq-config.json');
