 ...
 *
 * Config key : notifications.idleCooldownSeconds in ~/.qwen/.omq-config.json
 * Env var    : OMQ_IDLE_COOLDOWN_SECONDS  (overrides config)
 * State file : .omq/state/idle-notif-cooldown.json
 ...
 *
 * Resolution order:
 *   1. OMQ_IDLE_COOLDOWN_SECONDS env var
 *   2. notifications.idleCooldownSeconds in ~/.qwen/.omq-config.json
 *   3. Default: 60 seconds
 */
export function getIdleNotificationCooldownSeconds(): number {
  // 1. Environment variable override
  const envVal = process.env.OMQ_IDLE_COOLDOWN_SECONDS;
  if (envVal !== undefined) {
