 ...
 *
 * Note: "agent-call" event is not supported in OMQ (Qwen Code does not emit it).
 */
const DEFAULT_TEMPLATES: Record<NotificationEvent, string> = {
  ...
  "session-idle":
    "# Session Idle\n\n" +
    "Qwen Code has finished and is waiting for input.\n" +
    "{{#if reason}}\n**Reason:** {{reason}}{{/if}}" +
    "{{#if modesDisplay}}\n**Modes:** {{modesDisplay}}{{/if}}" +
    "{{tmuxTailBlock}}" +
    "\n\n{{footer}}",
  ...
  "ask-user-question":
    "# Input Needed\n" +
    "{{#if question}}\n**Question:** {{question}}\n{{/if}}" +
    "\nQwen Code is waiting for your response.\n\n{{footer}}",
};
