...
const OMQ_JOBS_DIR = join(homedir(), '.omq', 'team-jobs');
...
async function writeJobFiles(
  ...
  await mkdir(OMQ_JOBS_DIR, { recursive: true });
  await writeFile(join(OMQ_JOBS_DIR, `${jobId}.json`), JSON.stringify(job));
  await writeFile(join(OMQ_JOBS_DIR, `${jobId}-panes.json`), JSON.stringify(panes));
}

async function cleanupJobFiles(jobId: string): Promise<void> {
  await rm(join(OMQ_JOBS_DIR, `${jobId}.json`), { force: true });
  await rm(join(OMQ_JOBS_DIR, `${jobId}-panes.json`), { force: true });
}

async function loadTeamServer() {
  process.env.OMQ_TEAM_SERVER_DISABLE_AUTO_START = '1';
...
describe('team-server cleanup hardening', () => {
  it('intersects live-session candidates with team config + panes file identities before kill', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-team-cleanup-identity-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omq-team-cleanup-bin-'));
...
`,
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'omq-team-cleanup-team';
  ...
  it('does not broad-sweep session panes during cleanup target selection', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-team-cleanup-sweep-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omq-team-cleanup-sweep-bin-'));
    const tmuxLogPath = join(fakeBinDir, 'tmux.log');
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
    const jobId = `omq-${Date.now().toString(36)}`;
...
`,
      ...
      assert.ok(config);
      if (!config) return;
      config.tmux_session = 'omq-team-cleanup-sweep';
      config.workers[0]!.pane_id = '%2';
      await saveTeamConfig(config, cwd);
  ...
  it('returns unchanged legacy content[0].text plus additive structured JSON in content[1].text', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-team-cleanup-legacy-'));
    const fakeBinDir = await mkdtemp(join(tmpdir(), 'omq-team-cleanup-legacy-bin-'));
    const tmuxStubPath = join(fakeBinDir, 'tmux');
    const previousPath = process.env.PATH;
    const jobId = `omq-${Date.now().toString(36)}`;
  ...
  it('returns deterministic noop summary when no killable panes remain', async () => {
    ...
    try {
      await mkdir(OMQ_JOBS_DIR, { recursive: true });
      await writeFile(join(OMQ_JOBS_DIR, `${jobId}.json`), JSON.stringify({
        status: 'running',
        startedAt: Date.now(),
      }));
      await writeFile(join(OMQ_JOBS_DIR, `${jobId}-panes.json`), JSON.stringify({
        paneIds: [],
        leaderPaneId: '',
      }));
