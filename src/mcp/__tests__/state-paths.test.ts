...
describe('state paths', () => {
  ...
  it('enforces OMQ_MCP_WORKDIR_ROOTS allowlist when configured', async () => {
    const allowedRoot = await mkdtemp(join(tmpdir(), 'omq-allowed-root-'));
    const disallowedRoot = await mkdtemp(join(tmpdir(), 'omq-disallowed-root-'));
    const prev = process.env.OMQ_MCP_WORKDIR_ROOTS;
    process.env.OMQ_MCP_WORKDIR_ROOTS = allowedRoot;
    try {
      assert.equal(
      ...
      assert.throws(
        ...
        /outside allowed roots \(OMQ_MCP_WORKDIR_ROOTS\)/,
      );
    } finally {
      if (typeof prev === 'string') process.env.OMQ_MCP_WORKDIR_ROOTS = prev;
      else delete process.env.OMQ_MCP_WORKDIR_ROOTS;
  ...
  it('builds global state paths', () => {
    ...
    assert.equal(base, '/repo/.omq/state');
    assert.equal(getStateDir('/repo'), '/repo/.omq/state');
    assert.equal(getStatePath('team', '/repo'), '/repo/.omq/state/team-state.json');
  });

  it('builds session state paths', () => {
    assert.equal(getStateDir('/repo', 'sess1'), '/repo/.omq/state/sessions/sess1');
    assert.equal(
      getStatePath('ralph', '/repo', 'sess1'),
      '/repo/.omq/state/sessions/sess1/ralph-state.json'
    );
  });
  ...
  it('enumerates global-only path', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-state-paths-'));
    try {
