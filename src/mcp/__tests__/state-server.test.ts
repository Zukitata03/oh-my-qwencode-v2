...
describe('state-server directory initialization', () => {
  it('creates .omq/state for state tools without setup', async () => {
    process.env.OMQ_STATE_SERVER_DISABLE_AUTO_START = '1';
    const { handleStateToolCall } = await import('../state-server.js');

    const wd = await mkdtemp(join(tmpdir(), 'omq-state-server-test-'));
    try {
      const stateDir = join(wd, '.omq', 'state');
      const tmuxHookConfig = join(wd, '.omq', 'tmux-hook.json');
  ...
  it('bootstraps state-tool tmux-hook from the current tmux pane when available', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'omq-state-server-test-live-'));
    try {
      const tmuxHookConfig = join(wd, '.omq', 'tmux-hook.json');
    ...
    try {
      ...
      assert.deepEqual(
        JSON.parse(writeResponse.content[0]?.text || '{}'),
        {
          success: true,
          mode: 'deep-interview',
          path: join(wd, '.omq', 'state', 'deep-interview-state.json'),
        },
      );
  ...
  it('creates session-scoped state directory when session_id is provided', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'omq-state-server-test-'));
    try {
      const sessionDir = join(wd, '.omq', 'state', 'sessions', 'sess1');
  ...
  it('serializes concurrent state_write calls per mode file and preserves merged fields', async () => {
    ...
    try {
      ...
      const filePath = join(wd, '.omq', 'state', 'team-state.json');
