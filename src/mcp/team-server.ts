/**
 * OMQ Team Runner MCP Server
 * Provides omq_run_team_* tools for spawning and managing tmux CLI worker teams.
 * Storage: ~/.omq/team-jobs/
 */
...
const jobIdSchema = z.string().regex(/^omq-[a-z0-9]{1,12}$/);
...
const omqTeamJobs = new Map<string, OmqTeamJob>();

function getOmqJobsDir(): string {
  return join(homedir(), '.omq', 'team-jobs');
}

function persistJob(jobId: string, job: OmqTeamJob): void {
  try {
    const jobsDir = getOmqJobsDir();
...
function loadJobFromDisk(jobId: string): OmqTeamJob | undefined {
  try {
    return JSON.parse(readFileSync(join(getOmqJobsDir(), `${jobId}.json`), 'utf-8')) as OmqTeamJob;
  } catch (err) {
...
async function loadPaneIds(jobId: string): Promise<{ paneIds: string[]; leaderPaneId: string } | null> {
  try {
    const parsed = JSON.parse(await readFile(join(getOmqJobsDir(), `${jobId}-panes.json`), 'utf-8')) as {
      paneIds?: unknown;
...
async function resolveCleanupTargets(jobId: string, job: OmqTeamJob): Promise<CleanupSelection> {
...
const server = new Server(
  { name: 'omq-team-server', version: '0.1.0' },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    {
      name: 'omq_run_team_start',
      description: 'Spawn tmux CLI workers (qwen/gemini) in the background. Returns jobId immediately. Poll with omq_run_team_status.',
      inputSchema: {
        type: 'object',
        properties: {
          teamName: { type: 'string', description: 'Slug name for the team' },
          agentTypes: { type: 'array', items: { type: 'string' }, description: '"qwen", "claude", or "gemini" per worker' },
          tasks: {
            type: 'array',
            items: {
              type: 'object',
    ...
    {
      name: 'omq_run_team_status',
      description: 'Non-blocking status check for a background omq_run_team job. Returns status and result when done.',
      inputSchema: {
        type: 'object',
        properties: {
          job_id: { type: 'string', description: 'Job ID returned by omq_run_team_start' },
        },
        required: ['job_id'],
      },
    },
    {
      name: 'omq_run_team_wait',
      description: 'Block (poll internally) until a background omq_run_team job reaches a terminal state (completed or failed) or, in wake_on=event mode, until the next team event arrives. Uses exponential backoff (500ms to 2000ms). Auto-nudges idle teammate panes via tmux send-keys. If this wait call times out, workers are left running -- call omq_run_team_wait again to keep waiting, or omq_run_team_cleanup to stop them explicitly.',
      inputSchema: {
        type: 'object',
        properties: {
          job_id: { type: 'string', description: 'Job ID returned by omq_run_team_start' },
...
export async function handleTeamToolCall(request: {
  ...
  try {
    switch (name) {
      case 'omq_run_team_start': {
        const { teamName, agentTypes, tasks, cwd: inputCwd } = startSchema.parse(a);

        const jobId = `omq-${Date.now().toString(36)}`;
        const runtimeCliPath = join(dirname(fileURLToPath(import.meta.url)), '..', 'team', 'runtime-cli.js');

        const job: OmqTeamJob = { status: 'running', startedAt: Date.now(), teamName, cwd: inputCwd };
        omqTeamJobs.set(jobId, job);

        const child = spawn('node', [runtimeCliPath], {
          env: { ...process.env, OMQ_JOB_ID: jobId, OMQ_JOBS_DIR: getOmqJobsDir() },
        ...
        return {
          content: [{ type: 'text' as const, text: JSON.stringify({ jobId, pid: job.pid, message: 'Team started. Poll with omq_run_team_status.' }) }],
        };
      }

      case 'omq_run_team_status': {
        const { job_id: jobId } = statusSchema.parse(a);
        const job = omqTeamJobs.get(jobId) ?? loadJobFromDisk(jobId);
        if (!job) {
      ...
      case 'omq_run_team_wait': {
        ...
        const elapsed = ((Date.now() - (omqTeamJobs.get(jobId)?.startedAt ?? Date.now())) / 1000).toFixed(1);
        const timeoutOut: Record<string, unknown> = {
          error: `Timed out waiting for job ${jobId} after ${(timeoutMs / 1000).toFixed(0)}s -- workers are still running; call omq_run_team_wait again to keep waiting or omq_run_team_cleanup to stop them`,
          jobId,
          status: 'running',
          wake_on: wakeOn,
          elapsedSeconds: elapsed,
