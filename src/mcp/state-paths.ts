...
const WORKDIR_ALLOWLIST_ENV = 'OMQ_MCP_WORKDIR_ROOTS';
...
export function getBaseStateDir(workingDirectory?: string): string {
  if ((workingDirectory == null || workingDirectory === '') && typeof process.env.OMQ_TEAM_STATE_ROOT === 'string' && process.env.OMQ_TEAM_STATE_ROOT.trim() !== '') {
    try {
      return resolveWorkingDirectoryForState(process.env.OMQ_TEAM_STATE_ROOT.trim());
    } catch {}
  }
  return join(resolveWorkingDirectoryForState(workingDirectory), '.omq', 'state');
}
