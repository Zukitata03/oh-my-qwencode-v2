/**
 * OMQ Project Memory & Notepad MCP Server
 * Provides persistent project memory and session notepad tools
 * Storage: .omq/project-memory.json, .omq/notepad.md
 */
...
function getMemoryPath(wd: string): string {
  return join(wd, '.omq', 'project-memory.json');
}

function getNotepadPath(wd: string): string {
  return join(wd, '.omq', 'notepad.md');
}
