...
const SERVER_DISABLE_ENV: Record<McpServerName, string> = {
  state: 'OMQ_STATE_SERVER_DISABLE_AUTO_START',
  memory: 'OMQ_MEMORY_SERVER_DISABLE_AUTO_START',
  code_intel: 'OMQ_CODE_INTEL_SERVER_DISABLE_AUTO_START',
  trace: 'OMQ_TRACE_SERVER_DISABLE_AUTO_START',
  team: 'OMQ_TEAM_SERVER_DISABLE_AUTO_START',
};

const GLOBAL_DISABLE_ENV = 'OMQ_MCP_SERVER_DISABLE_AUTO_START';
...
export function autoStartStdioMcpServer(
  ...
  const shutdown = async () => {
    ...
    try {
      ...
      console.error(`[omq-${serverName}-server] shutdown failed`, error);
    }
  };
