...
describe('resolveRuntimeBinaryPath', () => {
  it('prefers explicit OMQ_RUNTIME_BINARY override', () => {
    const previous = process.env.OMQ_RUNTIME_BINARY;
    try {
      process.env.OMQ_RUNTIME_BINARY = '/custom/runtime';
      const actual = resolveRuntimeBinaryPath({
        debugPath: '/debug/runtime',
        releasePath: '/release/runtime',
        fallbackBinary: 'omq-runtime',
        exists: () => false,
      });
      assert.equal(actual, '/custom/runtime');
    } finally {
      if (typeof previous === 'string') process.env.OMQ_RUNTIME_BINARY = previous;
  ...
  it('falls back to PATH binary when local builds are unavailable', () => {
    ...
    assert.equal(actual, 'omq-runtime');
  });
});
