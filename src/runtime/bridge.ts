/**
 * TS Runtime Bridge — thin wrapper over omq-runtime binary.
 *
 * All semantic state mutations route through `execCommand()`.
 * All state queries read Rust-authored compatibility JSON files.
 * Set OMQ_RUNTIME_BRIDGE=0 to disable bridge (fallback to TS-direct).
 */
...
export function resolveRuntimeBinaryPath(options: RuntimeBinaryDiscoveryOptions = {}): string {
  ...
  const envOverride = process.env.OMQ_RUNTIME_BINARY?.trim();
  if (envOverride) return envOverride;

  const workspaceDebug = options.debugPath ?? resolve(__bridge_dirname, '../../target/debug/omq-runtime');
  if (exists(workspaceDebug)) return workspaceDebug;

  const workspaceRelease = options.releasePath ?? resolve(__bridge_dirname, '../../target/release/omq-runtime');
  if (exists(workspaceRelease)) return workspaceRelease;

  return options.fallbackBinary ?? 'omq-runtime';
}
...
export class RuntimeBridge {
  ...
  constructor(options: { stateDir?: string; binaryPath?: string } = {}) {
    this.enabled = process.env.OMQ_RUNTIME_BRIDGE !== '0';
    this.stateDir = options.stateDir;
    this.binaryPath = options.binaryPath ?? resolveRuntimeBinaryPath();
  }

  /** Whether the bridge is enabled (OMQ_RUNTIME_BRIDGE != '0'). */
  isEnabled(): boolean {
    return this.enabled;
  }
  ...
  private validateSchemaOnce(): void {
    ...
    try {
      ...
      if (missing.length > 0) {
        throw new Error(
          `omq-runtime schema missing commands: ${missing.join(', ')}. ` +
  ...
  private run(args: string[]): string {
    try {
      ...
      throw new Error(`omq-runtime ${args[0]} failed: ${stderr}`);
    }
  }
}
...
export function isBridgeEnabled(): boolean {
  return process.env.OMQ_RUNTIME_BRIDGE !== '0';
}
