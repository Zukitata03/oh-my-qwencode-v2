...
describe('session lifecycle manager', () => {
  it('resets session metrics files with zeroed counters', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-session-metrics-'));
    try {
      await resetSessionMetrics(cwd);

      const metricsPath = join(cwd, '.omq', 'metrics.json');
      const hudPath = join(cwd, '.omq', 'state', 'hud-state.json');
  ...
  it('writes session start/end lifecycle artifacts and archives session history', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-session-lifecycle-'));
    ...
    try {
      ...
      const sessionPath = join(cwd, '.omq', 'state', 'session.json');
      ...
      const historyPath = join(cwd, '.omq', 'logs', 'session-history.jsonl');
      ...
      const dailyLogPath = join(cwd, '.omq', 'logs', `omq-${todayIsoDate()}.jsonl`);
  ...
  it('treats invalid session JSON as absent state', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-session-invalid-'));
    try {
      const statePath = join(cwd, '.omq', 'state', 'session.json');
      await resetSessionMetrics(cwd);
...
describe('isSessionStale', () => {
  it('returns false for a live Linux process when identity matches', () => {
    const state = makeState({
      pid_start_ticks: 111,
      pid_cmdline: 'node omq',
    });

    const stale = isSessionStale(state, {
      platform: 'linux',
      isPidAlive: () => true,
      readLinuxIdentity: () => ({ startTicks: 111, cmdline: 'node omq' }),
    });
  ...
  it('returns true for PID reuse on Linux when start ticks mismatch', () => {
    ...
    const stale = isSessionStale(state, {
      platform: 'linux',
      isPidAlive: () => true,
      readLinuxIdentity: () => ({ startTicks: 222, cmdline: 'node omq' }),
    });
