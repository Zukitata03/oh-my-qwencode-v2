...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'omq-regression-205-'));
  try {
    await run(cwd);
  } finally {
...
describe('regression-205: detectStallPattern matches "if you want"', () => {
  ...
  it('ignores OMQ injection-marker lines when matching patterns', async () => {
    const { detectStallPattern, DEFAULT_STALL_PATTERNS } = await loadModule('notify-hook/auto-nudge.js');
    assert.equal(
      detectStallPattern('keep going [OMQ_TMUX_INJECT]', DEFAULT_STALL_PATTERNS),
      false,
    );
  });
...
describe('regression-205: notify-hook records pending stall state on "if you want" by default', () => {
  ...
  before(() => {
    originalTeamWorker = process.env.OMQ_TEAM_WORKER;
    originalTeamStateRoot = process.env.OMQ_TEAM_STATE_ROOT;
    delete process.env.OMQ_TEAM_WORKER;
    delete process.env.OMQ_TEAM_STATE_ROOT;
  });

  after(() => {
    if (originalTeamWorker === undefined) delete process.env.OMQ_TEAM_WORKER;
    else process.env.OMQ_TEAM_WORKER = originalTeamWorker;
    if (originalTeamStateRoot === undefined) delete process.env.OMQ_TEAM_STATE_ROOT;
    else process.env.OMQ_TEAM_STATE_ROOT = originalTeamStateRoot;
  });
  ...
  it('records pending stall state instead of injecting immediately', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.doesNotMatch(
        tmuxLog,
        /send-keys -t %99 -l yes, proceed \[OMQ_TMUX_INJECT\]/,
...
describe('regression-205: resolveTeamStateDirForWorker is exported from team-worker.js', () => {
  ...
  it('uses OMQ_TEAM_STATE_ROOT env var when set', async () => {
    const { resolveTeamStateDirForWorker } = await loadModule('notify-hook/team-worker.js');
    const saved = process.env.OMQ_TEAM_STATE_ROOT;
    ...
    try {
      ...
      if (saved === undefined) {
        ...
        process.env.OMQ_TEAM_STATE_ROOT = saved;
      }
    }
  });

  it('falls back to {cwd}/.omq/state when no env var and no team dir exists', async () => {
    const { resolveTeamStateDirForWorker } = await loadModule('notify-hook/team-worker.js');
    const savedRoot = process.env.OMQ_TEAM_STATE_ROOT;
    const savedLeader = process.env.OMQ_TEAM_LEADER_CWD;
    delete process.env.OMQ_TEAM_STATE_ROOT;
    ...
    try {
      ...
      assert.equal(result, join(cwd, '.omq', 'state'));
    } finally {
      if (savedRoot === undefined) {
        delete process.env.OMQ_TEAM_STATE_ROOT;
      } else {
        process.env.OMQ_TEAM_STATE_ROOT = savedRoot;
      }
      if (savedLeader === undefined) {
        delete process.env.OMQ_TEAM_LEADER_CWD;
      } else {
        process.env.OMQ_TEAM_LEADER_CWD = savedLeader;
      }
    }
  });
});
