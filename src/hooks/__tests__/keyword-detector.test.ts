...
describe('keyword detector swarm/team compatibility', () => {
  ...
  it('does not trigger team keyword from filesystem/team-state path text', () => {
    const match = detectPrimaryKeyword('You have 1 new message(s). Read .omq/state/team/execute-plan/mailbox/worker-3.json, act now, reply with concrete progress, then continue assigned work or next feasible task.');
...
describe('keyword detector skill-active-state lifecycle', () => {
  it('writes skill-active-state.json with planning phase when keyword activates', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-keyword-state-'));
  ...
  it('acquires a deep-interview input lock immediately on activation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-keyword-state-deep-interview-'));
    const stateDir = join(cwd, '.omq', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
      const result = await recordSkillActivation({
  ...
  it('releases the deep-interview input lock on abort via cancel keyword', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-keyword-state-deep-interview-abort-'));
    const stateDir = join(cwd, '.omq', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
  ...
  it('does not write state when no keyword is present', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-keyword-state-none-'));
    const stateDir = join(cwd, '.omq', 'state');
    try {
      await mkdir(stateDir, { recursive: true });
      const result = await recordSkillActivation({
  ...
  it('preserves activated_at for same-skill continuation', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-keyword-state-continuation-'));
    const stateDir = join(cwd, '.omq', 'state');
  ...
  it('resets activated_at when skill changes', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-keyword-state-skill-switch-'));
    const stateDir = join(cwd, '.omq', 'state');
    const statePath = join(stateDir, SKILL_ACTIVE_STATE_FILE);
    try {
      await mkdir(stateDir, { recursive: true });
  ...
  it('resets activated_at when keyword changes within the same skill', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-keyword-state-keyword-switch-'));
    const stateDir = join(cwd, '.omq', 'state');
    const statePath = join(stateDir, SKILL_ACTIVE_STATE_FILE);
    try {
      await mkdir(stateDir, { recursive: true });
...
describe('applyRalplanGate', () => {
  it('does not re-enter ralplan for a short approved team follow-up', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-keyword-gate-followup-'));
    try {
      const plansDir = join(cwd, '.omq', 'plans');
      await mkdir(plansDir, { recursive: true });
      await writeFile(
        join(plansDir, 'prd-issue-831.md'),
        '# Approved plan\n\nLaunch hint: omq team 3:executor "Execute approved issue 831 plan"\n',
      );
  ...
  it('does not re-enter ralplan for a short approved Korean team follow-up', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-keyword-gate-followup-ko-'));
    try {
      const plansDir = join(cwd, '.omq', 'plans');
      await mkdir(plansDir, { recursive: true });
      await writeFile(
