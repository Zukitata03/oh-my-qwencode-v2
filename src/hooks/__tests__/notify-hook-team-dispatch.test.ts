...
describe('notify-hook team dispatch consumer', () => {
  const originalTeamWorker = process.env.OMQ_TEAM_WORKER;
  const originalTeamStateRoot = process.env.OMQ_TEAM_STATE_ROOT;
  ...
  after(() => {
    if (originalTeamWorker === undefined) {
      ...
      process.env.OMQ_TEAM_WORKER = originalTeamWorker;
    }

    if (originalTeamStateRoot === undefined) {
      delete process.env.OMQ_TEAM_STATE_ROOT;
    } else {
      process.env.OMQ_TEAM_STATE_ROOT = originalTeamStateRoot;
    }
  });

  it('marks pending request as notified and preserves mailbox notified_at semantics', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-hook-team-dispatch-'));
    try {
  ...
  it('leader-fixed dispatch remains pending with leader_pane_missing_deferred when pane missing', async () => {
    ...
    try {
      ...
      const eventsPath = join(cwd, '.omq', 'state', 'team', 'alpha', 'events', 'events.ndjson');
      ...
      const dispatchLogPath = join(cwd, '.omq', 'logs', `team-dispatch-${new Date().toISOString().slice(0, 10)}.jsonl`);
  ...
  it('invokes omq-runtime exec via shared bridge fallback', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-hook-team-dispatch-'));
...
`,
  ...
  it('leader-fixed dispatch uses pane target only when leader_pane_id exists', async () => {
    ...
    try {
      ...
      await enqueueDispatchRequest('alpha', {
        ...
        to_worker: 'leader-fixed',
        message_id: msg.message_id,
        trigger_message: 'Read .omq/state/team/alpha/mailbox/leader-fixed.json; worker-1 sent a new message. Review it and decide the next concrete step.',
      }, cwd);
  ...
  it('leader-fixed dispatch prefers the canonical qwen pane over a stale HUD leader pane id', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-hook-team-dispatch-'));
    const fakeBinDir = join(cwd, 'fake-bin');
    const tmuxLogPath = join(cwd, 'tmux.log');
    const prevPath = process.env.PATH;
...
if [[ "$cmd" == "display-message" ]]; then
  ...
  if [[ "$fmt" == "#{pane_start_command}" && "$target" == "%91" ]]; then
    echo "node dist/cli/omq.js hud --watch"
    exit 0
  fi
...
if [[ "$cmd" == "list-panes" ]]; then
  printf "%%42\\t1\\tnode\\tqwen\\n%%91\\t0\\tnode\\tnode dist/cli/omq.js hud --watch\\n"
  exit 0
fi
exit 0
`;
  ...
  it('uses explicit stateDir when marking mailbox notified_at', async () => {
    ...
    const previousStateRoot = process.env.OMQ_TEAM_STATE_ROOT;
    try {
      process.env.OMQ_TEAM_STATE_ROOT = './custom-state-root';
      await initTeamState('alpha', 'task', 'executor', 1, cwd);
      const msg = await sendDirectMessage('alpha', 'worker-1', 'worker-1', 'hello', cwd);
      const queued = await enqueueDispatchRequest('alpha', {
        kind: 'mailbox',
      ...
      if (typeof previousStateRoot === 'string') process.env.OMQ_TEAM_STATE_ROOT = previousStateRoot;
  ...
  it('keeps retry_pending derived-only and does not persist transient tags', async () => {
    ...
    try {
      ...
      const rawRequests = JSON.parse(await readFile(join(cwd, '.omq', 'state', 'team', 'alpha', 'dispatch', 'requests.json'), 'utf8'));
  ...
  it('retries submit with isolated C-m and does not retype when trigger already present', async () => {
    ...
    try {
      ...
      process.env.PATH = `${fakeBinDir}:${previousPath || ''}`;
      process.env.OMQ_TEST_CAPTURE_FILE = captureFile;
      ...
      delete process.env.OMQ_TEST_CAPTURE_FILE;
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('retypes on every retry when trigger is not in narrow input area', async () => {
    ...
    try {
      ...
      process.env.PATH = `${fakeBinDir}:${previousPath || ''}`;
      process.env.OMQ_TEST_CAPTURE_SEQUENCE_FILE = captureSeqFile;
      process.env.OMQ_TEST_CAPTURE_COUNTER_FILE = captureCounterFile;
      ...
      delete process.env.OMQ_TEST_CAPTURE_SEQUENCE_FILE;
      delete process.env.OMQ_TEST_CAPTURE_COUNTER_FILE;
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('applies per-issue cooldown to avoid repeated reinjection in one drain tick', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-hook-team-dispatch-'));
    const previousIssueCooldown = process.env.OMQ_TEAM_DISPATCH_ISSUE_COOLDOWN_MS;
    try {
      process.env.OMQ_TEAM_DISPATCH_ISSUE_COOLDOWN_MS = '900000';
      ...
      if (typeof previousIssueCooldown === 'string') process.env.OMQ_TEAM_DISPATCH_ISSUE_COOLDOWN_MS = previousIssueCooldown;
      else delete process.env.OMQ_TEAM_DISPATCH_ISSUE_COOLDOWN_MS;
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('skips repeated same-issue reinjection during per-issue cooldown window', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-hook-team-dispatch-'));
    const previousCooldown = process.env.OMQ_TEAM_DISPATCH_ISSUE_COOLDOWN_MS;
    let injectCount = 0;
    try {
      process.env.OMQ_TEAM_DISPATCH_ISSUE_COOLDOWN_MS = '900000';
      await initTeamState('alpha', 'task', 'executor', 1, cwd);
      ...
      if (typeof previousCooldown === 'string') process.env.OMQ_TEAM_DISPATCH_ISSUE_COOLDOWN_MS = previousCooldown;
      else delete process.env.OMQ_TEAM_DISPATCH_ISSUE_COOLDOWN_MS;
      await rm(cwd, { recursive: true, force: true });
    }
  });
