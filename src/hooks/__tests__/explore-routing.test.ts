...
describe('explore-routing', () => {
  it('defaults USE_OMQ_EXPLORE_CMD to enabled and only disables explicit opt-out values', () => {
    assert.equal(isExploreCommandRoutingEnabled({}), true);
    assert.equal(isExploreCommandRoutingEnabled({ USE_OMQ_EXPLORE_CMD: '1' }), true);
    assert.equal(isExploreCommandRoutingEnabled({ USE_OMQ_EXPLORE_CMD: 'true' }), true);
    assert.equal(isExploreCommandRoutingEnabled({ USE_OMQ_EXPLORE_CMD: 'yes' }), true);
    assert.equal(isExploreCommandRoutingEnabled({ USE_OMQ_EXPLORE_CMD: 'on' }), true);
    assert.equal(isExploreCommandRoutingEnabled({ USE_OMQ_EXPLORE_CMD: '0' }), false);
    assert.equal(isExploreCommandRoutingEnabled({ USE_OMQ_EXPLORE_CMD: 'false' }), false);
    assert.equal(isExploreCommandRoutingEnabled({ USE_OMQ_EXPLORE_CMD: 'no' }), false);
    assert.equal(isExploreCommandRoutingEnabled({ USE_OMQ_EXPLORE_CMD: 'off' }), false);
  });
  ...
  it('builds advisory guidance whenever routing is not explicitly disabled', () => {
    ...
    assert.match(guidance, /USE_OMQ_EXPLORE_CMD/);
    assert.match(guidance, /default-on; opt out/i);
    assert.match(guidance, /agents SHOULD treat `omq explore` as the default first stop/i);
    assert.match(guidance, /use `omq explore` FIRST before attempting full code analysis/i);
    ...
    assert.equal(buildExploreRoutingGuidance({ USE_OMQ_EXPLORE_CMD: 'off' }), '');
  });
});
