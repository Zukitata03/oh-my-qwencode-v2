...
const RUNTIME_START = "<!-- OMQ:RUNTIME:START -->";
const RUNTIME_END = "<!-- OMQ:RUNTIME:END -->";
const WORKER_START = "<!-- OMQ:TEAM:WORKER:START -->";
const WORKER_END = "<!-- OMQ:TEAM:WORKER:END -->";

async function makeTempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), "omq-overlay-test-"));
  await mkdir(join(dir, ".omq", "state"), { recursive: true });
  return dir;
}
...
describe("generateOverlay", () => {
  ...
  it("generates overlay with no state files (empty but valid)", async () => {
    ...
    assert.ok(overlay.includes("<!-- OMQ:RUNTIME:START -->"));
    assert.ok(overlay.includes("<!-- OMQ:RUNTIME:END -->"));
  ...
  it("adds advisory explore routing guidance by default and hides it only on explicit opt-out", async () => {
    const previous = process.env.USE_OMQ_EXPLORE_CMD;
    try {
      delete process.env.USE_OMQ_EXPLORE_CMD;
      ...
      assert.match(defaultOverlay, /omq explore` FIRST before attempting full code analysis/i);

      process.env.USE_OMQ_EXPLORE_CMD = "off";
      ...
      if (typeof previous === "string")
        process.env.USE_OMQ_EXPLORE_CMD = previous;
      else delete process.env.USE_OMQ_EXPLORE_CMD;
    }
  });
  ...
  it("generates overlay with session-scoped active modes for current session", async () => {
    await mkdir(join(tempDir, ".omq", "state", "sessions", "sess1"), {
      recursive: true,
    });
    await writeFile(
      join(tempDir, ".omq", "state", "sessions", "sess1", "team-state.json"),
      JSON.stringify({
        active: true,
        iteration: 1,
        max_iterations: 5,
  ...
  it("generates overlay with project memory summary", async () => {
    await writeFile(
      join(tempDir, ".omq", "project-memory.json"),
      JSON.stringify({
  ...
  it("skips inactive modes", async () => {
    await writeFile(
      join(tempDir, ".omq", "state", "autopilot-state.json"),
  ...
  it("adds blocked ralph planning gate when PRD/test spec are missing", async () => {
    ...
    await mkdir(join(tempDir, ".omq", "plans"), { recursive: true });
  ...
  it("unlocks ralph planning gate when PRD and test spec exist", async () => {
    ...
    const plansDir = join(tempDir, ".omq", "plans");
...
describe("resolveSessionOrchestrationMode", () => {
  ...
  it("falls back to root team skill state only when no session-scoped skill state exists", async () => {
    ...
    await writeFile(
      join(tempDir, ".omq", "state", "skill-active-state.json"),
      JSON.stringify({ active: true, skill: "team" }),
    );
...
`;
  ...
  it("applyOverlay is idempotent (apply twice, no duplication)", async () => {
    ...
    const startCount = (secondApply.match(/<!-- OMQ:RUNTIME:START -->/g) || [])
      .length;
  ...
  it("handles stale markers from previous session", async () => {
    ...
    const startCount = (result.match(/<!-- OMQ:RUNTIME:START -->/g) || [])
      .length;
    assert.equal(startCount, 1);
  });
...
describe("session-scoped model instructions file", () => {
  let tempDir: string;
  let restoreQwenHome: (() => void) | undefined;

  before(async () => {
    tempDir = await makeTempDir();
    restoreQwenHome = setMockQwenHome(join(tempDir, "home", ".qwen"));
  });
  after(async () => {
    restoreQwenHome?.();
    await rm(tempDir, { recursive: true, force: true });
  ...
  it("writes user + project AGENTS.md + runtime overlay into session-scoped file", async () => {
    const userAgentsMd = join(tempDir, "home", ".qwen", "AGENTS.md");
    const projectAgentsMd = join(tempDir, "AGENTS.md");
    await mkdir(join(tempDir, "home", ".qwen"), { recursive: true });
    ...
    assert.match(sessionContent, /<!-- OMQ:RUNTIME:START -->/);
  ...
  it("deduplicates duplicate skill references when project and user scopes both install the same skill", async () => {
    const userAgentsMd = join(tempDir, "home", ".qwen", "AGENTS.md");
    const projectAgentsMd = join(tempDir, "AGENTS.md");
    const userSkillDir = join(tempDir, "home", ".qwen", "skills", "help");
    const projectSkillDir = join(tempDir, ".qwen", "skills", "help");
    await mkdir(join(tempDir, "home", ".qwen"), { recursive: true });
    ...
    await writeFile(
      ...
      [
        "# User instructions",
        "",
        "- help: user copy (file: /tmp/home/.qwen/skills/help/SKILL.md)",
      ].join("\n"),
    );
    await writeFile(
      projectAgentsMd,
      [
        ...
        "",
        "- help: project copy (file: /tmp/project/.qwen/skills/help/SKILL.md)",
      ].join("\n"),
    );
  ...
  it("writes overlay-only session file when no base AGENTS.md files exist", async () => {
    ...
    assert.ok(sessionContent.includes("<!-- OMQ:RUNTIME:START -->"));
    assert.ok(sessionContent.includes("<!-- OMQ:RUNTIME:END -->"));
  });
...
describe("hasOverlay", () => {
  ...
  it("returns false when only start marker", () => {
    assert.ok(!hasOverlay("<!-- OMQ:RUNTIME:START -->\nbroken"));
  });
});
