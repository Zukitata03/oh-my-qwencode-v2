...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'omq-notify-team-nudge-'));
  try {
    await run(cwd);
  } finally {
...
function runNotifyHook(
  ...
  return spawnSync(process.execPath, [NOTIFY_HOOK_SCRIPT.pathname, JSON.stringify(payload)], {
    ...
    env: {
      ...
      OMQ_TEAM_LEADER_NUDGE_MS: '10000',
      OMQ_TEAM_LEADER_STALE_MS: '10000',
      OMQ_TEAM_WORKER: '',
      OMQ_TEAM_STATE_ROOT: '',
      OMQ_TEAM_LEADER_CWD: '',
...
describe('notify-hook leader-side authority handoff', () => {
  ...
  it('does not drain pending dispatch requests from notify-hook leader context', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const fakeTmuxPath = join(fakeBinDir, 'tmux');
      const tmuxLogPath = join(cwd, 'tmux.log');
      await mkdir(join(cwd, '.omq', 'logs'), { recursive: true });
      await mkdir(fakeBinDir, { recursive: true });
      await writeFile(fakeTmuxPath, buildFakeTmux(tmuxLogPath));
      await chmod(fakeTmuxPath, 0o755);
  ...
  it('does not nudge stale leader when recent team status activity proves the leader is active', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omq-team-beta-active-status',
        leader_pane_id: '%92',
        workers: [
...
describe('notify-hook team leader nudge', () => {
  ...
  it('sends immediate all-workers-idle nudge for active team (leader context)', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /\[OMQ\] All 2 workers idle/, 'should emit all-workers-idle nudge');
      assert.match(tmuxLog, /\[OMQ_TMUX_INJECT\]/, 'should include injection marker');
  ...
  it('suggests shutdown when all workers are idle and the current task set is complete', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.match(tmuxLog, /\[OMQ\] All 2 workers idle\./);
      assert.match(tmuxLog, /Team idle-shutdown is complete and waiting on leader action/);
      assert.match(tmuxLog, /Next: decide whether to reconcile\/merge results or gracefully shut down: omq team shutdown/);
  ...
  it('suggests reusing the team when follow-up tasks are pending and worker panes are still reusable', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.doesNotMatch(tmuxLog, /omq team shutdown/);
    });
  });
  ...
  it('falls back to global team-state when session-scoped state is active but team-state.json remains global', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /\[OMQ\] All 2 workers idle/, 'global team-state fallback should still fire idle nudge');
    });
  });
  ...
  it('surfaces ack-like mailbox replies without work-start evidence as missing-start nudges', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Next: check worker-1 msg\/output, confirm task in omq team status ack-missing-start/);
  ...
  it('does not classify ack-like replies as missing-start after a worker has claimed work', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Next: read messages; keep orchestrating; if done, gracefully shut down: omq team shutdown ack-with-start/);
  ...
  it('does not re-nudge for the same fresh mailbox message on repeated notify-hook runs', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const first = runNotifyHook(cwd, fakeBinDir, { OMQ_TEAM_LEADER_NUDGE_MS: '600000' });
      assert.equal(first.status, 0, `notify-hook failed: ${first.stderr || first.stdout}`);
      const second = runNotifyHook(cwd, fakeBinDir, { OMQ_TEAM_LEADER_NUDGE_MS: '600000' });
...
`;
  ...
  it('nudges when worker panes are alive and leader is stale (no recent HUD turn)', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Next: check messages; keep orchestrating; if done, gracefully shut down: omq team shutdown beta/);
      assert.doesNotMatch(tmuxLog, /keep polling/);
      assert.match(tmuxLog, /\[OMQ_TMUX_INJECT\]/, 'should include injection marker');
    });
  });
  ...
  it('nudges when team progress is stalled even if timing signals are fresh or missing (fallback threshold)', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omq-team-stalled-progress',
        leader_pane_id: '%90',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
      ...
      const result = runNotifyHook(cwd, fakeBinDir, {
        OMQ_TEAM_PROGRESS_STALL_MS: '60000',
        OMQ_TEAM_LEADER_NUDGE_MS: '30000',
      });
      assert.equal(result.status, 0, `notify-hook failed: ${result.stderr || result.stdout}`);

      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.match(tmuxLog, /Team stalled-progress: leader stale, no progress 3m/);
      assert.match(tmuxLog, /Next: omq team status stalled-progress; read worker messages; unblock\/reassign or shutdown\./);
      assert.doesNotMatch(tmuxLog, /keep polling/);
      assert.match(tmuxLog, /\[OMQ_TMUX_INJECT\]/);
  ...
  it('nudges when team progress is stalled before the leader becomes stale (fallback threshold)', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omq-team-stalled-before-stale',
        leader_pane_id: '%89',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
          { name: 'worker-2', index: 2, pane_id: '%11' },
      ...
      const result = runNotifyHook(cwd, fakeBinDir, {
        OMQ_TEAM_PROGRESS_STALL_MS: '60000',
        OMQ_TEAM_LEADER_NUDGE_MS: '30000',
        OMQ_TEAM_LEADER_STALE_MS: '60000',
      });
      assert.equal(result.status, 0, `notify-hook failed: ${result.stderr || result.stdout}`);

      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.match(tmuxLog, /Team stalled-before-stale: worker panes stalled, no progress 3m/);
      assert.match(tmuxLog, /Next: omq team status stalled-before-stale; read worker messages; unblock\/reassign or shutdown\./);
      assert.doesNotMatch(tmuxLog, /keep polling/);
      assert.doesNotMatch(tmuxLog, /leader stale/);
  ...
  it('nudges stalled team after configurable worker turn stall window elapses (primary threshold)', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omq-team-worker-turn-stall-threshold',
        leader_pane_id: '%86',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
        ],
      ...
      const result = runNotifyHook(cwd, fakeBinDir, {
        OMQ_TEAM_PROGRESS_STALL_MS: '120000',
        OMQ_TEAM_WORKER_TURN_STALL_MS: '30000',
        OMQ_TEAM_LEADER_NUDGE_MS: '30000',
        OMQ_TEAM_LEADER_STALE_MS: '60000',
      });
      assert.equal(result.status, 0, `notify-hook failed: ${result.stderr || result.stdout}`);
  ...
  it('does not nudge stalled team when an in-progress worker is still advancing heartbeat turns', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omq-team-active-turns-no-stall',
        leader_pane_id: '%87',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
          { name: 'worker-2', index: 2, pane_id: '%11' },
  ...
  it('bounds repeated stalled-team nudges before leader stale by cooldown', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omq-team-stalled-before-stale-bounded',
        leader_pane_id: '%88',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
          { name: 'worker-2', index: 2, pane_id: '%11' },
      ...
      assert.match(tmuxLog, /Next: omq team status stalled-before-stale-bounded; read worker messages; unblock\/reassign or shutdown\./);
    });
  });
  ...
  it('does not treat leader and HUD panes as active worker panes when worker pane ids are known', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omq-team-stale-no-workers',
        leader_pane_id: '%92',
        hud_pane_id: '%93',
        workers: [
          { name: 'worker-1', index: 1, pane_id: '%10' },
  ...
  it('does not send a generic periodic leader nudge when the leader is not stale', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const result = runNotifyHook(cwd, fakeBinDir, { OMQ_TEAM_LEADER_NUDGE_MS: '30000' });
      assert.equal(result.status, 0, `notify-hook failed: ${result.stderr || result.stdout}`);
  ...
  it('uses a 30s cadence for stale leader follow-up nudges', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(teamDir, 'config.json'), {
        name: teamName,
        tmux_session: 'omq-team-stale-cadence',
        leader_pane_id: '%96',
      });
      ...
      const blocked = runNotifyHook(cwd, fakeBinDir, { OMQ_TEAM_LEADER_NUDGE_MS: '30000' });
      ...
      const allowed = runNotifyHook(cwd, fakeBinDir, { OMQ_TEAM_LEADER_NUDGE_MS: '30000' });
  ...
  it('bounds repeated all-workers-idle nudges by cooldown', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const first = runNotifyHook(cwd, fakeBinDir, { OMQ_TEAM_LEADER_ALL_IDLE_COOLDOWN_MS: '600000' });
      assert.equal(first.status, 0, `notify-hook failed: ${first.stderr || first.stdout}`);
      const second = runNotifyHook(cwd, fakeBinDir, { OMQ_TEAM_LEADER_ALL_IDLE_COOLDOWN_MS: '600000' });
      assert.equal(second.status, 0, `notify-hook failed: ${second.stderr || second.stdout}`);

      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      const sends = tmuxLog.match(/send-keys -t %98 -l \[OMQ\] All 2 workers idle/g) || [];
  ...
  it('includes stale_leader_with_messages reason when both conditions met', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /Next: read messages; keep orchestrating; if done, gracefully shut down: omq team shutdown delta/);
      assert.doesNotMatch(tmuxLog, /keep polling/);
      assert.match(tmuxLog, /\[OMQ_TMUX_INJECT\]/, 'should include injection marker');
