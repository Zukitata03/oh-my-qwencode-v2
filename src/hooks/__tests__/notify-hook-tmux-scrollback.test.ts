/**
 * Tests for issue #215: tmux scrollback preservation during OMQ output injection.
 *
...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'omq-notify-scroll-'));
  try {
    await run(cwd);
  } finally {
...
async function setupFixture(cwd: string, paneInMode: '0' | '1', skipIfScrolling = true) {
  ...
  const sessionId = 'omq-scroll-test';
  ...
  await writeJson(join(omqDir, 'tmux-hook.json'), {
    enabled: true,
