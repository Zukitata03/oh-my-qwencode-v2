...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'omq-notify-worker-idle-'));
  try {
    await run(cwd);
  } finally {
...
function runNotifyHookAsWorker(
  ...
  return spawnSync(process.execPath, [NOTIFY_HOOK_SCRIPT.pathname, JSON.stringify(payload)], {
    ...
    env: {
      ...
      OMQ_TEAM_WORKER_IDLE_COOLDOWN_MS: '500',
      OMQ_TEAM_ALL_IDLE_COOLDOWN_MS: '600000', // suppress all-idle to isolate per-worker
      TMUX: '',
      TMUX_PANE: '',
...
`;
  ...
  it('respects per-worker cooldown', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const result = runNotifyHookAsWorker(cwd, fakeBinDir, `${teamName}/worker-1`, {
        OMQ_TEAM_WORKER_IDLE_COOLDOWN_MS: '600000', // 10 minute cooldown
      });
      assert.equal(result.status, 0, `notify-hook failed: ${result.stderr || result.stdout}`);
  ...
  it('can be disabled via OMQ_TEAM_WORKER_IDLE_NOTIFY=false', async () => {
    await withTempWorkingDir(async (cwd) => {
      const stateDir = join(cwd, '.omq', 'state');
      const logsDir = join(cwd, '.omq', 'logs');
  ...
  it('can be disabled via OMQ_TEAM_WORKER_IDLE_NOTIFY=0', async () => {
    await withTempWorkingDir(async (cwd) => {
      const stateDir = join(cwd, '.omq', 'state');
      const logsDir = join(cwd, '.omq', 'logs');
  ...
  it('can be disabled via OMQ_TEAM_WORKER_IDLE_NOTIFY=off', async () => {
    await withTempWorkingDir(async (cwd) => {
      const stateDir = join(cwd, '.omq', 'state');
      const logsDir = join(cwd, '.omq', 'logs');
      ...
      const result = runNotifyHookAsWorker(cwd, fakeBinDir, `${teamName}/worker-1`, {
        OMQ_TEAM_WORKER_IDLE_NOTIFY: 'off',
      });
      assert.equal(result.status, 0, `notify-hook failed: ${result.stderr || result.stdout}`);
  ...
  it('existing all-workers-idle hook still fires alongside per-worker', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const result = runNotifyHookAsWorker(cwd, fakeBinDir, `${teamName}/worker-1`, {
        OMQ_TEAM_ALL_IDLE_COOLDOWN_MS: '500', // re-enable all-idle
      });
      assert.equal(result.status, 0, `notify-hook failed: ${result.stderr || result.stdout}`);
      ...
      assert.match(tmuxLog, /Next: run omq team status both-hooks, read unread worker messages, then decide whether to assign the next concrete task, reconcile results, or shut the team down/, 'all-workers-idle nudge should include a next action');
    });
  });
});
