...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'omq-notify-tmux-heal-'));
  try {
    await run(cwd);
  } finally {
...
describe('notify-hook tmux target healing', () => {
  it('falls back to global mode state when scoped session has no allowed active mode', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const sessionId = 'omq-abc123';
      const sessionStateDir = join(stateDir, 'sessions', sessionId);
      const fakeBinDir = join(cwd, 'fake-bin');
      const fakeTmuxPath = join(fakeBinDir, 'tmux');
      const configPath = join(omqDir, 'tmux-hook.json');
...
`;
  ...
  it('does not guess a pane by shared cwd when canonical qwen pane is unavailable', async () => {
    await withTempWorkingDir(async (cwd) => {
      const omqDir = join(cwd, '.omq');
      const stateDir = join(omqDir, 'state');
      const logsDir = join(omqDir, 'logs');
...
`;
  ...
  it('heals a stale HUD pane target back to the canonical qwen pane', async () => {
    await withTempWorkingDir(async (cwd) => {
      const omqDir = join(cwd, '.omq');
      const stateDir = join(omqDir, 'state');
      const logsDir = join(omqDir, 'logs');
      const sessionId = 'omq-hud-stale';
      const sessionStateDir = join(stateDir, 'sessions', sessionId);
      const fakeBinDir = join(cwd, 'fake-bin');
      const fakeTmuxPath = join(fakeBinDir, 'tmux');
      const configPath = join(omqDir, 'tmux-hook.json');
