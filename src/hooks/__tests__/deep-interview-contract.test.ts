...
describe("deep-interview Ouroboros contract", () => {
	...
	it("uses OMQ-native output paths", () => {
		assert.match(deepInterviewSkill, /\.omq\/interviews\//);
		assert.match(deepInterviewSkill, /\.omq\/specs\//);
	});
	...
	it("documents the autoresearch specialization contract", () => {
		...
		assert.match(
			deepInterviewSkill,
			/\.omq\/specs\/deep-interview-autoresearch-\{slug\}\.md/i,
		);
