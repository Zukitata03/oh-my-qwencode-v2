...
describe('notify-hook/operational-events – buildOperationalContext', () => {
  it('resolves a stable session_name from cwd + session id', async () => {
    ...
    const sessionId = 'omq-issue-663-session';
    ...
    try {
      ...
      assert.match(context.session_name || '', /^omq-/);
...
describe('notify-hook/auto-nudge – detectStallPattern', () => {
  ...
  it('ignores prior OMQ injection lines so injected text cannot self-trigger detection', async () => {
    const { detectStallPattern, DEFAULT_STALL_PATTERNS } = await loadModule('notify-hook/auto-nudge.js');
    const text = 'Completed the change.\nyes, proceed [OMQ_TMUX_INJECT]\nkeep going [OMQ_TMUX_INJECT]';
...
describe('notify-hook/payload-parser – language reminder injection', () => {
  ...
  it('injects language reminder for non-Latin input', async () => {
    ...
    const prompt = injectLanguageReminder('Continue from current mode state. [OMQ_TMUX_INJECT]', '帮我修复这个问题');
    assert.match(prompt, /\[OMQ_LANG_REMINDER\]/);
  ...
  it('does not inject reminder for Latin-only text', async () => {
    ...
    const prompt = injectLanguageReminder('Continue [OMQ_TMUX_INJECT]', 'Please fix issue 253');
    assert.equal(prompt, 'Continue [OMQ_TMUX_INJECT]');
  });
});
