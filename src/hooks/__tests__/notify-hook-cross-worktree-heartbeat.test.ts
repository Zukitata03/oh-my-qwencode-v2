...
async function withTempDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'omq-notify-cross-worktree-'));
  try {
    await run(cwd);
  } finally {
...
function runWorkerNotify(
  ...
  const inheritedEnv: NodeJS.ProcessEnv = {
    ...process.env,
    OMQ_TEAM_WORKER: teamWorker,
    TMUX: '',
    TMUX_PANE: '',
  };
  if (!Object.prototype.hasOwnProperty.call(extraEnv, 'OMQ_TEAM_STATE_ROOT')) {
    delete inheritedEnv.OMQ_TEAM_STATE_ROOT;
  }
  if (!Object.prototype.hasOwnProperty.call(extraEnv, 'OMQ_TEAM_LEADER_CWD')) {
    delete inheritedEnv.OMQ_TEAM_LEADER_CWD;
  }
...
describe('notify-hook cross-worktree heartbeat resolution', () => {
  it('writes heartbeat under OMQ_TEAM_STATE_ROOT even when payload cwd is a different worktree', async () => {
    await withTempDir(async (root) => {
      ...
      const leaderWorkerDir = join(leaderCwd, '.omq', 'state', 'team', teamName, 'workers', workerName);
      await mkdir(leaderWorkerDir, { recursive: true });
      await mkdir(workerCwd, { recursive: true });

      const result = runWorkerNotify(workerCwd, `${teamName}/${workerName}`, {
        OMQ_TEAM_STATE_ROOT: join(leaderCwd, '.omq', 'state'),
      });
      ...
      const wrongHeartbeatPath = join(workerCwd, '.omq', 'state', 'team', teamName, 'workers', workerName, 'heartbeat.json');
  ...
  it('keeps leader-owned heartbeat state when worker cwd uses the team worktree layout', async () => {
    await withTempDir(async (root) => {
      ...
      const workerName = 'worker-1';
      const workerCwd = join(leaderCwd, '.omq', 'team', teamName, 'worktrees', workerName);
  ...
  it('falls back to worker identity/config metadata when OMQ_TEAM_STATE_ROOT is absent', async () => {
    await withTempDir(async (root) => {
      const leaderCwd = join(root, 'leader');
      const workerCwd = join(root, 'worker-worktree');
      const teamName = 'cross-meta';
      const workerName = 'worker-1';
      const teamStateRoot = join(leaderCwd, '.omq', 'state');
