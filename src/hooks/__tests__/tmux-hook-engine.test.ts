...
describe('normalizeTmuxHookConfig', () => {
  ...
  it('normalizes valid config', () => {
    const config = normalizeTmuxHookConfig({
      enabled: true,
      target: { type: 'session', value: 'omq-work' },
      allowed_modes: ['team'],
      cooldown_ms: 2000,
    ...
    assert.deepEqual(config.target, { type: 'session', value: 'omq-work' });
...
describe('evaluateInjectionGuards', () => {
  const validConfig = normalizeTmuxHookConfig({
    enabled: true,
    target: { type: 'session', value: 'omq' },
    allowed_modes: ['ralph'],
    cooldown_ms: 1000,
    max_injections_per_session: 2,
    prompt_template: 'Continue [OMQ_TMUX_INJECT]',
...
describe('isPaneRunningShell', () => {
  ...
  it('returns false for agent processes', () => {
    ...
    assert.equal(isPaneRunningShell('qwen'), false);
...
describe('paneLooksReady', () => {
  ...
  it('accepts Qwen Code welcome-screen helper text', () => {
    assert.equal(paneLooksReady('How can I help you today?'), true);
  });

  it('accepts Qwen Code welcome-screen suggestion rows with a prompt glyph', () => {
...
describe('paneHasActiveTask', () => {
  it('detects Qwen Code and Claude activity markers', () => {
...
if [[ "$cmd" == "display-message" ]]; then
  ...
  if [[ "$format" == "#{pane_start_command}" && "$target" == "%42" ]]; then
    echo "node /pkg/dist/cli/omq.js hud --watch"
    exit 0
  fi
...
if [[ "$cmd" == "list-panes" ]]; then
  printf "%%42\tnode\tnode /pkg/dist/cli/omq.js hud --watch\n%%42\tnode\tqwen --model gpt-5\n"
  exit 0
fi
echo "unsupported" >&2
exit 1
`);
      ...
      assert.equal(resolveQwenPane(), '%42');
    } finally {
