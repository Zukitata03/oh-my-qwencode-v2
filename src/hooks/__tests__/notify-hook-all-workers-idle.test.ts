...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'omq-notify-all-idle-'));
  try {
    await run(cwd);
  } finally {
...
function runNotifyHookAsWorker(
  ...
  return spawnSync(process.execPath, [NOTIFY_HOOK_SCRIPT.pathname, JSON.stringify(payload)], {
    ...
    env: {
      ...
      OMQ_TEAM_WORKER: workerEnv,
      OMQ_TEAM_STATE_ROOT: join(cwd, '.omq', 'state'),
      OMQ_TEAM_LEADER_CWD: '',
      OMQ_MODEL_INSTRUCTIONS_FILE: '',
      OMQ_TEAM_WORKER_IDLE_NOTIFY: 'false',
      OMQ_TEAM_ALL_IDLE_COOLDOWN_MS: '500', // short cooldown for tests
      TMUX: '',
      TMUX_PANE: '',
      ...extraEnv,
    },
...
describe('notify-hook all-workers-idle notification', () => {
  ...
  it('writes deferred visibility once per cooldown window when leader pane is missing', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const first = runNotifyHookAsWorker(cwd, fakeBinDir, `${teamName}/worker-1`, { OMQ_TEAM_ALL_IDLE_COOLDOWN_MS: '600000' });
      assert.equal(first.status, 0, `notify-hook failed: ${first.stderr || first.stdout}`);
      const second = runNotifyHookAsWorker(cwd, fakeBinDir, `${teamName}/worker-1`, { OMQ_TEAM_ALL_IDLE_COOLDOWN_MS: '600000' });
...
`;
  ...
  it('does not notify when some workers are still working', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      if (existsSync(tmuxLogPath)) {
        const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
        assert.doesNotMatch(tmuxLog, /\[OMQ\] All .* idle/, 'should NOT send all-idle message when some workers are busy');
      }
    });
  });
  ...
  it('does not notify when a worker heartbeat is stale', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      if (existsSync(tmuxLogPath)) {
        const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
        assert.doesNotMatch(tmuxLog, /\[OMQ\] All .* idle/, 'stale heartbeat should suppress all-workers-idle notification');
      }
    });
  });
  ...
  it('does not notify when current worker is not idle', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      if (existsSync(tmuxLogPath)) {
        const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
        assert.doesNotMatch(tmuxLog, /\[OMQ\] All .* idle/);
      }
    });
  });
  ...
  it('respects cooldown: does not send repeated notifications', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const result = runNotifyHookAsWorker(cwd, fakeBinDir, `${teamName}/worker-1`, {
        OMQ_TEAM_ALL_IDLE_COOLDOWN_MS: '600000',
      });
      assert.equal(result.status, 0, `notify-hook failed: ${result.stderr || result.stdout}`);

      if (existsSync(tmuxLogPath)) {
        const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
        assert.doesNotMatch(tmuxLog, /\[OMQ\] All .* idle/, 'cooldown should block repeated notification');
      }
    });
  });
  ...
  it('does not fire for leader (non-team-worker) context', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      // Run as LEADER (no OMQ_TEAM_WORKER env var)
      const payload = {
        cwd,
        type: 'agent-turn-complete',
      ...
      const result = spawnSync(process.execPath, [NOTIFY_HOOK_SCRIPT.pathname, JSON.stringify(payload)], {
        ...
        env: {
          ...process.env,
          PATH: `${fakeBinDir}:${process.env.PATH || ''}`,
          OMQ_TEAM_WORKER: '', // empty = not a worker
          OMQ_TEAM_STATE_ROOT: '',
          OMQ_TEAM_LEADER_CWD: '',
          OMQ_MODEL_INSTRUCTIONS_FILE: '',
          TMUX: '',
      ...
      if (existsSync(tmuxLogPath)) {
        const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
        assert.doesNotMatch(tmuxLog, /\[OMQ\] All .* idle/, 'leader context should not send all-idle notification');
      }
    });
  });
  ...
  it('handles single worker team correctly with singular message', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.match(tmuxLog, /\[OMQ\] All 1 worker idle/, 'single worker uses singular form');
      assert.match(tmuxLog, /Next: run omq team status solo-team, read unread worker messages, then decide whether to assign the next concrete task, reconcile results, or shut the team down/, 'all-workers-idle notification should include a next action');
