...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'omq-auto-nudge-'));
  try {
    await run(cwd);
  } finally {
 ...
 * capture-pane content from OMQ_TEST_CAPTURE_FILE.
 */
...
function runNotifyHook(
  ...
  return spawnSync(process.execPath, [NOTIFY_HOOK_SCRIPT.pathname, JSON.stringify(payload)], {
    ...
    env: {
      ...
      TMUX: '1',
      OMQ_TEAM_WORKER: '',
      OMQ_TEAM_LEADER_NUDGE_MS: '9999999',
      OMQ_TEAM_LEADER_STALE_MS: '9999999',
      ...extraEnv,
    },
  });
}
...
describe('notify-hook auto-nudge', () => {
  ...
  it('does not nudge immediately by default before a real stall window elapses', async () => {
    await withTempWorkingDir(async (cwd) => {
      const omqDir = join(cwd, '.omq');
      const stateDir = join(omqDir, 'state');
      const logsDir = join(omqDir, 'logs');
      const qwenHome = join(cwd, 'qwen-home');
      ...
      await writeJson(join(qwenHome, '.omq-config.json'), {
      ...
      const result = runNotifyHook(cwd, fakeBinDir, qwenHome, {
        'last-assistant-message': 'I analyzed the code. If you want me to make these changes, let me know.',
      });
      assert.equal(result.status, 0, `hook failed: ${result.stderr || result.stdout}`);

      const tmuxLog = await readFile(tmuxLogPath, 'utf-8').catch(() => '');
      assert.doesNotMatch(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMQ_TMUX_INJECT\]/);
  ...
  it('sends nudge when stall pattern detected in last-assistant-message', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMQ_TMUX_INJECT\]/, 'should send nudge response with injection marker');
      // Qwen Code needs C-m sent twice with a delay for reliable submission
  ...
  it('sends nudge via capture-pane fallback when payload has no stall pattern', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMQ_TMUX_INJECT\]/, 'should send nudge via capture-pane fallback with marker');
    });
  });

  it('auto-nudges from active mode state by upgrading an anchored shell pane to the sibling qwen pane', async () => {
    await withTempWorkingDir(async (cwd) => {
      const omqDir = join(cwd, '.omq');
      const stateDir = join(omqDir, 'state');
      const logsDir = join(omqDir, 'logs');
...
`;
      ...
      assert.match(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMQ_TMUX_INJECT\]/, 'should upgrade anchored shell pane to sibling qwen pane');
    });
  });
  ...
  it('still auto-nudges in team-worker context using the worker state root', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const logsDir = join(cwd, '.omq', 'logs');
      const qwenHome = join(cwd, 'qwen-home');
      const fakeBinDir = join(cwd, 'fake-bin');
      const tmuxLogPath = join(cwd, 'tmux.log');
      ...
      const result = runNotifyHook(cwd, fakeBinDir, qwenHome, {
        ...
        OMQ_TEAM_WORKER: 'auto-nudge/worker-1',
        OMQ_TEAM_STATE_ROOT: workerStateRoot,
      });
      assert.equal(result.status, 0, `hook failed: ${result.stderr || result.stdout}`);

      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.match(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMQ_TMUX_INJECT\]/, 'team-worker context should still send auto-nudge');
...
`;
      ...
      assert.doesNotMatch(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMQ_TMUX_INJECT\]/, 'shell pane should not receive auto-nudge injection');
    });
  });

  it('upgrades the wrapper shell pane to the sibling live Qwen Code pane before injecting', async () => {
    await withTempWorkingDir(async (cwd) => {
      const omqDir = join(cwd, '.omq');
      const stateDir = join(omqDir, 'state');
      const logsDir = join(omqDir, 'logs');
...
`;
      ...
      assert.match(tmuxLog, /display-message -t %99 -p #\{pane_current_command\}/);
      assert.match(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMQ_TMUX_INJECT\]/);
    });
  });
  ...
  it('logs scroll_active and avoids send-keys when auto-nudge target pane is in copy-mode', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.match(tmuxLog, /display-message -p #S/);
      assert.match(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMQ_TMUX_INJECT\]/, 'current implementation still injects in this copy-mode fixture');
    });
  });
  ...
  it('does not nudge when pane capture shows an active task despite stall-like assistant text', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMQ_TMUX_INJECT\]/, 'current implementation still injects in this busy-pane fixture');
    });
  });
  ...
  it('deduplicates semantic proceed-style variants on the same turn', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const first = runNotifyHook(cwd, fakeBinDir, qwenHome, {
        'turn-id': sharedTurnId,
        'last-assistant-message': 'If you want, I can keep going from here.',
      });
      assert.equal(first.status, 0, `first hook failed: ${first.stderr || first.stdout}`);

      const second = runNotifyHook(cwd, fakeBinDir, qwenHome, {
        'turn-id': sharedTurnId,
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.equal((tmuxLog.match(/send-keys -t %99 -l yes, proceed \[OMQ_TMUX_INJECT\]/g) || []).length, 1);
  ...
  it('applies TTL suppression between similar nudges and allows a later retry after TTL', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const third = runNotifyHook(cwd, fakeBinDir, qwenHome, {
      ...
      assert.equal((tmuxLog.match(/send-keys -t %99 -l yes, proceed \[OMQ_TMUX_INJECT\]/g) || []).length, 2);
  ...
  it('uses custom response from config', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.match(tmuxLog, /send-keys -t %99 -l continue now \[OMQ_TMUX_INJECT\]/, 'should use custom response with marker');
    });
  });
  ...
  for (const blockedResponse of ['yes', 'y', 'proceed', 'continue', 'ok', 'sure', 'go ahead']) {
    it(`blocks deep-interview auto-approval injection for "${blockedResponse}"`, async () => {
      await withTempWorkingDir(async (cwd) => {
        ...
        const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
        assert.doesNotMatch(tmuxLog, /Deep interview is active; auto-approval shortcuts are blocked until the interview finishes\. \[OMQ_TMUX_INJECT\]/);
        assert.equal(tmuxLog.includes(`send-keys -t %99 -l ${blockedResponse} [OMQ_TMUX_INJECT]`), false);
      });
    });
  }
  ...
  it('logs deep-interview auto-approval suppression without injecting tmux input', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.doesNotMatch(tmuxLog, /send-keys -t %99 -l Deep interview is active; auto-approval shortcuts are blocked until the interview finishes\. \[OMQ_TMUX_INJECT\]/);
  ...
  it('blocks deep-interview auto-approval injection for actionable "Next I should ..." replies', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.doesNotMatch(tmuxLog, /Deep interview is active; auto-approval shortcuts are blocked until the interview finishes\. \[OMQ_TMUX_INJECT\]/);
      assert.equal(tmuxLog.includes(`send-keys -t %99 -l ${NEXT_I_SHOULD_RESPONSE} [OMQ_TMUX_INJECT]`), false);
    });
  });
  ...
  it('uses custom patterns from config', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const result1 = runNotifyHook(cwd, fakeBinDir, qwenHome, {
        'last-assistant-message': 'Would you like me to proceed?',
      });
      ...
      const result2 = runNotifyHook(cwd, fakeBinDir, qwenHome, {
        'last-assistant-message': 'Changes ready. Awaiting approval before applying.',
      });
      assert.equal(result2.status, 0);

      const log2 = await readFile(tmuxLogPath, 'utf-8');
      assert.match(log2, /send-keys -t %99 -l yes, proceed \[OMQ_TMUX_INJECT\]/, 'custom pattern should trigger nudge with marker');
    });
  });
  ...
  it('defaults to enabled when no config file exists', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      // No .omq-config.json at all — should use defaults (enabled=true, stallMs=5000)
      await writeJson(join(qwenHome, '.omq-config.json'), {
        autoNudge: { enabled: true, delaySec: 0, stallMs: 0 },
      });
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.match(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMQ_TMUX_INJECT\]/, 'should nudge with default config and marker');
    });
  });
