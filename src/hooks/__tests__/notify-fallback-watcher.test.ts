...
if [[ "$cmd" == "capture-pane" ]]; then
  if [[ -n "\${OMQ_TEST_CAPTURE_SEQUENCE_FILE:-}" && -f "\${OMQ_TEST_CAPTURE_SEQUENCE_FILE}" ]]; then
    counterFile="\${OMQ_TEST_CAPTURE_COUNTER_FILE:-\${OMQ_TEST_CAPTURE_SEQUENCE_FILE}.idx}"
    idx=0
    if [[ -f "$counterFile" ]]; then idx="$(cat "$counterFile")"; fi
    lineNo=$((idx + 1))
    line="$(sed -n "\${lineNo}p" "\${OMQ_TEST_CAPTURE_SEQUENCE_FILE}" || true)"
    if [[ -z "$line" ]]; then
      line="$(tail -n 1 "\${OMQ_TEST_CAPTURE_SEQUENCE_FILE}" || true)"
    fi
    printf "%s\\n" "$line"
    echo "$lineNo" > "$counterFile"
    exit 0
  fi
  if [[ -n "\${OMQ_TEST_CAPTURE_FILE:-}" && -f "\${OMQ_TEST_CAPTURE_FILE}" ]]; then
    cat "\${OMQ_TEST_CAPTURE_FILE}"
  fi
  exit 0
fi
...
describe('notify-fallback watcher', () => {
  it('one-shot mode forwards only recent task_complete events', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-once-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omq-fallback-home-'));
    ...
    try {
      await mkdir(join(wd, '.omq', 'logs'), { recursive: true });
      ...
      const turnLog = join(wd, '.omq', 'logs', `turns-${new Date().toISOString().split('T')[0]}.jsonl`);
  ...
  it('streaming mode buffers partial JSON lines until the newline arrives', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-stream-partial-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omq-fallback-home-'));
...
`
      ...
      const watcherScript = new URL('../../../dist/scripts/notify-fallback-watcher.js', import.meta.url).pathname;
      const notifyHook = new URL('../../../dist/scripts/notify-hook.js', import.meta.url).pathname;
      const watcherStatePath = join(wd, '.omq', 'state', 'notify-fallback-state.json');
      const turnLog = join(wd, '.omq', 'logs', `turns-${new Date().toISOString().split('T')[0]}.jsonl`);
      const child = spawn(
        process.execPath,
  ...
  it('streaming mode tails from EOF and does not replay backlog', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-stream-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omq-fallback-home-'));
    const sid = randomUUID();
    const sessionDir = todaySessionDir(tempHome);
  ...
  it('records explicit leader-only dispatch drain state and log visibility in one-shot mode', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-dispatch-state-'));
    try {
      ...
      const logPath = join(wd, '.omq', 'logs', `notify-fallback-${new Date().toISOString().split('T')[0]}.jsonl`);
  ...
  it('disables fallback watcher nudges when deep-interview state is active', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-deep-interview-suppressed-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    try {
      await mkdir(join(wd, '.omq', 'logs'), { recursive: true });
      await mkdir(join(wd, '.omq', 'state', 'team', 'dispatch-team'), { recursive: true });
      await mkdir(fakeBinDir, { recursive: true });
      await writeFile(join(fakeBinDir, 'tmux'), buildFakeTmux(tmuxLogPath));
      await chmod(join(fakeBinDir, 'tmux'), 0o755);

      await writeFile(join(wd, '.omq', 'state', 'deep-interview-state.json'), JSON.stringify({
        active: true,
        mode: 'deep-interview',
        current_phase: 'deep-interview',
      }, null, 2));
      await writeFile(join(wd, '.omq', 'state', 'ralph-state.json'), JSON.stringify({
        active: true,
        current_phase: 'executing',
        tmux_pane_id: '%42',
      }, null, 2));
      await writeFile(join(wd, '.omq', 'state', 'team-state.json'), JSON.stringify({
        active: true,
        team_name: 'dispatch-team',
        current_phase: 'team-exec',
      }, null, 2));
      await writeFile(join(wd, '.omq', 'state', 'hud-state.json'), JSON.stringify({
        last_turn_at: new Date(Date.now() - 300_000).toISOString(),
        turn_count: 3,
        last_agent_output: 'Would you like me to continue?',
      }, null, 2));
      await writeFile(join(wd, '.omq', 'state', 'team', 'dispatch-team', 'config.json'), JSON.stringify({
        name: 'dispatch-team',
        tmux_session: 'omq-team-dispatch-team',
        leader_pane_id: '%42',
      }, null, 2));
      ...
      assert.doesNotMatch(tmuxLog, /yes, proceed \[OMQ_TMUX_INJECT\]/);
    } finally {
      await rm(wd, { recursive: true, force: true });
    }
  });
  ...
  it('runs leader nudge checks from the fallback watcher so stale alerts do not wait for a leader turn', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-leader-nudge-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    try {
      await mkdir(join(wd, '.omq', 'logs'), { recursive: true });
  ...
  it('skips fallback watcher leader nudges when the leader is not stale even if mailbox messages exist', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-leader-nudge-fresh-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    try {
      await mkdir(join(wd, '.omq', 'logs'), { recursive: true });
      await mkdir(join(wd, '.omq', 'state', 'team', 'dispatch-team', 'mailbox'), { recursive: true });
      await mkdir(fakeBinDir, { recursive: true });
      await writeFile(join(fakeBinDir, 'tmux'), buildFakeTmux(tmuxLogPath));
      await chmod(join(fakeBinDir, 'tmux'), 0o755);
      ...
      await writeFile(join(wd, '.omq', 'state', 'team', 'dispatch-team', 'mailbox', 'leader-fixed.json'), JSON.stringify({
        worker: 'leader-fixed',
        messages: [
          {
            message_id: 'msg-1',
  ...
  it('auto-nudges stalled session output even when no active mode state exists', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-auto-nudge-stalled-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const qwenHome = join(wd, 'qwen-home');
    try {
      await mkdir(join(wd, '.omq', 'logs'), { recursive: true });
      await mkdir(join(wd, '.omq', 'state'), { recursive: true });
      await mkdir(fakeBinDir, { recursive: true });
      ...
      const result = spawnSync(
        ...
        [watcherScript, '--once', '--cwd', wd, '--notify-script', notifyHook, '--poll-ms', '50'],
        {
          ...
          env: buildCleanNotifyEnv({
            ...
            TMUX: '1',
            TMUX_PANE: '%42',
            OMQ_NOTIFY_FALLBACK_AUTO_NUDGE_STALL_MS: '5000',
          }),
        },
      );
      assert.equal(result.status, 0, result.stderr || result.stdout);
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf8');
      assert.match(tmuxLog, /send-keys -t %42 -l yes, proceed \[OMQ_TMUX_INJECT\]/);
  ...
  it('does not auto-nudge stalled-like output when the latest turn is still fresh', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-auto-nudge-fresh-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const qwenHome = join(wd, 'qwen-home');
    try {
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf8').catch(() => '');
      assert.doesNotMatch(tmuxLog, /send-keys -t %42 -l yes, proceed \[OMQ_TMUX_INJECT\]/);
  ...
  it('does not fallback auto-nudge a stalled hud snapshot that notify-hook already nudged', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-auto-nudge-dedup-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const qwenHome = join(wd, 'qwen-home');
    ...
    try {
      ...
      await writeFile(join(wd, '.omq', 'state', 'auto-nudge-state.json'), JSON.stringify({
        nudgeCount: 1,
  ...
  it('runs bounded non-turn team dispatch drain tick in leader context', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-dispatch-'));
    try {
      await initTeamState('dispatch-team', 'task', 'executor', 1, wd);
  ...
  it('skips dispatch drain in worker context (leader-only guard)', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-dispatch-worker-'));
    try {
      await initTeamState('dispatch-team', 'task', 'executor', 1, wd);
      const queued = await enqueueDispatchRequest('dispatch-team', {
        kind: 'inbox',
      ...
      const result = spawnSync(
        process.execPath,
        [watcherScript, '--once', '--cwd', wd, '--notify-script', notifyHook, '--poll-ms', '50', '--dispatch-max-per-tick', '1'],
        { encoding: 'utf-8', env: buildCleanNotifyEnv({ OMQ_TEAM_WORKER: 'dispatch-team/worker-1', OMQ_TEAM_STATE_ROOT: join(wd, '.omq', 'state') }) },
      );
      assert.equal(result.status, 0, result.stderr || result.stdout);
      const request = await readDispatchRequest('dispatch-team', queued.request.request_id, wd);
  ...
  it('watcher retry does not retype when pre-capture still contains trigger', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-dispatch-cm-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    ...
    try {
      ...
      const env = {
        ...buildCleanNotifyEnv(),
        PATH: `${fakeBinDir}:${process.env.PATH || ''}`,
        OMQ_TEST_CAPTURE_FILE: captureFile,
      };
  ...
  it('sends bounded periodic Ralph continue steer while Ralph state stays active', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-ralph-active-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const stateDir = join(wd, '.omq', 'state');
    const tmuxLogPath = join(wd, 'tmux.log');
    ...
    try {
      ...
      let sends = boundedLog.match(/send-keys -t %42 -l Ralph loop active continue \[OMQ_TMUX_INJECT\]/g) || [];
      ...
      sends = finalLog.match(/send-keys -t %42 -l Ralph loop active continue \[OMQ_TMUX_INJECT\]/g) || [];
  ...
  it('suppresses Ralph continue steer when hud progress is still fresh after cooldown', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-ralph-progress-fresh-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const stateDir = join(wd, '.omq', 'state');
    const tmuxLogPath = join(wd, 'tmux.log');
    const statePath = join(stateDir, 'notify-fallback-state.json');
    try {
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf8').catch(() => '');
      const sends = tmuxLog.match(/send-keys -t %42 -l Ralph loop active continue \[OMQ_TMUX_INJECT\]/g) || [];
  ...
  it('still sends Ralph continue steer when hud progress is stale after cooldown', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-ralph-progress-stale-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const stateDir = join(wd, '.omq', 'state');
    const tmuxLogPath = join(wd, 'tmux.log');
    const statePath = join(stateDir, 'notify-fallback-state.json');
  ...
  it('fails closed when Ralph hud progress is missing or invalid', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-ralph-progress-guard-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const stateDir = join(wd, '.omq', 'state');
    const tmuxLogPath = join(wd, 'tmux.log');
    const statePath = join(stateDir, 'notify-fallback-state.json');
  ...
  it('fails closed when active Ralph state has no bound tmux pane', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-ralph-pane-missing-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const stateDir = join(wd, '.omq', 'state');
    const tmuxLogPath = join(wd, 'tmux.log');
    const statePath = join(stateDir, 'notify-fallback-state.json');
    try {
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf8').catch(() => '');
      assert.equal(/send-keys -t %42 -l Ralph loop active continue \[OMQ_TMUX_INJECT\]/.test(tmuxLog), false);
  ...
  it('waits a full cadence from startup when persisted Ralph steer state is empty', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-ralph-startup-cooldown-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const stateDir = join(wd, '.omq', 'state');
    const tmuxLogPath = join(wd, 'tmux.log');
    const statePath = join(stateDir, 'notify-fallback-state.json');
  ...
  it('falls back to an aged persisted cooldown anchor when last_sent_at is invalid', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-ralph-invalid-last-sent-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const stateDir = join(wd, '.omq', 'state');
    const tmuxLogPath = join(wd, 'tmux.log');
    const statePath = join(stateDir, 'notify-fallback-state.json');
  ...
  it('stops Ralph continue steer immediately once Ralph state is terminal or cleared', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-ralph-terminal-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const stateDir = join(wd, '.omq', 'state');
  ...
  it('globally debounces Ralph continue steer across concurrent watcher instances', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-ralph-global-debounce-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const stateDir = join(wd, '.omq', 'state');
    const sharedTimestampPath = join(stateDir, 'ralph-last-steer-at');
  ...
  it('keeps team control-plane pumping when Ralph continue steer fails', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-control-plane-split-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    try {
      await mkdir(join(wd, '.omq', 'state'), { recursive: true });
      ...
      await writeFile(join(wd, '.omq', 'state', 'notify-fallback-state.json'), JSON.stringify({
        ralph_continue_steer: {
          last_sent_at: new Date(Date.now() - 61_000).toISOString(),
        },
      }, null, 2));
  ...
  it('retypes on every retry when trigger is not in narrow input area', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-dispatch-cm-fallback-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    ...
    try {
      ...
      const env = {
        ...buildCleanNotifyEnv(),
        PATH: `${fakeBinDir}:${process.env.PATH || ''}`,
        OMQ_TEST_CAPTURE_SEQUENCE_FILE: captureSeqFile,
        OMQ_TEST_CAPTURE_COUNTER_FILE: captureCounterFile,
      };
  ...
  it('exits when the tracked parent pid is gone', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-parent-exit-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omq-fallback-parent-home-'));
    const watcherScript = new URL('../../../dist/scripts/notify-fallback-watcher.js', import.meta.url).pathname;
    const notifyHook = new URL('../../../dist/scripts/notify-hook.js', import.meta.url).pathname;
    const logPath = join(wd, '.omq', 'logs', `notify-fallback-${new Date().toISOString().split('T')[0]}.jsonl`);
  ...
  it('ignores stale session-scoped Ralph state when the current session identity is stale', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-stale-session-ralph-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const stateDir = join(wd, '.omq', 'state');
  ...
  it('ignores stale root Ralph state when the current session has not started Ralph', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-stale-root-ralph-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const stateDir = join(wd, '.omq', 'state');
  ...
  it('keeps ticking for active session-scoped Ralph after parent loss, then stops once Ralph is terminal', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-parent-ralph-active-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omq-fallback-parent-ralph-home-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const stateDir = join(wd, '.omq', 'state');
    ...
    try {
      ...
      await waitFor(async () => {
        ...
        return /send-keys -t %42 -l Ralph loop active continue \[OMQ_TMUX_INJECT\]/.test(tmuxLog);
      }, 4000, 50);
  ...
  it('stays alive after parent exit while an active team still has live worker panes', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-parent-gone-team-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omq-fallback-parent-gone-team-home-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    const teamStatePath = join(wd, '.omq', 'state', 'team-state.json');
    let child: ReturnType<typeof spawn> | undefined;
  ...
  it('does not defer parent-loss shutdown for a team that is already terminal in phase.json', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-parent-gone-terminal-team-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omq-fallback-parent-gone-terminal-team-home-'));
    const fakeBinDir = join(wd, 'fake-bin');
    const tmuxLogPath = join(wd, 'tmux.log');
    let child: ReturnType<typeof spawn> | undefined;
    ...
    try {
      ...
      await writeFile(join(wd, '.omq', 'state', 'team', 'dispatch-team', 'phase.json'), JSON.stringify({
        current_phase: 'complete',
      }, null, 2));
      await writeFile(join(wd, '.omq', 'state', 'team', 'dispatch-team', 'config.json'), JSON.stringify({
        name: 'dispatch-team',
  ...
  it('replaces a stale watcher from the per-cwd pid file', async () => {
    ...
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-stale-pid-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omq-fallback-stale-home-'));
    const watcherScript = new URL('../../../dist/scripts/notify-fallback-watcher.js', import.meta.url).pathname;
    const notifyHook = new URL('../../../dist/scripts/notify-hook.js', import.meta.url).pathname;
    const pidPath = join(wd, '.omq', 'state', 'notify-fallback.pid');
  ...
  it('exits after the configured max lifetime', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-fallback-max-life-'));
    const tempHome = await mkdtemp(join(tmpdir(), 'omq-fallback-max-home-'));
    const watcherScript = new URL('../../../dist/scripts/notify-fallback-watcher.js', import.meta.url).pathname;
    const notifyHook = new URL('../../../dist/scripts/notify-hook.js', import.meta.url).pathname;
    const logPath = join(wd, '.omq', 'logs', `notify-fallback-${new Date().toISOString().split('T')[0]}.jsonl`);
    let child: ReturnType<typeof spawn> | undefined;
