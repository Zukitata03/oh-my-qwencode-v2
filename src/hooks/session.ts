/**
 * Session Lifecycle Manager for oh-my-qwencode
 *
...
import { omqStateDir, omqLogsDir } from '../utils/paths.js';
...
function sessionPath(cwd: string): string {
  return join(omqStateDir(cwd), SESSION_FILE);
}

function historyPath(cwd: string): string {
  return join(omqLogsDir(cwd), HISTORY_FILE);
}

/**
 * Reset session-scoped HUD/metrics files at launch so stale values do not leak
 * into a new Qwen Code session.
 */
export async function resetSessionMetrics(cwd: string): Promise<void> {
  ...
  await writeFile(join(omqDir, 'metrics.json'), JSON.stringify({
    total_turns: 0,
    session_turns: 0,
    last_activity: now,
...
export async function appendToLog(cwd: string, entry: Record<string, unknown>): Promise<void> {
  ...
  const logFile = join(logsDir, `omq-${date}.jsonl`);
