/**
 * AGENTS.md Runtime Overlay for oh-my-qwencode
 *
 * Dynamically injects session-specific context into AGENTS.md before Qwen Code
...
const START_MARKER = "<!-- OMQ:RUNTIME:START -->";
const END_MARKER = "<!-- OMQ:RUNTIME:END -->";
const WORKER_START_MARKER = "<!-- OMQ:TEAM:WORKER:START -->";
const WORKER_END_MARKER = "<!-- OMQ:TEAM:WORKER:END -->";
...
function lockPath(cwd: string): string {
  return join(cwd, ".omq", "state", "agents-md.lock");
}
...
async function readNotepadPriority(cwd: string): Promise<string> {
  const notePath = omqNotepadPath(cwd);
...
async function readProjectMemorySummary(cwd: string): Promise<string> {
  const memPath = omqProjectMemoryPath(cwd);
...
/**
 * Build a session-scoped AGENTS.md that combines user-level QWEN_HOME
...
export async function writeSessionModelInstructionsFile(
  ...
  const sourcePaths = [join(qwenHome(), "AGENTS.md"), join(cwd, "AGENTS.md")];
  ...
  for (const sourcePath of sourcePaths) {
    ...
    if (sourcePath === join(qwenHome(), "AGENTS.md")) {
