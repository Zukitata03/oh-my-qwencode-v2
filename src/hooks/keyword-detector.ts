 ...
 *
 * In OMQ, this logic is embedded in the AGENTS.md orchestration brain,
 * and can also be used by the notify hook for state tracking.
 *
 * When Qwen Code adds pre-hook support, this module can be promoted
...
export async function recordSkillActivation(input: RecordSkillActivationInput): Promise<SkillActiveState | null> {
  ...
  if (hasCancelIntent && hadDeepInterviewLock) {
    ...
    try {
      ...
      console.warn('[omq] warning: failed to persist keyword activation state', error);
    }
 ...
 * In OMQ they generate AGENTS.md instructions and serve as test infrastructure.
