...
export function hookPluginRootDir(cwd: string, pluginName: string): string {
  return join(cwd, '.omq', 'state', 'hooks', 'plugins', sanitizeHookPluginName(pluginName));
}
...
export function hookPluginLogPath(cwd: string, now = new Date()): string {
  ...
  return join(cwd, '.omq', 'logs', `hooks-${day}.jsonl`);
}

export function omqRootStateFilePath(cwd: string, fileName: string): string {
  return join(cwd, '.omq', 'state', fileName);
}
