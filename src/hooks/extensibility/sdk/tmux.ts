...
function resolveSessionPaneTarget(sessionName: string): HookPluginSendKeysResult {
  ...
  const canonicalRows = nonHudRows.filter((row) => /qwen/i.test(row.startCommand));
...
function resolveTmuxTarget(options: HookPluginSendKeysOptions): HookPluginSendKeysResult {
  ...
  const envPane = String(resolveQwenPane() || '').trim();
...
async function sendTmuxKeys(
  ...
  const marker = process.env.OMQ_HOOK_PLUGIN_LOOP_MARKER || '[OMQ_HOOK_PLUGIN]';
  ...
  const cooldownMs = typeof options.cooldownMs === 'number'
    ...
    : asPositiveNumber(process.env.OMQ_HOOK_PLUGIN_COOLDOWN_MS, DEFAULT_COOLDOWN_MS);
  const dedupeWindowMs = asPositiveNumber(process.env.OMQ_HOOK_PLUGIN_DEDUPE_MS, DEFAULT_DEDUPE_WINDOW_MS);
