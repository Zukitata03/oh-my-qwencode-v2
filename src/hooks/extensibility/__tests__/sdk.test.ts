...
async function writeOmqStateFile(cwd: string, fileName: string, value: unknown): Promise<void> {
...
describe('createHookPluginSdk', () => {
  describe('state', () => {
    it('reads undefined for missing key', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omq-sdk-'));
      try {
  ...
  describe('tmux.sendKeys', () => {
    ...
    it('returns loop_guard_input_marker when text contains loop marker', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omq-sdk-'));
      const originalMarker = process.env.OMQ_HOOK_PLUGIN_LOOP_MARKER;
      try {
        process.env.OMQ_HOOK_PLUGIN_LOOP_MARKER = '[TESTMARK]';
        const sdk = createHookPluginSdk({
          cwd,
          pluginName: 'test',
          event: makeEvent(),
        ...
        if (originalMarker === undefined) {
          delete process.env.OMQ_HOOK_PLUGIN_LOOP_MARKER;
        } else {
          process.env.OMQ_HOOK_PLUGIN_LOOP_MARKER = originalMarker;
        }
        await rm(cwd, { recursive: true, force: true });
      }
    });
    ...
    it('prefers non-HUD qwen pane when targeting a tmux session', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omq-sdk-'));
      const fakeBinDir = await mkdtemp(join(tmpdir(), 'omq-sdk-bin-'));
...
if [[ "$cmd" == "list-panes" ]]; then
  printf "%%42	1	node /pkg/dist/cli/omq.js hud --watch
%%42	0	qwen --model gpt-5
"
  exit 0
fi
...
`);
  ...
  describe('omq', () => {
    it('exposes only the explicit read-only omq readers', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omq-sdk-'));
      try {
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });

        assert.deepEqual(Object.keys(sdk.omq).sort(), ['hud', 'notifyFallback', 'session', 'updateCheck']);
        assert.equal(typeof sdk.omq.session.read, 'function');
        assert.equal(typeof sdk.omq.hud.read, 'function');
        assert.equal(typeof sdk.omq.notifyFallback.read, 'function');
        assert.equal(typeof sdk.omq.updateCheck.read, 'function');
        assert.equal('pluginState' in sdk, false);
        assert.equal('readJson' in sdk.omq, false);
        assert.equal('list' in sdk.omq, false);
        assert.equal('exists' in sdk.omq, false);
        assert.equal('write' in sdk.omq.session, false);
        assert.equal('delete' in sdk.omq.session, false);
        assert.equal('write' in sdk.omq.hud, false);
        assert.equal('delete' in sdk.omq.hud, false);
        assert.equal('write' in sdk.omq.notifyFallback, false);
        assert.equal('delete' in sdk.omq.notifyFallback, false);
        assert.equal('write' in sdk.omq.updateCheck, false);
        assert.equal('delete' in sdk.omq.updateCheck, false);
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
    });

    it('reads session state from .omq/state/session.json', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omq-sdk-'));
      try {
        await writeOmqStateFile(cwd, 'session.json', {
        ...
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        const state = await sdk.omq.session.read();
    ...
    it('returns null for invalid session state without session_id', async () => {
      ...
      try {
        ...
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        assert.equal(await sdk.omq.session.read(), null);
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
    });

    it('reads hud, notifyFallback, and updateCheck state from root-scoped omq files', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omq-sdk-'));
      try {
        await writeOmqStateFile(cwd, 'hud-state.json', {
          last_turn_at: '2026-01-01T00:00:00.000Z',
          turn_count: 3,
        });
        await writeOmqStateFile(cwd, 'notify-fallback-state.json', {
          pid: 1234,
          stopping: false,
          tracked_files: 2,
        });
        await writeOmqStateFile(cwd, 'update-check.json', {
          last_checked_at: '2026-01-01T00:00:00.000Z',
          last_seen_latest: '0.11.0',
        });

        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        assert.deepEqual(await sdk.omq.hud.read(), {
          last_turn_at: '2026-01-01T00:00:00.000Z',
          turn_count: 3,
        });
        assert.deepEqual(await sdk.omq.notifyFallback.read(), {
          pid: 1234,
          stopping: false,
          tracked_files: 2,
        });
        assert.deepEqual(await sdk.omq.updateCheck.read(), {
          last_checked_at: '2026-01-01T00:00:00.000Z',
          last_seen_latest: '0.11.0',
        });
      } finally {
    ...
    it('returns null for missing omq reader files', async () => {
      const cwd = await mkdtemp(join(tmpdir(), 'omq-sdk-'));
      try {
        const sdk = createHookPluginSdk({ cwd, pluginName: 'test', event: makeEvent() });
        assert.equal(await sdk.omq.session.read(), null);
        assert.equal(await sdk.omq.hud.read(), null);
        assert.equal(await sdk.omq.notifyFallback.read(), null);
        assert.equal(await sdk.omq.updateCheck.read(), null);
      } finally {
        await rm(cwd, { recursive: true, force: true });
      }
    });
...
describe('clearHookPluginState', () => {
  it('removes data.json and tmux.json for plugin', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-clear-'));
    try {
      const pluginDir = join(cwd, '.omq', 'state', 'hooks', 'plugins', 'my-plugin');
