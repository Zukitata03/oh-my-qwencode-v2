...
describe('hooksDir', () => {
  it('returns .omq/hooks under cwd', () => {
    assert.equal(hooksDir('/project'), join('/project', '.omq', 'hooks'));
  });
});
...
describe('ensureHooksDir', () => {
  it('creates .omq/hooks directory and returns path', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-ensure-'));
    try {
      const dir = await ensureHooksDir(cwd);
      assert.equal(dir, join(cwd, '.omq', 'hooks'));
...
describe('discoverHookPlugins', () => {
  it('returns empty array when hooks directory does not exist', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-discover-'));
    try {
...
describe('validateHookPluginExport', () => {
  it('returns valid for plugin with onHookEvent export', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-validate-'));
    try {
...
describe('loadHookPluginDescriptors', () => {
  it('returns empty array when no hooks directory', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-load-'));
    try {
