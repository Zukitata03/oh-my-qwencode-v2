...
/** Global OMQ config shape (subset relevant to code-simplifier) */
interface OmqGlobalConfig {
...
/**
 * Read the global OMQ config from ~/.omq/config.json.
 * Returns null if the file does not exist or cannot be parsed.
 *
 * @param configDir - Optional override for the home directory. When provided,
 *   the config is read from `<configDir>/.omq/config.json` instead of
 *   `~/.omq/config.json`. Useful for testing without relying on `os.homedir()`.
 */
export function readOmqConfig(configDir?: string): OmqGlobalConfig | null {
  const configPath = join(configDir ?? homedir(), '.omq', 'config.json');
  ...
  try {
    return JSON.parse(readFileSync(configPath, 'utf-8')) as OmqGlobalConfig;
  } catch {
    return null;
  }
}
...
export function isCodeSimplifierEnabled(configDir?: string): boolean {
  const config = readOmqConfig(configDir);
