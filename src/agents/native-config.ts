/**
 * Native agent config generators for Qwen Code.
 * Writes standalone TOML files under ~/.qwen/agents/ or ./.qwen/agents/.
 */
...
import { qwenAgentsDir } from "../utils/paths.js";

export const EXACT_GPT_5_4_MINI_MODEL = "qwen3.6-flash";
...
function readConfigTomlContent(
  ...
  const configPath = join(qwenHomeOverride ?? process.env.QWEN_HOME ?? "", "config.toml");
  if (qwenHomeOverride && existsSync(configPath)) {
...
function resolveFrontierModel(options: AgentModelResolutionOptions): string {
  const configTomlContent = readConfigTomlContent(
    options.qwenHomeOverride,
    options.configTomlContent,
  );
  return getRootModelName(configTomlContent)
    ?? getMainDefaultModel(options.qwenHomeOverride);
}
...
function resolveStandardModel(options: AgentModelResolutionOptions): string {
  ...
  return getStandardDefaultModel(options.qwenHomeOverride);
}
...
function resolveAgentModel(
  ...
  switch (agent.modelClass) {
    case "frontier":
      return resolveFrontierModel(options);
    case "fast":
      return getSparkDefaultModel(options.qwenHomeOverride);
    case "standard":
    default:
...
export function composeRoleInstructions(
  ...
  if (resolvedModel) {
    if (metadataLines.length === 0) {
      metadataLines.push("## OMQ Agent Metadata");
    }
...
export function generateStandaloneAgentToml(
  ...
  const lines = [
    `# oh-my-qwencode agent: ${config.name}`,
...
/**
 * Generate TOML content for a prompt-backed OMQ role agent.
 */
...
/**
 * Install prompt-backed native agent config .toml files to ~/.qwen/agents/
...
export async function installNativeAgentConfigs(
  ...
  const {
    ...
    dryRun = false,
    verbose = false,
    agentsDir = qwenAgentsDir(),
  } = options;
  const qwenHomeOverride = join(agentsDir, "..");
  ...
  for (const [name, agent] of Object.entries(AGENT_DEFINITIONS)) {
    ...
    const toml = generateAgentToml(agent, promptContent, { qwenHomeOverride });
