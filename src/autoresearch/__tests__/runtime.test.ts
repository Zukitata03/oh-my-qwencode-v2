...
describe('autoresearch runtime', () => {
  it('builds bootstrap instructions with mission, sandbox, and evaluator contract', async () => {
    ...
    try {
      ...
      const instructions = buildAutoresearchInstructions(contract, { runId: 'missions-demo-20260314t000000z', iteration: 1, baselineCommit: 'abc1234', lastKeptCommit: 'abc1234', resultsFile: 'results.tsv', candidateFile: '.omq/logs/autoresearch/missions-demo-20260314t000000z/candidate.json', keepPolicy: 'score_improvement' });
  ...
  it('allows untracked .omq runtime files when checking reset safety', async () => {
    const repo = await initRepo();
    try {
      await mkdir(join(repo, '.omq', 'logs'), { recursive: true });
      await mkdir(join(repo, '.omq', 'state'), { recursive: true });
      await writeFile(join(repo, '.omq', 'logs', 'hooks-2026-03-15.jsonl'), '{}\n', 'utf-8');
      await writeFile(join(repo, '.omq', 'metrics.json'), '{}\n', 'utf-8');
      await writeFile(join(repo, '.omq', 'state', 'hud-state.json'), '{}\n', 'utf-8');
  ...
  it('prepares runtime artifacts and persists autoresearch mode state', async () => {
    ...
    try {
      ...
      const worktreePath = join(repo, '.omq', 'worktrees', 'autoresearch-missions-demo-20260314t000000z');
...
describe('autoresearch parity decisions', () => {
  it('keeps improved candidates and resets discarded candidates back to the last kept commit', async () => {
    ...
    try {
      const contract = await makeContract(repo);
      const worktreePath = join(repo, '.omq', 'worktrees', 'autoresearch-missions-demo-20260314t010000z');
