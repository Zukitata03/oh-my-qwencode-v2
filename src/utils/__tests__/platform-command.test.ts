...
describe('buildPlatformCommandSpec', () => {
  it('wraps .cmd shims through ComSpec on Windows', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'omq-platform-cmd-'));
    try {
      const cmdPath = join(fakeBin, 'qwen.cmd');
      ...
      assert.match(spec.args[3] || '', /qwen\.cmd/i);
  ...
  it('launches .exe binaries directly on Windows', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'omq-platform-exe-'));
    try {
  ...
  it('prefers PowerShell shims over cmd shims when both exist', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'omq-platform-ps1-'));
    try {
      const ps1Path = join(fakeBin, 'qwen.ps1');
      const cmdPath = join(fakeBin, 'qwen.cmd');
...
describe('resolveCommandPathForPlatform', () => {
  it('prefers PATHEXT candidates on Windows', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'omq-platform-path-'));
    try {
      const exePath = join(fakeBin, 'tmux.exe');
      await writeFile(exePath, '');
      assert.equal(
  ...
  it('resolves PATH entries to absolute paths on POSIX', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'omq-platform-posix-'));
    try {
...
describe('spawnPlatformCommandSync', () => {
  it('passes the Windows-resolved spec into the spawn implementation', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'omq-platform-spawn-'));
    try {
      const cmdPath = join(fakeBin, 'qwen.cmd');
      await writeFile(cmdPath, '@echo off\r\n');
      const calls: Array<{
      ...
      assert.match((calls[0]?.args[3] || ''), /qwen\.cmd/i);
  ...
  it('does not force verbatim arguments for direct Windows executables', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'omq-platform-spawn-exe-'));
    try {
      const exePath = join(fakeBin, 'tmux.exe');
      await writeFile(exePath, '');
      const calls: Array<{
  ...
  it('launches Windows cmd shims successfully with the real spawn implementation', async () => {
    ...
    const fakeBin = await mkdtemp(join(tmpdir(), 'omq-platform-spawn-real-'));
    try {
      const cmdPath = join(fakeBin, 'qwen.cmd');
      await writeFile(cmdPath, '@echo off\r\necho fake-qwen 1.2.3\r\n');
      ...
      assert.equal((probed.result.stdout || '').trim(), 'fake-qwen 1.2.3');
    } finally {
      await rm(fakeBin, { recursive: true, force: true });
    }
  });
  ...
  it('retries blocked node-hosted scripts through process.execPath on non-Windows', () => {
    const scriptPath = '/tmp/omq-explore-stub.js';
