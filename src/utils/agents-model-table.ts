...
export const OMQ_MODELS_START_MARKER = '<!-- OMQ:MODELS:START -->';
export const OMQ_MODELS_END_MARKER = '<!-- OMQ:MODELS:END -->';
...
export function resolveAgentsModelTableContext(
  ...
  const { qwenHomeOverride, env = process.env } = options;
  const frontierModel =
    getRootModelName(configTomlContent) ??
    getEnvConfiguredMainDefaultModel(env, qwenHomeOverride) ??
    DEFAULT_FRONTIER_MODEL;
  const sparkModel =
    getEnvConfiguredSparkDefaultModel(env, qwenHomeOverride) ??
    getSparkDefaultModel(qwenHomeOverride) ??
    DEFAULT_SPARK_MODEL;
  const subagentDefaultModel =
    getEnvConfiguredStandardDefaultModel(env, qwenHomeOverride) ??
    getStandardDefaultModel(qwenHomeOverride) ??
    DEFAULT_STANDARD_MODEL;
...
export function upsertAgentsModelTable(
  ...
  const startIndex = content.indexOf(OMQ_MODELS_START_MARKER);
  const endIndex = content.indexOf(OMQ_MODELS_END_MARKER);

  if (startIndex >= 0 && endIndex > startIndex) {
    const replaceEnd = endIndex + OMQ_MODELS_END_MARKER.length;
