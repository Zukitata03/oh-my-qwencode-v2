...
describe('VULNERABILITY – old string-interpolation approach', () => {
   ...
   *   const cmd  = `node ${omqBin} hud --watch${presetArg}`;
   *   execSync(`tmux split-window -v -l 4 -c "${cwd}" '${cmd}'`);
   */
  function buildOldCommand(cwd: string, omqBin: string, preset?: string): string {
  ...
  it('cwd containing $() is injectable via double-quote context', () => {
    ...
    const shellCmd = buildOldCommand(maliciousCwd, '/usr/bin/omq.js');
  ...
  it("omqBin containing single quote breaks out of the '-quoted command", () => {
    const maliciousOmq = "/tmp/it';touch /tmp/pwned;echo '/omq.js";
    const shellCmd = buildOldCommand('/home/user', maliciousOmq);

    // The injected single quote terminates the tmux shell-command argument
    // early, allowing arbitrary commands to follow.
    assert.ok(
      shellCmd.includes("'node /tmp/it';touch /tmp/pwned;echo '/omq.js"),
...
describe('buildTmuxSplitArgs – shell injection hardening', () => {
  it('produces correct argv for normal inputs', () => {
    const args = buildTmuxSplitArgs('/home/user/project', '/usr/local/bin/omq.js');
    assert.deepEqual(args, [
      'split-window', '-v', '-l', String(HUD_TMUX_HEIGHT_LINES),
      // split height should come from shared HUD constants
      '-c', '/home/user/project',
      "node '/usr/local/bin/omq.js' hud --watch",
    ]);
  });

  it('cwd is an isolated array element – never shell-interpreted', () => {
    const maliciousCwd = '/tmp/foo"$(touch /tmp/pwned)"bar';
    const args = buildTmuxSplitArgs(maliciousCwd, '/usr/bin/omq.js');
  ...
  it("omqBin with single quote is properly escaped in command string", () => {
    const maliciousOmq = "/tmp/it's/omq.js";
    const args = buildTmuxSplitArgs('/home/user', maliciousOmq);
    const cmd = args[6];
    ...
    assert.equal(cmd, "node '/tmp/it'\\''s/omq.js' hud --watch");
  });

  it('omqBin with $() is neutralised by single-quote wrapping', () => {
    const maliciousOmq = '/tmp/$(id)/omq.js';
    const args = buildTmuxSplitArgs('/home/user', maliciousOmq);
    const cmd = args[6];

    // Inside single quotes, $() is literal.
    assert.equal(cmd, "node '/tmp/$(id)/omq.js' hud --watch");
  });

  it('omqBin with backticks is neutralised by single-quote wrapping', () => {
    const maliciousOmq = '/tmp/`whoami`/omq.js';
    const args = buildTmuxSplitArgs('/home/user', maliciousOmq);
    const cmd = args[6];

    assert.equal(cmd, "node '/tmp/`whoami`/omq.js' hud --watch");
  });

  it("omqBin with ';command' breakout attempt is neutralised", () => {
    const maliciousOmq = "/tmp/x';touch /tmp/pwned;echo '/omq.js";
    const args = buildTmuxSplitArgs('/home/user', maliciousOmq);
    const cmd = args[6];
    ...
    //
    // Raw expected value: node '/tmp/x'\'';touch /tmp/pwned;echo '\''/omq.js' hud --watch
    assert.equal(
      cmd,
      "node '/tmp/x'\\'';touch /tmp/pwned;echo '\\''/omq.js' hud --watch",
    );
  ...
  it('preset is appended safely', () => {
    const args = buildTmuxSplitArgs('/home/user', '/usr/bin/omq.js', 'minimal');
    const cmd = args[6];
    assert.ok(cmd.endsWith('--preset=minimal'));
  });

  it('absent preset produces no --preset flag', () => {
    const args = buildTmuxSplitArgs('/home/user', '/usr/bin/omq.js');
    const cmd = args[6];
  ...
  it('invalid preset is dropped (defense in depth)', () => {
    ...
    const cmd = args[6];
    assert.equal(cmd, "node '/usr/bin/omq.js' hud --watch");
    assert.ok(!cmd.includes('--preset='));
  });
});
