/**
 * OMQ HUD - CLI entry point
 *
 * Usage:
 *   omq hud              Show current HUD state
 *   omq hud --watch      Poll every 1s with terminal clear
 *   omq hud --json       Output raw state as JSON
 *   omq hud --preset=X   Use preset: minimal, focused, full
 *   omq hud --tmux       Open HUD in a tmux split pane (auto-detects orientation)
 */
 ...
 * passed as a literal argument to tmux (no shell expansion).  `omqBin` is
...
export function buildTmuxSplitArgs(
  ...
  const cmd = `node ${shellEscape(omqBin)} hud --watch${presetArg}`;
...
async function launchTmuxPane(cwd: string, flags: HudFlags): Promise<void> {
  ...
  if (!process.env.TMUX) {
    console.error('Not inside a tmux session. Start tmux first, then run: omq hud --tmux');
    process.exit(1);
  }

  const omqBin = process.argv[1]; // path to bin/omq.js
  const args = buildTmuxSplitArgs(cwd, omqBin, flags.preset);

  try {
    // Split bottom pane, 4 lines tall, running omq hud --watch.
    // execFileSync bypasses the shell – cwd and omqBin cannot inject commands.
