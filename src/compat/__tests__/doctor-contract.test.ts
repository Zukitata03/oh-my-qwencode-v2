...
const defaultTarget = join(repoRoot, 'dist', 'cli', 'omq.js');
...
function resolveCompatTarget(): { command: string; argsPrefix: string[] } {
  const override = process.env.OMQ_COMPAT_TARGET?.trim();
...
function normalizeInstallDoctorOutput(text: string, home: string, cwd: string): string {
  const repoStateDir = join(cwd, '.omq', 'state').replace(/\\/g, '/');
  return text
    .replaceAll(join(home, '.qwen').replace(/\\/g, '/'), '<QWEN_HOME>')
    .replaceAll(`/private${repoStateDir}`, '<REPO_STATE_DIR>')
    .replaceAll(repoStateDir, '<REPO_STATE_DIR>')
    .replace(/\\/g, '/')
    .split('\n')
    .map((line) => {
      if (line.startsWith('  [OK] Qwen Code:') || line.startsWith('  [XX] Qwen Code:')) {
        return '  [QWEN_CLI_STATUS]';
      }
      ...
      if (line.startsWith('Run "omq setup')) {
...
describe('compat doctor contract', () => {
  it('matches onboarding warning copy for first setup expectations', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-compat-doctor-'));
    const home = join(wd, 'home');
    const qwenHome = join(home, '.qwen');
    await mkdir(qwenHome, { recursive: true });
    await writeFile(join(qwenHome, 'config.toml'), '[mcp_servers.non_omq]\ncommand = "node"\n');

    try {
      const result = runCompatTarget(wd, ['doctor'], { HOME: home, QWEN_HOME: qwenHome });
  ...
  it('matches doctor --team resume_blocker behavior', async () => {
    const wd = await mkdtemp(join(tmpdir(), 'omq-compat-doctor-team-'));
    try {
      const teamRoot = join(wd, '.omq', 'state', 'team', 'alpha');
      await mkdir(join(teamRoot, 'workers', 'worker-1'), { recursive: true });
      await writeFile(join(teamRoot, 'config.json'), JSON.stringify({ name: 'alpha', tmux_session: 'omq-team-alpha' }));
