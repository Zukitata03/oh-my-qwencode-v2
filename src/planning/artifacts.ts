...
import { omqPlansDir } from '../utils/paths.js';
...
export function readPlanningArtifacts(cwd: string): PlanningArtifacts {
  const plansDir = omqPlansDir(cwd);
  const specsDir = join(cwd, '.omq', 'specs');
...
export function readApprovedExecutionLaunchHint(
  ...
  if (mode === 'team') {
    const teamPattern = /(?<command>(?:omq\s+team|\$team)\s+(?<ralph>ralph\s+)?(?<count>\d+)(?::(?<role>[a-z][a-z0-9-]*))?\s+(?<task>"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'))/gi;
  ...
  const ralphPattern = /(?<command>(?:omq\s+ralph|\$ralph)\s+(?<task>"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'))/gi;
