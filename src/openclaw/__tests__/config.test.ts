...
describe("getOpenClawConfig", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = join(tmpdir(), `omq-openclaw-test-${Date.now()}`);
    mkdirSync(tmpDir, { recursive: true });
    process.env.QWEN_HOME = join(tmpDir, ".qwen");
  });
  ...
  it("returns null when OMQ_OPENCLAW !== '1'", async () => {
  ...
  it("returns null when OMQ_OPENCLAW_CONFIG file does not exist", async () => {
    process.env.OMQ_OPENCLAW = "1";
    process.env.OMQ_OPENCLAW_CONFIG = join(tmpDir, "nonexistent.json");
    const { getOpenClawConfig, resetOpenClawConfigCache } = await import("../config.js");
    resetOpenClawConfigCache();
    const result = getOpenClawConfig();
    assert.equal(result, null);
  ...
  it("reads config from OMQ_OPENCLAW_CONFIG override file", async () => {
    process.env.OMQ_OPENCLAW = "1";
    ...
    process.env.OMQ_OPENCLAW_CONFIG = configPath;
    const { getOpenClawConfig, resetOpenClawConfigCache } = await import("../config.js");
    resetOpenClawConfigCache();
    const result = getOpenClawConfig();
...
describe("getOpenClawConfig generic alias normalization", () => {
  ...
  beforeEach(() => {
    originalEnv = { ...process.env };
    tmpDir = join(tmpdir(), `omq-openclaw-alias-test-${Date.now()}`);
    mkdirSync(tmpDir, { recursive: true });
    process.env.OMQ_OPENCLAW = "1";
    process.env.HOME = tmpDir;
    process.env.QWEN_HOME = join(tmpDir, ".qwen");
    delete process.env.OMQ_OPENCLAW_CONFIG;
  });
  ...
  it("normalizes custom_webhook_command alias to openclaw runtime config", async () => {
    const omqConfigPath = join(tmpDir, ".qwen", ".omq-config.json");
    mkdirSync(join(tmpDir, ".qwen"), { recursive: true });
    writeFileSync(omqConfigPath, JSON.stringify({
      notifications: {
        enabled: true,
