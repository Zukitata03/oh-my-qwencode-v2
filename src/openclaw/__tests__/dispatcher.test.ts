...
describe('resolveCommandTimeoutMs', () => {
  afterEach(() => {
    delete process.env.OMQ_OPENCLAW_COMMAND_TIMEOUT_MS;
  });
...
describe('wakeCommandGateway - command gate', () => {
  afterEach(() => {
    delete process.env.OMQ_OPENCLAW_COMMAND;
    delete process.env.OMQ_OPENCLAW_COMMAND_TIMEOUT_MS;
  });

  it('returns error when OMQ_OPENCLAW_COMMAND is not set', async () => {
    ...
    assert.ok(result.error?.includes('OMQ_OPENCLAW_COMMAND'));
  });

  it('succeeds when OMQ_OPENCLAW_COMMAND=1 and command exits 0', async () => {
    const { wakeCommandGateway } = await import('../dispatcher.js');
    process.env.OMQ_OPENCLAW_COMMAND = '1';
  ...
  it('uses env timeout when gateway timeout is not set', async () => {
    const { wakeCommandGateway } = await import('../dispatcher.js');
    process.env.OMQ_OPENCLAW_COMMAND = '1';
    process.env.OMQ_OPENCLAW_COMMAND_TIMEOUT_MS = '100';
