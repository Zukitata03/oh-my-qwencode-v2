 ...
 *
 * Reads OpenClaw config from the notifications.openclaw key in ~/.qwen/.omq-config.json.
 ...
 * Config file path can be overridden via OMQ_OPENCLAW_CONFIG env var (points to a separate file).
 */
  ...
  if (cliEnabled && cliAlias) {
    ...
    applyHooks(
      ...
      parseInstruction(
        ...
        "OMQ event {{event}} for {{projectPath}}",
      ),
      "custom_cli_command",
    );
  }
 ...
 *
 * Returns null when:
 * - OMQ_OPENCLAW env var is not "1"
 * - Config file does not exist
 * - Config file is invalid JSON
 * - Config has enabled: false
 *
 * Config is read from:
 * 1. OMQ_OPENCLAW_CONFIG env var path (separate file), if set
 * 2. notifications.openclaw key in ~/.qwen/.omq-config.json
...
export function getOpenClawConfig(): OpenClawConfig | null {
  // Activation gate: only active when OMQ_OPENCLAW=1
  if (process.env.OMQ_OPENCLAW !== "1") {
    return null;
  }
  ...
  try {
    const envOverride = process.env.OMQ_OPENCLAW_CONFIG;

    if (envOverride) {
      // OMQ_OPENCLAW_CONFIG points to a separate config file
    ...
    // Primary: read from notifications block in .omq-config.json
    const omqConfigPath = join(qwenHome(), ".omq-config.json");
    if (!existsSync(omqConfigPath)) {
      _cachedConfig = undefined;
      return null;
    }

    const fullConfig = JSON.parse(readFileSync(omqConfigPath, "utf-8")) as Record<string, unknown>;
