 ...
 *
 * NOTE: Qwen Code only supports a limited set of hook events.
 * pre-tool-use, post-tool-use, and keyword-detector are OMC-specific
 * and are NOT available in Qwen Code — excluded here intentionally.
 */
...
export interface OpenClawCommandGatewayConfig {
   ...
   * Precedence: gateway timeout > OMQ_OPENCLAW_COMMAND_TIMEOUT_MS > default (5000ms).
...
/** Top-level config schema for notifications.openclaw key in .omq-config.json */
