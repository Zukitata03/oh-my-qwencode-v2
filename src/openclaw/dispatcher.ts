 ...
 *
 * SECURITY: Command gateway requires OMQ_OPENCLAW_COMMAND=1 opt-in.
 ...
 * gateway timeout > OMQ_OPENCLAW_COMMAND_TIMEOUT_MS > default.
 */
export function resolveCommandTimeoutMs(
  gatewayTimeout?: number,
  envTimeoutRaw: string | undefined = process.env.OMQ_OPENCLAW_COMMAND_TIMEOUT_MS,
): number {
 ...
 * - Requires OMQ_OPENCLAW_COMMAND=1 opt-in (separate gate from OMQ_OPENCLAW)
 * - Timeout is configurable via gateway.timeout or OMQ_OPENCLAW_COMMAND_TIMEOUT_MS
...
export async function wakeCommandGateway(
  ...
  if (process.env.OMQ_OPENCLAW_COMMAND !== "1") {
    return {
      gateway: gatewayName,
      success: false,
      error: "Command gateway disabled (set OMQ_OPENCLAW_COMMAND=1 to enable)",
    };
  }
