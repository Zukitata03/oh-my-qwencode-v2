 ...
 *
 * Spawned by omq_run_team_start in state-server.ts.
 */
...
type TeamWorkerProvider = 'qwen' | 'claude' | 'gemini';
...
async function writePanesFile(
  ...
  const omqJobsDir = process.env.OMQ_JOBS_DIR;
  if (!jobId || !omqJobsDir) return;

  const panesPath = join(omqJobsDir, `${jobId}-panes.json`);
  await writeFile(
    panesPath + '.tmp',
...
export function normalizeAgentTypes(raw: string[], workerCount: number): TeamWorkerProvider[] {
  ...
  const invalid = providers.filter((entry) => entry !== 'qwen' && entry !== 'claude' && entry !== 'gemini');
  if (invalid.length > 0) {
    throw new Error(`Invalid agentTypes entries: ${invalid.join(', ')}. Expected qwen|claude|gemini.`);
  }
...
async function main(): Promise<void> {
  ...
  const stateRoot = join(cwd, '.omq', 'state');
  ...
  // Start the team — OMQ's startTeam takes individual parameters
  const agentType = 'executor';
  try {
    const providers = normalizeAgentTypes(agentTypes, workerCount);
    const previousCliMap = process.env.OMQ_TEAM_WORKER_CLI_MAP;
    try {
      process.env.OMQ_TEAM_WORKER_CLI_MAP = providers.join(',');
      ...
      if (typeof previousCliMap === 'string') process.env.OMQ_TEAM_WORKER_CLI_MAP = previousCliMap;
      else delete process.env.OMQ_TEAM_WORKER_CLI_MAP;
    }
  } catch (err) {
  ...
  // Persist pane IDs so MCP server can clean up explicitly via omq_run_team_cleanup.
...
const shouldAutoStart = process.env.OMQ_RUNTIME_CLI_DISABLE_AUTO_START !== '1';
