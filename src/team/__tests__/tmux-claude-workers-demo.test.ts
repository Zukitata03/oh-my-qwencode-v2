...
describe('tmux claude workers demo', () => {
  describe('mixed worker CLI map (like demo-team-e2e.sh)', () => {
    it('builds default CLI map with qwen first half, claude second half', () => {
      ...
      const workerCount = 6;
      const pivot = Math.floor((workerCount + 1) / 2); // 3 workers get qwen, 3 get claude
      ...
      // For 6 workers: first 3 qwen, last 3 claude
      assert.deepEqual(entries, ['qwen', 'qwen', 'qwen', 'claude', 'claude', 'claude']);

      // Verify via resolveTeamWorkerCliPlan
      const plan = resolveTeamWorkerCliPlan(workerCount, [], {
        OMQ_TEAM_WORKER_CLI_MAP: entries.join(','),
      });

      assert.deepEqual(plan, ['qwen', 'qwen', 'qwen', 'claude', 'claude', 'claude']);
    });

    it('handles odd worker count correctly (5 workers)', () => {
      // 5 workers: first 3 qwen, last 2 claude
      const workerCount = 5;
      const pivot = Math.floor((workerCount + 1) / 2); // 3

      const entries: string[] = [];
      for (let i = 1; i <= workerCount; i++) {
        entries.push(i <= pivot ? 'qwen' : 'claude');
      }

      assert.deepEqual(entries, ['qwen', 'qwen', 'qwen', 'claude', 'claude']);

      const plan = resolveTeamWorkerCliPlan(workerCount, [], {
        OMQ_TEAM_WORKER_CLI_MAP: entries.join(','),
      });

      assert.deepEqual(plan, ['qwen', 'qwen', 'qwen', 'claude', 'claude']);
    });

    it('handles even worker count correctly (6 workers)', () => {
      // 6 workers: first 3 qwen, last 3 claude
      const workerCount = 6;
      const pivot = Math.floor((workerCount + 1) / 2); // 3
  ...
  describe('claude worker launch spec', () => {
    ...
    it('claude worker drops qwen-specific flags', () => {
      ...
      // Claude should only have the skip-permissions flag, not qwen-specific args
      assert.equal(spec.workerCli, 'claude');
      assert.deepEqual(spec.args, ['--dangerously-skip-permissions']);

      // Should not include qwen-specific flags
    ...
    it('qwen worker preserves launch args', () => {
      const launchArgs = ['--model', 'qwen3.5-plus', '--json'];
      ...
      assert.ok(spec.args.includes('qwen3.5-plus'));
      assert.ok(spec.args.includes('--json'));
    });
  });

  describe('translateWorkerLaunchArgsForCli', () => {
    it('returns qwen args unchanged', () => {
      const args = ['--model', 'qwen3.5-plus', '--json'];
  ...
  describe('full demo scenario with 6 mixed workers', () => {
    it('creates correct launch specs for all workers in demo', () => {
      ...
      const cliMap = Array.from({ length: workerCount }, (_, i) =>
        i + 1 <= pivot ? 'qwen' : 'claude',
      ).join(',');

      const plan = resolveTeamWorkerCliPlan(workerCount, launchArgs, {
        OMQ_TEAM_WORKER_CLI_MAP: cliMap,
      });

      // Verify plan matches expected distribution
      // For 6 workers with bash pivot=$((7/2))=3: first 3 qwen, last 3 claude
      assert.deepEqual(plan, ['qwen', 'qwen', 'qwen', 'claude', 'claude', 'claude']);
      ...
      for (let i = 1; i <= workerCount; i++) {
        ...
        assert.equal(spec.env.OMQ_TEAM_WORKER, `${teamName}/worker-${i}`);
        assert.ok(spec.env.OMQ_LEADER_NODE_PATH, 'OMQ_LEADER_NODE_PATH must be set');
        assert.ok(spec.env.OMQ_LEADER_CLI_PATH, 'OMQ_LEADER_CLI_PATH must be set');
        assert.equal(spec.env.OMQ_TEAM_STATE_ROOT, `${cwd}/.omq/state`);
        ...
        if (workerCli === 'claude') {
          ...
          assert.ok(
            ...
            `worker-${i} (qwen) should have --model flag`,
          );
        }
      }
    });
    ...
    it('handles auto-resolved qwen from launch args', () => {
      // When launch args contain 'qwen' in model name and no CLI_MAP is set
      const plan = resolveTeamWorkerCliPlan(2, ['--model', 'qwen3.5-plus'], {});

      // Both should resolve to qwen because model name contains 'qwen' but not 'claude'
      assert.deepEqual(plan, ['qwen', 'qwen']);
    });
  });
  ...
  describe('edge cases for mixed worker teams', () => {
    ...
    it('handles single worker with qwen', () => {
      const plan = resolveTeamWorkerCliPlan(1, [], {
        OMQ_TEAM_WORKER_CLI_MAP: 'qwen',
      });

      assert.deepEqual(plan, ['qwen']);
    });

    it('handles single worker with gemini', () => {
      const plan = resolveTeamWorkerCliPlan(1, [], {
        OMQ_TEAM_WORKER_CLI_MAP: 'gemini',
      });
    ...
    it('rejects invalid CLI map with empty entries', () => {
      assert.throws(
        () => resolveTeamWorkerCliPlan(3, [], { OMQ_TEAM_WORKER_CLI_MAP: 'qwen,,claude' }),
    ...
    it('auto entries in CLI map resolve from launch args', () => {
      const plan = resolveTeamWorkerCliPlan(2, ['--model', 'claude-sonnet'], {
        OMQ_TEAM_WORKER_CLI_MAP: 'auto,auto',
      });
