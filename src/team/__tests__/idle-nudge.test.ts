...
if [[ "\$cmd" == "capture-pane" ]]; then
  if [[ "\${OMQ_FAIL_CAPTURE:-0}" == "1" ]]; then
    exit 1
  fi

  token=""
  if [[ -n "\${OMQ_CAPTURE_SEQ_FILE:-}" && -f "\${OMQ_CAPTURE_SEQ_FILE}" ]]; then
    token="\$(head -n 1 "\${OMQ_CAPTURE_SEQ_FILE}" || true)"
    if [[ -n "\$token" ]]; then
      tail -n +2 "\${OMQ_CAPTURE_SEQ_FILE}" > "\${OMQ_CAPTURE_SEQ_FILE}.tmp" || true
      mv "\${OMQ_CAPTURE_SEQ_FILE}.tmp" "\${OMQ_CAPTURE_SEQ_FILE}"
    fi
  fi
  if [[ -z "\$token" ]]; then
    token="\${OMQ_CAPTURE_TOKEN:-IDLE}"
  fi
...
if [[ "\$cmd" == "send-keys" ]]; then
  if [[ "\${OMQ_FAIL_SEND_KEYS:-0}" == "1" ]]; then
    exit 1
  fi
  exit 0
fi
...
async function withFakeTmux(run: (ctx: {
  ...
  const root = await mkdtemp(join(tmpdir(), 'omq-idle-nudge-test-'));
  ...
  const prevCaptureSeq = process.env.OMQ_CAPTURE_SEQ_FILE;
  const prevCaptureToken = process.env.OMQ_CAPTURE_TOKEN;
  const prevFailSendKeys = process.env.OMQ_FAIL_SEND_KEYS;
  const prevFailCapture = process.env.OMQ_FAIL_CAPTURE;
  ...
  try {
    ...
    process.env.OMQ_CAPTURE_SEQ_FILE = captureSeqPath;
    process.env.OMQ_CAPTURE_TOKEN = 'IDLE';
    delete process.env.OMQ_FAIL_SEND_KEYS;
    delete process.env.OMQ_FAIL_CAPTURE;
    ...
    if (typeof prevCaptureSeq === 'string') process.env.OMQ_CAPTURE_SEQ_FILE = prevCaptureSeq;
    else delete process.env.OMQ_CAPTURE_SEQ_FILE;

    if (typeof prevCaptureToken === 'string') process.env.OMQ_CAPTURE_TOKEN = prevCaptureToken;
    else delete process.env.OMQ_CAPTURE_TOKEN;

    if (typeof prevFailSendKeys === 'string') process.env.OMQ_FAIL_SEND_KEYS = prevFailSendKeys;
    else delete process.env.OMQ_FAIL_SEND_KEYS;

    if (typeof prevFailCapture === 'string') process.env.OMQ_FAIL_CAPTURE = prevFailCapture;
    else delete process.env.OMQ_FAIL_CAPTURE;
...
describe('idle-nudge', () => {
  ...
  it('throttles scans that happen too soon after the previous scan', async () => {
    await withFakeTmux(async ({ tmuxLogPath }) => {
      await withMockedNow(10_000, async (setNow) => {
        ...
        const first = await tracker.checkAndNudge(['%2'], undefined, 'omq-team-a');
        assert.deepEqual(first, ['%2']);
        const firstLog = await readFile(tmuxLogPath, 'utf-8');

        setNow(11_000); // < 5000ms scan interval
        const second = await tracker.checkAndNudge(['%2'], undefined, 'omq-team-a');
  ...
  it('never nudges the leader pane', async () => {
    await withFakeTmux(async ({ tmuxLogPath }) => {
      const tracker = new NudgeTracker({ delayMs: 0, maxCount: 3, message: 'nudge' });
      const nudged = await tracker.checkAndNudge(['%1'], '%1', 'omq-team-a');
  ...
  it('resets idle timer when pane becomes active before delay elapses', async () => {
    await withFakeTmux(async ({ setCaptureSequence }) => {
      ...
      await withMockedNow(10_000, async (setNow) => {
        ...
        const r1 = await tracker.checkAndNudge(['%2'], undefined, 'omq-team-a');
        assert.deepEqual(r1, []);

        setNow(16_000);
        const r2 = await tracker.checkAndNudge(['%2'], undefined, 'omq-team-a');
        assert.deepEqual(r2, []);

        setNow(22_000);
        const r3 = await tracker.checkAndNudge(['%2'], undefined, 'omq-team-a');
        assert.deepEqual(r3, []);

        setNow(28_000);
        const r4 = await tracker.checkAndNudge(['%2'], undefined, 'omq-team-a');
        assert.deepEqual(r4, []);

        setNow(39_000);
        const r5 = await tracker.checkAndNudge(['%2'], undefined, 'omq-team-a');
  ...
  it('does not count nudges when sendToWorker fails', async () => {
    await withFakeTmux(async () => {
      process.env.OMQ_FAIL_SEND_KEYS = '1';

      await withMockedNow(10_000, async () => {
        const tracker = new NudgeTracker({ delayMs: 0, maxCount: 3, message: 'nudge' });
        const nudged = await tracker.checkAndNudge(['%2'], undefined, 'omq-team-a');
        assert.deepEqual(nudged, []);
        assert.equal(tracker.totalNudges, 0);
        assert.deepEqual(tracker.getSummary(), {});
      });
  ...
  it('returns empty capture and non-idle when capture-pane command fails', async () => {
    await withFakeTmux(async () => {
      process.env.OMQ_FAIL_CAPTURE = '1';
