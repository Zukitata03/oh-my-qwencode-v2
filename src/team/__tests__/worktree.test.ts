...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'omq-worktree-test-'));
...
describe('worktree parser', () => {
  ...
  // leak into the Qwen Code shell as input.
...
describe('worktree planning', () => {
  it('plans dedicated autoresearch branch and path naming', async () => {
    ...
    try {
      ...
      assert.match(planned.worktreePath.replace(/\\/g, '/'), /\.omq\/worktrees\/autoresearch-demo-mission-20260314t000000z$/);
    } finally {
