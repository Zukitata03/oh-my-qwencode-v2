...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'omq-shutdown-fallback-'));
...
describe('shutdown fallback worktree reports', () => {
  it('shutdownTeam checkpoints dirty detached worker worktrees, merges them, and writes a report', async () => {
    ...
    const binDir = await mkdtemp(join(tmpdir(), 'omq-shutdown-fallback-bin-'));
...
`,
    ...
    try {
      ...
      const reportPath = join(preservedWorktreePath as string, '.omq', 'diff.md');
