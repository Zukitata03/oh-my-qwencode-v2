...
async function initRepo(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-worktree-repo-'));
...
function expectedLowComplexityModel(qwenHomeOverride?: string): string {
...
function withoutTeamWorkerEnv<T>(fn: () => T): T {
  const prev = process.env.OMQ_TEAM_WORKER;
  ...
  try {
    ...
    if (result instanceof Promise) {
      restoreImmediately = false;
      return result.finally(() => {
        if (typeof prev === 'string') process.env.OMQ_TEAM_WORKER = prev;
        else delete process.env.OMQ_TEAM_WORKER;
      }) as T;
    }
    return result;
  } finally {
...
function teamStateTestPath(cwd: string, ...parts: string[]): string {
  const stateRoot = process.env.OMQ_TEAM_STATE_ROOT ?? join(cwd, '.omq', 'state');
...
const ORIGINAL_OMQ_TEAM_STATE_ROOT = process.env.OMQ_TEAM_STATE_ROOT;
...
afterEach(() => {
  if (typeof ORIGINAL_OMQ_TEAM_STATE_ROOT === 'string') process.env.OMQ_TEAM_STATE_ROOT = ORIGINAL_OMQ_TEAM_STATE_ROOT;
  else delete process.env.OMQ_TEAM_STATE_ROOT;
});
...
describe('runtime', () => {
  it('resolveWorkerLaunchArgsFromEnv injects low-complexity default model when missing', () => {
    const args = resolveWorkerLaunchArgsFromEnv(
      { OMQ_TEAM_WORKER_LAUNCH_ARGS: '--no-alt-screen' },
      'explore',
    );
  ...
  it('resolveWorkerLaunchArgsFromEnv reads low-complexity model from config when present', async () => {
    ...
    const tempQwenHome = await mkdtemp(join(tmpdir(), 'omq-qwen-home-'));
    await writeFile(
      join(tempQwenHome, '.omq-config.json'),
    ...
    try {
      ...
      await rm(tempQwenHome, { recursive: true, force: true });
    }
  });
  ...
  it('resolveWorkerLaunchArgsFromEnv injects the frontier default model for executor workers', () => {
    ...
    assert.deepEqual(args, ['--no-alt-screen', '--model', 'qwen3.6-plus']);
  });
  ...
  it('resolveWorkerLaunchArgsFromEnv preserves explicit model in either syntax', () => {
    assert.deepEqual(
      resolveWorkerLaunchArgsFromEnv({ OMQ_TEAM_WORKER_LAUNCH_ARGS: '--model gpt-5' }, 'explore'),
      ['--model', 'gpt-5'],
    );
    assert.deepEqual(
      resolveWorkerLaunchArgsFromEnv({ OMQ_TEAM_WORKER_LAUNCH_ARGS: '--model=gpt-5.3' }, 'explore'),
      ['--model', 'gpt-5.3'],
    );
  });
  ...
  it('resolveWorkerLaunchArgsFromEnv prefers explicit env model over inherited leader model', () => {
    assert.deepEqual(
      resolveWorkerLaunchArgsFromEnv({ OMQ_TEAM_WORKER_LAUNCH_ARGS: '--model gpt-5' }, 'explore', 'gpt-4.1'),
      ['--model', 'gpt-5'],
    );
  });
  ...
  it('resolveWorkerLaunchArgsFromEnv logs model=gemini without thinking_level for gemini CLI', () => {
    ...
    try {
      const args = resolveWorkerLaunchArgsFromEnv(
        {
          OMQ_TEAM_WORKER_CLI: 'gemini',
          OMQ_TEAM_WORKER_LAUNCH_ARGS: '--model gemini-2.0-pro',
        },
        'executor',
      );
  ...
  it('resolveWorkerLaunchArgsFromEnv keeps qwen thinking_level logging as none for mixed CLI maps', () => {
    ...
    try {
      const args = resolveWorkerLaunchArgsFromEnv(
        {
          OMQ_TEAM_WORKER_CLI_MAP: 'qwen,claude',
          OMQ_TEAM_WORKER_LAUNCH_ARGS: '-c model_reasoning_effort="high" --model claude-3-7-sonnet',
        },
        'executor',
      );
      // Reasoning is silently ignored
  ...
  it('resolveWorkerLaunchArgsFromEnv keeps claude and gemini startup logs free of thinking_level during teammate allocation', () => {
    ...
    console.log = (...args: unknown[]) => { logs.push(args.join(' ')); };
    try {
      const qwenArgs = resolveWorkerLaunchArgsFromEnv(
        { OMQ_TEAM_WORKER_LAUNCH_ARGS: '--no-alt-screen' },
        'executor',
        undefined,
        'high',
      ...
      const claudeArgs = resolveWorkerLaunchArgsFromEnv(
        { OMQ_TEAM_WORKER_LAUNCH_ARGS: '--no-alt-screen --model claude-3-7-sonnet' },
        'executor',
        undefined,
        'low',
        'claude',
  ...
  it('waitForClaudeStartupEvidence requires first-start ACK/task progress before startup dispatch is treated as settled', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-claude-startup-'));
    try {
      ...
      await writeAtomic(
        join(cwd, '.omq', 'state', 'team', 'claude-startup', 'workers', 'worker-1', 'status.json'),
        JSON.stringify({
          state: 'working',
  ...
  it('waitForWorkerStartupEvidence ignores Qwen Code ACK-only startup replies until work is claimed', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-qwen-startup-'));
    try {
      await initTeamState('qwen-startup', 'startup evidence test', 'executor', 1, cwd);

      await sendWorkerMessage('qwen-startup', 'worker-1', 'leader-fixed', 'ACK', cwd);
      const ackOnly = await waitForWorkerStartupEvidence({
        teamName: 'qwen-startup',
        workerName: 'worker-1',
        workerCli: 'qwen',
        cwd,
        timeoutMs: 25,
      ...
      await writeAtomic(
        join(cwd, '.omq', 'state', 'team', 'qwen-startup', 'workers', 'worker-1', 'status.json'),
        JSON.stringify({
          state: 'working',
          current_task_id: 'task-1',
          updated_at: new Date().toISOString(),
  ...
  it('startTeam rejects nested team invocation inside worker context', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-'));
    const prev = process.env.OMQ_TEAM_WORKER;
    process.env.OMQ_TEAM_WORKER = 'alpha/worker-1';
    try {
  ...
  it('startTeam allows nested team invocation when parent governance enables it', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-nested-allow-'));
...
`,
    ...
    const parentManifestPath = join(cwd, '.omq', 'state', 'team', 'parent-team', 'manifest.v2.json');
    ...
    const prevWorker = process.env.OMQ_TEAM_WORKER;
    const prevStateRoot = process.env.OMQ_TEAM_STATE_ROOT;
    const prevLeaderCwd = process.env.OMQ_TEAM_LEADER_CWD;
    const prevLaunchMode = process.env.OMQ_TEAM_WORKER_LAUNCH_MODE;
    const prevWorkerCli = process.env.OMQ_TEAM_WORKER_CLI;

    process.env.PATH = `${binDir}:${prevPath ?? ''}`;
    delete process.env.TMUX;
    process.env.OMQ_TEAM_WORKER = 'parent-team/worker-1';
    process.env.OMQ_TEAM_STATE_ROOT = join(cwd, '.omq', 'state');
    process.env.OMQ_TEAM_LEADER_CWD = cwd;
    process.env.OMQ_TEAM_WORKER_LAUNCH_MODE = 'prompt';
    process.env.OMQ_TEAM_WORKER_CLI = 'gemini';
    ...
    try {
      ...
      if (typeof prevWorker === 'string') process.env.OMQ_TEAM_WORKER = prevWorker;
      else delete process.env.OMQ_TEAM_WORKER;
      if (typeof prevStateRoot === 'string') process.env.OMQ_TEAM_STATE_ROOT = prevStateRoot;
      else delete process.env.OMQ_TEAM_STATE_ROOT;
      if (typeof prevLeaderCwd === 'string') process.env.OMQ_TEAM_LEADER_CWD = prevLeaderCwd;
      else delete process.env.OMQ_TEAM_LEADER_CWD;
      if (typeof prevLaunchMode === 'string') process.env.OMQ_TEAM_WORKER_LAUNCH_MODE = prevLaunchMode;
      else delete process.env.OMQ_TEAM_WORKER_LAUNCH_MODE;
      if (typeof prevWorkerCli === 'string') process.env.OMQ_TEAM_WORKER_CLI = prevWorkerCli;
      else delete process.env.OMQ_TEAM_WORKER_CLI;
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('startTeam throws when tmux is not available', async () => {
    ...
    const prevLaunchMode = process.env.OMQ_TEAM_WORKER_LAUNCH_MODE;
    try {
      process.env.OMQ_TEAM_WORKER_LAUNCH_MODE = 'interactive';
      await assert.rejects(
  ...
  it('startTeam rejects dirty leader workspace before provisioning worker worktrees', async () => {
    ...
    try {
      ...
      await assert.rejects(
        ...
        /leader_workspace_dirty_for_worktrees:.*M README\.md.*\?\? notes\.txt.*commit_or_stash_before_omq_team/s,
      );
      ...
      assert.equal(existsSync(join(repo, '.omq', 'state', 'team', 'team-dirty-preflight')), false);
    } finally {
      if (typeof prevLaunchMode === 'string') process.env.OMQ_TEAM_WORKER_LAUNCH_MODE = prevLaunchMode;
      else delete process.env.OMQ_TEAM_WORKER_LAUNCH_MODE;
  ...
  it('startTeam launches gemini workers with startup prompt and no default model passthrough', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-gemini-'));
    const binDir = join(cwd, 'bin');
    const fakeGeminiPath = join(binDir, 'gemini');
...
`,
    ...
    const prevLaunchMode = process.env.OMQ_TEAM_WORKER_LAUNCH_MODE;
    const prevWorkerCli = process.env.OMQ_TEAM_WORKER_CLI;
    const prevLaunchArgs = process.env.OMQ_TEAM_WORKER_LAUNCH_ARGS;
    const prevCapture = process.env.OMQ_GEMINI_ARGV_CAPTURE_PATH;

    process.env.PATH = `${binDir}:${prevPath ?? ''}`;
    delete process.env.TMUX;
    process.env.OMQ_TEAM_WORKER_LAUNCH_MODE = 'prompt';
    process.env.OMQ_TEAM_WORKER_CLI = 'gemini';
    process.env.OMQ_TEAM_WORKER_LAUNCH_ARGS = '--model qwen3.5-plus';
    process.env.OMQ_GEMINI_ARGV_CAPTURE_PATH = capturePath;
    ...
    try {
      ...
      assert.deepEqual(argv, [
        ...
        'yolo',
        '-i',
        'Read .omq/state/team/team-gemini-prompt/workers/worker-1/inbox.md, start work now, report concrete progress, then continue assigned work or next feasible task.',
      ]);
      ...
      if (typeof prevLaunchArgs === 'string') process.env.OMQ_TEAM_WORKER_LAUNCH_ARGS = prevLaunchArgs;
      else delete process.env.OMQ_TEAM_WORKER_LAUNCH_ARGS;
      if (typeof prevCapture === 'string') process.env.OMQ_GEMINI_ARGV_CAPTURE_PATH = prevCapture;
      else delete process.env.OMQ_GEMINI_ARGV_CAPTURE_PATH;
      await rm(cwd, { recursive: true, force: true });
    }
  });

  it('startTeam preserves explicit qwen launch args while forcing bypass in prompt mode', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-qwen-explicit-launch-'));
    const binDir = join(cwd, 'bin');
    const fakeQwenPath = join(binDir, 'qwen');
    const capturePath = join(cwd, 'qwen-argv.json');
    await mkdir(binDir, { recursive: true });
    await writeFile(
      fakeQwenPath,
      `#!/usr/bin/env node
const fs = require('fs');
fs.writeFileSync(process.env.OMQ_QWEN_ARGV_CAPTURE_PATH, JSON.stringify(process.argv.slice(2), null, 2));
...
`,
    ...
    const prevWorkerCli = process.env.OMQ_TEAM_WORKER_CLI;
    const prevLaunchArgs = process.env.OMQ_TEAM_WORKER_LAUNCH_ARGS;
    const prevCapture = process.env.OMQ_QWEN_ARGV_CAPTURE_PATH;

    process.env.PATH = `${binDir}:${prevPath ?? ''}`;
    delete process.env.TMUX;
    process.env.OMQ_TEAM_WORKER_LAUNCH_MODE = 'prompt';
    process.env.OMQ_TEAM_WORKER_CLI = 'qwen';
    process.env.OMQ_TEAM_WORKER_LAUNCH_ARGS = '--model qwen3.5-plus -c model_reasoning_effort="low"';
    process.env.OMQ_QWEN_ARGV_CAPTURE_PATH = capturePath;

    let runtime: TeamRuntime | null = null;
    try {
      runtime = await withoutTeamWorkerEnv(() =>
        startTeam(
          'team-qwen-explicit-launch',
          'qwen prompt-mode team bootstrap',
          'explore',
          1,
          [{ subject: 's', description: 'd', owner: 'worker-1' }],
          cwd,
      ...
      if (typeof prevCapture === 'string') process.env.OMQ_QWEN_ARGV_CAPTURE_PATH = prevCapture;
      else delete process.env.OMQ_QWEN_ARGV_CAPTURE_PATH;
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('startTeam preserves routed task roles into team state and worker launch args', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-role-routing-'));
    const binDir = join(cwd, 'bin');
    const fakeQwenPath = join(binDir, 'qwen');
    const captureDir = join(cwd, 'captures');
    const promptsDir = join(cwd, '.qwen', 'prompts');
    await mkdir(binDir, { recursive: true });
...
const worker = String(process.env.OMQ_TEAM_WORKER || 'unknown').replace(/[^a-zA-Z0-9_-]+/g, '__');
const out = path.join(process.env.OMQ_ARGV_CAPTURE_DIR, worker + '.json');
...
`,
    ...
    const prevLaunchMode = process.env.OMQ_TEAM_WORKER_LAUNCH_MODE;
    const prevWorkerCli = process.env.OMQ_TEAM_WORKER_CLI;
    const prevCaptureDir = process.env.OMQ_ARGV_CAPTURE_DIR;

    process.env.PATH = `${binDir}:${prevPath ?? ''}`;
    delete process.env.TMUX;
    process.env.OMQ_TEAM_WORKER_LAUNCH_MODE = 'prompt';
    process.env.OMQ_TEAM_WORKER_CLI = 'qwen';
    process.env.OMQ_ARGV_CAPTURE_DIR = captureDir;
    ...
    try {
      ...
      const worker1Instructions = await readFile(join(cwd, '.omq', 'state', 'team', runtime.teamName, 'workers', 'worker-1', 'AGENTS.md'), 'utf-8');
      const worker2Instructions = await readFile(join(cwd, '.omq', 'state', 'team', runtime.teamName, 'workers', 'worker-2', 'AGENTS.md'), 'utf-8');
      ...
      assert.doesNotMatch(worker1Instructions, /exact qwen3.6-flash model/);
      assert.match(worker2Instructions, /You are operating as the \*\*writer\*\* role/);
      assert.match(worker2Instructions, /You are Writer\./);
      assert.match(worker2Instructions, /exact qwen3.6-flash model/);
      ...
      if (typeof prevCaptureDir === 'string') process.env.OMQ_ARGV_CAPTURE_DIR = prevCaptureDir;
      else delete process.env.OMQ_ARGV_CAPTURE_DIR;
      await rm(cwd, { recursive: true, force: true });
    }
  });

  it('startTeam does not apply mini guidance for exact-match negatives like qwen3.6-flash-tuned', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-mini-tuned-'));
    const binDir = join(cwd, 'bin');
    const fakeQwenPath = join(binDir, 'qwen');
    const captureDir = join(cwd, 'captures');
    const promptsDir = join(cwd, '.qwen', 'prompts');
...
`,
    ...
    process.env.OMQ_TEAM_WORKER_CLI = 'qwen';
    process.env.OMQ_ARGV_CAPTURE_DIR = captureDir;
    process.env.OMQ_TEAM_WORKER_LAUNCH_ARGS = '--model qwen3.6-flash-tuned';
    ...
    try {
      ...
      const workerInstructions = await readFile(join(cwd, '.omq', 'state', 'team', runtime.teamName, 'workers', 'worker-1', 'AGENTS.md'), 'utf-8');
      assert.match(workerInstructions, /You are operating as the \*\*writer\*\* role/);
      assert.match(workerInstructions, /You are Writer\./);
      assert.doesNotMatch(workerInstructions, /exact qwen3.6-flash model/);
      assert.doesNotMatch(workerInstructions, /strict execution order: inspect -> plan -> act -> verify/);
      assert.match(workerInstructions, /resolved_model: qwen3.6-flash-tuned/);
      ...
      assert.match(workerJoined, /--model qwen3.6-flash-tuned/);
  ...
  it('startTeam supports prompt launch mode without tmux and pipes trigger text via stdin', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-prompt-'));
    const binDir = join(cwd, 'bin');
    const fakeQwenPath = join(binDir, 'qwen');
    await mkdir(binDir, { recursive: true });
    await writeFile(
...
`,
  ...
  it('startTeam relaunch re-creates HUD pane and re-registers reconcile hooks after shutdown', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-relaunch-hud-'));
    const previousTmux = process.env.TMUX;
    const previousTmuxPane = process.env.TMUX_PANE;
    const previousLaunchMode = process.env.OMQ_TEAM_WORKER_LAUNCH_MODE;
    const previousWorkerCli = process.env.OMQ_TEAM_WORKER_CLI;
    let runtime: TeamRuntime | null = null;
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omq-runtime-relaunch-hud-bin-',
...
`,
      ...
      if (typeof previousLaunchMode === 'string') process.env.OMQ_TEAM_WORKER_LAUNCH_MODE = previousLaunchMode;
      else delete process.env.OMQ_TEAM_WORKER_LAUNCH_MODE;
      if (typeof previousWorkerCli === 'string') process.env.OMQ_TEAM_WORKER_CLI = previousWorkerCli;
      else delete process.env.OMQ_TEAM_WORKER_CLI;
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('startTeam routes detached worktree worker inbox and mailbox triggers through leader-root state references', async () => {
    const repo = await initRepo();
    const toolingDir = await mkdtemp(join(tmpdir(), 'omq-runtime-worktree-tools-'));
...
const fs = require('fs');
const path = require('path');
const logDir = process.env.OMQ_TEST_LOG_DIR;
fs.mkdirSync(logDir, { recursive: true });
fs.writeFileSync(path.join(logDir, 'env.json'), JSON.stringify({
  cwd: process.cwd(),
  teamStateRoot: process.env.OMQ_TEAM_STATE_ROOT || '',
  worker: process.env.OMQ_TEAM_WORKER || '',
}));
...
`,
    ...
    const prevLaunchMode = process.env.OMQ_TEAM_WORKER_LAUNCH_MODE;
    const prevWorkerCli = process.env.OMQ_TEAM_WORKER_CLI;
    const prevLogDir = process.env.OMQ_TEST_LOG_DIR;

    process.env.PATH = `${binDir}:${prevPath ?? ''}`;
    delete process.env.TMUX;
    process.env.OMQ_TEAM_WORKER_LAUNCH_MODE = 'prompt';
    process.env.OMQ_TEAM_WORKER_CLI = 'qwen';
    process.env.OMQ_TEST_LOG_DIR = logDir;
    ...
    try {
      ...
      assert.match(
        startupLog,
        /\$OMQ_TEAM_STATE_ROOT\/team\/team-detached-worktree-paths\/workers\/worker-1\/inbox\.md/,
      );
      assert.doesNotMatch(
        startupLog,
        /Read \.omq\/state\/team\/team-detached-worktree-paths\/workers\/worker-1\/inbox\.md/,
      );
      ...
      assert.equal(envLog.teamStateRoot, join(repo, '.omq', 'state'));
      ...
      assert.match(
        mailboxLog,
        /\$OMQ_TEAM_STATE_ROOT\/team\/team-detached-worktree-paths\/mailbox\/worker-1\.json/,
      );
      ...
      if (typeof prevLogDir === 'string') process.env.OMQ_TEST_LOG_DIR = prevLogDir;
      else delete process.env.OMQ_TEST_LOG_DIR;
...
`,
    ...
    try {
      ...
      assert.equal(existsSync(join(repo, '.omq', 'state', 'team', 'team-detached-worktree-shutdown')), false);
    } finally {
      if (runtime) {
        await shutdownTeam(runtime.teamName, repo, { force: true }).catch(() => {});
      }
  ...
  it('resumeTeam preserves detached worktree metadata for live prompt workers', async () => {
    const repo = await initRepo();
    const binDir = await mkdtemp(join(tmpdir(), 'omq-runtime-prompt-bin-'));
    const fakeQwenPath = join(binDir, 'qwen');
    const logDir = await mkdtemp(join(tmpdir(), 'omq-runtime-prompt-logs-'));
    const envLogPath = join(logDir, 'env.json');
    await writeFile(
      fakeQwenPath,
      `#!/usr/bin/env node
...
`,
    ...
    try {
      ...
      assert.equal(resumed?.config.workers[0]?.team_state_root, join(repo, '.omq', 'state'));
      ...
      assert.equal(identity.team_state_root, join(repo, '.omq', 'state'));
  ...
  it('shutdownTeam force-kills prompt workers that ignore SIGTERM', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-prompt-stubborn-'));
    const binDir = join(cwd, 'bin');
    const fakeQwenPath = join(binDir, 'qwen');
    await mkdir(binDir, { recursive: true });
    await writeFile(
...
`,
  ...
  it('monitorTeam surfaces reclaimed work pickup attempts when an idle worker is available', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-reassign-reclaimed-'));
    const prevTeamStateRoot = process.env.OMQ_TEAM_STATE_ROOT;
    delete process.env.OMQ_TEAM_STATE_ROOT;
    ...
    try {
      ...
      const taskPath = join(cwd, '.omq', 'state', 'team', 'team-runtime-reassign', 'tasks', `task-${task.id}.json`);
      ...
      const manifestPath = join(cwd, '.omq', 'state', 'team', 'team-runtime-reassign', 'manifest.v2.json');
      ...
      await writeAtomic(
        join(cwd, '.omq', 'state', 'team', 'team-runtime-reassign', 'workers', 'worker-1', 'status.json'),
        JSON.stringify({ state: 'working', current_task_id: task.id, updated_at: new Date().toISOString() }, null, 2),
      );
      await writeAtomic(
        join(cwd, '.omq', 'state', 'team', 'team-runtime-reassign', 'workers', 'worker-2', 'status.json'),
      ...
      if (typeof prevTeamStateRoot === 'string') process.env.OMQ_TEAM_STATE_ROOT = prevTeamStateRoot;
      else delete process.env.OMQ_TEAM_STATE_ROOT;
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it('monitorTeam reclaims expired task claims and surfaces the recovery in recommendations', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-reclaim-'));
    const prevTeamStateRoot = process.env.OMQ_TEAM_STATE_ROOT;
    delete process.env.OMQ_TEAM_STATE_ROOT;
    try {
      ...
      const taskPath = join(cwd, '.omq', 'state', 'team', 'team-runtime-reclaim', 'tasks', `task-${t.id}.json`);
      const current = JSON.parse(await readFile(taskPath, 'utf-8')) as any;
      current.claim.leased_until = new Date(Date.now() - 1000).toISOString();
      await writeAtomic(taskPath, JSON.stringify(current, null, 2));
  ...
  it('monitorTeam keeps phase in team-verify when completed code tasks lack verification evidence', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-verify-gate-'));
    const prevTeamStateRoot = process.env.OMQ_TEAM_STATE_ROOT;
    delete process.env.OMQ_TEAM_STATE_ROOT;
    try {
      ...
      const taskPath = join(cwd, '.omq', 'state', 'team', 'team-verify-gate', 'tasks', `task-${task.id}.json`);
  ...
  it('monitorTeam deactivates root team-state.json when the local phase becomes terminal', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-root-team-state-'));
    try {
      ...
      const rootStatePath = join(cwd, '.omq', 'state', 'team-state.json');
  ...
  it('monitorTeam emits worker_state_changed, worker_idle, and task_completed events based on transitions', async () => {
    ...
    try {
      ...
      await writeAtomic(
        join(cwd, '.omq', 'state', 'team', 'team-events', 'tasks', `task-${t.id}.json`),
        JSON.stringify({ ...t, status: 'completed', owner: 'worker-1' }, null, 2),
      );
      await writeAtomic(
        join(cwd, '.omq', 'state', 'team', 'team-events', 'workers', 'worker-1', 'status.json'),
        JSON.stringify({ state: 'idle', updated_at: new Date().toISOString() }, null, 2),
      );

      await monitorTeam('team-events', cwd);

      const eventsPath = join(cwd, '.omq', 'state', 'team', 'team-events', 'events', 'events.ndjson');
  ...
  it('monitorTeam persists integration ledger and cherry-picks unseen worker HEADs once', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'worker-1-branch', 'omq-runtime-worker-1-wt-');
  ...
  it('monitorTeam auto-commits dirty worker worktree before integration', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'wk1-ac-branch', 'omq-runtime-wk1-auto-commit-');
      ...
      assert.match(log, /omq\(team\): auto-checkpoint worker-1 \[1\]/, 'commit message should match auto-checkpoint pattern');
  ...
  it('monitorTeam uses merge for worker cleanly ahead of leader (hybrid merge path)', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'wk1-merge-branch', 'omq-runtime-wk1-merge-clean-');
  ...
  it('monitorTeam uses cherry-pick for diverged worker (hybrid cherry-pick path)', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'wk1-div-branch', 'omq-runtime-wk1-diverged-');
  ...
  it('monitorTeam rebases idle workers after integration lands on leader (cross-worker rebase)', async () => {
    ...
    let worker2Path = '';
    try {
      worker1Path = await addWorktree(repo, 'wk1-xr-branch', 'omq-runtime-wk1-cross-rebase-');
      worker2Path = await addWorktree(repo, 'wk2-xr-branch', 'omq-runtime-wk2-cross-rebase-');
  ...
  it('monitorTeam auto-resolves conflicts with -X theirs (worker wins on leader)', async () => {
    ...
    let workerPath = '';
    try {
      workerPath = await addWorktree(repo, 'wk1-cr-branch', 'omq-runtime-wk1-conflict-resolve-');
  ...
  it('monitorTeam skips rebase for workers with "working" status (idle-only gating)', async () => {
    ...
    let worker2Path = '';
    try {
      worker1Path = await addWorktree(repo, 'wk1-gate-branch', 'omq-runtime-wk1-rebase-gate-');
      worker2Path = await addWorktree(repo, 'wk2-gate-branch', 'omq-runtime-wk2-rebase-gate-');
  ...
  it('monitorTeam aborts failed rebase and leaves worktree in clean state', async () => {
    ...
    try {
      ...
      worker1Path = await addWorktree(repo, 'wk1-rf-branch', 'omq-runtime-wk1-rebase-fail-');
      worker2Path = await addWorktree(repo, 'wk2-rf-branch', 'omq-runtime-wk2-rebase-fail-');
      ...
      const reportPath = join(repo, '.omq', 'state', 'team', 'team-rebase-fail', 'integration-report.md');
  ...
  it('shutdownTeam cleans up state even when tmux session doesn\'t exist', async () => {
    ...
    try {
      ...
      const teamRoot = join(cwd, '.omq', 'state', 'team', 'team-shutdown');
  ...
  it('shutdownTeam blocks when pending tasks remain (shutdown gate)', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-shutdown-gate-'));
    try {
      ...
      const teamRoot = join(cwd, '.omq', 'state', 'team', 'team-shutdown-gate-pending');
  ...
  it('shutdownTeam honors governance cleanup override when active tasks remain', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-shutdown-gate-override-'));
    try {
      ...
      const manifestPath = join(cwd, '.omq', 'state', 'team', 'team-shutdown-gate-override', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as any;
      manifest.governance = {
        ...(manifest.governance || {}),
      ...
      const teamRoot = join(cwd, '.omq', 'state', 'team', 'team-shutdown-gate-override');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam honors legacy policy cleanup override after governance hydration', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-shutdown-gate-legacy-'));
    try {
      ...
      const manifestPath = join(cwd, '.omq', 'state', 'team', 'team-shutdown-gate-legacy', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as any;
      manifest.policy = {
        ...(manifest.policy || {}),
        cleanup_requires_all_workers_inactive: false,
      ...
      const teamRoot = join(cwd, '.omq', 'state', 'team', 'team-shutdown-gate-legacy');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam blocks when failed tasks remain (completion gate)', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-shutdown-gate-failed-'));
    try {
  ...
  it('shutdownTeam force=true bypasses shutdown gate and cleans up', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-shutdown-gate-force-'));
    try {
      ...
      const teamRoot = join(cwd, '.omq', 'state', 'team', 'team-shutdown-gate-force');
  ...
  it('shutdownTeam force=true emits shutdown_gate_forced audit event', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-shutdown-gate-forced-event-'));
    try {
      ...
      const eventsPath = join(cwd, '.omq', 'state', 'team', 'team-gate-forced-event', 'events', 'events.ndjson');
      ...
      const teamRoot = join(cwd, '.omq', 'state', 'team', 'team-gate-forced-event');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam handles persisted resize hook metadata during cleanup', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-resize-meta-'));
    try {
      const configPath = join(cwd, '.omq', 'state', 'team', 'team-resize-meta', 'config.json');
      const manifestPath = join(cwd, '.omq', 'state', 'team', 'team-resize-meta', 'manifest.v2.json');
      ...
      config.resize_hook_name = 'omq_resize_team_resize_meta_test';
      config.resize_hook_target = 'omq-team-team-resize-meta:0';
      await writeFile(configPath, JSON.stringify(config, null, 2));
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as Record<string, unknown>;
      manifest.resize_hook_name = 'omq_resize_team_resize_meta_test';
      manifest.resize_hook_target = 'omq-team-team-resize-meta:0';
      await writeFile(manifestPath, JSON.stringify(manifest, null, 2));

      await shutdownTeam('team-resize-meta', cwd);
      const teamRoot = join(cwd, '.omq', 'state', 'team', 'team-resize-meta');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam continues cleanup when resize hook unregister fails while session remains active', async () => {
    ...
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omq-runtime-fake-tmux-',
...
case "$1" in
  -V)
    ...
    ;;
  list-sessions)
    echo "omq-team-team-shutdown-gate-failed"
    exit 0
    ;;
  set-hook)
    if [ "\${2:-}" = "-u" ]; then
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          const config = JSON.parse(await readFile(configPath, 'utf-8')) as Record<string, unknown>;
          config.tmux_session = 'omq-team-team-shutdown-gate-failed';
          config.resize_hook_name = 'omq_resize_team_shutdown_gate_failed_test';
          config.resize_hook_target = 'omq-team-team-shutdown-gate-failed:0';
          await writeFile(configPath, JSON.stringify(config, null, 2));
          const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as Record<string, unknown>;
          manifest.tmux_session = 'omq-team-team-shutdown-gate-failed';
          manifest.resize_hook_name = 'omq_resize_team_shutdown_gate_failed_test';
          manifest.resize_hook_target = 'omq-team-team-shutdown-gate-failed:0';
          await writeFile(manifestPath, JSON.stringify(manifest, null, 2));
          ...
          const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
          assert.match(tmuxLog, /set-hook -u -t omq-team-team-shutdown-gate-failed:0 client-resized\[\d+\]/);
          assert.match(tmuxLog, /kill-session -t omq-team-team-shutdown-gate-failed/);
        },
      );
    } finally {
      await rm(cwd, { recursive: true, force: true });
  ...
  it('shutdownTeam emits shutdown_ack event when worker ack is received', async () => {
    ...
    try {
      ...
      const eventLogPath = join(cwd, '.omq', 'state', 'team', 'team-ack-evt', 'events', 'events.ndjson');
  ...
  it('shutdownTeam emits shutdown_ack event with accept reason for accepted acks', async () => {
    ...
    try {
      ...
      const eventLogPath = join(cwd, '.omq', 'state', 'team', 'team-ack-accept', 'events', 'events.ndjson');
      ...
      const teamRoot = join(cwd, '.omq', 'state', 'team', 'team-ack-accept');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam force=true ignores rejection and cleans up team state', async () => {
    ...
    try {
      ...
      const teamRoot = join(cwd, '.omq', 'state', 'team', 'team-force');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam ignores stale rejection ack from a prior request', async () => {
    ...
    try {
      ...
      const teamRoot = join(cwd, '.omq', 'state', 'team', 'team-stale-ack');
      assert.equal(existsSync(teamRoot), false);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('shutdownTeam applies best-effort teardown even when worker pane is already dead', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-shutdown-dead-pane-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omq-runtime-shutdown-dead-pane-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
  ...
  it('shutdownTeam restores a standalone HUD pane after tearing down the team HUD', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-shutdown-restore-hud-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omq-runtime-shutdown-restore-hud-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
  ...
  it('shutdownTeam preserves leader exclusion while tearing down the hud pane', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-shutdown-exclusions-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omq-runtime-shutdown-exclusions-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\\n' "$*" >> "${tmuxLogPath}"
case "$1" in
...
`,
        ...
        async ({ tmuxLogPath }) => {
          ...
          assert.ok(config);
          if (!config) return;
          config.tmux_session = 'omq-team-team-shutdown-exclusions';
          config.leader_pane_id = '%11';
          config.hud_pane_id = '%12';
  ...
  it('shutdownTeam still throws on failed tasks', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-normal-gate-'));
    try {
      ...
      const teamRoot = join(cwd, '.omq', 'state', 'team', 'team-normal-gate');
      assert.equal(existsSync(teamRoot), true);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it('resumeTeam returns null for prompt teams when worker handles are missing after restart', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-prompt-resume-'));
    ...
    const prevLeaderCwd = process.env.OMQ_TEAM_LEADER_CWD;
    delete process.env.OMQ_TEAM_STATE_ROOT;
    delete process.env.OMQ_TEAM_LEADER_CWD;

    try {
      await initTeamState('team-prompt-resume', 'prompt resume test', 'executor', 1, cwd);
      const configPath = join(cwd, '.omq', 'state', 'team', 'team-prompt-resume', 'config.json');
  ...
  it('assignTask enforces delegation_only policy for leader-fixed worker', async () => {
    ...
    try {
      ...
      const manifestPath = join(cwd, '.omq', 'state', 'team', 'team-delegation', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as any;
  ...
  it('assignTask rolls back claim when inbox write fails after claim', async () => {
    ...
    try {
      ...
      const workerDir = join(cwd, '.omq', 'state', 'team', 'team-inbox-fail', 'workers', 'worker-1');
  ...
  it('assignTask enforces plan approval for code-change tasks when required', async () => {
    ...
    try {
      ...
      const manifestPath = join(cwd, '.omq', 'state', 'team', 'team-approval', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8')) as any;
  ...
  it('monitorTeam does not re-notify already-notified mailbox messages (issue #116)', async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-no-spam-'));
    try {
      ...
      const mailboxDir = join(cwd, '.omq', 'state', 'team', 'team-no-spam', 'mailbox');
  ...
  it('monitorTeam only notifies once per new message even without notified_at (issue #116)', async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-new-msg-'));
    try {
      await initTeamState('team-new-msg', 'new msg test', 'executor', 1, cwd);

      const mailboxDir = join(cwd, '.omq', 'state', 'team', 'team-new-msg', 'mailbox');
      await mkdir(mailboxDir, { recursive: true });
  ...
  it('monitorTeam does not emit duplicate task_completed when transitionTaskStatus completed the task first (issue #161)', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-no-dup-'));
    try {
      ...
      const eventsPath = join(cwd, '.omq', 'state', 'team', 'team-no-dup', 'events', 'events.ndjson');
      const content = await readFile(eventsPath, 'utf-8');
  ...
  it('sendWorkerMessage keeps hook-preferred duplicate leader mailbox sends idempotent after notification', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-leader-dedupe-notified-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omq-runtime-leader-dedupe-notified-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\n' "$$*" >> "${tmuxLogPath}"
case "\${1:-}" in
...
`,
  ...
  it('sendWorkerMessage hook-preferred path persists leader mailbox guidance when leader pane exists', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-leader-inject-'));
    try {
      await withMockTmuxFixture(
        {
          dirPrefix: 'omq-runtime-leader-inject-bin-',
          tmuxScript: (tmuxLogPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${tmuxLogPath}"
case "\${1:-}" in
...
`,
  ...
  it('sendWorkerMessage hook-preferred path for leader waits for receipt then falls back to mailbox persistence', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-leader-hook-'));
    try {
  ...
  it('sendWorkerMessage transport_direct keeps leader-fixed request pending when leader_pane_id missing', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-leader-direct-'));
    try {
      ...
      const manifestPath = join(cwd, '.omq', 'state', 'team', 'team-leader-direct', 'manifest.v2.json');
      const manifest = JSON.parse(await readFile(manifestPath, 'utf-8'));
