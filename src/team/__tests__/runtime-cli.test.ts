...
async function loadRuntimeCliModule() {
  process.env.OMQ_RUNTIME_CLI_DISABLE_AUTO_START = '1';
...
describe('runtime-cli helpers', () => {
  it('normalizes per-worker providers and validates supported values', async () => {
    ...
    assert.deepEqual(
      runtimeCli.normalizeAgentTypes(['qwen', 'gemini'], 2),
      ['qwen', 'gemini'],
    );
    assert.deepEqual(
    ...
    assert.throws(
      () => runtimeCli.normalizeAgentTypes(['qwen', 'invalid'], 2),
      /Expected qwen\\|claude\\|gemini/,
    );
  });

  it('refreshes pane targets from live team config after scale changes', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-cli-live-'));
    try {
  ...
  it('gracefully shuts down only when the leader explicitly requests shutdown', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-cli-shutdown-'));
    ...
    try {
      ...
      const teamRoot = join(cwd, '.omq', 'state', 'team', 'shutdown-fallback');
  ...
  it('does not auto-shutdown merely because monitorTeam reaches complete', async () => {
    const cwd = await mkdtemp(join(tmpdir(), 'omq-runtime-cli-complete-'));
    const previousTeamStateRoot = process.env.OMQ_TEAM_STATE_ROOT;
    delete process.env.OMQ_TEAM_STATE_ROOT;
    try {
      ...
      const teamRoot = join(cwd, '.omq', 'state', 'team', 'runtime-cli-complete');
      assert.equal(existsSync(teamRoot), true);
