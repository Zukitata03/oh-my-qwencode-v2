...
function setMockQwenHome(qwenHomePath: string): () => void {
  const previous = process.env.QWEN_HOME;
  process.env.QWEN_HOME = qwenHomePath;
  return () => {
    if (typeof previous === "string") process.env.QWEN_HOME = previous;
...
describe("worker bootstrap", () => {
  it("worker skill lifecycle instructions are claim-safe (issue #448)", async () => {
    ...
    assert.match(workerSkill, /omq team api claim-task/);
    assert.match(workerSkill, /omq team api transition-task-status/);
    assert.match(workerSkill, /omq team api release-task-claim/);
    assert.match(
      workerSkill,
  ...
  it("generateWorkerOverlay produces markdown with correct start/end markers", () => {
    ...
    assert.match(overlay, /<!-- OMQ:TEAM:WORKER:START -->/);
    assert.match(overlay, /<!-- OMQ:TEAM:WORKER:END -->/);
  });
  ...
  it("generateWorkerOverlay includes the team name", () => {
    ...
    assert.match(overlay, /<leader_cwd>\/\.qwen\/skills\/worker\/SKILL\.md/);
    ...
    assert.match(overlay, /omq team api claim-task/);
    assert.match(overlay, /omq team api transition-task-status/);
    assert.match(overlay, /omq team api release-task-claim/);
    assert.doesNotMatch(
      overlay,
  ...
  it("applyWorkerOverlay appends to existing AGENTS.md content", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omq-worker-bootstrap-"));
    try {
      ...
      assert.match(content, /<!-- OMQ:TEAM:WORKER:START -->/);
      assert.match(content, /<!-- OMQ:TEAM:WORKER:END -->/);
    } finally {
  ...
  it("applyWorkerOverlay is idempotent (calling twice doesn't duplicate)", async () => {
    ...
    try {
      ...
      const content = await readFile(agentsMdPath, "utf8");
      const starts = content.match(/<!-- OMQ:TEAM:WORKER:START -->/g) ?? [];
      const ends = content.match(/<!-- OMQ:TEAM:WORKER:END -->/g) ?? [];
  ...
  it("stripWorkerOverlay removes the overlay section", async () => {
    ...
    try {
      ...
      assert.doesNotMatch(content, /<!-- OMQ:TEAM:WORKER:START -->/);
      assert.doesNotMatch(content, /<!-- OMQ:TEAM:WORKER:END -->/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it("applyWorkerOverlay reaps stale AGENTS lock directory", async () => {
    ...
    try {
      const agentsMdPath = join(cwd, "AGENTS.md");
      const lockPath = join(cwd, ".omq", "state", "agents-md.lock");
  ...
  it("generateInitialInbox includes worker name, team name, and all tasks", () => {
    ...
    assert.match(inbox, /omq team api claim-task/);
    assert.match(inbox, /omq team api transition-task-status/);
    assert.match(inbox, /omq team api release-task-claim/);
    assert.match(
      inbox,
      /\$\{QWEN_HOME:-~\/\.qwen\}\/skills\/worker\/SKILL\.md/,
    );
    assert.match(inbox, /\/\.qwen\/skills\/worker\/SKILL\.md/);
  ...
  it("generateTriggerMessage does not contain [OMQ_TMUX_INJECT]", () => {
    const message = generateTriggerMessage("worker-1", "team-safe");
    assert.equal(message.includes("[OMQ_TMUX_INJECT]"), false);
  });
  ...
  it("generateTriggerMessage contains the inbox path", () => {
    ...
    assert.match(
      message,
      /\.omq\/state\/team\/team-path\/workers\/worker-9\/inbox\.md/,
    );
  ...
  it("generateTriggerMessage uses provided state-root reference for worktree workers", () => {
    ...
    assert.match(
      message,
      /\$OMQ_TEAM_STATE_ROOT\/team\/team-path\/workers\/worker-9\/inbox\.md/,
    );
  ...
  it("generateMailboxTriggerMessage contains mailbox path and count", () => {
    ...
    assert.match(
      message,
      /Read .*\.omq\/state\/team\/team-mail\/mailbox\/worker-2\.json/,
    );
  ...
  it("generateMailboxTriggerMessage uses provided state-root reference for worktree workers", () => {
    ...
    assert.match(
      message,
      /read .*\$OMQ_TEAM_STATE_ROOT\/team\/team-mail\/mailbox\/worker-2\.json/i,
    );
  ...
  it("generateLeaderMailboxTriggerMessage tells the leader to read the mailbox and decide the next step", () => {
    ...
    assert.match(
      message,
      /Read .*\.omq\/state\/team\/team-mail\/mailbox\/leader-fixed\.json/,
    );
  ...
  it("generateLeaderMailboxTriggerMessage uses provided state-root reference for worktree leaders", () => {
    ...
    assert.match(
      message,
      /read .*\$OMQ_TEAM_STATE_ROOT\/team\/team-mail\/mailbox\/leader-fixed\.json/i,
    );
  ...
  it("writeTeamWorkerInstructionsFile composes user + project AGENTS.md with overlay", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omq-worker-bootstrap-"));
    const restoreQwenHome = setMockQwenHome(join(cwd, "home", ".qwen"));
    try {
      await mkdir(join(cwd, "home", ".qwen"), { recursive: true });
      await writeFile(
        join(cwd, "home", ".qwen", "AGENTS.md"),
        "# User Instructions\n\nStart globally.\n",
        "utf8",
      );
      await writeFile(
      ...
      assert.doesNotMatch(projectContent, /<!-- OMQ:TEAM:WORKER:START -->/);
    } finally {
      restoreQwenHome();
      await rm(cwd, { recursive: true, force: true });
    }
  ...
  it("writeTeamWorkerInstructionsFile deduplicates duplicate skill references in favor of project scope", async () => {
    ...
    const restoreQwenHome = setMockQwenHome(join(cwd, "home", ".qwen"));
    try {
      const userAgentsPath = join(cwd, "home", ".qwen", "AGENTS.md");
      const projectAgentsPath = join(cwd, "AGENTS.md");
      const userSkillDir = join(cwd, "home", ".qwen", "skills", "help");
      const projectSkillDir = join(cwd, ".qwen", "skills", "help");
      ...
      await writeFile(
        userAgentsPath,
        "- help user (file: /tmp/home/.qwen/skills/help/SKILL.md)\n",
        "utf8",
      );
      await writeFile(
        projectAgentsPath,
        "- help project (file: /tmp/project/.qwen/skills/help/SKILL.md)\n",
        "utf8",
      );
  ...
  it("generateWorkerRootAgentsContent includes hardcoded paths and role prompt without base AGENTS", () => {
    const content = generateWorkerRootAgentsContent({
      ...
      leaderCwd: "/repo",
      worktreePath: "/repo/.omq/team/root-team/worktrees/worker-3",
    });
  ...
  it("writeWorkerRoleInstructionsFile layers role prompt on top of team worker instructions", async () => {
    ...
    try {
      ...
      assert.match(content, /<!-- OMQ:TEAM:ROLE:START -->/);
      assert.match(content, /\*\*writer\*\* role/);
      assert.match(content, /<identity>Writer role prompt<\/identity>/);
      assert.doesNotMatch(content, /exact qwen3.6-flash model/);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
  ...
  it("generateWorkerRootAgentsContent hardcodes runtime paths and role prompt without inherited AGENTS", () => {
    const content = generateWorkerRootAgentsContent({
      ...
      workerRole: "writer",
      rolePromptContent: "<identity>You are Writer.</identity>",
      teamStateRoot: "/tmp/project/.omq/state",
      leaderCwd: "/tmp/project",
      worktreePath: "/tmp/project/.omq/team/root-team/worktrees/worker-2",
    });

    assert.match(content, /# Team Worker Runtime Instructions/);
    assert.match(content, /Inbox path: \/tmp\/project\/.omq\/state\/team\/root-team\/workers\/worker-2\/inbox\.md/);
    assert.match(content, /Mailbox path: \/tmp\/project\/.omq\/state\/team\/root-team\/mailbox\/worker-2\.json/);
    assert.match(content, /Leader mailbox path: \/tmp\/project\/.omq\/state\/team\/root-team\/mailbox\/leader-fixed\.json/);
  ...
  it("writeWorkerWorktreeRootAgentsFile writes disposable root AGENTS and remove restores tracked content", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omq-worker-root-agents-"));
    const worktree = join(cwd, "worktree");
    try {
      await mkdir(join(cwd, ".omq", "state", "team", "restore-team", "workers", "worker-1"), { recursive: true });
      ...
      const outPath = await writeWorkerWorktreeRootAgentsFile({
        ...
        teamStateRoot: join(cwd, ".omq", "state"),
        leaderCwd: cwd,
      ...
      await removeWorkerWorktreeRootAgentsFile("restore-team", "worker-1", join(cwd, ".omq", "state"), worktree);
