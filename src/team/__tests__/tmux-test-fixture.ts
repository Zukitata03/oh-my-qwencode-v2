...
export async function withTempTmuxSession<T>(
  ...
  const fixtureCwd = await mkdtemp(join(tmpdir(), 'omq-tmux-fixture-'));
  const sessionName = uniqueTmuxIdentifier('omq-test');
  const serverName = options.useAmbientServer ? '' : uniqueTmuxIdentifier('omq-fixture');
