...
│ >_ Qwen (v0.114.0)                 │
│                                            │
│ model:     qwen3.6-plus high   /model to change │
...
describe('chooseTeamLeaderPaneId', () => {
  it('keeps preferred pane when it is not HUD', () => {
    const panes = [
      { paneId: '%1', currentCommand: 'node', startCommand: "'qwen'" },
      { paneId: '%1', currentCommand: 'node', startCommand: "node omq hud --watch" },
    ];
  ...
  it('falls back to preferred pane when all panes are HUD panes', () => {
    const panes = [
      { paneId: '%1', currentCommand: 'node', startCommand: "node omq hud --watch" },
      { paneId: '%3', currentCommand: 'node', startCommand: "node omq hud --watch" },
    ];
    assert.equal(chooseTeamLeaderPaneId(panes, '%1'), '%1');
  });
});
...
describe('HUD resize hook command builders', () => {
  it('buildResizeHookName normalizes all segments into collision-safe tokens', () => {
    ...
    assert.equal(name, 'omq_resize_Team_A_Session_Main_0_12');
  });
  ...
  it('buildRegisterResizeHookArgs uses window target and numeric client-resized hook slot', () => {
    const args = buildRegisterResizeHookArgs('my-session:0', 'omq_resize_team_session_0_1', '%1');
  ...
  it('buildUnregisterResizeHookArgs removes the exact numeric hook slot', () => {
    const registered = buildRegisterResizeHookArgs('my-session:0', 'omq_resize_team_session_0_1', '%1');
    const unregistered = buildUnregisterResizeHookArgs('my-session:0', 'omq_resize_team_session_0_1');
  ...
  it('buildClientAttachedReconcileHookName normalizes all segments into collision-safe tokens', () => {
    ...
    assert.equal(name, 'omq_attached_Team_A_Session_Main_0_12');
  });

  it('buildRegisterClientAttachedReconcileArgs installs one-shot client-attached reconcile hook', () => {
    const args = buildRegisterClientAttachedReconcileArgs('my-session:0', 'omq_attached_team_session_0_1', '%1');
    assert.equal(args[0], 'set-hook');
    assert.equal(args[1], '-t');
    assert.equal(args[2], 'my-session:0');
  ...
  it('buildUnregisterClientAttachedReconcileArgs removes the exact numeric client-attached slot', () => {
    const registered = buildRegisterClientAttachedReconcileArgs('my-session:0', 'omq_attached_team_session_0_1', '%1');
    const unregistered = buildUnregisterClientAttachedReconcileArgs('my-session:0', 'omq_attached_team_session_0_1');
    assert.deepEqual(unregistered, ['set-hook', '-u', '-t', 'my-session:0', registered[3] as string]);
  });
  ...
  it('hook indices stay within signed 32-bit range (issue #240)', () => {
    ...
    const longName = 'omq_resize_' + 'a'.repeat(200);
  ...
  it('hook indices are deterministic across calls', () => {
    const name = 'omq_resize_team_session_0_1';
...
describe('sendToWorker validation', () => {
  it('rejects text over 200 chars', async () => {
    await assert.rejects(
      sendToWorker('omq-team-x', 1, 'a'.repeat(200)),
      /< 200/i
    );
  });
  ...
  it('rejects injection marker', async () => {
    await assert.rejects(
      sendToWorker('omq-team-x', 1, `hello [OMQ_TMUX_INJECT]`),
      /marker/i
    );
  });
...
`,
      async ({ logPath }) => {
        await sendToWorker('omq-team-x', 1, 'check inbox');
        const log = await readFile(logPath, 'utf-8');
        const acceptIndex = log.indexOf('send-keys -t omq-team-x:1 -l -- 2');
        const submitIndex = log.indexOf('send-keys -t omq-team-x:1 -l -- check inbox');
...
describe('paneLooksReady gate: status-only is not ready (#391)', () => {
  ...
  it('shouldAttemptAdaptiveRetry returns false for status-only capture (no prompt)', () => {
    // Capture has Qwen Code status bar but no prompt character — paneLooksReady
...
describe('buildWorkerStartupCommand', () => {
  it('auto-selects gemini worker CLI from gemini model', () => {
    ...
    const prevCli = process.env.OMQ_TEAM_WORKER_CLI;
    const prevBypass = process.env.OMQ_BYPASS_DEFAULT_SYSTEM_PROMPT;
    process.env.SHELL = '/bin/bash';
    delete process.env.OMQ_TEAM_WORKER_CLI; // auto
    ...
    try {
      ...
      if (typeof prevCli === 'string') process.env.OMQ_TEAM_WORKER_CLI = prevCli;
      ...
      else delete process.env.OMQ_BYPASS_DEFAULT_SYSTEM_PROMPT;
    }
  });
  ...
  it('respects explicit OMQ_TEAM_WORKER_CLI override', () => {
    const prevShell = process.env.SHELL;
    const prevCli = process.env.OMQ_TEAM_WORKER_CLI;
    const prevBypass = process.env.OMQ_BYPASS_DEFAULT_SYSTEM_PROMPT;
    process.env.SHELL = '/bin/bash';
    ...
    try {
      ...
      const qwenCmd = buildWorkerStartupCommand('alpha', 1, ['--model', 'claude-3-7-sonnet']);
      assert.match(qwenCmd, /exec .*qwen/);

      process.env.OMQ_TEAM_WORKER_CLI = 'claude';
  ...
  it('uses zsh with ~/.zshrc and exec qwen', () => {
    const prevShell = process.env.SHELL;
    ...
    try {
      ...
      assert.match(cmd, /OMQ_TEAM_WORKER=alpha\/worker-2/);
  ...
  it('injects canonical team state env vars when provided', () => {
    ...
    try {
      const cmd = buildWorkerStartupCommand(
        ...
        '/tmp/worker-cwd',
        {
          OMQ_TEAM_STATE_ROOT: '/tmp/leader/.omq/state',
          OMQ_TEAM_LEADER_CWD: '/tmp/leader',
        },
      );
      assert.match(cmd, /OMQ_TEAM_STATE_ROOT=\/tmp\/leader\/\.omq\/state/);
      assert.match(cmd, /OMQ_TEAM_LEADER_CWD=\/tmp\/leader/);
    } finally {
      if (typeof prevShell === 'string') process.env.SHELL = prevShell;
      else delete process.env.SHELL;
      if (typeof prevBypass === 'string') process.env.OMQ_BYPASS_DEFAULT_SYSTEM_PROMPT = prevBypass;
  ...
  it('resolves POSIX leader paths before building fish worker startup commands', async () => {
    const fakeBin = await mkdtemp(join(tmpdir(), 'omq-worker-startup-posix-'));
    ...
    try {
      ...
      const qwenPath = join(fakeBin, 'qwen');
      await writeFile(nodePath, '#!/bin/sh\n');
      await writeFile(qwenPath, '#!/bin/sh\n');
      ...
      assert.match(cmd, new RegExp(escapeRegExp(`OMQ_LEADER_NODE_PATH=${nodePath}`)));
      assert.match(cmd, new RegExp(escapeRegExp(`OMQ_LEADER_CLI_PATH=${qwenPath}`)));
  ...
  it('forces qwen bypass under explicit launch-arg profiles', () => {
    ...
    try {
      const profiles = [
        ...
        ['--model', 'qwen3.5-plus', '-c', 'model_reasoning_effort="low"'],
      ];
  ...
  it('injects model_instructions_file override by default', () => {
    ...
    process.env.SHELL = '/bin/bash';
    const prevBypass = process.env.OMQ_BYPASS_DEFAULT_SYSTEM_PROMPT;
    const prevInstr = process.env.OMQ_MODEL_INSTRUCTIONS_FILE;
    delete process.env.OMQ_BYPASS_DEFAULT_SYSTEM_PROMPT; // default enabled
    delete process.env.OMQ_MODEL_INSTRUCTIONS_FILE;
    try {
      ...
      if (typeof prevInstr === 'string') process.env.OMQ_MODEL_INSTRUCTIONS_FILE = prevInstr;
      else delete process.env.OMQ_MODEL_INSTRUCTIONS_FILE;
    }
  });


  it('uses per-worker OMQ_MODEL_INSTRUCTIONS_FILE from extraEnv when building process launch spec', () => {
    const prevBypass = process.env.OMQ_BYPASS_DEFAULT_SYSTEM_PROMPT;
    const prevInstr = process.env.OMQ_MODEL_INSTRUCTIONS_FILE;
    delete process.env.OMQ_BYPASS_DEFAULT_SYSTEM_PROMPT;
    delete process.env.OMQ_MODEL_INSTRUCTIONS_FILE;
    try {
      const spec = buildWorkerProcessLaunchSpec(
        ...
        ['-c', 'model_reasoning_effort="low"'],
        '/tmp/project',
        { OMQ_MODEL_INSTRUCTIONS_FILE: '/tmp/project/.omq/state/team/alpha/workers/worker-1/AGENTS.md' },
        'qwen',
      );
  ...
  it('translates model_instructions_file path for MSYS2/Git Bash environments', () => {
    const prevShell = process.env.SHELL;
    const prevBypass = process.env.OMQ_BYPASS_DEFAULT_SYSTEM_PROMPT;
    const prevInstructions = process.env.OMQ_MODEL_INSTRUCTIONS_FILE;
    const prevMsystem = process.env.MSYSTEM;
    const origPlatform = Object.getOwnPropertyDescriptor(process, 'platform');
    process.env.SHELL = '/bin/bash';
    delete process.env.OMQ_BYPASS_DEFAULT_SYSTEM_PROMPT; // default enabled
    process.env.OMQ_MODEL_INSTRUCTIONS_FILE = 'C:\\repo\\AGENTS.md';
    ...
    try {
      ...
      if (typeof prevInstructions === 'string') process.env.OMQ_MODEL_INSTRUCTIONS_FILE = prevInstructions;
      else delete process.env.OMQ_MODEL_INSTRUCTIONS_FILE;
...
describe('team worker CLI helpers', () => {
  it('resolveTeamWorkerCli auto-detects claude models', () => {
    ...
    assert.equal(resolveTeamWorkerCli(['--model', 'gpt-5'], {}), 'qwen');
    assert.equal(resolveTeamWorkerCli([], {}), 'qwen');
  });

  it('resolveTeamWorkerCli accepts explicit gemini override', () => {
    assert.equal(resolveTeamWorkerCli([], { OMQ_TEAM_WORKER_CLI: 'gemini' }), 'gemini');
  });

  it('resolveTeamWorkerCliPlan accepts gemini in CLI map', () => {
    const plan = resolveTeamWorkerCliPlan(3, [], { OMQ_TEAM_WORKER_CLI_MAP: 'qwen,gemini,claude' });
  ...
  it('translateWorkerLaunchArgsForCli omits non-gemini default models for gemini workers', () => {
    assert.deepEqual(
      translateWorkerLaunchArgsForCli('gemini', ['--model', 'qwen3.5-plus'], 'Read worker inbox'),
      ['--approval-mode', 'yolo', '-i', 'Read worker inbox'],
    );
  });
  ...
  it('resolveTeamWorkerCliPlan supports mixed per-worker CLI map', () => {
    const plan = resolveTeamWorkerCliPlan(
      4,
      [],
      { OMQ_TEAM_WORKER_CLI_MAP: 'qwen,qwen,gemini,claude' },
    );
    assert.deepEqual(plan, ['qwen', 'qwen', 'gemini', 'claude']);
  });
  ...
  it('resolveTeamWorkerCliPlan accepts single-value map and expands to all workers', () => {
    const plan = resolveTeamWorkerCliPlan(
      3,
      [],
      { OMQ_TEAM_WORKER_CLI_MAP: 'claude' },
    );
  ...
  it('resolveTeamWorkerCliPlan supports auto entries in CLI map', () => {
    const plan = resolveTeamWorkerCliPlan(
      2,
      ['--model', 'claude-3-7-sonnet'],
      { OMQ_TEAM_WORKER_CLI_MAP: 'auto,qwen' },
    );
    assert.deepEqual(plan, ['claude', 'qwen']);
  });

  it('resolveTeamWorkerCliPlan auto entries ignore OMQ_TEAM_WORKER_CLI override', () => {
    const plan = resolveTeamWorkerCliPlan(
      1,
      ['--model', 'claude-3-7-sonnet'],
      {
        OMQ_TEAM_WORKER_CLI: 'qwen',
        OMQ_TEAM_WORKER_CLI_MAP: 'auto',
      },
    );
  ...
  it('resolveTeamWorkerCliPlan rejects map lengths that do not match workerCount', () => {
    assert.throws(
      () => resolveTeamWorkerCliPlan(4, [], { OMQ_TEAM_WORKER_CLI_MAP: 'qwen,claude' }),
      /expected 1 or 4/i,
    );
  });
  ...
  it('resolveTeamWorkerCliPlan rejects empty entries in CLI map', () => {
    assert.throws(
      () => resolveTeamWorkerCliPlan(2, [], { OMQ_TEAM_WORKER_CLI_MAP: 'qwen,' }),
      /empty entries are not allowed/i,
    );
  });

  it('resolveTeamWorkerCliPlan reports invalid entry errors with OMQ_TEAM_WORKER_CLI_MAP', () => {
    assert.throws(
      () => resolveTeamWorkerCliPlan(1, [], { OMQ_TEAM_WORKER_CLI_MAP: 'claudee' }),
      /OMQ_TEAM_WORKER_CLI_MAP/i,
    );
  });

  it('resolveWorkerCliForSend prioritizes explicit worker CLI over map/global', () => {
    assert.equal(
      resolveWorkerCliForSend(2, 'claude', [], { OMQ_TEAM_WORKER_CLI_MAP: 'qwen,qwen' }),
      'claude',
    );
  });
  ...
  it('resolveWorkerCliForSend resolves per-worker map entry by index', () => {
    assert.equal(
      resolveWorkerCliForSend(2, undefined, [], { OMQ_TEAM_WORKER_CLI_MAP: 'qwen,claude' }),
      'claude',
    );
  });
  ...
  it('buildWorkerSubmitPlan preserves queue-first behavior for busy qwen workers', () => {
    const plan = buildWorkerSubmitPlan('auto', 'qwen', true, true);
...
describe('team worker launch mode helpers', () => {
  it('resolveTeamWorkerLaunchMode defaults to interactive and accepts prompt', () => {
    ...
    assert.equal(resolveTeamWorkerLaunchMode({ OMQ_TEAM_WORKER_LAUNCH_MODE: 'interactive' }), 'interactive');
    assert.equal(resolveTeamWorkerLaunchMode({ OMQ_TEAM_WORKER_LAUNCH_MODE: 'prompt' }), 'prompt');
    assert.equal(resolveTeamWorkerLaunchMode({ OMQ_TEAM_WORKER_LAUNCH_MODE: ' PROMPT ' }), 'prompt');
  });

  it('resolveTeamWorkerLaunchMode rejects unsupported values', () => {
    assert.throws(
      () => resolveTeamWorkerLaunchMode({ OMQ_TEAM_WORKER_LAUNCH_MODE: 'tmux' }),
      /Invalid OMQ_TEAM_WORKER_LAUNCH_MODE value/i,
    );
  });
  ...
  it('buildWorkerProcessLaunchSpec returns command/args/env for prompt process spawn', () => {
    ...
    try {
      const spec = buildWorkerProcessLaunchSpec(
        ...
        ['--model', 'qwen3.5-plus'],
        '/tmp/workspace',
        { OMQ_TEAM_STATE_ROOT: '/tmp/workspace/.omq/state' },
        'qwen',
      );
  ...
  it('buildWorkerProcessLaunchSpec includes leader node and CLI path env vars', () => {
    ...
    try {
      ...
      assert.ok(
        typeof spec.env.OMQ_LEADER_NODE_PATH === 'string' && spec.env.OMQ_LEADER_NODE_PATH.length > 0,
        'OMQ_LEADER_NODE_PATH must be set',
      );
      assert.ok(
        typeof spec.env.OMQ_LEADER_CLI_PATH === 'string' && spec.env.OMQ_LEADER_CLI_PATH.length > 0,
        'OMQ_LEADER_CLI_PATH must be set',
      );
      // command matches the resolved CLI path stored in env
      assert.equal(spec.command, spec.env.OMQ_LEADER_CLI_PATH);
    } finally {
      if (typeof prevBypass === 'string') process.env.OMQ_BYPASS_DEFAULT_SYSTEM_PROMPT = prevBypass;
      else delete process.env.OMQ_BYPASS_DEFAULT_SYSTEM_PROMPT;
    }
...
describe('tmux-dependent functions when tmux is unavailable', () => {
  ...
  it('waitForWorkerReady uses visible capture-pane argv without tail flags', async () => {
    await withMockTmuxFixture(
      'omq-tmux-worker-ready-visible-capture-',
      (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
      async ({ logPath }) => {
        assert.equal(waitForWorkerReady('omq-team-x', 1, 1_000), true);
        const log = await readFile(logPath, 'utf-8');
        assert.match(log, /capture-pane -t omq-team-x:1 -p/);
        assert.doesNotMatch(log, /capture-pane -t omq-team-x:1 -p -S/);
      },
    );
  });

  it('waitForWorkerReady accepts Qwen Code 0.114.0-style welcome helper text', async () => {
    await withMockTmuxFixture(
      'omq-tmux-worker-ready-hello-',
      (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
      async ({ logPath }) => {
        assert.equal(waitForWorkerReady('omq-team-x', 1, 1_000), true);
        const log = await readFile(logPath, 'utf-8');
        assert.match(log, /send-keys -t omq-team-x:1 -l -- 2/);
        assert.match(log, /send-keys -t omq-team-x:1 C-m/);
      },
    );
  });

  it('waitForWorkerReady leaves the Claude bypass prompt untouched when auto-accept is disabled', async () => {
    const previousAutoAccept = process.env.OMQ_TEAM_AUTO_ACCEPT_BYPASS;
    process.env.OMQ_TEAM_AUTO_ACCEPT_BYPASS = '0';
    try {
      await withMockTmuxFixture(
        'omq-tmux-claude-bypass-blocked-',
        (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
        async ({ logPath }) => {
          assert.equal(waitForWorkerReady('omq-team-x', 1, 250), false);
          const log = await readFile(logPath, 'utf-8');
          assert.doesNotMatch(log, /send-keys/);
        },
      );
    } finally {
      if (typeof previousAutoAccept === 'string') process.env.OMQ_TEAM_AUTO_ACCEPT_BYPASS = previousAutoAccept;
      else delete process.env.OMQ_TEAM_AUTO_ACCEPT_BYPASS;
    }
  });

  it('waitForWorkerReady returns false on timeout', () => {
    withEmptyPath(() => {
      assert.equal(waitForWorkerReady('omq-team-x', 1, 1), false);
    });
  });
});
...
describe('dismissTrustPromptIfPresent capture shape', () => {
  it('uses visible capture-pane argv without tail flags', async () => {
    const previousAutoTrust = process.env.OMQ_TEAM_AUTO_TRUST;
    delete process.env.OMQ_TEAM_AUTO_TRUST;
    try {
      await withMockTmuxFixture(
        'omq-tmux-dismiss-trust-visible-capture-',
        (logPath) => `#!/bin/sh
set -eu
printf '%s\n' "$*" >> "${logPath}"
case "$1" in
...
`,
        async ({ logPath }) => {
          assert.equal(dismissTrustPromptIfPresent('omq-team-x', 1), true);
          const log = await readFile(logPath, 'utf-8');
          assert.match(log, /capture-pane -t omq-team-x:1 -p/);
          assert.doesNotMatch(log, /capture-pane -t omq-team-x:1 -p -S/);
        },
      ...
      if (typeof previousAutoTrust === 'string') process.env.OMQ_TEAM_AUTO_TRUST = previousAutoTrust;
      else delete process.env.OMQ_TEAM_AUTO_TRUST;
    }
  });
});
...
describe('dismissTrustPromptIfPresent', () => {
  it('returns false when tmux is unavailable', () => {
    withEmptyPath(() => {
      assert.equal(dismissTrustPromptIfPresent('omq-team-x', 1), false);
    });
  });

  it('returns false when OMQ_TEAM_AUTO_TRUST is disabled', () => {
    const prev = process.env.OMQ_TEAM_AUTO_TRUST;
    process.env.OMQ_TEAM_AUTO_TRUST = '0';
    try {
      assert.equal(dismissTrustPromptIfPresent('omq-team-x', 1), false);
    } finally {
      if (typeof prev === 'string') process.env.OMQ_TEAM_AUTO_TRUST = prev;
      else delete process.env.OMQ_TEAM_AUTO_TRUST;
    }
  });

  it('returns false when OMQ_TEAM_AUTO_TRUST is unset (auto-trust enabled) but tmux unavailable', () => {
    const prev = process.env.OMQ_TEAM_AUTO_TRUST;
    delete process.env.OMQ_TEAM_AUTO_TRUST;
    try {
      withEmptyPath(() => {
...
describe('isWorkerAlive', () => {
  it('does not require pane_current_command to match "qwen"', () => {
    // This was a real failure mode: tmux reports pane_current_command=node for the Qwen Code TUI,
    // which caused workers to be treated as dead and the leader to clean up state too early.
    withEmptyPath(() => {
      assert.equal(isWorkerAlive('omq-team-x', 1), false);
    });
  });
});
...
describe('enableMouseScrolling', () => {
  it('returns false when tmux is unavailable', () => {
    ...
    withEmptyPath(() => {
      assert.equal(enableMouseScrolling('omq-team-x'), false);
    });
  });
...
describe('enableMouseScrolling scroll and copy setup (issue #206)', () => {
  ...
  it('does not throw when WSL2 env is set and tmux is unavailable (regression + #206)', () => {
    ...
    try {
      withEmptyPath(() => {
        assert.doesNotThrow(() => enableMouseScrolling('omq-team-x'));
      });
    } finally {
      if (typeof prev === 'string') process.env.WSL_DISTRO_NAME = prev;
      else delete process.env.WSL_DISTRO_NAME;
...
`,
      async ({ logPath }) => {
        assert.equal(enableMouseScrolling('omq-team-x'), true);
        const tmuxLog = await readFile(logPath, 'utf-8');
        assert.match(tmuxLog, /set-option -t omq-team-x mouse on/);
        assert.match(tmuxLog, /set-option -t omq-team-x set-clipboard on/);
...
describe('killWorker leader pane guard', () => {
  it('returns immediately when workerPaneId matches leaderPaneId', () => {
    ...
    withEmptyPath(() => {
      assert.doesNotThrow(() => killWorker('omq-team-x:0', 1, '%5', '%5'));
    });
  });
  ...
  it('proceeds (gracefully) when pane ids differ', () => {
    ...
    withEmptyPath(() => {
      assert.doesNotThrow(() => killWorker('omq-team-x:0', 1, '%5', '%6'));
    });
  });

  it('proceeds when leaderPaneId is not provided', () => {
    withEmptyPath(() => {
      assert.doesNotThrow(() => killWorker('omq-team-x:0', 1, '%5'));
    });
  });
});
...
`,
      async ({ logPath }) => {
        const summary = await teardownWorkerPanes(['%1', '%1', '%3'], {
          leaderPaneId: '%1',
          hudPaneId: '%1',
          graceMs: 1,
        });

        assert.equal(summary.excluded.leader, 2);
        assert.equal(summary.excluded.hud, 0);
