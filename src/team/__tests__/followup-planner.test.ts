...
describe('followup-planner', () => {
  it('resolves available agent types from explicit prompt directories', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'omq-followup-roster-'));
    try {
  ...
  it('builds concrete team staffing guidance from the available roster', () => {
    ...
    assert.equal(plan.launchHints.shellCommand, 'omq team 3:executor "Fix flaky integration tests and update README"');
  ...
  it('builds concrete ralph staffing guidance from the available roster', () => {
    ...
    assert.equal(plan.launchHints.shellCommand, 'omq ralph "Investigate auth regression and verify the fix"');
