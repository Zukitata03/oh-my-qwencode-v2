...
import { qwenPromptsDir } from '../utils/paths.js';
...
function summarizeWorktreeDiffWithSparkShell(worktreePath: string): string | null {
  ...
  const result = runCommand('omq', ['sparkshell', 'sh', '-lc', shellCommand], worktreePath);
...
function autoCommitDirtyWorktree(
  ...
  const msg = `omq(team): auto-checkpoint ${worker.name} [${taskId}]`;
...
async function integrateWorkerCommitsIntoLeader(params: {
  ...
  for (const worker of config.workers) {
    ...
    if (workerIsAheadOfLeader) {
      ...
      const merge = runGitCommand(repoRoot, ['merge', '--no-ff', '-X', 'theirs', '-m', `omq(team): merge ${worker.name}`, branchRef], cwd);
...
async function prepareShutdownMergeReport(
  ...
  if (hadChanges) {
    ...
    if (!addResult.ok) {
      return {
        workerName: worker.name,
        worktreePath,
        reportPath: join(worktreePath, '.omq', 'diff.md'),
        sourceRef: null,
    ...
    const commitResult = runGitCommand(
      repoRoot,
      ['commit', '--no-verify', '-m', `omq(team): checkpoint ${worker.name} shutdown changes`],
      worktreePath,
    );
    if (commitResult.ok) {
  ...
  const reportPath = join(worktreePath, '.omq', 'diff.md');
  ...
  await mkdir(join(worktreePath, '.omq'), { recursive: true });
...
async function prepareWorkerWorktreeShutdownReports(config: TeamConfig, leaderCwd: string): Promise<void> {
  for (const worker of config.workers) {
    ...
    try {
      ...
      await mkdir(join(worktreePath, '.omq'), { recursive: true }).catch(() => {});
...
const MODEL_INSTRUCTIONS_FILE_ENV = 'OMQ_MODEL_INSTRUCTIONS_FILE';
const TEAM_STATE_ROOT_ENV = 'OMQ_TEAM_STATE_ROOT';
const TEAM_LEADER_CWD_ENV = 'OMQ_TEAM_LEADER_CWD';
...
async function assertNestedTeamAllowed(cwd: string): Promise<void> {
  const workerContext = parseTeamWorkerContext(process.env.OMQ_TEAM_WORKER);
...
function doesStartupEvidenceSettle(
  ...
  if (workerCli === 'qwen' && evidence === 'leader_ack') return false;
  return true;
}
...
function shouldSkipWorkerReadyWait(env: NodeJS.ProcessEnv): boolean {
  return env.OMQ_TEAM_SKIP_READY_WAIT === '1';
}
...
export function resolveWorkerLaunchArgsFromEnv(
  ...
  const inheritedArgs = (typeof inheritedLeaderModel === 'string' && inheritedLeaderModel.trim() !== '')
    ...
    : [];
  const fallbackModel = resolveAgentDefaultModel(agentType, env.QWEN_HOME);

  // Detect if an explicit reasoning override exists before resolving (for log source labelling)
  const preEnvArgs = splitWorkerLaunchArgs(env.OMQ_TEAM_WORKER_LAUNCH_ARGS);
  ...
  const resolved = resolveTeamWorkerLaunchArgs({
    existingRaw: env.OMQ_TEAM_WORKER_LAUNCH_ARGS,
    inheritedArgs,
    fallbackModel,
    preferredReasoning,
  });
  ...
  if (effectiveWorkerCli === 'claude') {
    console.log('[omq:team] worker startup resolution: model=claude source=local-settings');
  } else if (effectiveWorkerCli === 'gemini') {
    console.log('[omq:team] worker startup resolution: model=gemini source=local-settings');
  } else {
    console.log(`[omq:team] worker startup resolution: model=${resolvedModel} thinking_level=${thinkingLevel} source=${source}`);
  }
...
function resolveEffectiveWorkerCliForStartupLog(
  ...
  const rawCliMap = String(env.OMQ_TEAM_WORKER_CLI_MAP ?? '').trim();
  if (rawCliMap !== '') {
    ...
    if (entries.length > 0) {
      const autoCli = resolveTeamWorkerCli(resolvedLaunchArgs, {
        ...env,
        OMQ_TEAM_WORKER_CLI: 'auto',
      });
      const resolvedMap = entries.map((entry): 'qwen' | 'claude' | 'gemini' | null => {
        if (entry === 'auto') return autoCli;
        if (entry === 'qwen' || entry === 'claude' || entry === 'gemini') return entry;
        return null;
      });
      if (resolvedMap.every((entry) => entry === 'claude')) return 'claude';
      if (resolvedMap.every((entry) => entry === 'gemini')) return 'gemini';
      if (resolvedMap.some((entry) => entry === 'qwen')) return 'qwen';
    }
  }
...
export async function startTeam(
  ...
  let sessionName = `omq-team-${sanitized}`;
  ...
  const sharedWorkerLaunchArgs = resolveTeamWorkerLaunchArgs({
    existingRaw: process.env.OMQ_TEAM_WORKER_LAUNCH_ARGS,
    fallbackModel: resolveAgentDefaultModel(agentType, process.env.QWEN_HOME),
  });
  ...
  try {
    ...
    config = await initTeamState(
      ...
      leaderCwd,
      DEFAULT_MAX_WORKERS,
      { ...process.env, OMQ_TEAM_DISPLAY_MODE: displayMode, OMQ_TEAM_WORKER_LAUNCH_MODE: workerLaunchMode },
      {
    ...
    for (let i = 1; i <= workerCount; i++) {
      ...
      const workerRole = taskRoles.length > 0 && uniqueTaskRoles.size === 1
        ? taskRoles[0]
        : agentType;
      const rawRolePromptContent = await loadRolePrompt(workerRole, join(leaderCwd, '.qwen', 'prompts'))
        ?? await loadRolePrompt(workerRole, qwenPromptsDir());
    ...
    const workerStartups = workerBootstrapPlans.map((plan) => {
      ...
      if (plan.workerWorkspace.worktreePath) {
        env.OMQ_TEAM_WORKTREE_PATH = plan.workerWorkspace.worktreePath;
      }
      if (plan.workerWorkspace.worktreeBranch) {
        env.OMQ_TEAM_WORKTREE_BRANCH = plan.workerWorkspace.worktreeBranch;
      }
      if (typeof plan.workerWorkspace.worktreeDetached === 'boolean') {
        env.OMQ_TEAM_WORKTREE_DETACHED = plan.workerWorkspace.worktreeDetached ? '1' : '0';
      }
      return {
    ...
    for (let i = 1; i <= workerCount; i++) {
      ...
      if (workerTasks.map(t => t.role).filter(Boolean).length > 0 && new Set(workerTasks.map(t => t.role).filter(Boolean)).size > 1) {
        console.log(`[omq:team] ${workerName}: mixed task roles [${[...new Set(workerTasks.map(t => t.role).filter(Boolean))].join(', ')}], falling back to ${agentType}`);
      }
      ...
      // Retry dispatch up to 3 times to handle Qwen Code trust prompts that may
...
export async function shutdownTeam(teamName: string, cwd: string, options: ShutdownOptions = {}): Promise<void> {
  ...
  if (!config) {
    ...
    try {
      destroyTeamSession(`omq-team-${sanitized}`);
    } catch (err) {
      process.stderr.write(`[team/runtime] operation failed: ${err}\n`);
    }
  ...
  if (anyAliveAfterWait && !force) {
    // Workers may have accepted shutdown but not exited (Qwen Code TUI requires explicit exit).
...
async function findActiveTeams(cwd: string, leaderSessionId: string): Promise<string[]> {
  const root = join(cwd, '.omq', 'state', 'team');
  ...
  for (const e of entries) {
    ...
    const workerLaunchMode = cfg?.worker_launch_mode
      ...
      ?? 'interactive';
    const tmuxSession = (manifest?.tmux_session || cfg?.tmux_session || `omq-team-${teamName}`).split(':')[0];
    if (leaderSessionId) {
...
async function resolveLeaderSessionId(cwd: string): Promise<string> {
  const fromEnv = process.env.OMQ_SESSION_ID || process.env.QWEN_SESSION_ID || process.env.SESSION_ID;
  if (fromEnv && fromEnv.trim() !== '') return fromEnv.trim();

  const p = join(cwd, '.omq', 'state', 'session.json');
...
async function dispatchCriticalInboxInstruction(params: {
  ...
  const requiresObservedStartupEvidence = requireWorkerStartupEvidence === true
    && (workerCli === 'claude' || workerCli === 'qwen');
