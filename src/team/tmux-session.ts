...
const OMQ_TEAM_WORKER_CLI_MAP_ENV = 'OMQ_TEAM_WORKER_CLI_MAP';
const OMQ_TEAM_WORKER_LAUNCH_MODE_ENV = 'OMQ_TEAM_WORKER_LAUNCH_MODE';
const OMQ_TEAM_AUTO_INTERRUPT_RETRY_ENV = 'OMQ_TEAM_AUTO_INTERRUPT_RETRY';
...
const OMQ_LEADER_NODE_PATH_ENV = 'OMQ_LEADER_NODE_PATH';
const OMQ_LEADER_CLI_PATH_ENV = 'OMQ_LEADER_CLI_PATH';

export type TeamWorkerCli = 'qwen' | 'claude' | 'gemini';
...
function isHudWatchPane(pane: TmuxPaneInfo): boolean {
  ...
  return /\bomq\b.*\bhud\b.*--watch/i.test(start);
}
...
function normalizeTeamWorkerCliMode(raw: string | undefined, sourceEnv: string = OMQ_TEAM_WORKER_CLI_ENV): TeamWorkerCliMode {
  const normalized = String(raw ?? 'auto').trim().toLowerCase();
  if (normalized === '' || normalized === 'auto') return 'auto';
  if (normalized === 'qwen' || normalized === 'claude' || normalized === 'gemini') return normalized;
  throw new Error(`Invalid ${sourceEnv} value "${raw}". Expected: auto, qwen, claude, gemini`);
}

export function resolveTeamWorkerLaunchMode(
  env: NodeJS.ProcessEnv = process.env,
): TeamWorkerLaunchMode {
  const raw = String(env[OMQ_TEAM_WORKER_LAUNCH_MODE_ENV] ?? 'interactive').trim().toLowerCase();
  if (raw === '' || raw === 'interactive') return 'interactive';
  if (raw === 'prompt') return 'prompt';
  throw new Error(`Invalid ${OMQ_TEAM_WORKER_LAUNCH_MODE_ENV} value "${env[OMQ_TEAM_WORKER_LAUNCH_MODE_ENV]}". Expected: interactive, prompt`);
}
...
export function resolveTeamWorkerCli(launchArgs: string[] = [], env: NodeJS.ProcessEnv = process.env): TeamWorkerCli {
  const mode = normalizeTeamWorkerCliMode(env[OMQ_TEAM_WORKER_CLI_ENV]);
...
export function resolveTeamWorkerCliPlan(
  ...
  const rawMap = String(env[OMQ_TEAM_WORKER_CLI_MAP_ENV] ?? '').trim();
  ...
  if (entries.length === 0 || entries.every((part) => part.length === 0)) {
    throw new Error(
      `Invalid ${OMQ_TEAM_WORKER_CLI_MAP_ENV} value "${env[OMQ_TEAM_WORKER_CLI_MAP_ENV]}". `
        + `Expected comma-separated values: auto|qwen|claude|gemini.`,
    );
  }
  ...
  if (entries.length !== 1 && entries.length !== workerCount) {
    throw new Error(
      `Invalid ${OMQ_TEAM_WORKER_CLI_MAP_ENV} length ${entries.length}; `
  ...
  return expanded.map((entry) => {
    const mode = normalizeTeamWorkerCliMode(entry, OMQ_TEAM_WORKER_CLI_MAP_ENV);
...
export function translateWorkerLaunchArgsForCli(workerCli: TeamWorkerCli, args: string[], initialPrompt?: string): string[] {
  ...
  // All other launch args are dropped to avoid Qwen Code-only flags and model/config overrides.
  void args;
...
export function assertTeamWorkerCliBinaryAvailable(
  ...
  throw new Error(
    `Selected team worker CLI "${workerCli}" is not available on PATH. `
      + `Install "${workerCli}" or set ${OMQ_TEAM_WORKER_CLI_ENV}=qwen|claude|gemini.`,
  );
}
...
function buildModelInstructionsOverride(cwd: string, env: NodeJS.ProcessEnv): string {
  const filePath = translatePathForMsys(env[OMQ_MODEL_INSTRUCTIONS_FILE_ENV] || join(cwd, 'AGENTS.md'));
...
export function buildWorkerProcessLaunchSpec(
  ...
  const workerEnv: Record<string, string> = {
    OMQ_TEAM_WORKER: `${teamName}/worker-${workerIndex}`,
    [OMQ_LEADER_NODE_PATH_ENV]: resolveLeaderNodePath(),
    [OMQ_LEADER_CLI_PATH_ENV]: resolvedCliPath,
  };
 ...
 * OMQ requires tmux, which is unavailable on native Windows.
 */
...
export function createTeamSession(
  ...
  try {
    ...
    const omqEntry = process.argv[1];
    if (omqEntry && omqEntry.trim() !== '') {
      const hudCmd = `node ${shellQuoteSingle(translatePathForMsys(omqEntry))} hud --watch`;
    ...
    // history navigation in the Qwen Code input field. (issue #103)
    // Opt-out: set OMQ_TEAM_MOUSE=0 in the environment.
    if (process.env.OMQ_TEAM_MOUSE !== '0') {
...
export function restoreStandaloneHudPane(
  ...
  const omqEntry = process.argv[1];
  if (!omqEntry || omqEntry.trim() === '') return null;
 ...
 *
 * This helper is intentionally limited to session-scoped options so OMQ
...
function dismissClaudeBypassPermissionsPromptIfPresent(target: string, captured: string): boolean {
  if (process.env.OMQ_TEAM_AUTO_ACCEPT_BYPASS === '0') return false;
...
function resolveSendStrategyFromEnv(): 'auto' | 'queue' | 'interrupt' {
  const raw = String(process.env.OMQ_TEAM_SEND_STRATEGY || '')
    .trim()
    .toLowerCase();
...
function resolveWorkerCliFromMapForSend(
  ...
  try {
    const mode = normalizeTeamWorkerCliMode(selectedRaw, OMQ_TEAM_WORKER_CLI_MAP_ENV);
 ...
 * 2) per-worker OMQ_TEAM_WORKER_CLI_MAP entry (worker index aware)
 * 3) global/default OMQ_TEAM_WORKER_CLI behavior
 */
...
export function buildWorkerSubmitPlan(
  ...
  return {
    ...
    queueFirstRound: workerCli === 'qwen' && queueRequested,
    rounds: 6,
    submitKeyPressesPerRound: workerCli === 'claude' ? 1 : 2,
    allowAdaptiveRetry: workerCli === 'qwen' && allowAdaptiveRetry,
  };
}
...
// Poll tmux capture-pane for a worker-ready Qwen Code/Claude prompt or welcome screen.
...
export function waitForWorkerReady(
  ...
  const sendRobustEnter = (): void => {
    ...
    // Trust + follow-up splash can require two submits in Qwen Code TUI.
  ...
  const check = (): boolean => {
    ...
    if (paneHasTrustPrompt(result.stdout)) {
      ...
      // Opt-out by setting OMQ_TEAM_AUTO_TRUST=0.
      if (process.env.OMQ_TEAM_AUTO_TRUST !== '0') {
        sendRobustEnter();
        promptDismissed = true;
        return false;
      }
...
/**
 * Detect and auto-dismiss a Qwen Code "Trust this directory?" prompt in a worker pane.
 * Returns true if a trust prompt was found and dismissed, false otherwise.
 * Opt-out: set OMQ_TEAM_AUTO_TRUST=0 to disable auto-dismissal.
 */
export function dismissTrustPromptIfPresent(
  ...
  if (process.env.OMQ_TEAM_AUTO_TRUST === '0') return false;
...
export async function sendToWorker(
  ...
  const allowAutoInterruptRetry = process.env[OMQ_TEAM_AUTO_INTERRUPT_RETRY_ENV] !== '0';
  ...
  // - Qwen Code: queue-first Tab+C-m when configured/busy, then double C-m rounds.
  ...
  // Fail-open by default: Qwen Code may keep the last submitted line visible even after executing it.
  // If you need strictness for debugging, set OMQ_TEAM_STRICT_SUBMIT=1.
  const strict = process.env.OMQ_TEAM_STRICT_SUBMIT === '1';
  if (strict) {
...
// List all tmux sessions matching omq-team-* pattern
 ...
 * workers and runtime paths may message `leader-fixed` via `omq team api`
