...
import { omqStateDir } from '../utils/paths.js';
...
export interface TeamConfig {
  ...
  workers: WorkerInfo[];
  created_at: string;
  tmux_session: string; // "omq-team-{name}"
  next_task_id: number;
  leader_cwd?: string;
...
function resolveDisplayModeFromEnv(env: NodeJS.ProcessEnv): TeamPolicy['display_mode'] {
  const raw = readEnvValue(env, ['OMQ_TEAM_DISPLAY_MODE', 'OMQ_TEAM_MODE']);
  if (!raw) return 'auto';
...
function resolveWorkerLaunchModeFromEnv(env: NodeJS.ProcessEnv): TeamPolicy['worker_launch_mode'] {
  const raw = readEnvValue(env, ['OMQ_TEAM_WORKER_LAUNCH_MODE']);
  if (!raw || raw === 'interactive') return 'interactive';
  if (raw === 'prompt') return 'prompt';
  throw new Error(`Invalid OMQ_TEAM_WORKER_LAUNCH_MODE value "${raw}". Expected: interactive, prompt`);
}
...
function resolvePermissionsSnapshot(env: NodeJS.ProcessEnv): PermissionsSnapshot {
  ...
  const approvalMode = readEnvValue(env, [
    'OMQ_APPROVAL_MODE',
    'QWEN_APPROVAL_MODE',
    'QWEN_APPROVAL_POLICY',
    'CLAUDE_CODE_APPROVAL_MODE',
  ]);
  if (approvalMode) snapshot.approval_mode = approvalMode;

  const sandboxMode = readEnvValue(env, ['OMQ_SANDBOX_MODE', 'QWEN_SANDBOX_MODE', 'SANDBOX_MODE']);
  if (sandboxMode) snapshot.sandbox_mode = sandboxMode;

  const network = parseOptionalBoolean(readEnvValue(env, ['OMQ_NETWORK_ACCESS', 'QWEN_NETWORK_ACCESS', 'NETWORK_ACCESS']));
...
async function resolveLeaderSessionId(cwd: string, env: NodeJS.ProcessEnv): Promise<string> {
  const fromEnv = readEnvValue(env, ['OMQ_SESSION_ID', 'QWEN_SESSION_ID', 'SESSION_ID']);
  if (fromEnv) return fromEnv;

  const sessionPath = join(omqStateDir(cwd), 'session.json');
  try {
...
// Creates: .omq/state/team/{name}/, workers/{worker-1}..{worker-N}/, tasks/
...
export async function initTeamState(
  ...
  const leaderWorkerId = readEnvValue(env, ['OMQ_TEAM_WORKER']) ?? 'leader-fixed';
  ...
  const config: TeamConfig = {
    ...
    tmux_session: `omq-team-${teamName}`,
    next_task_id: 1,
