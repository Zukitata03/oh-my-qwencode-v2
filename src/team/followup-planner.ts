...
import { qwenPromptsDir, packageRoot } from '../utils/paths.js';
...
function buildLaunchHints(
  ...
  if (mode === 'team') {
    return {
      shellCommand: `omq team ${recommendedHeadcount}:${fallbackRole} ${toQuotedCliArg(task)}`,
  ...
  return {
    shellCommand: `omq ralph ${toQuotedCliArg(task)}`,
...
function buildVerificationPlan(
  ...
  if (mode === 'team') {
    ...
    return {
      ...
      checkpoints: [
        'Launch via `omq team ...` (or `$team ...`) so the team runtime owns both parallel delivery and coordinated verification.',
