...
import { qwenHome, listInstalledSkillDirectories } from "../utils/paths.js";
import { sleep } from "../utils/sleep.js";

const TEAM_OVERLAY_START = "<!-- OMQ:TEAM:WORKER:START -->";
const TEAM_OVERLAY_END = "<!-- OMQ:TEAM:WORKER:END -->";
const SKILL_REFERENCE_PATTERN = /\/skills\/([^/\s`]+)\/SKILL\.md\b/g;
const AGENTS_LOCK_PATH = [".omq", "state", "agents-md.lock"];
...
This file is generated for a live OMQ team worker run and is disposable.
...
2. Load the worker skill from the first existing path:
   - \`${"${QWEN_HOME:-~/.qwen}"}/skills/worker/SKILL.md\`
   - \`${options.leaderCwd}/.qwen/skills/worker/SKILL.md\`
   - \`${options.leaderCwd}/skills/worker/SKILL.md\`
3. Send startup ACK before task work:

   \`omq team api send-message --input "{\"team_name\":\"${options.teamName}\",\"from_worker\":\"${options.workerName}\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: ${options.workerName} initialized\"}" --json\`

4. Resolve canonical team state root in this order: \`OMQ_TEAM_STATE_ROOT\` env -> worker identity \`team_state_root\` -> config/manifest \`team_state_root\` -> local cwd fallback.
...
6. Use claim-safe lifecycle APIs only:
   ...
   - \`omq team api transition-task-status --json\`
   - \`omq team api release-task-claim --json\` only for rollback to pending
7. Use mailbox delivery flow:
   - \`omq team api mailbox-list --input "{\"team_name\":\"${options.teamName}\",\"worker\":\"${options.workerName}\"}" --json\`
   - \`omq team api mailbox-mark-delivered --input "{\"team_name\":\"${options.teamName}\",\"worker\":\"${options.workerName}\",\"message_id\":\"<MESSAGE_ID>\"}" --json\`
8. Preserve leader steering via inbox/mailbox nudges; task payload stays in inbox/task JSON, not this file.
9. Do not pass \`workingDirectory\` to legacy team_* MCP tools; use \`omq team api\` CLI interop.
...
2. Load the worker skill instructions from the first path that exists:
   - \`${"${QWEN_HOME:-~/.qwen}"}/skills/worker/SKILL.md\`
   - \`<leader_cwd>/.qwen/skills/worker/SKILL.md\`
   - \`<leader_cwd>/skills/worker/SKILL.md\` (repo fallback)
3. Send an ACK to the lead using CLI interop \`omq team api send-message --json\` (to_worker="leader-fixed") once initialized
4. Resolve canonical team state root in this order:
   - OMQ_TEAM_STATE_ROOT env
   - worker identity team_state_root
   - team config/manifest team_state_root
   - local cwd fallback (.omq/state)
...
7. Request a claim via CLI interop (\`omq team api claim-task --json\`); do not directly set lifecycle fields in the task file
...
10. On completion/failure, use lifecycle transition APIs:
   - \`omq team api transition-task-status --json\` with from \`"in_progress"\` to \`"completed"\` or \`"failed"\`
   - Include \`result\` (for completed) or \`error\` (for failed) in the transition patch
11. Use \`omq team api release-task-claim --json\` only for rollback/requeue to \`pending\` (not for completion)
...
15. For legacy team_* MCP tools (hard-deprecated), switch to \`omq team api\` CLI interop; do not pass workingDirectory unless the lead explicitly tells you to

## Message Protocol
When calling \`omq team api send-message\`, you MUST always include:
...
Keep the body short and deterministic so all worker CLIs (Qwen Code/Claude) behave consistently.

Example:
omq team api send-message --input "{\"team_name\":\"${teamName}\",\"from_worker\":\"<your-worker-name>\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: <your-worker-name> initialized\"}" --json
...
1. Read: \`omq team api mailbox-list --input "{\"team_name\":\"${teamName}\",\"worker\":\"<your-worker-name>\"}" --json\`
2. Mark delivered: \`omq team api mailbox-mark-delivered --input "{\"team_name\":\"${teamName}\",\"worker\":\"<your-worker-name>\",\"message_id\":\"<MESSAGE_ID>\"}" --json\`
...
- You may spawn Qwen Code native subagents when parallel execution improves throughput.
 ...
 * QWEN_HOME AGENTS.md, the project's AGENTS.md (if any), and the worker
...
export async function writeTeamWorkerInstructionsFile(
  ...
  const userAgentsPath = join(qwenHome(), "AGENTS.md");
 ...
 * This is written to .omq/state/team/{team}/workers/{worker}/inbox.md by the lead.
 */
...
1. Load and follow the worker skill from the first existing path:
   - \`${"${QWEN_HOME:-~/.qwen}"}/skills/worker/SKILL.md\`
   - \`${leaderCwd}/.qwen/skills/worker/SKILL.md\`
...
2. Send startup ACK to the lead mailbox BEFORE any task work (run this exact command):
   ...
   \`omq team api send-message --input "{\"team_name\":\"${teamName}\",\"from_worker\":\"${workerName}\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: ${workerName} initialized\"}" --json\`
...
7. Request a claim via CLI interop (\`omq team api claim-task --json\`) to claim it
8. Complete the work described in the task
9. After completing work, commit your changes before reporting completion:
   \`git add -A && git commit -m "task: <task-subject>"\`
   This ensures your changes are available for incremental integration into the leader branch.
10. Complete/fail it via lifecycle transition API (\`omq team api transition-task-status --json\`) from \`"in_progress"\` to \`"completed"\` or \`"failed"\` (include \`result\`/\`error\`)
11. Use \`omq team api release-task-claim --json\` only for rollback to \`pending\`
...
14. For legacy team_* MCP tools (hard-deprecated), use \`omq team api\`; do not pass \`workingDirectory\` unless the lead explicitly asks (if resolution fails, use leader cwd: \`${leaderCwd}\`)

## Mailbox Delivery Protocol (Required)
When you are notified about mailbox messages, always follow this exact flow:

1. List mailbox:
   \`omq team api mailbox-list --input "{\"team_name\":\"${teamName}\",\"worker\":\"${workerName}\"}" --json\`
2. For each undelivered message, mark delivery:
   \`omq team api mailbox-mark-delivered --input "{\"team_name\":\"${teamName}\",\"worker\":\"${workerName}\",\"message_id\":\"<MESSAGE_ID>\"}" --json\`

Use terse ACK bodies (single line) for consistent parsing across Qwen Code and Claude workers.
...
## Message Protocol
When using \`omq team api send-message\`, ALWAYS include from_worker with YOUR worker name:
- from_worker: "${workerName}"
- to_worker: "leader-fixed" (for leader) or "worker-N" (for peers)

Example: omq team api send-message --input "{\"team_name\":\"${teamName}\",\"from_worker\":\"${workerName}\",\"to_worker\":\"leader-fixed\",\"body\":\"ACK: initialized\"}" --json
...
3. Request a claim via CLI interop (\`omq team api claim-task --json\`)
4. Complete the work
5. After completing work, commit your changes before reporting completion:
   \`git add -A && git commit -m "task: <task-subject>"\`
   This ensures your changes are available for incremental integration into the leader branch.
6. Complete/fail via lifecycle transition API (\`omq team api transition-task-status --json\`) from \`"in_progress"\` to \`"completed"\` or \`"failed"\` (include \`result\`/\`error\`)
7. Use \`omq team api release-task-claim --json\` only for rollback to \`pending\`
...
3. After writing the ack, exit your Qwen Code session.

Type \`exit\` or press Ctrl+C to end your Qwen Code session.
`;
}
...
export function generateTriggerMessage(
  workerName: string,
  teamName: string,
  teamStateRoot: string = ".omq/state",
): string {
