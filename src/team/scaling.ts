  ...
  return await withScalingLock(sanitized, leaderCwd, async (): Promise<ScaleUpResult | ScaleError> => {
    ...
    const rollbackScaleUp = async (
      ...
      for (const taskId of createdTaskIds) {
        await rm(join(leaderCwd, '.omq', 'state', 'team', sanitized, 'tasks', `task-${taskId}.json`), { force: true }).catch(() => {});
      }
    ...
    for (let i = 0; i < count; i++) {
      ...
      const workerDirPath = join(leaderCwd, '.omq', 'state', 'team', sanitized, 'workers', workerName);
      ...
      if (uniqueTaskRoles.size > 1) {
        console.log(`[omq:scaling] ${workerName}: mixed task roles [${[...uniqueTaskRoles].join(', ')}], falling back to ${agentType}`);
      }
      ...
      const rolePromptContent = rawRolePromptContent
        ...
        : null;
      const teamInstructionsPath = join(leaderCwd, '.omq', 'state', 'team', sanitized, 'worker-agents.md');
      ...
      const extraEnv: Record<string, string> = {
        ...
        OMQ_MODEL_INSTRUCTIONS_FILE: instructionsFilePath,
      };
      if (workerWorkspace) {
        extraEnv.OMQ_TEAM_WORKTREE_PATH = workerWorkspace.worktreePath;
        if (workerWorkspace.branchName) {
          extraEnv.OMQ_TEAM_WORKTREE_BRANCH = workerWorkspace.branchName;
        }
        extraEnv.OMQ_TEAM_WORKTREE_DETACHED = workerWorkspace.detached ? '1' : '0';
      }
      ...
      if (!skipReadyWait) {
        ...
        if (!ready) {
          console.log(`[omq:scaling] Warning: worker ${workerName} did not become ready within timeout`);
        }
      }
