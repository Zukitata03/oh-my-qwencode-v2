...
describe('CI Rust gates', () => {
  ...
  it('uses the current crates/omq-sparkshell manifest for Rust coverage', () => {
    const workflowPath = join(process.cwd(), '.github', 'workflows', 'ci.yml');
    const workflow = readFileSync(workflowPath, 'utf-8');

    assert.match(workflow, /crates\/omq-sparkshell\/Cargo\.toml/);
    assert.doesNotMatch(workflow, /native\/omq-sparkshell\/Cargo\.toml/);
  });
  ...
  it('uses the current sparkshell crate manifest in the Rust coverage lane', () => {
    ...
    assert.match(workflow, /cargo llvm-cov --manifest-path crates\/omq-sparkshell\/Cargo\.toml --summary-only/);
    assert.match(workflow, /cargo llvm-cov --manifest-path crates\/omq-sparkshell\/Cargo\.toml --lcov --output-path coverage\/rust\/omq-sparkshell\.lcov/);
    assert.doesNotMatch(workflow, /native\/omq-sparkshell\/Cargo\.toml/);
  });
});
