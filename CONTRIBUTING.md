# Contributing to oh-my-qwencode
...
```bash
unset OMQ_TEAM_WORKER OMQ_TEAM_STATE_ROOT OMQ_TEAM_LEADER_CWD OMQ_TEAM_WORKER_CLI OMQ_TEAM_WORKER_CLI_MAP OMQ_TEAM_WORKER_LAUNCH_ARGS
```

## Project structure

- `src/` -- TypeScript source (CLI, config, agents, MCP servers, hooks, modes, team, verification)
- `prompts/` -- 30 agent prompt markdown files (installed to `~/.qwen/prompts/`)
- `skills/` -- 39 skill directories with `SKILL.md` (installed to `~/.qwen/skills/`)
...
2. Run `omq setup --force` to install it to `~/.qwen/prompts/`
3. Use `/prompts:my-agent` in Qwen Code
...
2. Run `omq setup --force` to install it to `~/.qwen/skills/`
3. Use `$my-skill` in Qwen Code
...
```text
docs: clarify setup steps for Qwen Code users
```
