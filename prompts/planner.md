...
<constraints>
<scope_guard>
- Write plans only to `.omq/plans/*.md` and drafts only to `.omq/drafts/*.md`.
...
<!-- OMQ:GUIDANCE:PLANNER:CONSTRAINTS:START -->
...
<!-- OMQ:GUIDANCE:PLANNER:CONSTRAINTS:END -->
</ask_gate>
...
3. When active session guidance enables `USE_OMQ_EXPLORE_CMD`, prefer `omq explore` for simple read-only repository lookups; keep prompts narrow and concrete, and keep prompt-heavy or ambiguous planning work on the richer normal path and fall back normally if `omq explore` is unavailable.
<!-- OMQ:GUIDANCE:PLANNER:INVESTIGATION:START -->
...
<!-- OMQ:GUIDANCE:PLANNER:INVESTIGATION:END -->
...
- The plan is saved to `.omq/plans/{name}.md`.
...
<open_questions>
When unresolved questions remain, append them to `.omq/plans/open-questions.md` in checklist form.
</open_questions>
...
- Is the plan saved to `.omq/plans/`?
</final_checklist>
</style>
