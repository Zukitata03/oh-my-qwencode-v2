...
pub const DEFAULT_SPARK_MODEL: &str = "qwen3.5-plus";
pub const DEFAULT_FRONTIER_MODEL: &str = "gpt-5.4";

pub fn resolve_model() -> String {
    env::var("OMQ_SPARKSHELL_MODEL")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .or_else(|| {
            env::var("OMQ_DEFAULT_SPARK_MODEL")
                .ok()
                .filter(|value| !value.trim().is_empty())
        })
        .or_else(|| {
            env::var("OMQ_SPARK_MODEL")
                .ok()
                .filter(|value| !value.trim().is_empty())
        })
...
pub fn resolve_fallback_model() -> String {
    env::var("OMQ_SPARKSHELL_FALLBACK_MODEL")
        .ok()
        .filter(|value| !value.trim().is_empty())
        .or_else(|| {
            env::var("OMQ_DEFAULT_FRONTIER_MODEL")
                .ok()
                .filter(|value| !value.trim().is_empty())
        })
...
pub fn read_summary_timeout_ms() -> u64 {
    env::var("OMQ_SPARKSHELL_SUMMARY_TIMEOUT_MS")
        .ok()
...
pub fn summarize_output(
    ...
    if !status_ok {
        ...
        if should_retry {
            ...
            if !fallback_ok {
                ...
                return Err(SparkshellError::SummaryBridge(format!(
                    "qwen exec failed for primary model `{model}` ({primary_message}) and fallback model `{fallback_model}` ({fallback_message})"
                )));
            }
            return normalize_summary(&fallback_stdout).ok_or_else(|| {
                SparkshellError::SummaryBridge(
                    "qwen exec fallback returned no valid summary sections".to_string(),
                )
            });
        }
        let message = if stderr.trim().is_empty() {
            ...
            format!("qwen exec exited unsuccessfully: {}", stderr.trim())
        };
        return Err(SparkshellError::SummaryBridge(message));
    }
    normalize_summary(&stdout).ok_or_else(|| {
        SparkshellError::SummaryBridge("qwen exec returned no valid summary sections".to_string())
    })
}
...
fn run_qwen_exec(
    ...
    let mut stdin = child
        .stdin
        .take()
        .ok_or_else(|| SparkshellError::SummaryBridge("failed to open qwen stdin".to_string()))?;
    let mut stdout = child
        .stdout
        .take()
        .ok_or_else(|| SparkshellError::SummaryBridge("failed to open qwen stdout".to_string()))?;
    let mut stderr = child
        .stderr
        .take()
        .ok_or_else(|| SparkshellError::SummaryBridge("failed to open qwen stderr".to_string()))?;
    ...
    let stdout_bytes = stdout_reader
        .join()
        .map_err(|_| SparkshellError::SummaryBridge("failed reading qwen stdout".to_string()))?;
    let stderr_bytes = stderr_reader
        .join()
        .map_err(|_| SparkshellError::SummaryBridge("failed reading qwen stderr".to_string()))?;
...
mod tests {
    ...
    fn model_resolution_prefers_sparkshell_override() {
        let _guard = env_lock();
        unsafe {
            env::set_var("OMQ_SPARKSHELL_MODEL", "spark-a");
            env::set_var("OMQ_DEFAULT_SPARK_MODEL", "spark-b");
        }
        assert_eq!(resolve_model(), "spark-a");
        unsafe {
            env::remove_var("OMQ_SPARKSHELL_MODEL");
            env::remove_var("OMQ_DEFAULT_SPARK_MODEL");
            env::remove_var("OMQ_SPARK_MODEL");
        }
    }
    ...
    fn fallback_model_resolution_prefers_override_then_default_frontier() {
        let _guard = env_lock();
        unsafe {
            env::remove_var("OMQ_SPARKSHELL_FALLBACK_MODEL");
            env::remove_var("OMQ_DEFAULT_FRONTIER_MODEL");
        }
        assert_eq!(resolve_fallback_model(), DEFAULT_FRONTIER_MODEL);

        unsafe {
            env::set_var("OMQ_DEFAULT_FRONTIER_MODEL", "frontier-a");
        }
        assert_eq!(resolve_fallback_model(), "frontier-a");

        unsafe {
            env::set_var("OMQ_SPARKSHELL_FALLBACK_MODEL", "frontier-b");
        }
    ...
    fn timeout_defaults_when_unset() {
        let _guard = env_lock();
        unsafe {
            env::remove_var("OMQ_SPARKSHELL_SUMMARY_TIMEOUT_MS");
        }
    ...
    fn model_resolution_ignores_blank_override_and_uses_secondary_env() {
        let _guard = env_lock();
        unsafe {
            env::set_var("OMQ_SPARKSHELL_MODEL", "   ");
            env::set_var("OMQ_DEFAULT_SPARK_MODEL", "spark-b");
        }
    ...
    fn timeout_defaults_for_zero_and_invalid_values() {
        let _guard = env_lock();
        unsafe {
            env::set_var("OMQ_SPARKSHELL_SUMMARY_TIMEOUT_MS", "0");
        }
        assert_eq!(read_summary_timeout_ms(), DEFAULT_SUMMARY_TIMEOUT_MS);

        unsafe {
            env::set_var("OMQ_SPARKSHELL_SUMMARY_TIMEOUT_MS", "bogus");
        }
        assert_eq!(read_summary_timeout_ms(), DEFAULT_SUMMARY_TIMEOUT_MS);
