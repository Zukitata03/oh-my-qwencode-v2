...
fn sparkshell_bin() -> &'static str {
    env!("CARGO_BIN_EXE_omq-sparkshell")
}
...
#[test]
fn summary_mode_uses_qwen_exec_and_model_override() {
    let temp = unique_temp_dir("qwen-success");
    ...
    let output = Command::new(sparkshell_bin())
        .env("PATH", path)
        .env("OMQ_SPARKSHELL_LINES", "1")
        .env("OMQ_SPARKSHELL_MODEL", "spark-test-model")
        .arg("sh")
        .arg("-c")
        .arg("printf 'one\ntwo\n'")
        .output()
...
fn summary_failure_falls_back_to_raw_output_with_notice() {
    let temp = unique_temp_dir("qwen-fail");
    let qwen = temp.join("qwen");
    write_executable(
        &qwen,
        "#!/bin/sh\nprintf '%s\n' 'bridge failed' >&2\nexit 9\n",
...
fn summary_mode_retries_with_fallback_model_when_spark_is_unavailable() {
    let temp = unique_temp_dir("qwen-fallback-model");
    let qwen = temp.join("qwen");
    let args_log = temp.join("args.log");
    write_executable(
        &qwen,
...
",
    ...
    let output = Command::new(sparkshell_bin())
        ...
        .env("OMQ_SPARKSHELL_LINES", "1")
        .env("OMQ_SPARKSHELL_MODEL", "spark-test-model")
        .env("OMQ_SPARKSHELL_FALLBACK_MODEL", "frontier-test-model")
        .arg("sh")
        .arg("-c")
        .arg("printf 'one\ntwo\n'")
        .output()
...
fn summary_mode_reports_both_models_when_fallback_also_fails() {
    let temp = unique_temp_dir("qwen-fallback-model-fail");
    let qwen = temp.join("qwen");
    write_executable(
        &qwen,
        "#!/bin/sh
...
fn summary_mode_preserves_child_exit_code() {
    let temp = unique_temp_dir("qwen-exit");
    let qwen = temp.join("qwen");
    write_executable(
        &qwen,
...
fn raw_mode_keeps_boundary_output_without_summary() {
    ...
    let qwen = temp.join("qwen");
    let qwen_log = temp.join("qwen.log");
    write_executable(
        &qwen,
        &format!(
            "#!/bin/sh
...
"
    ...
    assert!(
        !qwen_log.exists(),
        "qwen should not run at the raw/summary boundary"
    );
...
#[test]
fn summary_failure_when_qwen_is_missing_falls_back_to_raw_output() {
    let empty_path = unique_temp_dir("missing-qwen");
    let output = Command::new(sparkshell_bin())
