...
mod tests {
    ...
    fn persist_and_load_round_trip() {
        let dir = std::env::temp_dir().join("omq-runtime-test-persist");
    ...
    fn compatibility_view_writes_section_files() {
        let dir = std::env::temp_dir().join("omq-runtime-test-compat");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn queue_dispatch_with_metadata_persists_and_round_trips() {
        let dir = std::env::temp_dir().join("omq-runtime-test-metadata");
        let _ = std::fs::remove_dir_all(&dir);

        let mut engine = RuntimeEngine::new().with_state_dir(&dir);
        let meta = serde_json::json!({"priority": "high", "worker_type": "qwen"});
        engine
            .process(RuntimeCommand::QueueDispatch {
