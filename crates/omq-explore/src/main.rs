...
const QWEN_BIN_ENV: &str = "OMQ_EXPLORE_QWEN_BIN";
const HARNESS_ROOT_ENV: &str = "OMQ_EXPLORE_ROOT";
    ...
    I: Iterator<Item = OsString>,
{
    ...
    let spark_attempt = invoke_qwen(&args, &args.spark_model, &prompt_contract)
    ...
    eprintln!(
        "[omq explore] spark model `{}` unavailable or failed (exit {}). Falling back to `{}`.",
        args.spark_model, spark_attempt.status_code, args.fallback_model
    );
    if !spark_attempt.stderr.trim().is_empty() {
        eprintln!(
            "[omq explore] spark stderr: {}",
            spark_attempt.stderr.trim()
        );
    }

    let fallback_attempt = invoke_qwen(&args, &args.fallback_model, &prompt_contract)
...
fn print_attempt_output(attempt: AttemptResult) -> Result<(), String> {
    ...
    Err(
        "qwen completed successfully but did not produce the expected markdown output artifact"
            .to_string(),
    )
}
...
fn usage() -> &'static str {
    "Usage: omq-explore --cwd <dir> --prompt <text> --prompt-file <explore-prompt.md> --model-spark <model> --model-fallback <model>"
}

fn invoke_qwen(args: &Args, model: &str, prompt_contract: &str) -> io::Result<AttemptResult> {
    let qwen_launch = resolve_qwen_launch();
    ...
    let mut command = Command::new(&qwen_launch.program);
    command.args(&qwen_launch.leading_args);
    command
        .arg("exec")
        .arg("-C")
        .arg(&args.cwd)
        .args(qwen_support_dir_args())
        .arg("-m")
        .arg(model)
        .arg("-s")
        .arg("read-only")
...
fn resolve_qwen_launch() -> QwenLaunch {
    let qwen_binary = resolve_qwen_binary();
    qwen_launch_for_binary(&qwen_binary).unwrap_or_else(|| QwenLaunch {
        program: qwen_binary,
        leading_args: Vec::new(),
    })
}

fn resolve_qwen_binary() -> String {
    if let Some(value) = env::var(QWEN_BIN_ENV)
        .ok()
    ...
    resolve_host_command("qwen")
        .map(|path| path.display().to_string())
        .unwrap_or_else(|| "qwen".to_string())
}

fn qwen_launch_for_binary(qwen_binary: &str) -> Option<QwenLaunch> {
    let interpreter = read_shebang_interpreter(Path::new(qwen_binary))?;
    let (program, mut leading_args) = resolve_shebang_launch(&interpreter)?;
    leading_args.push(qwen_binary.to_string());
    Some(QwenLaunch {
        program,
        leading_args,
    })
...
fn qwen_support_dir_args() -> Vec<String> {
    discover_qwen_support_dirs()
        .into_iter()
        .flat_map(|dir| ["--add-dir".to_string(), dir.display().to_string()])
        .collect()
}

fn discover_qwen_support_dirs() -> Vec<PathBuf> {
    let mut dirs = Vec::new();
    if let Some(home) = env::var_os("HOME").filter(|value| !value.is_empty()) {
        let home = PathBuf::from(home);
        for relative in [".omq", ".qwen"] {
...
fn temp_output_path() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    env::temp_dir().join(format!("omq-explore-{}-{}.md", std::process::id(), nanos))
}

fn compose_exec_prompt(user_prompt: &str, prompt_contract: &str) -> String {
    format!(
        concat!(
            "You are OMQ Explore, a low-cost read-only repository exploration harness.\\n",
...
fn build_direct_wrapper(self_exe: &Path, command: &str) -> Result<String, String> {
    ...
    Ok(format!(
        "#!/bin/sh\nprintf '%s\\n' {} >&2\nexit 127\n",
        shell_quote(&format!(
            "omq explore allowlisted host command `{command}` is unavailable on this host"
        )),
    ))
}
...
fn validate_direct_command(command_name: &str, args: &[String]) -> Result<(), String> {
    if !ALLOWED_DIRECT_COMMANDS.contains(&command_name) {
        return Err(format!(
            "command `{command_name}` is not on the omq explore allowlist"
        ));
    }
    ...
    match command_name {
        "rg" => {
            ...
            {
                return Err("ripgrep `--pre` is not allowed in omq explore".to_string());
            }
            if args.iter().any(|arg| arg == "-") {
                return Err("ripgrep stdin (`-`) is not allowed in omq explore".to_string());
            }
        }
        "grep" => {
            if args.iter().any(|arg| arg == "-") {
                return Err("grep stdin (`-`) is not allowed in omq explore".to_string());
            }
            if non_option_operands(args).len() < 2 {
                return Err(
                    "grep requires a pattern and at least one file/path in omq explore".to_string(),
                );
            }
        }
        "find" => {
            if args.iter().any(|arg| {
                ...
                return Err(
                    "find actions that execute, delete, or write files are not allowed in omq explore"
                        .to_string(),
                );
            }
        }
        "cat" => {
            ...
            if operands.is_empty() {
                return Err("cat requires at least one file/path in omq explore".to_string());
            }
            if operands.contains(&"-") {
                return Err("cat stdin (`-`) is not allowed in omq explore".to_string());
            }
        }
        "head" | "wc" => {
            let operands = non_option_operands(args);
            if operands.is_empty() {
                return Err(format!(
                    "{command_name} requires at least one file/path in omq explore"
                ));
            }
            if operands.contains(&"-") {
                return Err(format!(
                    "{command_name} stdin (`-`) is not allowed in omq explore"
                ));
            }
        }
        "tail" => {
            let operands = non_option_operands(args);
            if operands.is_empty() {
                return Err("tail requires at least one file/path in omq explore".to_string());
            }
            if operands.contains(&"-") {
                return Err("tail stdin (`-`) is not allowed in omq explore".to_string());
            }
            if args.iter().any(|arg| {
                matches!(arg.as_str(), "-f" | "-F" | "--retry") || arg.starts_with("--follow")
            }) {
                return Err("tail follow/retry modes are not allowed in omq explore".to_string());
            }
        }
        _ => {}
    }
...
fn validate_repo_paths(command_name: &str, args: &[String]) -> Result<(), String> {
    ...
    for operand in candidate_paths {
        ...
        if !normalized.starts_with(&repo_root) {
            return Err(format!(
                "path `{operand}` escapes the omq explore repository root {}",
                repo_root.display()
            ));
        }
        if let Some(canonical_candidate) = canonicalize_existing_prefix(&normalized) {
            if let Some(canonical_repo_root) = &canonical_repo_root {
                if !canonical_candidate.starts_with(canonical_repo_root) {
                    return Err(format!(
                        "path `{operand}` resolves outside the omq explore repository root {}",
...
mod tests {
    ...
    fn parse_args_accepts_full_contract() {
        ...
        assert_eq!(args.spark_model, "qwen3.5-plus");
    ...
    #[test]
    fn resolve_qwen_binary_prefers_env_override() {
        let _guard = env_lock();
        unsafe {
            env::set_var(QWEN_BIN_ENV, "/tmp/qwen-stub");
        }
        assert_eq!(resolve_qwen_binary(), "/tmp/qwen-stub");
        unsafe {
            env::remove_var(QWEN_BIN_ENV);
        }
    }

    #[test]
    fn resolve_qwen_binary_resolves_bare_env_override_from_path() {
        let _guard = env_lock();
        let root = temp_allowlist_dir().expect("temp root");
        let bin_dir = root.path.join("bin");
        create_dir_all(&bin_dir).expect("create bin");
        let fake_qwen = bin_dir.join("qwen-custom");
        write(&fake_qwen, b"#!/bin/sh\nexit 0\n").expect("write fake qwen");
        write_executable(&fake_qwen, "#!/bin/sh\nexit 0\n").expect("chmod fake qwen");

        let original_path = env::var_os("PATH");
        unsafe {
            env::set_var(QWEN_BIN_ENV, "qwen-custom");
            env::set_var("PATH", &bin_dir);
        }

        let resolved = resolve_qwen_binary();
        ...
        assert_eq!(resolved, fake_qwen.display().to_string());
    }

    #[test]
    fn qwen_launch_for_env_node_shebang_uses_host_node_absolute_path() {
        let root = temp_allowlist_dir().expect("temp root");
        let script_path = root.path.join("qwen-script");
        write(&script_path, b"#!/usr/bin/env node\nconsole.log(\"ok\");\n").expect("write script");

        let launch = qwen_launch_for_binary(script_path.to_str().expect("script path"))
    ...
    #[test]
    fn discover_qwen_support_dirs_includes_home_omq_and_qwen_when_present() {
        let _guard = env_lock();
        let root = temp_allowlist_dir().expect("temp root");
        let home_dir = root.path.join("home");
        create_dir_all(home_dir.join(".omq")).expect("create .omq");
        create_dir_all(home_dir.join(".qwen")).expect("create .qwen");
        let original_home = env::var_os("HOME");
        unsafe {
            env::set_var("HOME", &home_dir);
        }

        let dirs = discover_qwen_support_dirs();

        match original_home {
            Some(value) => unsafe { env::set_var("HOME", value) },
            None => unsafe { env::remove_var("HOME") },
        }
        assert_eq!(dirs, vec![home_dir.join(".omq"), home_dir.join(".qwen")]);
    }
    ...
    fn run_with_args_reports_both_failed_attempts_with_codes_and_stderr() {
        ...
        let fake_qwen = root.path.join("qwen-stub");
        write_executable(
            &fake_qwen,
            r#"#!/bin/sh
model=""
...
"#,
        ...
        unsafe {
            env::set_var(QWEN_BIN_ENV, &fake_qwen);
        }
