...
Status: contributor-facing contract for OMQ prompt and orchestration surfaces.
...
- generated top-level Qwen Code config guidance: `src/config/generator.ts`
...
OMQ also has a narrow **instruction-composition seam** for subagents/workers whose **final resolved model** is exactly `gpt-5.4-mini`.
...
This contract is about **how OMQ prompts should behave**.
It is not the same thing as OMQ's routing metadata.
...
## The 4 core GPT-5.4 patterns OMQ currently enforces
...
Contributors should preserve the rule that prompts keep using tools when correctness depends on retrieval, diagnostics, tests, or verification. OMQ should not stop at a plausible answer if proof is still missing.
...
OMQ also uses **scenario-style examples** to make the contract concrete for "continue", "make a PR", and "merge if CI green" flows.
