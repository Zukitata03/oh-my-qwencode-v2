...
- Prevents non-Gemini default models such as `qwen3.5-plus` from being passed through to Gemini workers unless the configured model is explicitly a Gemini model (`#585`).
...
Gemini workers launched through OMQ team prompt mode are now started with an explicit initial instruction:
