...
3. OMQ emits a warning for clarity
    ...
    "openclaw": {
      ...
      "hooks": {
        "session-end": {
          "enabled": true,
          "gateway": "local",
          "instruction": "OMQ task completed for {{projectPath}}"
        },
        "ask-user-question": {
          "enabled": true,
          "gateway": "local",
          "instruction": "OMQ needs input: {{question}}"
        }
      }
    }
  }
...
```json
{
  "notifications": {
    ...
    "custom_webhook_command": {
      ...
      "instruction": "OMQ event {{event}} for {{projectPath}}"
    },
    "custom_cli_command": {
      "enabled": true,
...
These aliases are normalized by OMQ into internal OpenClaw gateway mappings.

## Option C: Clawdbot agent-command workflow (recommended for dev)

Use this when you want OMQ hook events to trigger **agent turns** (not plain
...
>
> Command gateway timeout precedence: `gateways.<name>.timeout` > `OMQ_OPENCLAW_COMMAND_TIMEOUT_MS` > `5000`.
> For `clawdbot agent` workflows, use `120000` (2 minutes) to avoid premature timeout.
>
> **Production best practices:**
> - Use `|| true` at the end of the command to prevent OMQ hook failures from blocking sessions
  ...
  "notifications": {
    ...
    "openclaw": {
      ...
      "gateways": {
        "local": {
          "type": "command",
          "command": "(clawdbot agent --session-id omq-hooks --message {{instruction}} --thinking minimal --deliver --reply-channel discord --reply-to 'channel:1468539002985644084' --timeout 120 --json >>/tmp/omq-openclaw-agent.jsonl 2>&1 || true)",
          "timeout": 120000
        }
      },
      "hooks": {
...
```text
OMQ 훅={{event}} 프로젝트={{projectName}} 세션={{sessionId}}.
반드시 한국어로 응답하세요.
OMQ tmux 세션: {{tmuxSession}}.
SOUL.md 및 #omc-dev 맥락을 참고해 필요한 후속 액션이 있으면 즉시 안내하세요.
```

### 2) Track which OMQ tmux session emitted the hook
...
clawdbot agent --session-id omq-hooks \
  --message "OMQ hook retry 점검: session={{sessionId}} tmux={{tmuxSession}}" \
...
curl -sS -X POST http://127.0.0.1:18789/hooks/wake \
  ...
  -d '{"text":"OMQ wake smoke test","mode":"now"}'
```
...
```bash
curl -sS -o /tmp/omq-openclaw-agent-check.json -w "HTTP %{http_code}\n" \
  -X POST http://127.0.0.1:18789/hooks/agent \
  -H "Authorization: Bearer ${HOOKS_TOKEN}" \
  -H "Content-Type: application/json" \
  -d '{"message":"OMQ delivery verification","instruction":"OMQ delivery verification","event":"session-end","sessionId":"manual-check"}'
```
...
# gate checks
test "$OMQ_OPENCLAW" = "1" && echo "OMQ_OPENCLAW=1" || echo "missing OMQ_OPENCLAW=1"
test "$OMQ_OPENCLAW_COMMAND" = "1" && echo "OMQ_OPENCLAW_COMMAND=1" || echo "missing OMQ_OPENCLAW_COMMAND=1"
```
...
- **Command gateway disabled**: set both `OMQ_OPENCLAW=1` and `OMQ_OPENCLAW_COMMAND=1`.
- **Command killed by `SIGTERM`**: increase `gateways.<name>.timeout` (recommend `120000` for clawdbot agent) or set `OMQ_OPENCLAW_COMMAND_TIMEOUT_MS`.
- **Hook failures blocking sessions**: ensure command ends with `|| true` to prevent OMQ from waiting on clawdbot failures.
