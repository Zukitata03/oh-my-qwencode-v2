...
- use `omq setup` and need predictable install scope behavior,
- run `omq team`/tmux workflows and want the latest reliability fixes,
...
- `omq setup` now supports scope-aware install modes (`user`, `project`). Legacy `project-local` values are auto-migrated.
...
| `$deepinit` | `omq agents-init [path]` | Lightweight CLI successor for AGENTS.md bootstrap only; immediate child directories only, unmanaged files preserved unless `--force`. |
| `$learn-about-omq` / `$learner` / `$writer-memory` | No in-repo replacement | Remove stale references from workflows/docs. |
...
- [ ] Confirm removed references are gone from local notes/scripts:
  ```bash
  rg -n "deep-executor|scientist|pipeline|project-session-manager|\bpsm\b|ultrapilot|learn-about-omq|writer-memory|learner|deepinit|\brelease\b" README.md docs scripts .omq -S
  ```
...
- [ ] If using spark worker routing, verify flags are available:
  ```bash
  omq --help | rg "spark|madmax-spark"
  ```
