...
4. **Lifecycle/process handling**
   - `bin/omq.js` now awaits `main(...)` and exits explicitly.
...
- Set `OMQ_TEAM_MOUSE=0`, restart session, verify mouse mode is not forcibly enabled.
