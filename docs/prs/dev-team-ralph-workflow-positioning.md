...
## Summary
This PR documents one of OMQ's strongest high-control workflows: `ralplan -> team -> ralph`. The goal is to make the README explain not only that team mode exists, but why it matters even when `$ultrawork` already provides parallel execution.
...
## Expected outcomes
- Better onboarding for users evaluating OMQ's orchestration model
