# PR Draft: Rebase `feat/omq-sparkshell` onto `experimental/dev`

## Target branch
`experimental/dev`

## Summary
This PR rebases the `omq sparkshell` feature line onto `experimental/dev` and preserves the follow-on team-status inspection work built on top of it.

It adds the native Rust-backed `omq sparkshell <command> [args...]` flow, plus a long sequence of `omq team status` improvements that surface pane-aware inspection metadata and ready-to-run sparkshell commands for leader triage.

## What’s included
### `omq sparkshell`
- adds CLI dispatch/help for `omq sparkshell`
- adds `src/cli/sparkshell.ts` for native binary discovery and launch
- adds the Rust crate under `native/omq-sparkshell/`
- adds packaging/build helpers:
  - `scripts/build-sparkshell.mjs`
  - `scripts/test-sparkshell.mjs`
- stages the packaged native binary under `bin/native/linux-x64/omq-sparkshell`
- adds focused CLI + packaging tests

### `omq team status` inspection improvements
...
- excluded `native/omq-sparkshell` from the root Cargo workspace so manifest-path cargo commands work again
...
- `npm run build:full`
- `cargo test --manifest-path native/omq-sparkshell/Cargo.toml`
- `node scripts/build-sparkshell.mjs`
- `node scripts/test-sparkshell.mjs`
- `node bin/omq.js sparkshell cargo --version`
- `node bin/omq.js sparkshell npm --version`
- `node bin/omq.js sparkshell git log --oneline -3`
- `npm test`
...
- Native Rust build artifacts under `native/omq-sparkshell/target/` were removed after verification.
