...
After `omq setup`, native agent configs in `~/.omq/agents/` include new sections:
`## OMQ Posture Overlay`, `## Model-Class Guidance`, and `## OMQ Agent Metadata`.
...
`bin/omq.js` failed on Windows with `ERR_UNSUPPORTED_ESM_URL_SCHEME` because `import()` received a bare absolute path (`C:\...`) instead of a `file://` URL.
...
15 prompt files and the runtime native-config generator still referenced `qwen3.5-plus` and `o3` model aliases that were removed from the config layer in v0.8.2. With posture routing active, these stale references could confuse tier/model-class guidance.
...
2f4862a docs(omq): remove remaining legacy model alias references
0d2115c fix(omq): remove legacy model aliases from prompts and runtime metadata
8fb3aa0 fix(bin): use file:// URL for dynamic import on Windows
f448108 feat(omq): add posture-aware agent routing metadata
```
