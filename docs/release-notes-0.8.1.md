...
If you use `$team`/`omq team` heavily, this release makes the runtime more consistent by standardizing around **CLI-first interop**.
...
- For notification setup, prefer:
  - `omq configure-notifications` (or skill equivalent)
...
Thanks for using **oh-my-qwencode**. If anything regresses, please open an issue with repro steps and logs.
