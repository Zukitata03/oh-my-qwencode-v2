# `omq autoresearch` Full-Parity Review Notes
...
- `.omq/plans/prd-autoresearch-full-parity.md`
- `.omq/plans/test-spec-autoresearch-full-parity.md`
...
- `omq autoresearch` exposes both fresh launch and `--resume <run-id>` flows.
...
**Pass.**
- `buildAutoresearchInstructions()` tells the launched Qwen Code session to perform exactly one experiment cycle and write the candidate artifact before exit.
- `runAutoresearchLoop()` relaunches Qwen Code after `processAutoresearchCandidate()` unless the run aborts/errors.

### 2. Repo-root state authority
**Pass.**
- repo-root active-run lock lives at `.omq/state/autoresearch-state.json`.
- per-run manifest, candidate, ledger, and latest evaluator files live under `.omq/logs/autoresearch/<run-id>/`.
