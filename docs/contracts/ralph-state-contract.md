...
Ralph runtime state is stored at `.omq/state/{scope}/ralph-state.json` and MUST use this schema:
...
explicit follow-up, but there is no built-in `omq team ralph ...` linked launch
path anymore.
...
1. If `session_id` is present (explicit argument or current `.omq/state/session.json`), session scope (`.omq/state/sessions/{session_id}/...`) is authoritative.
2. Root scope (`.omq/state/*.json`) is compatibility fallback only.
...
- Canonical PRD: `.omq/plans/prd-{slug}.md`
- Canonical progress ledger: `.omq/state/{scope}/ralph-progress.json`
- Legacy compatibility migration:
  - `.omq/prd.json` migrates one-way to canonical PRD markdown when no canonical PRD exists.
  - `.omq/progress.txt` migrates one-way to canonical `ralph-progress.json` when no canonical ledger exists.
