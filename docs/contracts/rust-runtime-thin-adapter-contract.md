...
|---|---|---|
| `omq team status` | `.omq/state/team/<team>/config.json`, `manifest.v2.json`, `tasks/*.json`, `approvals/*.json`, `workers/*` | Manifest-backed team config is authoritative when both config and manifest exist. |
| `omq doctor --team` | `.omq/state/team/<team>/config.json`, `manifest.v2.json`, `workers/*/status.json`, `workers/*/heartbeat.json`, `.omq/state/hud-state.json` | Manifest-backed tmux/session identity is authoritative when both config and manifest exist. |
| HUD readers | `.omq/state/session.json`, `.omq/state/sessions/<session>/team-state.json`, `.omq/state/team-state.json`, `.omq/state/ralph-state.json` | Session-scoped files are authoritative when a session is active; root files are compatibility fallback only. |

## Rust-authored runtime files

The `RuntimeEngine` (in `crates/omq-runtime-core/src/engine.rs`) writes the following files via `persist()` and `write_compatibility_view()`:
