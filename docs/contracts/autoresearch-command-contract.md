# `omq autoresearch` parity contract

`omq autoresearch` is a thin supervisor that drives one Qwen Code experiment session per iteration while OMQ owns the durable keep/discard/reset loop.

## CLI

```bash
omq autoresearch <mission-dir> [qwen-args...]
omq autoresearch --resume <run-id> [qwen-args...]
omq autoresearch --help
```

- Fresh launch always creates a new run-tagged lane.
- `--resume <run-id>` loads `.omq/logs/autoresearch/<run-id>/manifest.json`.
- A second launch is rejected while repo-root `.omq/state/autoresearch-state.json` points at an active run.
...
- worktree `<repo>.omq-worktrees/autoresearch-<mission-slug>-<run-tag>`
- repo-root run artifacts under `.omq/logs/autoresearch/<run-id>/`

Repo-root state responsibilities:
- `.omq/state/autoresearch-state.json` = active-run pointer/lock only
- `.omq/logs/autoresearch/<run-id>/manifest.json` = authoritative per-run state
- `.omq/logs/autoresearch/<run-id>/candidate.json` = candidate handoff from the just-finished Qwen Code session
- `.omq/logs/autoresearch/<run-id>/iteration-ledger.json` = durable iteration history
- `.omq/logs/autoresearch/<run-id>/latest-evaluator-result.json` = latest evaluator output
