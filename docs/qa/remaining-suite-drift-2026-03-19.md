...
Execution surface: active OMQ team worker pane (`worker-3`) with local verification run from repository root after clearing `OMQ_TEAM_*` env vars.
...
```bash
unset OMQ_TEAM_STATE_ROOT OMQ_TEAM_WORKER OMQ_TEAM_LEADER_CWD
npm run build
npm run lint
npm test
...
- `runs qwen exec with session-scoped instructions that preserve AGENTS and overlay content`

Observed mismatch:

- expected `instructions-path: .../.omq/state/sessions/omq-*/AGENTS.md`
- actual `instructions-path: .../.omq/team/continue-from-clean-commit-810/worktrees/worker-3/AGENTS.md`

Interpretation:

- The test expects the `omq exec` path to always use a generated session-scoped overlay file.
...
```text
expected: /instructions-path:.*\/\.omq\/state\/sessions\/omq-.*\/AGENTS\.md/
actual: fake-qwen:exec --model gpt-5 say hi -c model_instructions_file="/home/.../.omq/team/continue-from-clean-commit-810/worktrees/worker-3/AGENTS.md"
```
...
- The full-suite evidence for this snapshot is stored in `.omq/context/task-3-npm-test-20260319.log`.
- Focused repro logs were stored in:
  - `.omq/context/task-3-exec-test.log`
  - `.omq/context/task-3-codebase-map-test.log`
