# Heavy / Manual Stress Lane: `omq-explore` and `omq-sparkshell`
...
| Command | Result |
|---|---|
| `node bin/omq.js explore --help` | PASS |
| `node bin/omq.js sparkshell --help` | PASS |
| `node bin/omq.js sparkshell git --version` | PASS |
| `node bin/omq.js version` | PASS |
| `for i in $(seq 1 20); do node bin/omq.js sparkshell git --version; done` | PASS (bounded sample verified locally) |
| `seq 1 5 | xargs -I{} -P5 node bin/omq.js sparkshell git --version` | PASS (bounded sample verified locally) |
...
**Goal**
- Confirm `omq-sparkshell --tmux-pane` preserves operator-critical facts when pane tails are large and noisy.
...
```bash
node bin/omq.js sparkshell git --version
```

Suggested repeated-run shape:

```bash
for i in $(seq 1 20); do node bin/omq.js sparkshell git --version; done
```

Suggested concurrent-run shape:

```bash
seq 1 5 | xargs -I{} -P5 node bin/omq.js sparkshell git --version
```
...
**Goal**
- Stress `omq-explore` and `omq-sparkshell` summarization with outputs that bury signal under distractors.
...
```bash
node bin/omq.js sparkshell --help
node bin/omq.js explore --help
```
