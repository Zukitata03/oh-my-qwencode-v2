...
| CLI help smoke | `node bin/omq.js --help` | PASS |
| Version smoke | `node bin/omq.js version` | PASS (`oh-my-qwencode v0.9.0`) |
| Version sync | `node scripts/check-version-sync.mjs --tag v0.9.0` | PASS |
| Ask help smoke | `node bin/omq.js ask --help` | PASS |
| HUD help smoke | `node bin/omq.js hud --help` | PASS |
| Doctor smoke | `node bin/omq.js doctor` | PASS (`10 passed, 0 warnings, 0 failed`) |
| Status smoke | `node bin/omq.js status` | PASS |
| Setup dry-run smoke | `node bin/omq.js setup --dry-run` | PASS |
| Explore help smoke | `node bin/omq.js explore --help` | PASS |
| Explore prompt-file smoke | `node bin/omq.js explore --prompt-file /tmp/omq-explore-smoke.txt` | PASS |
| Explore→sparkshell routing smoke | `OMQ_SPARKSHELL_LINES=1 node bin/omq.js explore --prompt 'git log --oneline -10'` | PASS (summary output emitted) |
| Sparkshell help smoke | `node bin/omq.js sparkshell --help` | PASS |
| Sparkshell direct smoke | `node bin/omq.js sparkshell git --version` | PASS (`git version 2.34.1`) |
| Sparkshell summary smoke | `OMQ_SPARKSHELL_LINES=1 node bin/omq.js sparkshell git log --oneline -10` | PASS (summary output emitted) |
| Sparkshell tmux-pane smoke | `node bin/omq.js sparkshell --tmux-pane %2141 --tail-lines 120` | PASS |
| Full test suite | `npm test` | PASS (`2375` pass / `0` fail) |
| Packed tarball dry run | `npm pack --dry-run` | PASS (`oh-my-qwencode-0.9.0.tgz`) |
...
- `omq explore` is intentionally constrained; release validation should keep checking that shell-only/read-only boundaries stay intact while sparkshell routing is enabled.
- `omq sparkshell --tmux-pane` is operator-critical for team debugging, so pane summarization behavior should be treated as a release-facing feature, not a hidden internal detail.
