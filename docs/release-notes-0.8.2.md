...
- `$team` / team runtime can now launch **Gemini CLI workers** alongside Qwen Code and Claude (`#576`, `#579`, related issue `#573`).
...
- `omq setup` now skips the deprecated **`[tui]`** config section for Qwen Code `>= 0.107.0` (`#572`, fixes `#564`).
...
OMQ team mode now supports **Gemini** as a worker CLI provider in addition to Qwen Code and Claude.

Included in this update:
- Gemini worker launch support in runtime/session resolution
- mixed CLI maps with Qwen Code / Claude / Gemini workers
...
- low-complexity spark default remains **`qwen3.5-plus`**
...
Setup now respects the catalog manifest and current Qwen Code compatibility more strictly:
...
- skips writing the deprecated `[tui]` section when Qwen Code is `>= 0.107.0`

**Why this matters:**
- cleaner installs and upgrades
- fewer stale shipped assets after upgrades
- fewer setup/config issues on newer Qwen Code versions
...
- #572 — fix(setup): skip `[tui]` section for Qwen Code >= `0.107.0`
- #571 — docs: improve OpenClaw gateway configuration examples

### Related issues tagged in this release
- #564 — setup/config breakage caused by deprecated `[tui]` generation on newer Qwen Code versions
- #573 — feat(team): add Gemini CLI worker support to OMQ team mode
