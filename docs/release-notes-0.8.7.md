...
OMQ's instruction surfaces now have a cleaner shared contract across the root orchestrator, templates, generated guidance, and the prompt catalog.
...
OMQ's MCP stdio entrypoints now share one idempotent shutdown path instead of duplicating raw transport bootstrap logic in each server.
...
OMQ no longer blocks native Windows purely because the platform is `win32`.
...
577c416 fix(mcp): centralize stdio lifecycle teardown for OMQ servers (#626) (#627)
