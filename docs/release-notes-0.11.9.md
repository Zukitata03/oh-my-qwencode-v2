...
`0.11.9` is a focused patch release after `0.11.8` that hardens deep-interview / ralplan coordination, repairs setup behavior around Qwen Code-managed TUI configs, and keeps live worker supervision / HUD state visibility aligned with active sessions.
...
- setup no longer rebreaks Qwen Code-managed TUI configs, and default explore-routing guidance stays aligned with setup adoption
