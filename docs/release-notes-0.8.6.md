...
OMQ team orchestration can now wait on canonical team events in addition to terminal completion.

This release adds:
- additive `wake_on=event` / `after_event_id` support to `omq_run_team_wait`
...
- `omq team await <team-name>` CLI support
