...
The code-reviewer agent SHOULD consult Qwen Code for cross-validation.

### Protocol
1. **Form your OWN review FIRST** - Complete the review independently
2. **Consult for validation** - Cross-check findings with Qwen Code
...
Use `mcp__x__ask_qwen` with `agent_role: "code-reviewer"`.
If ToolSearch finds no MCP tools, fall back to the `code-reviewer` agent.

**Note:** Qwen Code calls can take up to 1 hour. Consider the review timeline before consulting.
