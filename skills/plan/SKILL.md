...
- When session guidance enables `USE_OMQ_EXPLORE_CMD`, prefer `omq explore` for simple read-only repository lookups during planning; keep prompts narrow and concrete, and keep prompt-heavy or ambiguous planning work on the richer normal path and fall back normally if `omq explore` is unavailable.
...
3. **Architect** reviews for architectural soundness using `ask_qwen` with `agent_role: "architect"`. Architect review **MUST** include: strongest steelman counterargument (antithesis) against the favored option, at least one meaningful tradeoff tension, and (when possible) a synthesis path. In deliberate mode, Architect should explicitly flag principle violations. **Wait for this step to complete before proceeding to step 4.** Do NOT run steps 3 and 4 in parallel.
4. **Critic** evaluates against quality criteria using `ask_qwen` with `agent_role: "critic"`. Critic **MUST** verify principle-option consistency, fair alternative exploration, risk mitigation clarity, testable acceptance criteria, and concrete verification steps. Critic **MUST** explicitly reject shallow alternatives, driver contradictions, vague risks, or weak verification. In deliberate mode, Critic **MUST** reject missing/weak pre-mortem or missing/weak expanded test plan. Run only after step 3 is complete.
...
6. **Apply improvements**: When reviewers approve with improvement suggestions, merge all accepted improvements into the plan file before proceeding. Final consensus output **MUST** include an **ADR** section with: **Decision**, **Drivers**, **Alternatives considered**, **Why chosen**, **Consequences**, **Follow-ups**. Specifically:
   ...
   f. For the `$team` path, add an explicit launch-hint block with concrete `omq team` / `$team` commands and a **team verification path** (what team proves before shutdown, what Ralph verifies after handoff)
...
2. Evaluate via Critic using `ask_qwen` with `agent_role: "critic"`
...
- For consensus/ralplan execution handoff: **Available-Agent-Types Roster**, **Follow-up Staffing Guidance** (including suggested reasoning levels by lane), explicit `omq team` / `$team` **Launch Hints**, and **Team Verification Path**
...
- Use `ask_qwen` with `agent_role: "planner"` for planning validation on large-scope plans
- Use `ask_qwen` with `agent_role: "analyst"` for requirements analysis
- Use `ask_qwen` with `agent_role: "critic"` for plan review in consensus and review modes
- If ToolSearch finds no MCP tools or Qwen Code is unavailable, fall back to equivalent OMQ prompt agents -- never block on external tools
...
- In consensus mode, execution follow-up handoff **MUST** include an explicit available-agent-types roster plus concrete staffing / role-allocation guidance grounded in that roster, suggested reasoning levels by lane, explicit `omq team` / `$team` launch hints, and a team verification path
</Tool_Usage>
