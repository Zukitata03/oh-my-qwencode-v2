...
0. **Pre-context Intake (required before Phase 0 starts)**:
   ...
   - Load the latest relevant snapshot from `.omq/context/{slug}-*.md` when available.
   - If no snapshot exists, create `.omq/context/{slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`) with:
     - Task statement
     - Desired outcome
...
1. **Phase 0 - Expansion**: Turn the user's idea into a detailed spec
   - If `.omq/specs/deep-interview-*.md` exists for this task: reuse it and skip redundant expansion work
   ...
   - Output: `.omq/plans/autopilot-spec.md`
...
2. **Phase 1 - Planning**: Create an implementation plan from the spec
   ...
   - Output: `.omq/plans/autopilot-impl.md`
...
6. **Phase 5 - Cleanup**: Clear all mode state via OMQ MCP tools on successful completion
...
- Use `ask_qwen` with `agent_role: "architect"` for Phase 4 architecture validation
- Use `ask_qwen` with `agent_role: "security-reviewer"` for Phase 4 security review
- Use `ask_qwen` with `agent_role: "code-reviewer"` for Phase 4 quality review
- Agents form their own analysis first, then consult Qwen Code for cross-validation
- If ToolSearch finds no MCP tools or Qwen Code is unavailable, proceed without it -- never block on external tools
</Tool_Usage>

## State Management

Use `omq_state` MCP tools for autopilot lifecycle state.
