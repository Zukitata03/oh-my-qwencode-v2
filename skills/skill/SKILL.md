...
Meta-skill for managing oh-my-qwencode skills via CLI-like commands.
...
**Behavior:**
1. Scan user skills at `~/.qwen/skills/`
2. Scan project skills at `.qwen/skills/`
3. Parse YAML frontmatter for metadata
4. Display in organized table format:

```
USER SKILLS (~/.qwen/skills/):
...
```
/oh-my-qwencode:<name> example-arg
```
...
✓ Created skill at ~/.qwen/skills/custom-logger/SKILL.md
...
3. **If confirmed:**
   - Delete entire skill directory (e.g., `~/.qwen/skills/<name>/`)
...
```
Skill: custom-logger
Scope: user (~/.qwen/skills/custom-logger/)
Description: Enhanced logging with structured output
Triggers: log, logger, logging
Argument Hint: <level> [message]
Quality: 95% (if available)
Usage Count: 42 times (if available)
File Path: /home/user/.qwen/skills/custom-logger/SKILL.md
...
Description: Enhanced logging with structured output
Triggers: log, logger, logging
File: ~/.qwen/skills/custom-logger/SKILL.md
...
User: yes
Assistant: ✓ Copied 'error-handler' to .qwen/skills/
...
USER_SKILLS_DIR="$HOME/.qwen/skills"
...
echo "=== USER-LEVEL SKILLS (~/.qwen/skills/) ==="
if [ -d "$HOME/.qwen/skills" ]; then
  USER_COUNT=$(find "$HOME/.qwen/skills" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $USER_COUNT"

  if [ $USER_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find "$HOME/.qwen/skills" -name "*.md" -type f -exec sh -c '
      FILE="$1"
...
echo ""
echo "=== PROJECT-LEVEL SKILLS (.qwen/skills/) ==="
if [ -d ".qwen/skills" ]; then
  PROJECT_COUNT=$(find ".qwen/skills" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $PROJECT_COUNT"

  if [ $PROJECT_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find ".qwen/skills" -name "*.md" -type f -exec sh -c '
      FILE="$1"
      NAME=$(grep -m1 "^name:" "$FILE" 2>/dev/null | sed "s/name: //")
      DESC=$(grep -m1 "^description:" "$FILE" 2>/dev/null | sed "s/description: //")
      MODIFIED=$(stat -c "%y" "$FILE" 2>/dev/null || stat -f "%Sm" "$FILE" 2>/dev/null)
...
Then ask for scope:
- **User-level** (~/.qwen/skills/) - Available across all projects
- **Project-level** (.qwen/skills/) - Only for this project
...
**Automatic Application**: Qwen Code detects triggers and applies skills automatically - no need to remember or search for solutions.

**Version Control**: Project-level skills (.qwen/skills/) are committed with your code, so the whole team benefits.

**Evolving Knowledge**: Skills improve over time as you discover better approaches and refine triggers.

**Reduced Token Usage**: Instead of re-solving the same problems, Qwen Code applies known patterns efficiently.
...
✓ User skills directory exists: ~/.qwen/skills/
✓ Project skills directory exists: .qwen/skills/
