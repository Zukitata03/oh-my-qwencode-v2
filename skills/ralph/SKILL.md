...
0. **Pre-context intake (required before planning/execution loop starts)**:
   - Assemble or load a context snapshot at `.omq/context/{task-slug}-{timestamp}.md` (UTC `YYYYMMDDTHHMMSSZ`).
   ...
   - If request ambiguity is high, gather brownfield facts first. When session guidance enables `USE_OMQ_EXPLORE_CMD`, prefer `omq explore` for simple read-only repository lookups with narrow, concrete prompts; otherwise use the richer normal explore path. Then run `$deep-interview --quick <task>` to close critical gaps.
...
5. **Visual task gate (when screenshot/reference images are present)**:
   ...
   - Persist verdict to `.omq/state/{scope}/ralph-progress.json` including numeric + qualitative feedback.
...
7.5 **Mandatory Deslop Pass**:
   - After Step 7 passes, run `oh-my-qwencode:ai-slop-cleaner` on **all files changed during the Ralph session**.
...
- Use `ask_qwen` with `agent_role: "architect"` for verification cross-checks when changes are security-sensitive, architectural, or involve complex multi-system integration
- Skip Qwen Code consultation for simple feature additions, well-tested changes, or time-critical verification
- If ToolSearch finds no MCP tools or Qwen Code is unavailable, proceed with architect agent verification alone -- never block on external tools
...
Use the `omq_state` MCP server tools (`state_write`, `state_read`, `state_clear`) for Ralph lifecycle state.
...
1. Run deep-interview in quick mode before creating PRD artifacts:
   ...
   - Persist interview output to `.omq/interviews/{slug}-{timestamp}.md`
2. Create canonical PRD/progress artifacts:
   - PRD: `.omq/plans/prd-{slug}.md`
   - Progress ledger: `.omq/state/{scope}/ralph-progress.json` (session scope when available, else root scope)
...
5. Initialize canonical progress ledger at `.omq/state/{scope}/ralph-progress.json`
...
Workflow: Detect flag, extract task, create `.omq/plans/prd-{slug}.md`, create `.omq/state/{scope}/ralph-progress.json`, begin ralph loop.

### Legacy compatibility
- If `.omq/prd.json` exists and canonical PRD is absent, migrate one-way into `.omq/plans/prd-{slug}.md`.
- If `.omq/progress.txt` exists and canonical progress ledger is absent, import one-way into `.omq/state/{scope}/ralph-progress.json`.
