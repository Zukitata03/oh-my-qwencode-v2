---
name: cancel
description: Cancel any active OMQ mode (autopilot, ralph, ultrawork, ecomode, ultraqa, swarm, ultrapilot, pipeline, team)
---
...
4. Team artifacts (`.omq/state/team/*/`, tmux sessions matching `omq-team-*`) are best-effort cleared as part of the legacy fallback.

Every `state_clear` command honors the `session_id` argument, so even force mode still uses the session-aware paths first before deleting legacy files.

Legacy compatibility list (removed only under `--force`/`--all`):
- `.omq/state/autopilot-state.json`
- `.omq/state/ralph-state.json`
- `.omq/state/ralph-plan-state.json`
- `.omq/state/ralph-verification.json`
- `.omq/state/ultrawork-state.json`
- `.omq/state/ecomode-state.json`
- `.omq/state/ultraqa-state.json`
- `.omq/state/swarm.db`
- `.omq/state/swarm.db-wal`
- `.omq/state/swarm.db-shm`
- `.omq/state/swarm-active.marker`
- `.omq/state/swarm-tasks.db`
...
- `.omq/state/hud-state.json`
- `.omq/state/subagent-tracking.json`
- `.omq/state/subagent-tracker.lock`
- `.omq/state/rate-limit-daemon.pid`
- `.omq/state/rate-limit-daemon.log`
- `.omq/state/checkpoints/` (directory)
- `.omq/state/sessions/` (empty directory cleanup after clearing sessions)
...
3. If a `session_id` was supplied to `/cancel`, skip legacy fallback entirely and operate solely within that session path; otherwise, consult legacy files in `.omq/state/*.json` only if the state tools report no active session. Swarm remains a shared SQLite/marker mode outside session scoping.
...
Teams are detected by checking for config files in `.omq/state/team/`:

```bash
# Check for active teams
ls .omq/state/team/*/config.json 2>/dev/null
```
...
```
For each team found in .omq/state/team/:
  1. Read config.json to get team_name and workers list
  2. For each worker:
     a. Write shutdown inbox to .omq/state/team/{name}/workers/{worker}/inbox.md
...
After graceful pass:
  ...
  2. Destroy the tmux session: tmux kill-session -t omq-team-{name}
```

**Cleanup:**
```
  1. Strip AGENTS.md team worker overlay (<!-- OMQ:TEAM:WORKER:START/END -->)
  2. Remove team state directory: rm -rf .omq/state/team/{name}/
...
1. Check `.omq/state/team/*/config.json` for active teams
...
5. Destroy tmux session: `tmux kill-session -t omq-team-{name}`
6. Strip AGENTS.md overlay
7. Remove state: `rm -rf .omq/state/team/{name}/`
...
if [[ -f .omq/state/autopilot-state.json ]]; then
  # Clean up ralph if active
  if [[ -f .omq/state/ralph-state.json ]]; then
    RALPH_STATE=$(cat .omq/state/ralph-state.json)
    LINKED_UW=$(echo "$RALPH_STATE" | jq -r '.linked_ultrawork // false')

    # Clean linked ultrawork first
    if [[ "$LINKED_UW" == "true" ]] && [[ -f .omq/state/ultrawork-state.json ]]; then
      rm -f .omq/state/ultrawork-state.json
      echo "Cleaned up: ultrawork (linked to ralph)"
    fi

    # Clean ralph
    rm -f .omq/state/ralph-state.json
    rm -f .omq/state/ralph-verification.json
    echo "Cleaned up: ralph"
  fi

  # Clean up ultraqa if active
  if [[ -f .omq/state/ultraqa-state.json ]]; then
    rm -f .omq/state/ultraqa-state.json
    echo "Cleaned up: ultraqa"
  fi

  # Mark autopilot inactive but preserve state
  CURRENT_STATE=$(cat .omq/state/autopilot-state.json)
  CURRENT_PHASE=$(echo "$CURRENT_STATE" | jq -r '.phase // "unknown"')
  echo "$CURRENT_STATE" | jq '.active = false' > .omq/state/autopilot-state.json
...
if [[ -f .omq/state/ralph-state.json ]]; then
  ...
  # Clean linked ultrawork first
  if [[ "$LINKED_UW" == "true" ]] && [[ -f .omq/state/ultrawork-state.json ]]; then
    UW_STATE=$(cat .omq/state/ultrawork-state.json)
  ...
  # Clean ralph state
  rm -f .omq/state/ralph-state.json
  rm -f .omq/state/ralph-plan-state.json
  rm -f .omq/state/ralph-verification.json
...
```bash
if [[ -f .omq/state/ultrawork-state.json ]]; then
...
```bash
echo "No active OMQ modes detected."
echo ""
echo "Checked for:"
echo "  - Autopilot (.omq/state/autopilot-state.json)"
echo "  - Ralph (.omq/state/ralph-state.json)"
echo "  - Ultrawork (.omq/state/ultrawork-state.json)"
echo "  - UltraQA (.omq/state/ultraqa-state.json)"
echo ""
...
5. Team artifacts (`.omq/state/team/*/`, tmux sessions matching `omq-team-*`) remain best-effort cleanup items invoked during the legacy/global pass.
...
1. **Kill all team tmux sessions**: `tmux list-sessions -F '#{session_name}' 2>/dev/null | grep '^omq-team-'` and kill each
2. **Remove team state directories**: `rm -rf .omq/state/team/*/`
3. **Strip AGENTS.md overlay**: Remove content between `<!-- OMQ:TEAM:WORKER:START -->` and `<!-- OMQ:TEAM:WORKER:END -->`

### Force Clear Addition

When `--force` is used, also clean up:
```bash
rm -rf .omq/state/team/                  # All team state
