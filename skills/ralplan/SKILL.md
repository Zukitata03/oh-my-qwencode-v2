...
4. If ambiguity remains high, gather brownfield facts first. When session guidance enables `USE_OMQ_EXPLORE_CMD`, prefer `omq explore` for simple read-only repository lookups with narrow, concrete prompts; otherwise use the richer normal explore path. Then run `$deep-interview --quick <task>` before continuing.
