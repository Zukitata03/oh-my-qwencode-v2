...
description: Configure OMQ notifications - unified entry point for all platforms
triggers:
...
jq \
  ...
  --arg instruction "${INSTRUCTION:-OMQ event {{event}} for {{projectPath}}}" \
...
> Activation gate: OpenClaw-backed dispatch is active only when `OMQ_OPENCLAW=1`.
> For command gateways, also require `OMQ_OPENCLAW_COMMAND=1`.
> Optional timeout env override: `OMQ_OPENCLAW_COMMAND_TIMEOUT_MS` (ms).
...
- OMQ shell-escapes template substitutions for command gateways (including `{{instruction}}`).
...
- Timeout precedence: `gateways.<name>.timeout` > `OMQ_OPENCLAW_COMMAND_TIMEOUT_MS` > `5000`.
...
- **Error handling**: Append `|| true` to prevent OMQ hook failures from blocking the session.
...
```bash
jq \
  --arg command "(clawdbot agent --session-id omq-hooks --message {{instruction}} --thinking minimal --deliver --reply-channel discord --reply-to 'channel:1468539002985644084' --timeout 120 --json >>/tmp/omq-openclaw-agent.jsonl 2>&1 || true)" \
  '.notifications = (.notifications // {enabled: true}) |
   .notifications.enabled = true |
   ...
   .notifications.openclaw.hooks["session-start"] = {
     enabled: true,
     gateway: "local",
     instruction: "OMQ hook=session-start project={{projectName}} session={{sessionId}} tmux={{tmuxSession}}. 한국어로 상태를 공유하고 SOUL.md를 참고해 필요한 후속 조치를 #omc-dev에 안내하세요."
   } |
   .notifications.openclaw.hooks["session-idle"] = {
     enabled: true,
     gateway: "local",
     instruction: "OMQ hook=session-idle project={{projectName}} session={{sessionId}} tmux={{tmuxSession}}. 한국어로 idle 상황을 간단히 공유하고 진행중인 작업 팔로업을 안내하세요."
   } |
   .notifications.openclaw.hooks["ask-user-question"] = {
     enabled: true,
     gateway: "local",
     instruction: "OMQ hook=ask-user-question session={{sessionId}} tmux={{tmuxSession}} question={{question}}. 한국어로 사용자 응답 필요를 #omc-dev에 알리고 즉시 액션 아이템을 제시하세요."
   } |
   .notifications.openclaw.hooks["stop"] = {
     enabled: true,
     gateway: "local",
     instruction: "OMQ hook=session-stop project={{projectName}} session={{sessionId}} tmux={{tmuxSession}}. 한국어로 중단 상태와 정리 액션을 SOUL.md 기준으로 전달하세요."
   } |
   .notifications.openclaw.hooks["session-end"] = {
     enabled: true,
     gateway: "local",
     instruction: "OMQ hook=session-end project={{projectName}} session={{sessionId}} tmux={{tmuxSession}} reason={{reason}}. 한국어로 완료 요약을 1줄로 남기고 필요한 후속 조치를 안내하세요."
   }' "$CONFIG_FILE" > "$CONFIG_FILE.tmp" && mv "$CONFIG_FILE.tmp" "$CONFIG_FILE"
```
...
```bash
clawdbot agent --session-id omq-hooks --message "OMQ hook test via clawdbot agent path" \
...
```bash
# 1) identify active OMQ tmux sessions
tmux list-sessions -F '#{session_name}' | rg '^omq-' || true
...
tail -n 120 /tmp/omq-openclaw-agent.jsonl | jq -s '.[] | {timestamp: (.timestamp // .time), status: (.status // .error // "ok")}'

# 4) check for recent errors in logs
rg '"error"|"failed"|"timeout"' /tmp/omq-openclaw-agent.jsonl | tail -20
```
...
- env gates: `OMQ_REPLY_ENABLED=true`, and for Discord `OMQ_REPLY_DISCORD_USER_IDS=...`
...
- Config path (`~/.qwen/.omq-config.json`)
