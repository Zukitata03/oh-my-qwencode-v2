---
name: worker
description: Team worker protocol (ACK, mailbox, task lifecycle) for tmux-based OMQ teams
---

# Worker Skill

This skill is for a Qwen Code session that was started as an OMQ Team worker (a tmux pane spawned by `$team`).

## Identity

You MUST be running with `OMQ_TEAM_WORKER` set. It looks like:

`<team-name>/worker-<n>`

Example: `alpha/worker-2`

## Load Worker Skill Path (Claude/Qwen Code)

When a worker inbox tells you to load this skill, resolve the first existing path:

1. `${QWEN_HOME:-~/.qwen}/skills/worker/SKILL.md`
2. `~/.qwen/skills/worker/SKILL.md`
3. `<leader_cwd>/.qwen/skills/worker/SKILL.md`
4. `<leader_cwd>/skills/worker/SKILL.md` (repo fallback)

## Startup Protocol (ACK)

1. Parse `OMQ_TEAM_WORKER` into:
   - `teamName` (before the `/`)
...
Use CLI interop:
- `omq team api send-message --input <json> --json` with `{team_name, from_worker, to_worker:"leader-fixed", body}`
...
6. Claim the task (do NOT start work without a claim) using claim-safe lifecycle CLI interop (`omq team api claim-task --json`).
7. Do the work.
8. Complete/fail the task via lifecycle transition CLI interop (`omq team api transition-task-status --json`) from `in_progress` to `completed` or `failed`.
   - Do NOT directly write lifecycle fields (`status`, `owner`, `result`, `error`) in task files.
9. Use `omq team api release-task-claim --json` only for rollback/requeue to `pending` (not for completion).
...
Use CLI interop:
- `omq team api mailbox-list --json` to read
- `omq team api mailbox-mark-delivered --json` to acknowledge delivery
...
- Prefer inbox/mailbox/task state and `omq team api ... --json` operations.
...
If the lead sends a shutdown request, follow the shutdown inbox instructions exactly, write your shutdown ack file, then exit the Qwen Code session.
