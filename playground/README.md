...
This folder collects **small, reproducible research-style demos** used to showcase `omq autoresearch` on harder optimization problems.
...
- evaluator-driven keep/discard loops that are easy to inspect under `.omq/logs/autoresearch/`
...
|---|---|---|---|---|
| OMQ self-optimization | `missions/in-action-cat-shellout-demo/` | `scripts/eval-in-action-cat-shellout-demo.js` | ✅ completed | kept commit `99ebf16` / cherry-picked as `8478261` removing the autoresearch manifest `cat` shell-out |
...
|---|---:|---:|---:|
| OMQ self-optimization | n/a | behavior-preserving cleanup | n/a |
...
### 1. OMQ self-optimization
- Mission: `missions/in-action-cat-shellout-demo/`
- Evaluator: `scripts/eval-in-action-cat-shellout-demo.js`
- What it demonstrates: a tiny self-hosted code optimization loop where autoresearch removes an unnecessary shell-out from OMQ itself.
...
```bash
omq autoresearch missions/noisy-bayesopt-highdim
```

Then inspect:

```bash
RUN_ID=$(find .omq/logs/autoresearch -maxdepth 1 -mindepth 1 -type d -printf '%f\n' | sort | tail -n 1)
cat .omq/logs/autoresearch/$RUN_ID/manifest.json
cat .omq/logs/autoresearch/$RUN_ID/candidate.json
cat .omq/logs/autoresearch/$RUN_ID/iteration-ledger.json
```
...
- runtime autoresearch logs under `.omq/logs/`
